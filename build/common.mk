...
#           would place them in `GOBIN`, but not `CODEINFRA_ROOT`
#
#  - install: copies the bits we plan to ship into a layout in
#             `CODEINFRA_ROOT` that looks like what a customer would get
#             when they download and install Codeinfra. For JavaScript
...
#              are often integration tests which will use `codeinfra` to
#              deploy example Codeinfra projects, creating cloud
...
#                     $(CODEINFRA_ROOT)/node_modules/$(NODE_MODULE_NAME)
#
...
# Our install targets place items item into $CODEINFRA_ROOT.
CODEINFRA_ROOT ?= $$HOME/.codeinfra-dev
...
CODEINFRA_BIN          := $(CODEINFRA_ROOT)/bin
CODEINFRA_NODE_MODULES := $(CODEINFRA_ROOT)/node_modules
...
install::
	$(call STEP_MESSAGE)
	@# Implicitly creates CODEINFRA_ROOT.
	@mkdir -p $(CODEINFRA_BIN)
	@mkdir -p $(CODEINFRA_NODE_MODULES)
...
install::
	[ ! -e "$(CODEINFRA_NODE_MODULES)/$(NODE_MODULE_NAME)" ] || rm -rf "$(CODEINFRA_NODE_MODULES)/$(NODE_MODULE_NAME)"
	mkdir -p "$(CODEINFRA_NODE_MODULES)/$(NODE_MODULE_NAME)"
	cp -r bin/. "$(CODEINFRA_NODE_MODULES)/$(NODE_MODULE_NAME)"
	cp yarn.lock "$(CODEINFRA_NODE_MODULES)/$(NODE_MODULE_NAME)"
	rm -rf "$(CODEINFRA_NODE_MODULES)/$(NODE_MODULE_NAME)/node_modules"
	cd "$(CODEINFRA_NODE_MODULES)/$(NODE_MODULE_NAME)" && \
