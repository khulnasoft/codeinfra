...
# standard Codeinfra build container.

set -o errexit
set -o pipefail

CODEINFRA_BUILD_CONTAINER_VERSION=v0.3.0


# First build the image for the Codeinfra Build Container
echo "* Building Codeinfra Build Container:"
docker build build-container -t "codeinfra/codeinfra-build-container:${CODEINFRA_BUILD_CONTAINER_VERSION}" --platform linux/x86_64

# Run as the _current_ user, so that the generated files are owned by the current user, not root.
USER=$(id -u):$(id -g)

DOCKER_RUN="docker run --user $USER -it --rm -w /local -v $(pwd)/../sdk/proto/go:/go  -v $(pwd)/../sdk/python:/python -v $(pwd)/../sdk/nodejs:/nodejs -v $(pwd):/local codeinfra/codeinfra-build-container:${CODEINFRA_BUILD_CONTAINER_VERSION}"
...
$DOCKER_RUN /bin/bash -c 'set -x && GO_CODEINFRARPC=/go && \
    ...
    protoc \
      ...
      $PROTO_FILES && \
    rm -rf "$GO_CODEINFRARPC"/* && \
    cp -r "$TEMP_DIR"/codeinfra/* "$GO_CODEINFRARPC"'

# Protoc for JavaScript has a bug where it emits Google Closure Compiler directives in the module prologue that mutate
# the global object, which causes side-by-side bugs in codeinfra/codeinfra (codeinfra/codeinfra#2401). The protoc compiler
...
# with
#   var proto = { codeinfrarpc: ... }, global = proto;
#
...
# on that global state (https://github.com/khulnasoft/codeinfra/pull/2403#issuecomment-458673703).
$DOCKER_RUN /bin/bash -c 'set -x && JS_CODEINFRARPC=/nodejs/proto && \
    JS_PROTOFLAGS="import_style=commonjs,binary"   && \
    PROTO_FILES=$(find . -name "*.proto") && \
    echo -e "\tJavaScript: $JS_CODEINFRARPC [$JS_PROTOFLAGS]" && \
    ...
    protoc --js_out=$JS_PROTOFLAGS:$TEMP_DIR --ts_out=grpc_js:${TEMP_DIR} --grpc_out=grpc_js,minimum_node_version=6:$TEMP_DIR \
        ...
        $PROTO_FILES && \
    find $TEMP_DIR && \
    find "$TEMP_DIR/codeinfra" -type f -name "*.ts" -exec sed -i "s|../codeinfra/|./|" {} \; && \
    find "$TEMP_DIR/codeinfra" -type f -name "*.ts" -exec sed -i "s|/./|/|" {} \; && \
    find "$TEMP_DIR/codeinfra" -type f -name "*.js" -exec sed -i "s|^var global = .*;|var proto = { codeinfrarpc: { codegen: { }, testing: { } } }, global = proto;|" {} \; && \
    find "$TEMP_DIR/codeinfra" -type f -name "*.js" -exec sed -i "s|require('\''../codeinfra/|require('\''./|" {} \; && \
    find "$TEMP_DIR/codeinfra" -type f -name "*.js" -exec sed -i "s|require('\''../../codeinfra/|require('\''../|" {} \; && \
    rm -rf "$JS_CODEINFRARPC"/* && \
    cp "$TEMP_DIR"/google/protobuf/*.js "$JS_CODEINFRARPC" && \
    cp "$TEMP_DIR"/google/protobuf/*.ts "$JS_CODEINFRARPC" && \
    cp -r "$TEMP_DIR"/codeinfra/* "$JS_CODEINFRARPC"'
...
$DOCKER_RUN /bin/bash -c 'PY_CODEINFRARPC=/python/lib/codeinfra/runtime/proto/ && \
    PROTO_FILES=$(find . -name "*.proto") && \
    echo -e "\tPython: $PY_CODEINFRARPC" && \
    ...
    sed -i "s/^from codeinfra import \([^ ]*\)_pb2 as \([^ ]*\)$/from . import \1_pb2 as \2/" "$TEMP_DIR"/codeinfra/*.py && \
    sed -i "s/^from codeinfra.codegen import \([^ ]*\)_pb2 as \([^ ]*\)$/from .codegen import \1_pb2 as \2/" "$TEMP_DIR"/codeinfra/*.py && \
    sed -i "s/^import grpc$/import grpc\nimport grpc.aio\nimport typing/" "$TEMP_DIR"/codeinfra/*.pyi && \
    sed -i "s/: grpc\.Server/: typing.Union[grpc.Server, grpc.aio.Server]/" "$TEMP_DIR"/codeinfra/*.pyi && \
    sed -i "s/@abc.abstractmethod//" "$TEMP_DIR"/codeinfra/*.pyi && \
    rm -rf "$PY_CODEINFRARPC"/* && \
    cp -r "$TEMP_DIR"/codeinfra/* "$PY_CODEINFRARPC" && \
    cp -r "$TEMP_DIR"/google/protobuf/* "$PY_CODEINFRARPC"'
