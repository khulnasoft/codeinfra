# Codeinfra Architecture Overview

Broadly speaking, Codeinfra is composed of five components:
...
These components interact to provide the feature set exposed by the Codeinfra CLI and SDKs, including desired-state
deployments using standard programming languages, remote state storage and secret encryption, and the ability to bridge
the gap between existing and Codeinfra-managed infrastructure.

These components are composed like so:

![Codeinfra CLI](./codeinfra-cli.svg)
