...
programming model, it is important that Codeinfra deals in values with well-defined
semantics. The _Codeinfra type system_ specifies these semantics. It is the responsibility of
...
The core primitives of the Codeinfra type system form a superset of the JSON type system, and
...
Perhaps the most important type in the Codeinfra type system is `Output<T>`. A value of type
`Output<T>` represents a node in a Codeinfra program graph, and behaves like a `Promise<T>`
...
will not be run because it is part of a `codeinfra preview`. Previews typically produce
