...
resources for management by Codeinfra. For example:

- Migrating from manually-managed resources to IaC
- Migrating from other IaC platforms to Codeinfra
- Migrating resources between Codeinfra stacks

At a minimum, importing a resource involves adding the resource's state to the destination
stack's statefile. Once the resource has been added to the stack, the Codeinfra CLI is able
to manage the resource like any other. In order to do anything besides delete the resource,
however, the user must also add a definition for the resource to their Codeinfra program.

Both of the import approaches used by Codeinfra aim to prevent the accidental modification or
...
successfully imported, the stack's Codeinfra program must contain a definition for the
...
The oldest method supported of importing resources into a stack is the [`import`](https://www.codeinfra.com/docs/intro/concepts/resources/#import)
[resource option](https://www.codeinfra.com/docs/intro/concepts/resources/#options). When set,
...
a resource in one `codeinfra up`, then successfully run another `codeinfra up` without removing
...
requires that the desired state described by Codeinfra program for a resource being imported
...
Codeinfra program `Iₚ`, the engine performs the following sequence of operations:
...
2. Process the [`ignoreChanges` resource option](https://www.codeinfra.com/docs/intro/concepts/resources/#import)
...
If all of these steps succeed, the user is left with a definition for `R` in their Codeinfra
...
a technical limitation of the Codeinfra engine. In order to actually perform the diff--an
...
likely be negatively affected by a lack of output from the Codeinfra CLI, which only displays
...
The second, newer method of importing resources into a stack is the [`codeinfra import`
command](https://www.codeinfra.com/docs/cli/commands/codeinfra_import/). This command accepts a
list of import specs to import, imports the resources into the destination stack, and
generates definitions for the resources in the language used by the stack's Codeinfra program.
...
During a `codeinfra import`, given a resource `R` of type `T` with import ID X and an empty
...
of the updated stack that do not differ. The Codeinfra CLI then passes the inputs `Iₚ` stored
...
state in the language used by the destination stack's Codeinfra program. The user can then
copy the generated definition into their Codeinfra program.

Graphically, the import process looks something like this:

![`codeinfra import` diagram](./codeinfra-import.svg)

### Challenges

The primary challenge in generating appropriate code for `codeinfra import` lies in
