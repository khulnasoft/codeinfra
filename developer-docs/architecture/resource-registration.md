...
A Codeinfra program declares the desired states of its stack's resources by sending `RegisterResource` requests to the
Codeinfra engine. Each `RegisterResource` request contains the type, name, and parent (if any) of the resource, a
...
methods to perform it. After the action completes, the engine returns the new state of the resource to the Codeinfra
program.
...
RegisterResourceRequest{
	...
	name: "myBucket",
	parent: "urn:codeinfra:dev::project::codeinfra:codeinfra:Stack::project",
	custom: true,
	object: {},
	version: "4.16.0",
}
...
RegisterResourceRequest{
	...
	custom: true,
	object: {},
	provider: "urn:codeinfra:dev::vpc-2::codeinfra:providers:aws::us-west-2::308b79ee-8249-40fb-a203-de190cb8faa8",
	version: "4.16.0",
}
```
...
Once the Codeinfra program has exited, the step generator determines which existing resources must be deleted by taking
...
1. If the resource has been marked for replacement out of band (e.g. by the use of the `--target-replace` command-line
   option of the Codeinfra CLI), the resource must be replaced.
...
deployment. Once the Codeinfra program has exited and the [step generator] has issued all required deletions, the step
