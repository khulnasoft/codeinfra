...
construction of component resources within the scope of a Codeinfra stack. Custom resources
...
resources with the Codeinfra engine.

#### URNs

Each resource registered with the Codeinfra engine is logically identified by its
...
```ebnf
urn = "urn:codeinfra:" stack "::" project "::" qualified type name "::" name ;
...
the Codeinfra engine, and is only meaningful to the provider as a means to identify a
...
[well-defined lifecycle](#custom-resource-lifecycle) within the scope of a Codeinfra stack.
...
with a connection to the Codeinfra engine.

### Data Exchange Types

The values exchanged between Codeinfra resource providers and the Codeinfra engine are a
superset of the values expressible in JSON.

Codeinfra supports the following data types:
...
- [`Archive`](#assets-and-archives), which represents a map from strings to `Asset`s or
  `Archive`s
- [`ResourceReference`](#resource-references), which represents a reference to a [Codeinfra
  resource](#resources)
...
A `ResourceReference` represents a reference to a [Codeinfra resource](#resources). Although
...
Each provider constitutes the implementation of a single Codeinfra package. Each Codeinfra
...
per-language SDKs for the Codeinfra package, but is also used for importing resources,
...
Clients of a provider (e.g. the Codeinfra CLI) must obey the provider lifecycle. This
...
Within the scope of a Codeinfra stack, each provider instance has a corresponding provider
resource. Provider resources are custom resources that are managed by the Codeinfra engine,
...
Within the context of the Codeinfra CLI, the list of available factories is the list of
installed resource plugins plus the builtin `codeinfra` provider. The list of installed
resource plugins can be viewed by running `codeinfra plugin ls`.
...
Because it has no connection to the Codeinfra engine during configuration, a provider's
...
2. [`DiffConfig`](#diffconfig), which indicates whether or not the new configuration can
   ...
   [preview](#preview) or [update](#update) of a Codeinfra stack).
...
A custom resource has a well-defined lifecycle within the scope of a Codeinfra stack. When a
custom resource is registered by a Codeinfra program, the Codeinfra engine first determines
...
If a managed resource registered by a Codeinfra program is not re-registered by the next
successful execution of a Codeinfra program in the resource's stack, the engine deletes the
...
[ignoreChanges](https://www.codeinfra.com/docs/concepts/options/ignorechanges/) resource
