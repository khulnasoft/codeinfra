PROJECT_NAME := Codeinfra SDK
SDKS         ?= nodejs python go
SUB_PROJECTS := $(SDKS:%=sdk/%)

include build/common.mk

PROJECT         := github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra

PKG_CODEGEN := github.com/khulnasoft/codeinfra/pkg/v3/codegen
...
INTEGRATION_PKG := github.com/khulnasoft/codeinfra/tests/integration
PERFORMANCE_PKG := github.com/khulnasoft/codeinfra/tests/performance
TESTS_PKGS      := $(shell cd ./tests && go list -tags all ./... | grep -v tests/templates | grep -v ^${INTEGRATION_PKG}$ | grep -v ^${PERFORMANCE_PKG}$)
VERSION         := $(if ${CODEINFRA_VERSION},${CODEINFRA_VERSION},$(shell ./scripts/codeinfra-version.sh))

# Relative paths to directories with go.mod files that should be linted.
LINT_GOLANG_PKGS := sdk pkg tests sdk/go/codeinfra-language-go sdk/nodejs/cmd/codeinfra-language-nodejs sdk/python/cmd/codeinfra-language-python cmd/codeinfra-test-language
...
build:: build_proto build_display_wasm go.ensure
	cd pkg && go install -ldflags "-X github.com/khulnasoft/codeinfra/sdk/v3/go/common/version.Version=${VERSION}" ${PROJECT}

build_display_wasm:: go.ensure
	cd pkg && GOOS=js GOARCH=wasm go build -o ../bin/codeinfra-display.wasm -ldflags "-X github.com/khulnasoft/codeinfra/sdk/v3/go/common/version.Version=${VERSION}" ./backend/display/wasm

install:: .ensure.phony go.ensure
	cd pkg && GOBIN=$(CODEINFRA_BIN) go install -ldflags "-X github.com/khulnasoft/codeinfra/sdk/v3/go/common/version.Version=${VERSION}" ${PROJECT}

build_debug::
	cd pkg && go install -gcflags="all=-N -l" -ldflags "-X github.com/khulnasoft/codeinfra/sdk/v3/go/common/version.Version=${VERSION}" ${PROJECT}

build_cover::
	cd pkg && go build -cover -o ../bin/codeinfra \
		-coverpkg github.com/khulnasoft/codeinfra/pkg/v3/...,github.com/khulnasoft/codeinfra/sdk/v3/... \
		-ldflags "-X github.com/khulnasoft/codeinfra/sdk/v3/go/common/version.Version=${VERSION}" ${PROJECT}

install_cover:: build_cover
	cp bin/codeinfra $(CODEINFRA_BIN)
...
test_lifecycle:
	@cd pkg && $(GO_TEST) github.com/khulnasoft/codeinfra/pkg/v3/engine/lifecycletest

test_lifecycle_fuzz: GO_TEST_RACE = false
test_lifecycle_fuzz: export CODEINFRA_LIFECYCLE_TEST_FUZZ := 1
test_lifecycle_fuzz:
	@cd pkg && $(GO_TEST) github.com/khulnasoft/codeinfra/pkg/v3/engine/lifecycletest \
		-run '^TestFuzz$$' \
...
test_integration_%:
	@cd tests && CODEINFRA_INTEGRATION_TESTS=$(subset) $(GO_TEST) $(INTEGRATION_PKG)
...
schema-%: curl.ensure jq.ensure
	...
	@[ -f pkg/codegen/testing/test/testdata/${name}-${version}.json ] || \
		curl "https://raw.githubusercontent.com/khulnasoft/codeinfra-${name}/v${version}/provider/cmd/codeinfra-resource-${name}/schema.json" \
...
work:
	...
	go work init \
		cmd/codeinfra-test-language \
		pkg \
		sdk \
		sdk/go/codeinfra-language-go \
		sdk/nodejs/cmd/codeinfra-language-nodejs \
		sdk/python/cmd/codeinfra-language-python \
		tests
