...
on--it is important that the Codeinfra engine manages information consistently
according to some agreed upon set of semantics. To this end Codeinfra defines a
...
The core of Codeinfra's type system is an extension of that offered by
...
* [`ResourceReference`](res-refs), which represents a reference to a
  [resource](resources) managed by Codeinfra.
...
[](codeinfrarpc.ResourceProvider.Invoke)s (that is, provider function calls which
...
Resources are the fundamental unit of Codeinfra-managed infrastructure, such as a
...
[](codeinfrarpc.ResourceProvider.Update), and [](codeinfrarpc.ResourceProvider.Delete)
...
[*Component
resources*](https://www.codeinfra.com/docs/concepts/resources/components/) are
...
resources with the Codeinfra engine in a [](codeinfrarpc.ResourceProvider.Construct)
operation.
...
[*`Type`*](https://www.codeinfra.com/docs/iac/concepts/resources/names/#types)
...
last actual state that Codeinfra recorded. A resource's [*uniform resource name*,
or
*`URN`*](https://www.codeinfra.com/docs/iac/concepts/resources/names/#urns)[^urn-uniqueness]
serves as its identifier to the Codeinfra engine and is built from the stack and
project a resource is located in, followed by its type, parent type and name:

```
urn:codeinfra:<stack>::<project>::<qualified-type>::<name>
```
...
If a resource has been [created](codeinfrarpc.ResourceProvider.Create) or
[read](codeinfrarpc.ResourceProvider.Read) by a [provider](providers), it will also
...
`Output<T>` is perhaps the most important member of Codeinfra's type system,
representing a node in a Codeinfra program graph (such as the output of a [resource
...
operation that will not be run, for example because it is part of a `codeinfra
...
[^explicit-unknowns]:
    There are also cases where unknown values are used explicitly by the Codeinfra
...
[`ignoreChanges`](https://www.codeinfra.com/docs/iac/concepts/options/ignorechanges/)
for, or when identifying the set of properties responsible for a particular
[](codeinfrarpc.DiffResponse). Example property paths include:
...
properties. To this end, the Codeinfra type system specifies the following
