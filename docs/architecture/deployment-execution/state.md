...
*State* is the metadata that Codeinfra stores about the infrastructure it manages,
and, among other things, is key to enabling Codeinfra to work out when to create,
update, replace and delete resources. A *snapshot* is a view of a Codeinfra state
...
A *backend* is an API and storage endpoint used by the Codeinfra CLI to coordinate
...
persisted to a medium controlled and managed by the Codeinfra user. Under the hood,
Codeinfra uses the [Go Cloud Development Kit](https://gocloud.dev/) (specifically,
...
[Codeinfra Cloud](https://www.codeinfra.com/product/codeinfra-cloud/) is the primary
example of this.
...
[`Snapshot.VerifyIntegrity`](gh-file:codeinfra#pkg/resource/deploy/snapshot.go)
method is responsible for performing these checks. When a snapshot has an
integrity error, the Codeinfra CLI will refuse to operate on it.[^sie-p1] Note that the
Codeinfra CLI will *not refuse to write a snapshot with integrity errors*, since
...
* *The program may change between operations.* If you are debugging or
  ...
  operation -- see [](gh-issue:codeinfra#17117) for an example of these kinds of
  interactions).
...
* [Fix snapshot integrity on pending replacement](gh-issue:codeinfra#17146)
* [Propagate deleted parents of untargeted resources](gh-issue:codeinfra#17117)
* [Better handle property dependencies and `deletedWith`](gh-issue:codeinfra#16088)
* [Rewrite `DeletedWith` properties when renaming stacks](gh-issue:codeinfra#16216)
