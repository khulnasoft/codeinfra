...
As a Codeinfra program is executed by the language host, it will make
[](codeinfrarpc.ResourceMonitor.RegisterResource) calls to the engine in order to
declare resources and their desired state. Each
[](codeinfrarpc.RegisterResourceRequest) contains:

* The resource's type and name.
* The resource's [parent](https://www.codeinfra.com/docs/concepts/options/parent/),
  if it has one.
* A reference to the
  [provider](https://www.codeinfra.com/docs/concepts/resources/providers/) that
  manages the resource, if an explicit one has been specified. If no reference
  has been specified, Codeinfra will use a [default provider](#default-providers)
  instance for the resource's package and version.
* Values for the resource's input properties.
* Any [resource options](https://www.codeinfra.com/docs/concepts/options/) that
...
<gh-file:codeinfra#pkg/resource/deploy/source_eval.go>) implements the
[](codeinfrarpc.ResourceMonitor) interface, which is the primary communication
...
[](codeinfrarpc.RegisterResourceRequest)s as follows:
...
* If the request registers a [remote component](component-providers), the
  ...
  [](codeinfrarpc.ResourceProvider.Construct) call to the component provider and
  await the result.
...
* When a result is received (either in response to a
  [](codeinfrarpc.ResourceProvider.Construct) call or a `RegisterResourceEvent`),
...
The *step generator* (<gh-file:codeinfra#pkg/resource/deploy/step_generator.go>) is
...
5. If the resource is not being imported, the step generator will continue by
   calling the provider's [](codeinfrarpc.ResourceProvider.Check) method with both
   ...
   that may be used in later calls to [](codeinfrarpc.ResourceProvider.Diff),
   [](codeinfrarpc.ResourceProvider.Create), and
   [](codeinfrarpc.ResourceProvider.Update).
...
11. If the resource is being replaced, call the resource provider's
    [](codeinfrarpc.ResourceProvider.Check) method again, this time sending no
...
[](codeinfrarpc.ResourceProvider.Diff) method, there are a number of cases where
...
4. In all other cases, the engine will call the provider's
   [](codeinfrarpc.ResourceProvider.Diff) method to determine whether the resource
   must be updated, replaced, or left as-is.

(step-generation-deletions)=
### Deletions

Once the Codeinfra program has exited, the step generator determines the set of
...
By default, Codeinfra will replace resources by first creating the replacement and
then deleting the original (create-before-replace). There are cases however
where Codeinfra will delete the original first (a delete-before-replace):

* If the resource's provider specifies that the resource must be deleted before
  it can be replaced as using a [](codeinfrarpc.DiffResponse)'s
  `deleteBeforeReplace` field.
* If the program specifies the [`deleteBeforeReplace` resource
  option](https://www.codeinfra.com/docs/concepts/options/deletebeforereplace/) for
  the resource.
...
2. Remove from this set any resources that would *not themselves be replaced* by
   ...
   and calling the provider's [](codeinfrarpc.ResourceProvider.Diff) method.
...
[`dependsOn`](https://www.codeinfra.com/docs/concepts/options/dependson/), its
...
failure and cancels the deployment. Once the Codeinfra program has exited and the
...
sequenceDiagram
    ...
    Note over SG: Codeinfra program exits
