...
The Codeinfra engine is responsible for orchestrating *deployments*. A deployment
...
* [Resource events](resource-registration) result in [steps](step-generation), which
  ...
  [](codeinfrarpc.ResourceProvider.Create) method in order to create a new
  resource.
...
* `destroy` uses a *null source*
  (<gh-file:codeinfra#pkg/resource/deploy/source_null.go>), which simply returns no
...
* `refresh` uses an *error source*
  (<gh-file:codeinfra#pkg/resource/deploy/source_error.go>), which returns an error
...
For `preview` and `update` operations, the Codeinfra program specifies the desired
...
(<gh-file:codeinfra#pkg/resource/deploy/source_eval.go>) that boots up a language
...
* [](codeinfrarpc.ResourceMonitor) is the primary interface used to manage
  resources. A resource monitor supports [registering
  resources](codeinfrarpc.ResourceMonitor.RegisterResource), [reading
  resources](codeinfrarpc.ResourceMonitor.ReadResource), [invoking
  functions](codeinfrarpc.ResourceMonitor.Invoke), and many more operations key to
  the operation of a Codeinfra program. An evaluation source contains a
...
* [](codeinfrarpc.Engine) exposes auxilliary operations that are not specific to a
  particular resource, such as [logging](codeinfrarpc.Engine.Log) and [state
  management](codeinfrarpc.Engine.SetRootResource).
* [](codeinfrarpc.Callbacks) allows callers to [execute
  functions](codeinfrarpc.Callbacks.Invoke) in the language host, such as
  [transforms](https://www.codeinfra.com/docs/concepts/options/transforms/), which
...
in a number of processes to execute a Codeinfra program, as elucidated in the
following pages.
