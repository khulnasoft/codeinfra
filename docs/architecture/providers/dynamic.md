...
[*Dynamic
providers*](https://www.codeinfra.com/docs/concepts/resources/dynamic-providers/)
are a Codeinfra feature that allows the core logic of a provider to be defined and
managed within the context of a Codeinfra program. This is in contrast to a normal
...
* The SDK defines two types:
  ...
  * That of *dynamic resources* -- those that are managed by a dynamic provider.
    ...
    core resource type so that all dynamic resources *have the same Codeinfra
    package* -- `codeinfra-nodejs` for NodeJS and `codeinfra-python` for Python.

  These are located in <gh-file:codeinfra#sdk/nodejs/dynamic/index.ts> in
  NodeJS/TypeScript and
  <gh-file:codeinfra#sdk/python/lib/codeinfra/dynamic/dynamic.py> in Python.
* The SDK also defines a "real" provider that implements the gRPC interface and
  ...
  <gh-file:codeinfra#sdk/nodejs/cmd/dynamic-provider/index.ts> for NodeJS and
  <gh-file:codeinfra#sdk/python/lib/codeinfra/dynamic/__main__.py> for Python.
...
* When a dynamic resource class is instantiated, it *captures the provider
  ...
  * In NodeJS, serialization is performed by capturing and mangling the source
    ...
    <gh-file:codeinfra#sdk/nodejs/runtime/closure> for the gory details.
  * In Python, serialization is performed by pickling the dynamic provider
    instance -- see <gh-file:codeinfra#sdk/python/lib/codeinfra/dynamic/dynamic.py>'s
...
* When the engine receives requests pertaining to dynamic resources, the fixed
  package (`codeinfra-nodejs` or `codeinfra-python`) will cause it to make provider
...
* Even when code works once, or in one context, it might not work later on. If
  ...
  Codeinfra state means that on later hydration, a program that worked before might
  not work again.
