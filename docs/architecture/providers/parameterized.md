...
*Parameterized providers* are a feature of Codeinfra that allows a caller to change
a provider's behaviour at runtime in response to a
[](codeinfrarpc.ResourceProvider.Parameterize) call. Where a
[](codeinfrarpc.ResourceProvider.Configure) call allows a caller to influence
provider behaviour at a high level (e.g. by specifying the region in which an
AWS provider should operate), a [](codeinfrarpc.ResourceProvider.Parameterize) call
...
* Dynamically bridging Terraform providers. The
  [`codeinfra-terraform-bridge`](https://github.com/khulnasoft/codeinfra-terraform-bridge)
  can be used to build a Codeinfra provider that wraps a Terraform provider. This
  ...
  provider. The resulting provider can then be published as a Codeinfra plugin and
  its [](codeinfrarpc.ResourceProvider.GetSchema) method used to generate
  ...
  providers.
  [`codeinfra-terraform-provider`](https://github.com/khulnasoft/codeinfra-terraform-provider)
  ...
  `codeinfra-terraform-provider` accepts the name of the Terraform provider to
  bridge and uses the existing `codeinfra-terraform-bridge` machinery to perform
...
* Managing Kubernetes clusters with [custom resource definitions
  ...
  standard set of APIs (`Pod`, `Service`, and so on). By default, the Codeinfra
...
As hinted at by the above examples, [](codeinfrarpc.ResourceProvider.Parameterize)
encodes a provider-specific *parameter* that is used to influence the provider's
behaviour. The parameter passed in the [](codeinfrarpc.ParameterizeRequest) can
take two forms, corresponding to the two contexts in which parameterization
typically occurs:

* When generating an SDK (e.g. using a `codeinfra package add` command), we need to
...
turn would live in a binary named `codeinfra-resource-aws`. In the presence of
