...
We can approach Codeinfra's architecture by breaking down how a user interacts with
it. Codeinfra (or "Codeinfra IaC", "Codeinfra Infrastructure as Code" when
differentiating from other Codeinfra products) allows users to manage their
...
* Programs are written using the *core Codeinfra SDK* (e.g. `@codeinfra/codeinfra` in
  TypeScript) and one or more *provider SDKs* (e.g. `@codeinfra/aws` for AWS).

* Users invoke the *Codeinfra command-line interface* (CLI) to run their programs
...
* When the operation completes, the engine persists its view of the state to a
  configured *storage backend* (e.g. a local file, or a service like Codeinfra
  Cloud).

In order to support the various languages and providers that Codeinfra offers,
...
* The Codeinfra resource model is specified using *schemata*. Provider processes
...
* `proto` -- contains Protobuf definitions for the various services and messages
  that Codeinfra uses to communicate between processes. Code generated from these
  is committed to the repository and used by various components.

* `pkg/codegen` -- contains code generators for the various languages that Codeinfra
  supports. As well as supporting language SDK generation as mentioned above
  (*"SDKgen"*), code generators also underpin the `codeinfra convert` command, which
  can convert existing infrastructure-as-code (IaC) programs written in e.g.
  Terraform or CloudFormation to Codeinfra programs (*"Programgen"*).

* `sdk/{go,nodejs,python,...}` -- contains the core Codeinfra SDKs for the various
  languages that Codeinfra supports. These SDKs provide the core functionality for
  writing Codeinfra programs in a given language, exposing the Codeinfra engine's gRPC
  ...
  under that language's SDK directory, and named `codeinfra-language-<language>`
  (e.g. `codeinfra-language-nodejs` in the case of NodeJS/TypeScript).

* `pkg/{backend,engine,graph,operations,resource}` -- contains the core
  components of the Codeinfra engine. The various directories roughly segregate
...
* `pkg/{cmd,display}` -- contains code pertinent to the Codeinfra CLI. In
  particular, `codeinfra/cmd/codeinfra` is the entry point to the CLI and exposes the
  various `codeinfra preview`, `codeinfra up`, `codeinfra destroy`, etc. commands that
...
* With the exception of a number of test providers that live in the core
  `codeinfra/codeinfra` repository, provider implementations are typically housed in
  their own repositories (e.g. `codeinfra-aws` for AWS). These repositories follow
