...
func construct(ctx context.Context, req *codeinfrarpc.ConstructRequest, engineConn *grpc.ClientConn,
	...
	codeinfraCtx, err := NewContext(ctx, runInfo)
	if err != nil {
	...
	for _, urn := range req.GetDependencies() {
		dependencies = append(dependencies, codeinfraCtx.newDependencyResource(URN(urn)))
	}

	providers := make(map[string]ProviderResource, len(req.GetProviders()))
	for pkg, ref := range req.GetProviders() {
		resource, err := createProviderResource(codeinfraCtx, ref)
		if err != nil {
			return nil, err
		}
	...
	var parent Resource
	if req.GetParent() != "" {
		parent = codeinfraCtx.newDependencyResource(URN(req.GetParent()))
	}
	opts := resourceOption(func(ro *resourceOptions) {
		...
		if urn := req.DeletedWith; urn != "" {
			ro.DeletedWith = codeinfraCtx.newDependencyResource(URN(urn))
		}
	...
	urn, state, err := constructF(codeinfraCtx, req.GetType(), req.GetName(), inputs, opts)
	if err != nil {
	...
	if err = codeinfraCtx.wait(); err != nil {
		return nil, err
	}
	...
	rpcProps, err := plugin.MarshalProperties(
		resolvedProps,
		plugin.MarshalOptions{KeepSecrets: true, KeepUnknowns: keepUnknowns, KeepResources: codeinfraCtx.state.keepResources})
	if err != nil {
		return nil, fmt.Errorf("marshaling properties: %w", err)
	}

	// Convert the property dependencies map for RPC and remove duplicates.
	rpcPropertyDeps := make(map[string]*codeinfrarpc.ConstructResponse_PropertyDependencies)
	for k, deps := range propertyDeps {
		...
		rpcPropertyDeps[k] = &codeinfrarpc.ConstructResponse_PropertyDependencies{
			Urns: urns,
		}
	}
...
func copyInputTo(ctx *Context, v resource.PropertyValue, dest reflect.Value) error {
	...
	case v.IsOutput():
		...
		if !dest.Type().Implements(outputType) && !dest.Type().Implements(inputType) {
			return fmt.Errorf("expected destination type to implement codeinfra.Input or codeinfra.Output, got %v", dest.Type())
		}
...
func copyToStruct(ctx *Context, v resource.PropertyValue, typ reflect.Type, dest reflect.Value) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		tag := typ.Field(i).Tag.Get("codeinfra")
...
func constructInputsCopyTo(ctx *Context, inputs map[string]interface{}, args interface{}) error {
	...
	for k, v := range inputs {
		...
		for i := 0; i < typ.NumField(); i++ {
			...
			field := typ.Field(i)
			tag, has := field.Tag.Lookup("codeinfra")
			tag = strings.Split(tag, ",")[0] // tagName,flag => tagName
			if !has || tag != k {
				continue
			}
			...
			if len(ci.deps) > 0 {
				return fmt.Errorf("copying input %q: %s.%s is typed as %v but must be a type that implements codeinfra.Input or "+
					"codeinfra.Output for input with dependencies", k, typ, field.Name, field.Type)
			}
			...
			if secret {
				return fmt.Errorf("copying input %q: %s.%s is typed as %v but must be a type that implements codeinfra.Input or "+
					"codeinfra.Output for secret input", k, typ, field.Name, field.Type)
			}
...
func call(ctx context.Context, req *codeinfrarpc.CallRequest, engineConn *grpc.ClientConn,
	...
	result, failures, err := callF(codeinfraCtx, req.GetTok(), args)
	if err != nil {
	...
	// Convert the property dependencies map for RPC and remove duplicates.
	rpcPropertyDeps := make(map[string]*codeinfrarpc.CallResponse_ReturnDependencies)
	for k, deps := range propertyDeps {
		sort.Slice(deps, func(i, j int) bool { return deps[i] < deps[j] })
		...
		rpcPropertyDeps[k] = &codeinfrarpc.CallResponse_ReturnDependencies{
			Urns: urns,
		}
	}

	var rpcFailures []*codeinfrarpc.CheckFailure
	if len(failures) > 0 {
		rpcFailures = make([]*codeinfrarpc.CheckFailure, len(failures))
		for i, v := range failures {
			failure := v.(callFailure)
			rpcFailures[i] = &codeinfrarpc.CheckFailure{
