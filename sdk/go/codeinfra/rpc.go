...
func marshalInputs(props Input) (resource.PropertyMap, map[string][]URN, []URN, error) {
	...
	case reflect.Struct:
		...
		for i := 0; i < numFields; i++ {
			...
			tag := destField.Tag.Get("codeinfra")
...
func unmarshalResourceReference(ctx *Context, ref resource.ResourceReference) (Resource, error) {
	...
	isProvider := tokens.Token(resType).HasModuleMember() && resType.Module() == "codeinfra:providers"
	if isProvider {
...
// keep things as plain types where possible (e.g. a string property value will just be a `codeinfra.String`, not an
// `OutputString`). It will use `codeinfra.Output` for values that are either Computed (will always be a
// `codeinfra.AnyOutput`), secret, or an output property value.
	...
	unmarshal = func(v resource.PropertyValue) (Input, error) {
		...
		case v.IsOutput():
			...
			if v.OutputValue().Known {
				...
				if element != nil {
					// element will be an Input/Output type like codeinfra.String or codeinfra.AnyOutput. We want
...
// RegisterResourcePackage register a resource package with the Codeinfra runtime.
...
// RegisterResourceModule register a resource module with the Codeinfra runtime.
