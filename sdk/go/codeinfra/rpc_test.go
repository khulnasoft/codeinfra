...
import (
	...
	rarchive "github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource/archive"
	rasset "github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource/asset"
...
func (rp *testResourcePackage) ConstructProvider(ctx *Context, name, typ, urn string) (ProviderResource, error) {
	if typ != "codeinfra:providers:test" {
...
type test struct {
	S                              string                 `codeinfra:"s"`
	A                              bool                   `codeinfra:"a"`
	B                              int                    `codeinfra:"b"`
	StringAsset                    Asset                  `codeinfra:"cStringAsset"`
	FileAsset                      Asset                  `codeinfra:"cFileAsset"`
	RemoteAsset                    Asset                  `codeinfra:"cRemoteAsset"`
	AssetArchive                   Archive                `codeinfra:"dAssetArchive"`
	FileArchive                    Archive                `codeinfra:"dFileArchive"`
	RemoteArchive                  Archive                `codeinfra:"dRemoteArchive"`
	E                              interface{}            `codeinfra:"e"`
	Array                          []interface{}          `codeinfra:"fArray"`
	Map                            map[string]interface{} `codeinfra:"fMap"`
	G                              string                 `codeinfra:"g"`
	H                              string                 `codeinfra:"h"`
	I                              string                 `codeinfra:"i"`
	CustomResource                 CustomResource         `codeinfra:"jCustomResource"`
	ComponentResource              ComponentResource      `codeinfra:"jComponentResource"`
	ProviderResource               ProviderResource       `codeinfra:"jProviderResource"`
	PreviewCustomResource          CustomResource         `codeinfra:"jPreviewCustomResource"`
	PreviewProviderResource        ProviderResource       `codeinfra:"jPreviewProviderResource"`
	MissingCustomResource          CustomResource         `codeinfra:"kCustomResource"`
	MissingComponentResource       ComponentResource      `codeinfra:"kComponentResource"`
	MissingProviderResource        ProviderResource       `codeinfra:"kProviderResource"`
	MissingPreviewCustomResource   CustomResource         `codeinfra:"kPreviewCustomResource"`
	MissingPreviewProviderResource ProviderResource       `codeinfra:"kPreviewProviderResource"`
}
...
func TestMarshalRoundtrip(t *testing.T) {
	...
	providerURN := resource.NewURN("stack", "project", "", "codeinfra:providers:test", "test")
	previewCustomURN := resource.NewURN("stack", "project", "", "test:index:custom", "preview")
	previewProviderURN := resource.NewURN("stack", "project", "", "codeinfra:providers:test", "preview")
	...
	missingProviderURN := resource.NewURN("stack", "project", "", "codeinfra:providers:missing", "test")
	missingPreviewCustomURN := resource.NewURN("stack", "project", "", "missing:index:custom", "preview")
	missingPreviewProviderURN := resource.NewURN("stack", "project", "", "codeinfra:providers:missing", "preview")
	...
	inputs := testInputs{
		...
		RemoteAsset: NewRemoteAsset("https://codeinfra.com/fake/txt"),
		...
		RemoteArchive: NewRemoteArchive("https://codeinfra.com/fake/archive.zip"),
		E:             out,
	...
	if assert.NoError(t, err) {
		...
		if assert.NoError(t, err) {
			if assert.NotNil(t, resV) {
				...
				assert.Equal(t, "https://codeinfra.com/fake/txt", res["cRemoteAsset"].(Asset).URI())
				...
				assert.Equal(t, "https://codeinfra.com/fake/archive.zip", res["dRemoteArchive"].(Archive).URI())
...
type nestedTypeInputs struct {
	Foo StringInput `codeinfra:"foo"`
	Bar IntInput    `codeinfra:"bar"`
}
...
type testResourceArgs struct {
	URN URN `codeinfra:"urn"`
	ID  ID  `codeinfra:"id"`

	Any     interface{}            `codeinfra:"any"`
	Archive Archive                `codeinfra:"archive"`
	Array   []interface{}          `codeinfra:"array"`
	Asset   Asset                  `codeinfra:"asset"`
	Bool    bool                   `codeinfra:"bool"`
	Float64 float64                `codeinfra:"float64"`
	Int     int                    `codeinfra:"int"`
	Map     map[string]interface{} `codeinfra:"map"`
	String  string                 `codeinfra:"string"`

	Nested nestedType `codeinfra:"nested"`
}
...
type testResource struct {
	...
	Any     AnyOutput     `codeinfra:"any"`
	Archive ArchiveOutput `codeinfra:"archive"`
	Array   ArrayOutput   `codeinfra:"array"`
	Asset   AssetOutput   `codeinfra:"asset"`
	Bool    BoolOutput    `codeinfra:"bool"`
	Float64 Float64Output `codeinfra:"float64"`
	Int     IntOutput     `codeinfra:"int"`
	Map     MapOutput     `codeinfra:"map"`
	String  StringOutput  `codeinfra:"string"`

	Nested nestedTypeOutput `codeinfra:"nested"`
}
...
func TestResourceState(t *testing.T) {
	...
	resolved, _, _, _ := marshalInputs(&testResourceInputs{
		Any:     String("foo"),
		Archive: NewRemoteArchive("https://codeinfra.com/fake/archive.zip"),
...
type foo struct {
	TemplateOptions *TemplateOptions `codeinfra:"templateOptions"`
}
...
type TemplateOptions struct {
	...
	TagSpecifications []TemplateTagSpecification `codeinfra:"tagSpecifications"`
}
...
type TemplateTagSpecificationArgs struct {
	Name StringPtrInput `codeinfra:"name"`
	Tags StringMapInput `codeinfra:"tags"`
}
...
type bucketObjectArgs struct {
	Source AssetOrArchive `codeinfra:"source"`
}
...
type myNestedOutputArgs struct {
	Nested interface{} `codeinfra:"nested"`
}
...
func TestUnmarshalPropertyMap(t *testing.T) {
	...
	t.Run("output", func(t *testing.T) {
		...
		dependencies := []resource.URN{"urn:codeinfra:test_stack::test_project::pkg:index:type::name"}
		...
		assertDeps := func(actual []Resource) {
			...
			require.NoError(t, err)
			assert.True(t, known)
			assert.Equal(t, URN("urn:codeinfra:test_stack::test_project::pkg:index:type::name"), value.(URN))
		}
	...
	t.Run("resource", func(t *testing.T) {
		...
		actual, err := unmarshalPropertyMap(ctx, resource.PropertyMap{
			"unknown id": resource.NewResourceReferenceProperty(resource.ResourceReference{
				URN: "urn:codeinfra:test_stack::test_project::pkg:index:type::name",
			}),
		})
		require.NoError(t, err)
		...
		require.NoError(t, err)
		assert.Equal(t, URN("urn:codeinfra:test_stack::test_project::pkg:index:type::name"), value)
		assert.True(t, known)
		assert.False(t, secret)
	})
}
