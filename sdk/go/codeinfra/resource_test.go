...
func TestUninitializedParentResource(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			parent: &myComponent{},
			wantErr: "WARNING: Ignoring component resource *codeinfra.myComponent " +
		...
		},
		{
			...
			wantErr: "WARNING: Ignoring resource *codeinfra.myCustomResource " +
				"(parent of my-resource :: test:index:MyResource) " +
...
func TestComponentResourcePropagatesProvider(t *testing.T) {
	...
	t.Run("provider option", func(t *testing.T) {
		...
		err := RunErr(func(ctx *Context) error {
			...
			require.NoError(t,
				ctx.RegisterResource("codeinfra:providers:test", "prov", nil /* props */, &prov),
...
//
// Regression test for https://github.com/khulnasoft/codeinfra/issues/12430
func TestRemoteComponentResourcePropagatesProvider(t *testing.T) {
	...
	err := RunErr(func(ctx *Context) error {
		var prov struct{ ProviderResourceState }
		require.NoError(t,
			ctx.RegisterResource("codeinfra:providers:aws", "myprovider", nil /* props */, &prov),
			"error registering provider")
		...
		NewResourceF: func(args MockResourceArgs) (string, resource.PropertyMap, error) {
			switch args.Name {
			case "myprovider":
				assert.Equal(t, "codeinfra:providers:aws", args.RegisterRPC.Type)
...
func TestComponentResourceMultipleSingletonProviders(t *testing.T) {
	...
	newTestProvider := func(ctx *Context, pkg, name string) ProviderResource {
		...
		require.NoError(t,
			ctx.RegisterResource("codeinfra:providers:"+pkg, name, nil /* props */, &prov),
			"error registering provider")
		return &prov
	}
...
func (dt *dependenciesTracker) Wrap(cl codeinfrarpc.ResourceMonitorClient) codeinfrarpc.ResourceMonitorClient {
	m := newInterceptingResourceMonitor(cl)
	m.afterRegisterResource = func(in *codeinfrarpc.RegisterResourceRequest,
		resp *codeinfrarpc.RegisterResourceResponse,
		err error,
	) {
		var deps []URN
...
type interceptingResourceMonitor struct {
	...
	afterRegisterResource func(req *codeinfrarpc.RegisterResourceRequest, resp *codeinfrarpc.RegisterResourceResponse, err error)
}

func newInterceptingResourceMonitor(inner codeinfrarpc.ResourceMonitorClient) *interceptingResourceMonitor {
...
func (i *interceptingResourceMonitor) RegisterResource(
	ctx context.Context,
	in *codeinfrarpc.RegisterResourceRequest,
