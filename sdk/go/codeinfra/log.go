...
// to the Codeinfra log stream.  These events will be printed in the terminal while the Codeinfra app
...
func (log *logState) Debug(msg string, args *LogArgs) error {
	return log._log(codeinfrarpc.LogSeverity_DEBUG, msg, args)
}
...
func (log *logState) Info(msg string, args *LogArgs) error {
	return log._log(codeinfrarpc.LogSeverity_INFO, msg, args)
}

// Warn logs a warning to indicate that something went wrong, but not catastrophically so.
func (log *logState) Warn(msg string, args *LogArgs) error {
	return log._log(codeinfrarpc.LogSeverity_WARNING, msg, args)
}

// Logs a fatal condition. Consider returning a non-nil error object
// after calling Error to stop the Codeinfra program.
func (log *logState) Error(msg string, args *LogArgs) error {
	return log._log(codeinfrarpc.LogSeverity_ERROR, msg, args)
}

func (log *logState) _log(severity codeinfrarpc.LogSeverity, message string, args *LogArgs) error {
	if log.join != nil {
		log.join.Add(1)
		defer log.join.Done()
	}
	...
	logRequest := &codeinfrarpc.LogRequest{
		Severity:  severity,
