...
func TestRegisterInputType(t *testing.T) {
	...
	assert.PanicsWithError(t, "expected codeinfra.Foo to implement internal.Input", func() {
		RegisterInputType(reflect.TypeOf((*Foo)(nil)).Elem(), FooArgs{})
	})
	assert.PanicsWithError(t, "expected codeinfra.FooArgs to implement interface codeinfra.FooInput", func() {
...
func TestJSONMarshalNested(t *testing.T) {
	...
	json := JSONMarshal(out)
	v, known, secret, deps, err := await(json)
	assert.EqualError(t, err, "json: error calling MarshalJSON for type codeinfra.AnyOutput: outputs can not be marshaled to JSON")
	assert.True(t, known)
	assert.False(t, secret)
	assert.Nil(t, deps)
	assert.Nil(t, v)
...
func TestDeferredOutputStruct(t *testing.T) {
	...
	require.Equal(t, internal.OutputPending, internal.GetOutputStatus(deferred))
	resolve(codeinfrax.Val[SomeInterface](SomeStruct{A: 42}))
	v, known, secret, deps, err := await(deferred)
	assert.NoError(t, err)
	assert.True(t, known)
	assert.False(t, secret)
...
func TestDeferredOutputNil(t *testing.T) {
	...
	deferred, resolve := DeferredOutput[*string](context.Background())
	require.Equal(t, internal.OutputPending, internal.GetOutputStatus(deferred))
	resolve(codeinfrax.Val[*string](nil))
	v, known, secret, deps, err := await(deferred)
	assert.NoError(t, err)
	assert.True(t, known)
	assert.False(t, secret)
