...
var ErrPlugins = errors.New("codeinfra: plugins requested")

// A RunOption is used to control the behavior of Run and RunErr.
type RunOption func(*RunInfo)

// Run executes the body of a Codeinfra program, granting it access to a deployment context that it may use
// to register resources and orchestrate deployment activities.  This connects back to the Codeinfra engine using gRPC.
...
// RunErr executes the body of a Codeinfra program, granting it access to a deployment context that it may use
// to register resources and orchestrate deployment activities.  This connects back to the Codeinfra engine using gRPC.
...
func runErrInner(body RunFunc, logError func(*Context, error), opts ...RunOption) error {
	...
	// boilerplate to a minimum in the average Codeinfra Go program.
	info := getEnvInfo()
	if info.getPlugins {
		return ErrPlugins
	}
...
// RunWithContext runs the body of a Codeinfra program using the given Context for information about the target stack,
...
func RunWithContext(ctx *Context, body RunFunc) error {
	...
	err := ctx.RegisterResource(
		"codeinfra:codeinfra:Stack", fmt.Sprintf("%s-%s", info.Project, info.Stack), nil, &stack)
	if err != nil {
		return err
	}
...
// RunFunc executes the body of a Codeinfra program.  It may register resources using the deployment context
// supplied as an arguent and any non-nil return value is interpreted as a program error by the Codeinfra runtime.
...
type RunInfo struct {
	...
	wrapResourceMonitorClient func(codeinfrarpc.ResourceMonitorClient) codeinfrarpc.ResourceMonitorClient
}
...
const (
	// EnvOrganization is the envvar used to read the current Codeinfra organization name.
	EnvOrganization = "CODEINFRA_ORGANIZATION"
	// EnvProject is the envvar used to read the current Codeinfra project name.
	EnvProject = "CODEINFRA_PROJECT"
	// EnvStack is the envvar used to read the current Codeinfra stack name.
	EnvStack = "CODEINFRA_STACK"
	// EnvConfig is the envvar used to read the current Codeinfra configuration variables.
	EnvConfig = "CODEINFRA_CONFIG"
	// EnvConfigSecretKeys is the envvar used to read the current Codeinfra configuration keys that are secrets.
	//nolint:gosec
	EnvConfigSecretKeys = "CODEINFRA_CONFIG_SECRET_KEYS"
	// EnvParallel is the envvar used to read the current Codeinfra degree of parallelism.
	EnvParallel = "CODEINFRA_PARALLEL"
	// EnvDryRun is the envvar used to read the current Codeinfra dry-run setting.
	EnvDryRun = "CODEINFRA_DRY_RUN"
	// EnvMonitor is the envvar used to read the current Codeinfra monitor RPC address.
	EnvMonitor = "CODEINFRA_MONITOR"
	// EnvEngine is the envvar used to read the current Codeinfra engine RPC address.
	EnvEngine = "CODEINFRA_ENGINE"
	// envPlugins is the envvar used to request that the Codeinfra program print its set of required plugins and exit.
	envPlugins = "CODEINFRA_PLUGINS"
)
