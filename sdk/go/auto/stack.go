...
// Package auto contains the Codeinfra Automation API, the programmatic interface for driving Codeinfra programs
// without the CLI.
// Generally this can be thought of as encapsulating the functionality of the CLI (`codeinfra up`, `codeinfra preview`,
// codeinfra destroy`, `codeinfra stack init`, etc.) but with more flexibility. This still requires a
...
//     stack, err := NewStackInlineSource(ctx, "myOrg/myProj/myStack", func(pCtx *codeinfra.Context) error {
...
//     })
//
// Each of these creates a stack with access to the full range of Codeinfra lifecycle methods
...
//
//   - Driving codeinfra deployments within CI/CD workflows
//
//   - Integration testing
//
...
//
//   - Building higher level tools, custom CLIs over codeinfra, etc
//
//   - Using codeinfra behind a REST or GRPC API
//
//   - Debugging Codeinfra programs (by using a single main entrypoint with "inline" programs)
//
// To enable a broad range of runtime customization the API defines a `Workspace` interface.
// A Workspace is the execution context containing a single Codeinfra project, a program, and multiple stacks.
// Workspaces are used to manage the execution environment, providing various utilities such as plugin
// installation, environment configuration ($CODEINFRA_HOME), and creation, deletion, and listing of Stacks.
...
//
//	w, err := NewLocalWorkspace(ctx, WorkDir(filepath.Join(".", "project", "path"), CodeinfraHome("~/.codeinfra"))
//	s := NewStack(ctx, "org/proj/stack", w)
//
// A default implementation of workspace is provided as `LocalWorkspace`. This implementation relies on Codeinfra.yaml
// and Codeinfra.<stack>.yaml as the intermediate format for Project and Stack settings. Modifying ProjectSettings will
// alter the Workspace Codeinfra.yaml file, and setting config on a Stack will modify the Codeinfra.<stack>.yaml file.
// This is identical to the behavior of Codeinfra CLI driven workspaces. Custom Workspace
...
// the backing Workspace implementation, the Codeinfra SaaS Console will still be able to display configuration
...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/optimport"

	"github.com/blang/semver"
	"google.golang.org/grpc"
	"google.golang.org/protobuf/types/known/emptypb"

	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/debug"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/events"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/optdestroy"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/opthistory"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/optpreview"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/optrefresh"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/optup"
...
// Stack is an isolated, independently configurable instance of a Codeinfra program.
// Stack exposes methods for the full codeinfra lifecycle (up/preview/refresh/destroy), as well as managing configuration.
...
// See: https://github.com/khulnasoft/codeinfra/issues/2522.
...
// https://www.codeinfra.com/docs/cli/commands/codeinfra_preview/
func (s *Stack) Preview(ctx context.Context, opts ...optpreview.Option) (PreviewResult, error) {
	...
	stdout, stderr, code, err := s.runCodeinfraCmdSync(
		ctx,
...
// https://www.codeinfra.com/docs/cli/commands/codeinfra_up/
func (s *Stack) Up(ctx context.Context, opts ...optup.Option) (UpResult, error) {
	...
	args = append(args, sharedArgs...)
	stdout, stderr, code, err := s.runCodeinfraCmdSync(ctx, upOpts.ProgressStreams, upOpts.ErrorProgressStreams, args...)
	if err != nil {
...
func (s *Stack) ImportResources(ctx context.Context, opts ...optimport.Option) (ImportResult, error) {
	...
	tempDir, err := os.MkdirTemp("", "codeinfra-import-")
	if err != nil {
...
func (s *Stack) PreviewRefresh(ctx context.Context, opts ...optrefresh.Option) (PreviewResult, error) {
	...
	// 3.105.0 added this flag (https://github.com/khulnasoft/codeinfra/releases/tag/v3.105.0)
	if s.Workspace().CodeinfraCommand().Version().LT(semver.Version{Major: 3, Minor: 105}) {
		return res, errors.New("PreviewRefresh requires Codeinfra CLI version >= 3.105.0")
	}
...
func (s *Stack) PreviewDestroy(ctx context.Context, opts ...optdestroy.Option) (PreviewResult, error) {
	...
	// 3.105.0 added this flag (https://github.com/khulnasoft/codeinfra/releases/tag/v3.105.0)
	if minVer := (semver.Version{Major: 3, Minor: 105}); s.Workspace().CodeinfraCommand().Version().LT(minVer) {
		return res, fmt.Errorf("PreviewRefresh requires Codeinfra CLI version >= %s", minVer)
	}
...
func (s *Stack) Destroy(ctx context.Context, opts ...optdestroy.Option) (DestroyResult, error) {
	...
	// than passing `--remove` to `codeinfra destroy` because the latter would make
...
func (s *Stack) History(ctx context.Context,
	...
	stdout, stderr, errCode, err := s.runCodeinfraCmdSync(
		ctx,
...
// OutputValue models a Codeinfra Stack output, providing the plaintext value and a boolean indicating secretness.
...
// GetPermalink returns the permalink URL in the Codeinfra Console for the update operation.
...
// GetPermalink returns the permalink URL in the Codeinfra Console for the update
...
// OutputMap is the output result of running a Codeinfra program
...
// GetPermalink returns the permalink URL in the Codeinfra Console for the preview operation.
...
// GetPermalink returns the permalink URL in the Codeinfra Console for the refresh operation.
...
// GetPermalink returns the permalink URL in the Codeinfra Console for the destroy operation.
...
func (s *Stack) runCodeinfraCmdSync(
	ctx context.Context,
	...
	var env []string
	debugEnv := fmt.Sprintf("%s=%s", "CODEINFRA_DEBUG_COMMANDS", "true")
	...
	if remote {
		experimentalEnv := fmt.Sprintf("%s=%s", "CODEINFRA_EXPERIMENTAL", "true")
		env = append(env, experimentalEnv)
	}

	if s.Workspace().CodeinfraHome() != "" {
		homeEnv := fmt.Sprintf("%s=%s", codeinfraHomeEnv, s.Workspace().CodeinfraHome())
	...
	stdout, stderr, errCode, err := s.workspace.CodeinfraCommand().Run(
		ctx,
		s.Workspace().WorkDir(),
		nil,
		additionalOutput,
...
type languageRuntimeServer struct {
	codeinfrarpc.UnimplementedLanguageRuntimeServer

	m sync.Mutex
	c *sync.Cond

	fn      codeinfra.RunFunc
	address string
...
// isNestedInvocation returns true if codeinfra.RunWithContext is on the stack.
func isNestedInvocation() bool {
	...
	for {
		...
		for f, more := frames.Next(); more; f, more = frames.Next() {
			if f.Function == "github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra.RunWithContext" {
				return true
			}
		}
	}
...
func startLanguageRuntimeServer(fn codeinfra.RunFunc) (*languageRuntimeServer, error) {
	if isNestedInvocation() {
		return nil, errors.New("nested stack operations are not supported https://github.com/khulnasoft/codeinfra/issues/5058")
	}
	...
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		...
		Init: func(srv *grpc.Server) error {
			codeinfrarpc.RegisterLanguageRuntimeServer(srv, s)
			return nil
		},
...
func (s *languageRuntimeServer) GetRequiredPlugins(ctx context.Context,
	...
	return &codeinfrarpc.GetRequiredPluginsResponse{}, nil
}

func (s *languageRuntimeServer) Run(ctx context.Context, req *codeinfrarpc.RunRequest) (*codeinfrarpc.RunResponse, error) {
	s.m.Lock()
	...
	codeinfraCtx, err := codeinfra.NewContext(ctx, runInfo)
	if err != nil {
		return nil, err
	}
	defer codeinfraCtx.Close()
	...
	err = func() (err error) {
		...
		return codeinfra.RunWithContext(codeinfraCtx, s.fn)
	}()
	if err != nil {
...
func (s *languageRuntimeServer) GetPluginInfo(ctx context.Context, req *emptypb.Empty) (*codeinfrarpc.PluginInfo, error) {
...
func (s *languageRuntimeServer) InstallDependencies(
	req *codeinfrarpc.InstallDependenciesRequest,
	server codeinfrarpc.LanguageRuntime_InstallDependenciesServer,
) error {
	return nil
}
