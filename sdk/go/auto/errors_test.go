...
func TestConcurrentUpdateError(t *testing.T) {
	...
	// TODO[codeinfra/codeinfra#8122] - investigate underlying sporadic 404 error
...
func TestCompilationErrorGo(t *testing.T) {
	...
	ctx := context.Background()
	sName := ptesting.RandomStackName()
	stackName := FullyQualifiedStackName(codeinfraOrg, compilationErrProj, sName)
...
func TestSelectStack404Error(t *testing.T) {
	...
	ctx := context.Background()
	sName := ptesting.RandomStackName()
	stackName := FullyQualifiedStackName(codeinfraOrg, "testproj", sName)
...
func TestRuntimeErrorGo(t *testing.T) {
	...
	ctx := context.Background()
	sName := ptesting.RandomStackName()
	stackName := FullyQualifiedStackName(codeinfraOrg, runtimeErrProj, sName)
...
func TestRuntimeErrorInlineGo(t *testing.T) {
	...
	// initialize
	s, err := NewStackInlineSource(ctx, stackName, runtimeErrProj, func(ctx *codeinfra.Context) error {
		panic("great sadness")
	})
	if err != nil {
		t.Errorf("failed to initialize stack, err: %v", err)
...
func TestRuntimeErrorPython(t *testing.T) {
	...
	err = pyCmd.Run()
	if err != nil {
		t.Errorf("failed to link venv against in-source codeinfra: %v", err)
		t.FailNow()
	}
