...
// CodeinfraCommand manages the Codeinfra CLI and runs operations.
type CodeinfraCommand interface {
...
type CodeinfraCommandOptions struct {
	...
	Root string
	// SkipVersionCheck is used to disable the validation of the found Codeinfra
	// binary.
...
// defaults to $HOME/.codeinfra/versions/$VERSION.
func (opts *CodeinfraCommandOptions) withDefaults() (*CodeinfraCommandOptions, error) {
	newOpts := &CodeinfraCommandOptions{
	...
	if newOpts.Root == "" {
		...
		newOpts.Root = filepath.Join(home, ".codeinfra", "versions", newOpts.Version.String())
	}

	return newOpts, nil
}

type codeinfraCommand struct {
	version semver.Version
	command string
}

// NewCodeinfraCommand creates a Codeinfra instance that uses the installation in
// `opts.Root`.  Defaults to using the Codeinfra binary found in $PATH if no
...
// variable `CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK` or setting
...
func NewCodeinfraCommand(opts *CodeinfraCommandOptions) (CodeinfraCommand, error) {
	if opts == nil {
		opts = &CodeinfraCommandOptions{}
	}
	command := "codeinfra"
	if opts.Root != "" {
		command = filepath.Join(opts.Root, "bin", "codeinfra")
	...
	out, err := cmd.Output()
	if err != nil {
		return codeinfraCommand{}, fmt.Errorf("failed to run `codeinfra version`: %w", err)
	}
	...
	version, err := parseAndValidateCodeinfraVersion(minimum, currentVersion, skipVersionCheck)
	if err != nil {
		return codeinfraCommand{}, err
	}

	return codeinfraCommand{
		version: version,
		command: command,
	}, nil
}

// InstallCodeinfraCommand downloads and installs the Codeinfra CLI. By default the
// CLI version matching the current SDK release is installed in
// $HOME/.codeinfra/versions/$VERSION. Set `opts.Root` to specify a different
// directory, and `opts.Version` to install a custom version.
func InstallCodeinfraCommand(ctx context.Context, opts *CodeinfraCommandOptions) (
	CodeinfraCommand,
	error,
) {
	if opts == nil {
	...
	return NewCodeinfraCommand(opts)
}
...
func installWindows(ctx context.Context, version semver.Version, root string) error {
	scriptPath, err := downloadToTmpFile(ctx, "https://get.codeinfra.com/install.ps1", "install-*.ps1")
	if err != nil {
		return err
	}
...
func installPosix(ctx context.Context, version semver.Version, root string) error {
	scriptPath, err := downloadToTmpFile(ctx, "https://get.codeinfra.com/install.sh", "install-*.sh")
	if err != nil {
		return err
	}
	defer os.Remove(scriptPath)
...
// Run executes a Codeinfra CLI command.
func (p codeinfraCommand) Run(ctx context.Context,
	workdir string,
	stdin io.Reader,
	additionalOutput []io.Writer,
	additionalErrorOutput []io.Writer,
	...
	if filepath.IsAbs(p.command) {
		codeinfraBin := filepath.Dir(p.command)
		env = fixupPath(env, codeinfraBin)
	}
	cmd.Env = env
...
// The version of the current Codeinfra CLI installation.
func (p codeinfraCommand) Version() semver.Version {
	return p.version
}
...
// codeinfraBin as the first path.
func fixupPath(env []string, codeinfraBin string) []string {
	...
	if pathIndex >= 0 {
		pathEntry := codeinfraBin
		oldPath := env[pathIndex][5:]
		if oldPath != "" {
			pathEntry = codeinfraBin + string(os.PathListSeparator) + oldPath
		}
		newEnv[pathIndex] = "PATH=" + pathEntry
	} else {
		newEnv = append(newEnv, "PATH="+codeinfraBin)
	}
	return newEnv
}

//nolint:lll
func parseAndValidateCodeinfraVersion(minVersion semver.Version, currentVersion string, optOut bool) (semver.Version, error) {
	version, err := semver.ParseTolerant(currentVersion)
	if err != nil && !optOut {
		return semver.Version{}, fmt.Errorf("Unable to parse Codeinfra CLI version (skip with %s=true): %w", env.SkipVersionCheck.Var().Name(), err)
	}
	if optOut {
		return version, nil
	}
	if minVersion.Major < version.Major {
		return semver.Version{}, fmt.Errorf("Major version mismatch. You are using Codeinfra CLI version %s with Automation SDK v%v. Please update the SDK.", currentVersion, minVersion.Major) //nolint
	}
	if minVersion.GT(version) {
		return semver.Version{}, fmt.Errorf("Minimum version requirement failed. The minimum CLI version requirement is %s, your current CLI version is %s. Please update the Codeinfra CLI.", minimumVersion, currentVersion) //nolint
	}
	return version, nil
}
