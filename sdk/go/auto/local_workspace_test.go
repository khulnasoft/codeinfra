...
import (
	...
	resourceConfig "github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource/config"
...
var codeinfraOrg = getTestOrg()

const (
	pName         = "testproj"
	agent         = "codeinfra/codeinfra/test"
	codeinfraTestOrg = "moolumi"
)

type mockCodeinfraCommand struct {
...
func (m *mockCodeinfraCommand) Version() semver.Version {
	return m.version
}

func (m *mockCodeinfraCommand) Run(ctx context.Context,
	workdir string,
	stdin io.Reader,
...
func TestWorkspaceSecretsProvider(t *testing.T) {
	...
	stackName := FullyQualifiedStackName(codeinfraOrg, pName, sName)

	mkstack := func(passphrase string) Stack {
		opts := []LocalWorkspaceOption{
			SecretsProvider("passphrase"),
			EnvVars(map[string]string{
				"CODEINFRA_CONFIG_PASSPHRASE": passphrase,
			}),
		}

		// initialize
		s, err := UpsertStackInlineSource(ctx, stackName, pName, func(ctx *codeinfra.Context) error {
			c := config.New(ctx, "")
	...
	defer func() {
		err := os.Unsetenv("CODEINFRA_CONFIG_PASSPHRASE")
...
func TestRemoveWithForce(t *testing.T) {
	...
	const permalinkSearchStr = "https://app.codeinfra.com"
...
func TestNewStackInlineSource(t *testing.T) {
	...
	// initialize
	s, err := NewStackInlineSource(ctx, stackName, pName, func(ctx *codeinfra.Context) error {
		c := config.New(ctx, "")
		ctx.Export("exp_static", codeinfra.String("foo"))
		ctx.Export("exp_cfg", codeinfra.String(c.Get("bar")))
		ctx.Export("exp_secret", c.GetSecret("buzz"))
	...
	// -- codeinfra refresh --preview-only --
	...
	// -- codeinfra destroy --preview-only --
...
func TestNestedStackFails(t *testing.T) {
	...
	sName := ptesting.RandomStackName()
	parentstackName := FullyQualifiedStackName(codeinfraOrg, "parent", sName)
	nestedstackName := FullyQualifiedStackName(codeinfraOrg, "nested", sName)

	nestedStack, err := NewStackInlineSource(testCtx, nestedstackName, "nested", func(ctx *codeinfra.Context) error {
		ctx.Export("exp_static", codeinfra.String("foo"))
		return nil
	})
	if err != nil {
	...
	// initialize
	s, err := NewStackInlineSource(testCtx, parentstackName, "parent", func(ctx *codeinfra.Context) error {
...
func TestGetAllConfigCorrectArgs(t *testing.T) {
	...
	workspace, err := NewLocalWorkspace(ctx, WorkDir(pDir), Codeinfra(&m))
	require.NoError(t, err)
...
// This test requires the existence of a Codeinfra.dev.yaml file because we are reading the nested
...
func TestNestedConfig(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	stackName := FullyQualifiedStackName(codeinfraOrg, "nested_config", "dev")
	...
	// make sure the config agrees with the config loaded by Codeinfra.
...
func TestEnvFunctions(t *testing.T) {
	if getTestOrg() != codeinfraTestOrg {
		t.Skip("Skipping test because the required environments are in the moolumi org.")
	}
	t.Parallel()

	ctx := context.Background()
	stackName := FullyQualifiedStackName(codeinfraOrg, pName, ptesting.RandomStackName())
...
func TestTagFunctions(t *testing.T) {
	...
	assert.Equal(t, pName, tags["codeinfra:project"])
...
func TestStackImportResources(t *testing.T) {
	...
	ctx := context.Background()
	sName := ptesting.RandomStackName()
	stackName := FullyQualifiedStackName(codeinfraOrg, "import", sName)
...
func TestSupportsStackOutputs(t *testing.T) {
	...
	s, err := NewStackInlineSource(ctx, stackName, pName, func(ctx *codeinfra.Context) error {
		...
		nestedObj := codeinfra.Map{
			"not_a_secret": codeinfra.String("foo"),
			"is_a_secret":  codeinfra.ToSecret("iamsecret"),
		}
		ctx.Export("exp_static", codeinfra.String("foo"))
		ctx.Export("exp_cfg", codeinfra.String(c.Get("bar")))
		ctx.Export("exp_secret", c.GetSecret("buzz"))
...
func TestCodeinfraVersion(t *testing.T) {
	t.Parallel()
	...
	version := ws.CodeinfraVersion()
	assert.NotEqual(t, "v0.0.0", version)
	assert.Regexp(t, `(\d+\.)(\d+\.)(\d+)(-.*)?`, version)
}

func TestCodeinfraCommand(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	codeinfraCommand, err := NewCodeinfraCommand(nil)
	require.NoError(t, err, "failed to create codeinfra command: %s", err)
	ws, err := NewLocalWorkspace(ctx, Codeinfra(codeinfraCommand))
...
func TestClIWithoutRemoteSupport(t *testing.T) {
	...
	// We inspect the output of `codeinfra preview --help` to determine if the
	// CLI supports remote operations. Set the output to `some output` to
	// simulate a CLI version without remote support.
	m := mockCodeinfraCommand{stdout: "some output"}

	_, err := NewLocalWorkspace(context.Background(), Codeinfra(&m), remote(true))
...
func TestByPassesRemoteCheck(t *testing.T) {
	...
	// simulate a CLI version without remote support.
	m := mockCodeinfraCommand{stdout: "some output"}
	envVars := map[string]string{"CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK": "true"}

	_, err := NewLocalWorkspace(context.Background(), Codeinfra(&m), EnvVars(envVars), remote(true))
...
func TestProjectSettingsRespected(t *testing.T) {
	...
	stack, err := NewStackInlineSource(ctx, stackName, badProjectName, func(ctx *codeinfra.Context) error {
		return nil
...
func TestSaveStackSettings(t *testing.T) {
	...
	opts := []LocalWorkspaceOption{
		SecretsProvider("passphrase"),
		EnvVars(map[string]string{
			"CODEINFRA_CONFIG_PASSPHRASE": "password",
		}),
	}
...
func TestListStacks(t *testing.T) {
	...
	m := mockCodeinfraCommand{
		stdout: `[{"name": "testorg1/testproj1/teststack1",
				   "current": false,
				   "url": "https://app.codeinfra.com/testorg1/testproj1/teststack1"},
				  {"name": "testorg1/testproj1/teststack2",
				   "current": false,
				   "url": "https://app.codeinfra.com/testorg1/testproj1/teststack2"}]`,
		stderr:   "",
		exitCode: 0,
		err:      nil,
	}
	...
	assert.Equal(t, "https://app.codeinfra.com/testorg1/testproj1/teststack1", stacks[0].URL)
	assert.Equal(t, "testorg1/testproj1/teststack2", stacks[1].Name)
	assert.Equal(t, false, stacks[1].Current)
	assert.Equal(t, "https://app.codeinfra.com/testorg1/testproj1/teststack2", stacks[1].URL)
}
...
func TestListAllStacks(t *testing.T) {
	...
	m := mockCodeinfraCommand{
		stdout: `[{"name": "testorg1/testproj1/teststack1",
				  ...
				  {"name": "testorg1/testproj2/teststack2",
				   "current": false,
				   "url": "https://app.codeinfra.com/testorg1/testproj2/teststack2"}]`,
		stderr:   "",
		exitCode: 0,
		err:      nil,
	}
	...
	assert.Equal(t, false, stacks[1].Current)
	assert.Equal(t, "https://app.codeinfra.com/testorg1/testproj2/teststack2", stacks[1].URL)
}
...
func TestInstallWithUseLanguageVersionTools(t *testing.T) {
	...
	require.ErrorContains(t, err, "UseLanguageVersionTools requires Codeinfra CLI version >= 3.130.0")
...
func BenchmarkBulkSetConfigMixed(b *testing.B) {
	ctx := context.Background()
	stackName := FullyQualifiedStackName(codeinfraOrg, "set_config_mixed", "dev")

	// initialize
	s, err := NewStackInlineSource(ctx, stackName, "set_config_mixed", func(ctx *codeinfra.Context) error { return nil })
	if err != nil {
...
func BenchmarkBulkSetConfigPlain(b *testing.B) {
	ctx := context.Background()
	stackName := FullyQualifiedStackName(codeinfraOrg, "set_config_plain", "dev")

	// initialize
	s, err := NewStackInlineSource(ctx, stackName, "set_config_plain", func(ctx *codeinfra.Context) error { return nil })
	if err != nil {
		b.Errorf("failed to initialize stack, err: %v", err)
		b.FailNow()
	}
...
func getTestOrg() string {
	testOrg := codeinfraTestOrg
	if _, set := os.LookupEnv("CODEINFRA_TEST_ORG"); set {
		testOrg = os.Getenv("CODEINFRA_TEST_ORG")
	}
	return testOrg
}
...
func NewMyResource(ctx *codeinfra.Context, name string, opts ...codeinfra.ResourceOption) (*MyResource, error) {
