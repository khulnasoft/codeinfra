...
func Example() {
	...
	stackA, err := NewStackInlineSource(ctx, stackName, projA, func(pCtx *codeinfra.Context) error {
		pCtx.Export("outputA", codeinfra.String("valueA"))
		return nil
	})
	if err != nil {
	...
	stackB, err := NewStackInlineSource(ctx, stackName, projB, func(pCtx *codeinfra.Context) error {
		// output a new value "valueA/valueB"
		pCtx.Export(
			"outputB",
			codeinfra.Sprintf(
				"%s/%s",
				codeinfra.String(aRes.Outputs["outputA"].Value.(string)), codeinfra.String("valueB"),
			),
		)
		return nil
	})
...
func ExampleIsUnexpectedEngineError() {
	...
	if err != nil && IsUnexpectedEngineError(err) {
		// oops, you've probably uncovered a bug in the codeinfra engine.
...
func ExampleGitRepo_privateKeyPath() {
	...
	repo := GitRepo{
		URL:         "git@github.com:codeinfra/test-repo.git",
		ProjectPath: "goproj",
		Auth: &GitAuth{
...
func ExampleLocalWorkspace() {
	...
	// install the codeinfra-aws plugin
...
func ExampleLocalWorkspace_GetAllConfig() {
	...
	// create a workspace from a local project
	w, _ := NewLocalWorkspace(ctx, WorkDir(filepath.Join(".", "program")))
	// get all config from the workspace for the stack (read from Codeinfra.stack.yaml)
...
func ExampleNewLocalWorkspace() {
	...
	// project settings file (Codeinfra.yaml) in this location for information about the Codeinfra program.
	wd := WorkDir(filepath.Join("..", "path", "to", "codeinfra", "project"))
	// CodeinfraHome customizes the location of $CODEINFRA_HOME where metadata is stored and plugins are installed.
	ph := CodeinfraHome(filepath.Join("~", ".codeinfra"))
...
func ExampleLocalWorkspace_ProjectSettings() {
	...
	if err != nil {
		// no Codeinfra.yaml was found, so create a default
...
func ExampleLocalWorkspace_RefreshConfig() {
	...
	// this overwrites config in codeinfra.stackA.yaml
...
func ExampleLocalWorkspace_SetProgram() {
	...
	// create a workspace from a local project
	w, _ := NewLocalWorkspace(ctx, WorkDir(filepath.Join(".", "program")))
	program := func(pCtx *codeinfra.Context) error {
		pCtx.Export("an output", codeinfra.String("an output value"))
		return nil
	}
...
func ExampleLocalWorkspace_StackSettings() {
	...
	if err != nil {
		// no codeinfra.stackA.yaml was found, so create a default
...
func ExampleNewStackInlineSource() {
	...
	stack, _ := NewStackInlineSource(ctx, stackName, projName, func(pCtx *codeinfra.Context) error {
		pCtx.Export("outputA", codeinfra.String("valueA"))
		return nil
	})
	// Stack.Up runs the inline codeinfra.RunFunc specified above.
	stack.Up(ctx)
	// we can update the Workspace program for subsequent updates if desired
	stack.Workspace().SetProgram(func(pCtx *codeinfra.Context) error {
		pCtx.Export("outputAA", codeinfra.String("valueAA"))
		return nil
	})
	stack.Up(ctx)
}
...
func ExampleUpsertStackInlineSource() {
	...
	// LocalWorkspace on behalf of the user.
	stack, _ := UpsertStackInlineSource(ctx, stackName, projName, func(pCtx *codeinfra.Context) error {
		pCtx.Export("outputA", codeinfra.String("valueA"))
		return nil
	})
	// Stack.Up runs the inline codeinfra.RunFunc specified above.
...
func ExampleSelectStackInlineSource() {
	...
	stack, _ := SelectStackInlineSource(ctx, stackName, projName, func(pCtx *codeinfra.Context) error {
		pCtx.Export("outputA", codeinfra.String("valueA"))
		return nil
	})
	// Stack.Up runs the inline codeinfra.RunFunc specified above.
...
func ExampleStack_Info() {
	...
	// url to view the Stack in the Codeinfra SaaS or other backend
...
func ExampleStack_RefreshConfig() {
	...
	stackName := FullyQualifiedStackName("org", "project", "stack")
	stack, _ := SelectStackLocalSource(ctx, stackName, filepath.Join(".", "program"))
	// get the last deployed config from stack and overwrite Codeinfra.stack.yaml
