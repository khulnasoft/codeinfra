...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/optlist"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto/optremove"
...
// A Workspace is the execution context containing a single Codeinfra project, a program,
...
// ($CODEINFRA_HOME), and creation, deletion, and listing of Stacks.
// LocalWorkspace relies on Codeinfra.yaml and Codeinfra.<stack>.yaml as the intermediate format
// for Project and Stack settings. Modifying ProjectSettings will
// alter the Workspace Codeinfra.yaml file, and setting config on a Stack will modify the Codeinfra.<stack>.yaml file.
// This is identical to the behavior of Codeinfra CLI driven workspaces.
type LocalWorkspace struct {
	workDir                       string
	codeinfraHome                    string
	program                       codeinfra.RunFunc
	...
	codeinfraCommand                 CodeinfraCommand
...
// LocalWorkspace reads settings from the Codeinfra.yaml in the workspace.
...
// LocalWorkspace writes this value to a Codeinfra.yaml file in Workspace.WorkDir().
func (l *LocalWorkspace) SaveProjectSettings(ctx context.Context, settings *workspace.Project) error {
	codeinfraYamlPath := filepath.Join(l.WorkDir(), "Codeinfra.yaml")
	return settings.Save(codeinfraYamlPath)
}

// StackSettings returns the settings object for the stack matching the specified stack name if any.
// LocalWorkspace reads this from a Codeinfra.<stack>.yaml file in Workspace.WorkDir().
func (l *LocalWorkspace) StackSettings(ctx context.Context, stackName string) (*workspace.ProjectStack, error) {
	...
	for _, ext := range settingsExtensions {
		stackPath := filepath.Join(l.WorkDir(), fmt.Sprintf("Codeinfra.%s%s", name, ext))
...
// LocalWorkspace writes this value to a Codeinfra.<stack>.yaml file in Workspace.WorkDir()
func (l *LocalWorkspace) SaveStackSettings(
	...
	name := getStackSettingsName(stackName)
	stackYamlPath := filepath.Join(l.WorkDir(), fmt.Sprintf("Codeinfra.%s.yaml", name))
...
// An extensibility point to perform workspace cleanup (CLI operations may create/modify a Codeinfra.stack.yaml)
// LocalWorkspace does not utilize this extensibility point.
...
func (l *LocalWorkspace) AddEnvironments(ctx context.Context, stackName string, envs ...string) error {
	// 3.95 added this command (https://github.com/khulnasoft/codeinfra/releases/tag/v3.95.0)
	if l.codeinfraCommand.Version().LT(semver.Version{Major: 3, Minor: 95}) {
		return errors.New("AddEnvironments requires Codeinfra CLI version >= 3.95.0")
	}
	args := []string{"config", "env", "add"}
	args = append(args, envs...)
	args = append(args, "--yes", "--stack", stackName)
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, args...)
	if err != nil {
...
func (l *LocalWorkspace) ListEnvironments(ctx context.Context, stackName string) ([]string, error) {
	// 3.99 added this command (https://github.com/khulnasoft/codeinfra/releases/tag/v3.99.0)
	if l.codeinfraCommand.Version().LT(semver.Version{Major: 3, Minor: 99}) {
		return nil, errors.New("ListEnvironments requires Codeinfra CLI version >= 3.99.0")
	}
...
func (l *LocalWorkspace) RemoveEnvironment(ctx context.Context, stackName string, env string) error {
	// 3.95 added this command (https://github.com/khulnasoft/codeinfra/releases/tag/v3.95.0)
	if l.codeinfraCommand.Version().LT(semver.Version{Major: 3, Minor: 95}) {
		return errors.New("RemoveEnvironments requires Codeinfra CLI version >= 3.95.0")
	}
...
// scoped to the current workspace. LocalWorkspace reads this config from the matching Codeinfra.stack.yaml file.
...
// LocalWorkspace reads this config from the matching Codeinfra.stack.yaml file.
...
// LocalWorkspace writes this value to the matching Codeinfra.<stack>.yaml file in Workspace.WorkDir().
...
// LocalWorkspace writes the config to the matching Codeinfra.<stack>.yaml file in Workspace.WorkDir().
...
// It will remove any matching values in the Codeinfra.<stack>.yaml file in Workspace.WorkDir().
...
// It will overwrite all configuration in the Codeinfra.<stack>.yaml file in Workspace.WorkDir().
func (l *LocalWorkspace) RefreshConfig(ctx context.Context, stackName string) (ConfigMap, error) {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "config", "refresh", "--force", "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) GetTag(ctx context.Context, stackName string, key string) (string, error) {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "stack", "tag", "get", key, "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) SetTag(ctx context.Context, stackName string, key string, value string) error {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "stack", "tag", "set", key, value, "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) RemoveTag(ctx context.Context, stackName string, key string) error {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "stack", "tag", "rm", key, "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) ListTags(ctx context.Context, stackName string) (map[string]string, error) {
	...
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "stack", "tag", "ls", "--json", "--stack", stackName)
	if err != nil {
...
// WorkDir returns the working directory to run Codeinfra CLI commands.
// LocalWorkspace expects that this directory contains a Codeinfra.yaml file.
// For "Inline" Codeinfra programs created from NewStackInlineSource, a Codeinfra.yaml
// is created on behalf of the user if none is specified.
func (l *LocalWorkspace) WorkDir() string {
	return l.workDir
}

// CodeinfraCommand returns the CodeinfraCommand instance that is used to execute commands.
func (l *LocalWorkspace) CodeinfraCommand() CodeinfraCommand {
	return l.codeinfraCommand
}

// CodeinfraHome returns the directory override for CLI metadata if set.
// This customizes the location of $CODEINFRA_HOME where metadata is stored and plugins are installed.
func (l *LocalWorkspace) CodeinfraHome() string {
	return l.codeinfraHome
}

// CodeinfraVersion returns the version of the underlying Codeinfra CLI/Engine.
func (l *LocalWorkspace) CodeinfraVersion() string {
	return l.codeinfraCommand.Version().String()
}

// WhoAmI returns the currently authenticated user
func (l *LocalWorkspace) WhoAmI(ctx context.Context) (string, error) {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "whoami")
	if err != nil {
...
// logged-in Codeinfra identity.
func (l *LocalWorkspace) WhoAmIDetails(ctx context.Context) (WhoAmIResult, error) {
	// 3.58 added the --json flag (https://github.com/khulnasoft/codeinfra/releases/tag/v3.58.0)
	if l.codeinfraCommand.Version().GTE(semver.Version{Major: 3, Minor: 58}) {
		var whoAmIDetailedInfo WhoAmIResult
		stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "whoami", "--json")
		if err != nil {
...
func (l *LocalWorkspace) ChangeStackSecretsProvider(
	...
	stdout, stderr, errCode, err := l.runCodeinfraInputCmdSync(ctx, reader, nil, nil, args...)
	if err != nil {
...
func (l *LocalWorkspace) SelectStack(ctx context.Context, stackName string) error {
	...
	// but we will ensure the stack exists by calling `codeinfra stack`.
	args := []string{"stack"}
	if !l.remote {
...
// This queries underlying backend and may return stacks not present in the Workspace (as Codeinfra.<stack>.yaml files).
...
func (l *LocalWorkspace) InstallPlugin(ctx context.Context, name string, version string) error {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "plugin", "install", "resource", name, version)
	if err != nil {
...
func (l *LocalWorkspace) InstallPluginFromServer(
	...
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(
...
func (l *LocalWorkspace) RemovePlugin(ctx context.Context, name string, version string) error {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "plugin", "rm", "resource", name, version, "--yes")
	if err != nil {
...
func (l *LocalWorkspace) ListPlugins(ctx context.Context) ([]workspace.PluginInfo, error) {
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "plugin", "ls", "--json")
	if err != nil {
...
// Program returns the program `codeinfra.RunFunc` to be used for Preview/Update if any.
// If none is specified, the stack will refer to ProjectSettings for this information.
func (l *LocalWorkspace) Program() codeinfra.RunFunc {
	return l.program
}

// SetProgram sets the program associated with the Workspace to the specified `codeinfra.RunFunc`.
func (l *LocalWorkspace) SetProgram(fn codeinfra.RunFunc) {
	l.program = fn
}
...
func (l *LocalWorkspace) ExportStack(ctx context.Context, stackName string) (apitype.UntypedDeployment, error) {
	...
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "stack", "export", "--show-secrets", "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) ImportStack(ctx context.Context, stackName string, state apitype.UntypedDeployment) error {
	...
	stdout, stderr, errCode, err := l.runCodeinfraCmdSync(ctx, "stack", "import", "--file", f.Name(), "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) StackOutputs(ctx context.Context, stackName string) (OutputMap, error) {
	// standard outputs
	outStdout, outStderr, code, err := l.runCodeinfraCmdSync(ctx, "stack", "output", "--json", "--stack", stackName)
	if err != nil {
		return nil, newAutoError(fmt.Errorf("could not get outputs: %w", err), outStdout, outStderr, code)
	}

	// secret outputs
	secretStdout, secretStderr, code, err := l.runCodeinfraCmdSync(ctx,
...
func (l *LocalWorkspace) Install(ctx context.Context, opts *InstallOptions) error {
	...
	if opts != nil && opts.UseLanguageVersionTools {
		// Codeinfra 3.130.0 introduced the `--use-language-version-tools` flag.
		if l.codeinfraCommand.Version().LT(semver.Version{Major: 3, Minor: 130}) {
			return errors.New("UseLanguageVersionTools requires Codeinfra CLI version >= 3.130.0")
		}
	...
	stdout, stderr, errCode, err := l.runCodeinfraInputCmdSync(ctx, nil, stdoutWriters, stderrWriters, args...)
	if err != nil {
...
func (l *LocalWorkspace) runCodeinfraInputCmdSync(
	ctx context.Context,
	stdin io.Reader,
	...
	var env []string
	if l.CodeinfraHome() != "" {
		homeEnv := fmt.Sprintf("%s=%s", codeinfraHomeEnv, l.CodeinfraHome())
	...
	return l.CodeinfraCommand().Run(ctx,
		l.WorkDir(),
		stdin,
		additionalOutputs,
...
func (l *LocalWorkspace) runCodeinfraCmdSync(
	ctx context.Context,
	args ...string,
) (string, string, int, error) {
	return l.runCodeinfraInputCmdSync(ctx, nil, nil, nil, args...)
}

// supportsCodeinfraCmdFlag runs a command with `--help` to see if the specified flag is found within the resulting
// output, in which case we assume the flag is supported.
func (l *LocalWorkspace) supportsCodeinfraCmdFlag(ctx context.Context, flag string, args ...string) (bool, error) {
	env := []string{
		"CODEINFRA_DEBUG_COMMANDS=true",
		"CODEINFRA_EXPERIMENTAL=true",
	}

	// Run the command with `--help`, and then we'll look for the flag in the output.
	stdout, _, _, err := l.CodeinfraCommand().Run(ctx, l.WorkDir(), nil, nil, nil, env, append(args, "--help")...)
	if err != nil {
		return false, err
	}
...
func NewLocalWorkspace(ctx context.Context, opts ...LocalWorkspaceOption) (Workspace, error) {
	...
	if lwOpts.WorkDir != "" {
		...
		dir, err := os.MkdirTemp("", "codeinfra_auto")
		if err != nil {
	...
	var codeinfraCommand CodeinfraCommand
	if lwOpts.CodeinfraCommand != nil {
		codeinfraCommand = lwOpts.CodeinfraCommand
	} else {
		p, err := NewCodeinfraCommand(&CodeinfraCommandOptions{SkipVersionCheck: optOut})
		if err != nil {
			return nil, err
		}
		codeinfraCommand = p
	...
	l := &LocalWorkspace{
		...
		program:                       program,
		codeinfraHome:                    lwOpts.CodeinfraHome,
	...
	if !optOut && l.remote {
		// See if `--remote` is present in `codeinfra preview --help`'s output.
		supportsRemote, err := l.supportsCodeinfraCmdFlag(ctx, "--remote", "preview")
		if err != nil {
			return nil, err
		}
		if !supportsRemote {
			return nil, errors.New("Codeinfra CLI does not support remote operations; please upgrade")
		}
	}
...
type localWorkspaceOptions struct {
	...
	WorkDir string
	// Program is the Codeinfra Program to execute. If none is supplied,
	// the program identified in $WORKDIR/Codeinfra.yaml will be used instead.
	Program codeinfra.RunFunc
	// CodeinfraHome overrides the metadata directory for codeinfra commands.
	// This customizes the location of $CODEINFRA_HOME where metadata is stored and plugins are installed.
	CodeinfraHome string
	// CodeinfraCommand is the CodeinfraCommand instance to use. If none is
	// supplied, the workspace will create an instance using the CodeinfraCommand
	// CLI found in $PATH.
	CodeinfraCommand CodeinfraCommand
	...
	// Repo is a git repo with a Codeinfra Project to clone into the WorkDir.
	Repo *GitRepo
	...
	Remote bool
	// Remote environment variables to be passed to the remote Codeinfra operation.
	RemoteEnvVars map[string]EnvVarValue
	// PreRunCommands is an optional list of arbitrary commands to run before the remote Codeinfra operation is invoked.
	PreRunCommands []string
	// RemoteSkipInstallDependencies sets whether to skip the default dependency installation step
	RemoteSkipInstallDependencies bool
	// RemoteExecutorImage is the image to use for the remote Codeinfra operation.
	RemoteExecutorImage *ExecutorImage
	// RemoteAgentPoolID is the agent pool (also called deployment runner pool) to use for the remote Codeinfra operation.
	RemoteAgentPoolID string
...
// See Workdir, Program, CodeinfraHome, Project, Stacks, and Repo for concrete options.
...
// GitRepo contains info to acquire and setup a Codeinfra program from a git repository.
type GitRepo struct {
	// URL to clone git repo
	URL string
	// Optional path relative to the repo root specifying location of the codeinfra program.
...
// Program is the Codeinfra Program to execute. If none is supplied,
// the program identified in $WORKDIR/Codeinfra.yaml will be used instead.
func Program(program codeinfra.RunFunc) LocalWorkspaceOption {
	return localWorkspaceOption(func(lo *localWorkspaceOptions) {
		lo.Program = program
	})
}
...
// CodeinfraHome overrides the metadata directory for codeinfra commands.
func CodeinfraHome(dir string) LocalWorkspaceOption {
	return localWorkspaceOption(func(lo *localWorkspaceOptions) {
		lo.CodeinfraHome = dir
	})
}
...
// supplied, the workspace will create an instance using the CodeinfraCommand
// CLI found in $PATH.
func Codeinfra(codeinfra CodeinfraCommand) LocalWorkspaceOption {
	return localWorkspaceOption(func(lo *localWorkspaceOptions) {
		lo.CodeinfraCommand = codeinfra
	})
}
...
// These values will be passed to the remote Codeinfra operation.
...
// any available Settings files (Codeinfra.yaml, Codeinfra.<stack>.yaml).
...
// Settings files (Codeinfra.yaml, Codeinfra.<stack>.yaml).
...
// any available Settings files (Codeinfra.yaml, Codeinfra.<stack>.yaml) that are cloned into the Workspace.
...
// any available Settings files (Codeinfra.yaml, Codeinfra.<stack>.yaml) that are cloned
...
// s, o/p/s, u/p/s o/s
// so just return the last chunk which is what will be used in codeinfra.<stack>.yaml
