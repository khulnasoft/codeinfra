...
// Input is a generic input for a Codeinfra resource or function.
//
...
// All such types MUST embed *codeinfra.OutputState.
//
// For example, OutputOf[int] is satisfied by codeinfrax.Output[int]
// as well as codeinfra.IntOutput.
...
// It encodes the relationship between resources in a Codeinfra application.
// The value may be unknown, a secret, and may have dependencies.
//
// Output[T] implements codeinfrax.Input[T], as well as codeinfra.Input.
...
// Untyped converts an Output[T] to a codeinfra.Output.
//
// The concrete type of the returned value will be the most specific known
// implementation of codeinfra.Output that matches the element type of the input,
// assuming it was registered with codeinfra.RegisterOutputType.
//
// Use this to call legacy APIs that expect a concrete codeinfra.Output.
// For example,
//
//	var o codeinfrax.Output[string] = // ...
//	legacyAPI(o.Untyped().(codeinfra.StringOutput))
...
// ConvertTyped builds an [Output] with the given untyped codeinfra.Output,
...
//
// The above is true for all output types generated by the Codeinfra SDK.
//
// As an example, you can use Cast to convert interchangeably between
// Output[[]string], ArrayOutput[string], and StringArrayOutput.
//
//	var o codeinfrax.Output[[]string] = // ...
//	ao := codeinfrax.Cast[codeinfrax.ArrayOutput[string]](o)
//	sao := codeinfrax.Cast[codeinfra.StringArrayOutput](o)
