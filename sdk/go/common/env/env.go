...
import "github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/env"
...
// That Codeinfra is running in experimental mode.
//
...
var SkipUpdateCheck = env.Bool("SKIP_UPDATE_CHECK", "Disable checking for a new version of codeinfra.")

var Dev = env.Bool("DEV", "Enable features for hacking on codeinfra itself.")

var SkipCheckpoints = env.Bool("SKIP_CHECKPOINTS", "Skip saving state checkpoints and only save "+
	"the final deployment. See #10668.")

var DebugCommands = env.Bool("DEBUG_COMMANDS", "List commands helpful for debugging codeinfra itself.")
...
var DebugGRPC = env.String("DEBUG_GRPC", `Enables debug tracing of Codeinfra gRPC internals.
...
// Environment variables which affect Codeinfra AI integrations
var (
	AIServiceEndpoint = env.String("AI_SERVICE_ENDPOINT", "Endpoint for Codeinfra AI service")
)
