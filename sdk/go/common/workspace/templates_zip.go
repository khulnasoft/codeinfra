...
var ErrCodeinfraCloudUnauthorized = errors.New("unauthorized")
...
func isCodeinfraHostResponse(resp *http.Response) bool {
	return resp.Header.Get("X-Codeinfra-Request-ID") != ""
}
...
func RetrieveZIPTemplateFolder(templateURL *url.URL, tempDir string, opts ...RequestOption) (string, error) {
	...
	if packageResponse.StatusCode == http.StatusUnauthorized && isCodeinfraHostResponse(packageResponse) {
		return "", fmt.Errorf("failed to download template from codeinfra host: %w", ErrCodeinfraCloudUnauthorized)
	}
	...
	hasCodeinfraYAML := false
	for _, file := range archive.File {
		if !file.FileInfo().IsDir() && file.FileInfo().Name() == "Codeinfra.yaml" {
			hasCodeinfraYAML = true
			break
		}
	}
	if !hasCodeinfraYAML {
		return "", errors.New("template does not contain a Codeinfra.yaml file")
	}
