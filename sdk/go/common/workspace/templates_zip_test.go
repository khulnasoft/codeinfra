...
func TestRetrieveZIPTemplates_FailsWhenCodeinfraYAMLIsMissing(t *testing.T) {
	// Arrange.
	cases := map[string][]string{
		"empty.zip":          {},
		"no-codeinfra-yaml.zip": {"foo", "bar/baz"},
	}
	...
	for path := range cases {
		...
		// Assert.
		assert.ErrorContains(t, err, "template does not contain a Codeinfra.yaml file")
	}
}

//nolint:paralleltest // uses shared server URL
func TestRetrieveZIPTemplates_SucceedsWhenCodeinfraYAMLIsPresent(t *testing.T) {
	// Arrange.
	cases := map[string][]string{
		"just-codeinfra-yaml.zip":                    {"Codeinfra.yaml"},
		"codeinfra-yaml-and-flat-files.zip":          {"Codeinfra.yaml", "foo"},
		"codeinfra-yaml-and-nested-files.zip":        {"Codeinfra.yaml", "bar/baz"},
		"codeinfra-yaml-and-mixture.zip":             {"Codeinfra.yaml", "foo", "bar/baz"},
		"codeinfra-yaml-at-top-level-and-nested.zip": {"Codeinfra.yaml", "foo", "bar/Codeinfra.yaml"},
	}
...
func TestRetrieveZIPTemplates_RaisesDetectableCodeinfraCloud401Error(t *testing.T) {
	t.Parallel()

	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		// How we id a Codeinfra Cloud from an arbitrary http response.
		rw.Header().Set("X-Codeinfra-Request-ID", "123")
	...
	// Make sure we can detect the error as a ErrCodeinfraCloudUnauthorized.
	// Allows us to handle it properly and retry w/ credentials etc if needed.
	assert.ErrorIs(t, err, ErrCodeinfraCloudUnauthorized)
}
