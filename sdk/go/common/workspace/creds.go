...
// CodeinfraCredentialsPathEnvVar is a path to the folder where credentials are stored.
...
// Credentials hold the information necessary for authenticating Codeinfra Cloud API requests.  It contains
...
// getCredsFilePath returns the path to the Codeinfra credentials file on disk, regardless of
// whether it exists or not.
func getCredsFilePath() (string, error) {
	// Allow the folder we use to store credentials to be overridden by tests
	codeinfraFolder := os.Getenv(CodeinfraCredentialsPathEnvVar)
	if codeinfraFolder == "" {
		folder, err := GetCodeinfraHomeDir()
		if err != nil {
			return "", fmt.Errorf("failed to get the home path: %w", err)
		}
		codeinfraFolder = folder
	}

	err := os.MkdirAll(codeinfraFolder, 0o700)
	if err != nil {
		return "", fmt.Errorf("failed to create '%s': %w", codeinfraFolder, err)
	}

	return filepath.Join(codeinfraFolder, "credentials.json"), nil
}
...
func GetStoredCredentials() (Credentials, error) {
	...
	if err = json.Unmarshal(c, &creds); err != nil {
		return Credentials{}, fmt.Errorf("failed to read Codeinfra credentials file. Please fix "+
...
func getConfigFilePath() (string, error) {
	...
	return filepath.Join(codeinfraFolder, "config.json"), nil
}

func GetCodeinfraConfig() (CodeinfraConfig, error) {
	configFile, err := getConfigFilePath()
	if err != nil {
		return CodeinfraConfig{}, err
	}

	c, err := os.ReadFile(configFile)
	if err != nil {
		if os.IsNotExist(err) {
			return CodeinfraConfig{}, nil
		}
		return CodeinfraConfig{}, fmt.Errorf("reading '%s': %w", configFile, err)
	}

	var config CodeinfraConfig
	if err = json.Unmarshal(c, &config); err != nil {
		return CodeinfraConfig{}, fmt.Errorf("failed to read Codeinfra config file: %w", err)
	}
...
func StoreCodeinfraConfig(config CodeinfraConfig) error {
	...
	// updated atomically to ensure concurrent `codeinfra` CLI operations are safe.
...
func SetBackendConfigDefaultOrg(backendURL, defaultOrg string) error {
	...
	return StoreCodeinfraConfig(config)
}
