...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/gitutil"
...
const (
	...
	// a project directory.
	legacyCodeinfraTemplateManifestFile = ".codeinfra.template.yaml"

	// codeinfraLocalTemplatePathEnvVar is a path to the folder where templates are stored.
	// It is used in sandboxed environments where the classic template folder may not be writable.
	codeinfraLocalTemplatePathEnvVar = "CODEINFRA_TEMPLATE_PATH"

	// codeinfraLocalPolicyTemplatePathEnvVar is a path to the folder where policy templates are stored.
	// It is used in sandboxed environments where the classic template folder may not be writable.
	codeinfraLocalPolicyTemplatePathEnvVar = "CODEINFRA_POLICY_TEMPLATE_PATH"
)

// These are variables instead of constants in order that they can be set using the `-X`
// `ldflag` at build time, if necessary.
var (
	// The Git URL for Codeinfra program templates
	codeinfraTemplateGitRepository = "https://github.com/pulumi/templates.git"
	// The branch name for the template repository
	codeinfraTemplateBranch = "master"
	// The Git URL for Codeinfra Policy Pack templates
	codeinfraPolicyTemplateGitRepository = "https://github.com/pulumi/templates-policy.git"
	// The branch name for the policy pack template repository
	codeinfraPolicyTemplateBranch = "master"
)
...
const (
	// TemplateKindCodeinfraProject is a template for a Codeinfra stack.
	TemplateKindCodeinfraProject TemplateKind = 0
...
func (repo TemplateRepository) Templates() ([]Template, error) {
	...
	// See if there's a Codeinfra.yaml in the directory.
...
func (repo TemplateRepository) PolicyTemplates() ([]PolicyPackTemplate, error) {
	...
	// See if there's a CodeinfraPolicy.yaml in the directory.
	...
	// Otherwise, read all subdirectories to find the ones
	// that contain a CodeinfraPolicy.yaml.
	infos, err := os.ReadDir(path)
	if err != nil {
		return nil, err
	}
...
type Template struct {
	Dir         string                                // The directory containing Codeinfra.yaml.
...
type PolicyPackTemplate struct {
	Dir         string // The directory containing CodeinfraPolicy.yaml.
	Name        string // The name of the template.
	Description string // Description of the template.
	Error       error  // Non-nil if the template is broken.
}
...
// cleanupLegacyTemplateDir deletes an existing ~/.codeinfra/templates directory if it isn't a git repository.
func cleanupLegacyTemplateDir(templateKind TemplateKind) error {
	...
	if templateKind == TemplateKindPolicyPack {
		url = codeinfraPolicyTemplateGitRepository
	} else {
		url = codeinfraTemplateGitRepository
	}
...
func RetrieveTemplates(ctx context.Context, templateNamePathOrURL string, offline bool,
	...
	return retrieveCodeinfraTemplates(ctx, templateNamePathOrURL, offline, templateKind)
}
...
func retrieveURLTemplates(
	...
	// Create a temp dir.
	var temp string
	if temp, err = os.MkdirTemp("", "codeinfra-template-"); err != nil {
...
// retrieveCodeinfraTemplates retrieves the "template repository" for Codeinfra templates.
// Instead of retrieving to a temporary directory, the Codeinfra templates are managed from
// ~/.codeinfra/templates.
func retrieveCodeinfraTemplates(
	...
	if !offline {
		// Clone or update the codeinfra/templates repo.
		repo := codeinfraTemplateGitRepository
		branch := plumbing.NewBranchReferenceName(codeinfraTemplateBranch)
		if templateKind == TemplateKindPolicyPack {
			repo = codeinfraPolicyTemplateGitRepository
			branch = plumbing.NewBranchReferenceName(codeinfraPolicyTemplateBranch)
		}
...
func LoadTemplate(path string) (Template, error) {
	...
	// TODO handle other extensions like Codeinfra.yml and Codeinfra.json?
	proj, err := LoadProject(filepath.Join(path, "Codeinfra.yaml"))
	if err != nil {
		return Template{}, err
	}
		...
		func(entry os.DirEntry, source string, dest string) error {
			...
			if !isBinary(b) {
				...
				if strings.HasSuffix(source, ".yaml") {
					// Use the sanitized project name and description for the Codeinfra.yaml file.
					name = yamlName
...
func LoadPolicyPackTemplate(path string) (PolicyPackTemplate, error) {
	...
	pack, err := LoadPolicyPack(filepath.Join(path, "CodeinfraPolicy.yaml"))
	if err != nil {
		return PolicyPackTemplate{}, err
	}
...
func GetTemplateDir(templateKind TemplateKind) (string, error) {
	envVar := codeinfraLocalTemplatePathEnvVar
	if templateKind == TemplateKindPolicyPack {
		envVar = codeinfraLocalPolicyTemplatePathEnvVar
	}
	...
	if templateKind == TemplateKindPolicyPack {
		return GetCodeinfraPath(TemplatePolicyDir)
	}

	// Use the classic template directory if there is no override.
	return GetCodeinfraPath(TemplateDir)
}
...
func walkFiles(sourceDir string, destDir string, projectName string,
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			...
			if name == legacyCodeinfraTemplateManifestFile {
				continue
			}
