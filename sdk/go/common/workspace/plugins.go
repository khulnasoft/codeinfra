...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/archive"
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/httputil"
...
var enableLegacyPluginBehavior = os.Getenv("CODEINFRA_ENABLE_LEGACY_PLUGIN_SEARCH") != ""
...
func (err *MissingError) Error() string {
	...
	if err.version != nil {
		return fmt.Sprintf("no %[1]s plugin 'codeinfra-%[1]s-%[2]s' found in the workspace at version v%[3]s%[4]s",
			err.kind, err.name, err.version, includePath)
	}

	return fmt.Sprintf("no %[1]s plugin 'codeinfra-%[1]s-%[2]s' found in the workspace%[3]s",
...
type PluginSource interface {
	...
	// schema option. Example: "github://api.github.com/khulnasoft/codeinfra-aws".
	URL() string
}
...
func standardAssetName(name string, kind apitype.PluginKind, version semver.Version, opSy, arch string) string {
	return fmt.Sprintf("codeinfra-%s-%s-v%s-%s-%s.tar.gz", kind, name, version, opSy, arch)
}

// getCodeinfraSource can download a plugin from get.codeinfra.com
type getCodeinfraSource struct {
	name string
	kind apitype.PluginKind
}

func newGetCodeinfraSource(name string, kind apitype.PluginKind) *getCodeinfraSource {
	return &getCodeinfraSource{name: name, kind: kind}
}

func (source *getCodeinfraSource) Download(
	ctx context.Context,
	version semver.Version, opSy string, arch string,
	getHTTPResponse func(*http.Request) (io.ReadCloser, int64, error),
) (io.ReadCloser, int64, error) {
	serverURL := "https://get.codeinfra.com/releases/plugins"
...
func newGithubSource(url *url.URL, name string, kind apitype.PluginKind) (*githubSource, error) {
	...
	repository := "codeinfra-" + name
	if kind == apitype.ConverterPlugin {
		// Converter plugins are expected at a different repo path, e.g.
		// github.com/khulnasoft/codeinfra-converter-aws rather than github.com/khulnasoft/codeinfra-aws which would clash
		// with the providers of the same name.
		repository = "codeinfra-converter-" + name
		if name == "yaml" {
			// We special case the yaml converter plugin to be in the codeinfra-yaml repo. It's not ideal but its
			...
			repository = "codeinfra-yaml"
		}
	}

	if kind == apitype.ToolPlugin {
		// Convention for tool plugins is to have them in a repository named "codeinfra-tool-<name>".
		if !strings.HasPrefix(name, "codeinfra-tool-") {
			// if it is not already fully qualified, we will prefix it with "codeinfra-tool-"
			repository = "codeinfra-tool-" + name
		}
	}
...
func (source *httpSource) Download(
	...
	endpoint := fmt.Sprintf("%s/%s",
		serverURL,
		url.QueryEscape(fmt.Sprintf("codeinfra-%s-%s-v%s-%s-%s.tar.gz", source.kind, source.name, version, opSy, arch)))
...
// fallbackSource handles our current default logic of trying the codeinfra public github then get.codeinfra.com.
...
func (source *fallbackSource) GetLatestVersion(
	...
	// Try and get this package from our public codeinfra github
	public, err := newGithubSource(urlMustParse("github://api.github.com/codeinfra"), source.name, source.kind)
	if err != nil {
		return nil, err
	}
...
func (source *fallbackSource) Download(
	...
	// Try and get this package from public codeinfra github
	public, err := newGithubSource(urlMustParse("github://api.github.com/codeinfra"), source.name, source.kind)
	if err != nil {
		return nil, -1, err
	}
	...
	logging.Infof("Failed to download from GitHub, falling back to get.codeinfra.com: %v", err)

	// Fallback to get.codeinfra.com
	codeinfra := newGetCodeinfraSource(source.name, source.kind)
	return codeinfra.Download(ctx, version, opSy, arch, getHTTPResponse)
}
...
type PluginSpec struct {
	...
	PluginDir         string             // if set, will be used as the root plugin dir instead of ~/.codeinfra/plugins.
...
func (spec PluginSpec) File() string {
	return fmt.Sprintf("codeinfra-%s-%s", spec.Kind, spec.Name)
}
...
// location, by default `~/.codeinfra/plugins/<kind>-<name>-<version>/`.  A plugin may contain multiple files,
// however the primary loadable executable must be named `codeinfra-<kind>-<name>`.
...
func (spec PluginSpec) GetSource() (PluginSource, error) {
	...
	if spec.PluginDownloadURL != "" {
		...
		// Use our default fallback behaviour of github then get.codeinfra.com
...
func buildHTTPRequest(ctx context.Context, pluginEndpoint string, authorization string) (*http.Request, error) {
	...
	userAgent := fmt.Sprintf("codeinfra-cli/1 (%s; %s)", version.Version, runtime.GOOS)
...
func (d *pluginDownloader) tryDownloadToFile(ctx context.Context, pkgPlugin PluginSpec) (string, error, error) {
	file, err := os.CreateTemp("" /* default temp dir */, "codeinfra-plugin-tar")
	if err != nil {
		return "", nil, err
	}
...
func (p singleFilePlugin) writeToDir(finalDir string) error {
	...
	finalPath := filepath.Join(finalDir, fmt.Sprintf("codeinfra-%s-%s", p.Kind, p.Name))
...
// format. Previous versions of Codeinfra extracted the tarball to a temp directory first, and then renamed the temp
...
func (spec PluginSpec) InstallWithContext(ctx context.Context, content PluginContent, reinstall bool) error {
	...
	// Cleanup any temp dirs from failed installations of this plugin from previous versions of Codeinfra.
	...
	proj, err := LoadPluginProject(filepath.Join(finalDir, "CodeinfraPlugin.yaml"))
	if err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("loading CodeinfraPlugin.yaml: %w", err)
	}
	if proj != nil {
		...
		// TODO[codeinfra/codeinfra#1334]: move to the language plugins so we don't have to hard code here.
		switch runtime {
		case "nodejs":
			var b bytes.Buffer
		...
		case "python":
			// TODO[codeinfra/codeinfra/issues/16287]: Support toolchain options for installing plugins.
...
// cleanupTempDirs cleans up leftover temp dirs from failed installs with previous versions of Codeinfra.
...
//
// Deprecated: IsPluginKind type was moved to "github.com/khulnasoft/codeinfra/sdk/v3/go/common/apitype"
...
func HasPluginGTE(spec PluginSpec) (bool, error) {
	...
	// search that we'd do at runtime. This ensures that `codeinfra plugin install` works the same way that the runtime
...
func GetPolicyDir(orgName string) (string, error) {
	return GetCodeinfraPath(PolicyDir, orgName)
}
...
func GetPolicyPath(orgName, name, version string) (string, bool, error) {
	...
	policyPackPath := path.Join(policiesDir, fmt.Sprintf("codeinfra-analyzer-%s-v%s", name, version))
...
func GetPluginDir() (string, error) {
	return GetCodeinfraPath(PluginDir)
}
...
// We currently bundle some plugins with "codeinfra" and thus expect them to be next to the codeinfra binary.
...
func IsPluginBundled(kind apitype.PluginKind, name string) bool {
	return (kind == apitype.LanguagePlugin && name == "nodejs") ||
		...
		(kind == apitype.ResourcePlugin && name == "codeinfra-nodejs") ||
		(kind == apitype.ResourcePlugin && name == "codeinfra-python") ||
...
// possible to opt out of this behavior by setting CODEINFRA_IGNORE_AMBIENT_PLUGINS to any non-empty value.
...
//   - if found in the codeinfra dir's installed plugins, a PluginInfo and path to the executable
...
func getPluginInfoAndPath(
	...
	// plugins (see codeinfra/codeinfra#956 for some of the reasons why this isn't the case today). For now, they
	// ship next to the `codeinfra` binary. While we encourage this folder to be on the $PATH (and so the check
	// above would have normally found the plugin) it's possible someone is running `codeinfra` with an explicit
	// path on the command line or has done symlink magic such that `codeinfra` is on the path, but the bundled
	// plugins are not, or has simply set IGNORE_AMBIENT_PLUGINS. So, if possible, look next to the instance
	// of `codeinfra` that is running to find this bundled plugin.
	...
	if ambientPath != "" {
		if ambientPath != bundledPath {
			// They don't match _but_ it might be they just don't match because the codeinfra install is symlinked,
			// e.g. /opt/homebrew/bin/codeinfra-language-nodejs -> /opt/homebrew/Cellar/codeinfra/3.77.0/bin/codeinfra-language-nodejs
...
// pluginRegexp matches plugin directory names: codeinfra-KIND-NAME-VERSION.
var pluginRegexp = regexp.MustCompile(
	"^(?P<Kind>[a-z]+)-" + // KIND
		"(?P<Name>[a-zA-Z0-9-]*[a-zA-Z0-9])-" + // NAME
		"v(?P<Version>.*)$") // VERSION

// installingPluginRegexp matches the name of temporary folders. Previous versions of Codeinfra first extracted
