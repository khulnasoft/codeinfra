...
//nolint:paralleltest // uses shared state in codeinfra dir
func TestRetrieveNonExistingTemplate(t *testing.T) {
	tests := []struct {
		...
		{
			testName:     "TemplateKindCodeinfraProject",
			templateKind: TemplateKindCodeinfraProject,
		},
		{
	...
	templateName := "not-the-template-that-exists-in-codeinfra-repo-nor-on-disk"
...
func TestRetrieveStandardTemplate(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.testName, func(t *testing.T) {
			...
			// Root should point to Codeinfra templates directory
			// (e.g. ~/.codeinfra/templates or ~/.codeinfra/templates-policy)
...
func TestRetrieveHttpsTemplate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			testName:        "TemplateKindCodeinfraProject",
			templateKind:    TemplateKindCodeinfraProject,
			templateURL:     "https://github.com/khulnasoft/codeinfra/tree/test-examples/examples/minimal",
			yamlFile:        "Codeinfra.yaml",
			expectedSubPath: []string{"examples", "minimal"},
		},
		{
			testName:        "TemplateKindPolicyPack",
			templateKind:    TemplateKindPolicyPack,
			templateURL:     "https://github.com/khulnasoft/codeinfra/tree/test-examples/examples/policy-packs/aws-ts-advanced",
			yamlFile:        "CodeinfraPolicy.yaml",
...
func TestRetrieveHttpsTemplateOffline(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			testName:     "TemplateKindCodeinfraProject",
			templateKind: TemplateKindCodeinfraProject,
			templateURL:  "https://github.com/khulnasoft/codeinfra-aws/tree/master/examples/minimal",
		},
		{
			testName:     "TemplateKindPolicyPack",
			templateKind: TemplateKindPolicyPack,
...
func TestCopyTemplateFiles(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			files:    []string{"main.go", "Codeinfra.yaml", "Codeinfra.dev.yaml"},
		},
		{
			testName:    "NestedProject",
			directories: []string{"src"},
			files:       []string{"src/main.go", "Codeinfra.yaml", "Codeinfra.dev.yaml"},
		},
	}
	...
	t.Run("Overwrite file over empty directory: force=false", func(t *testing.T) {
		...
		// change the Codeinfra.dev.yaml file in the destination dir to a dir
		err = os.RemoveAll(copyDestDir + "/Codeinfra.dev.yaml")
		assert.NoError(t, err)

		err = os.Mkdir(copyDestDir+"/Codeinfra.dev.yaml", 0o700)
		assert.NoError(t, err)
	...
	t.Run("Overwrite file over non-empty directory: force=true", func(t *testing.T) {
		...
		err = os.WriteFile(copyDestDir+"/Codeinfra.dev.yaml/README.md", []byte("testing"), 0o600)
		assert.NoError(t, err)
