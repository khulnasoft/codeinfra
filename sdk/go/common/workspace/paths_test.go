...
func TestDetectProjectAndPath(t *testing.T) {
	...
	yamlPath := filepath.Join(tmpDir, "Codeinfra.yaml")
...
func TestProjectStackPath(t *testing.T) {
	...
	tests := []struct {
		...
		"name: some_project\ndescription: Some project\nruntime: nodejs\n",
		expectedPath("Codeinfra.my_stack.yaml"),
	}, {
		"WithStackConfigDir",
		"name: some_project\ndescription: Some project\nruntime: nodejs\nstackConfigDir: stacks\n",
		expectedPath(filepath.Join("stacks", "Codeinfra.my_stack.yaml")),
	}, {
		"WithConfig",
		"name: some_project\ndescription: Some project\nruntime: nodejs\nconfig: stacks\n",
		expectedPath(filepath.Join("stacks", "Codeinfra.my_stack.yaml")),
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err = os.WriteFile(
				filepath.Join(tmpDir, "Codeinfra.yaml"),
...
func TestDetectProjectUnreadableParent(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/12481
...
runtime: nodejs`
	...
	yamlDeployPath := filepath.Join(tmpDir, "Codeinfra.stack.deploy.yaml")
	yamlDeployContents := ""
...
func TestDetectPolicyPackPathAt(t *testing.T) {
	...
	policyPackPath := filepath.Join(tmpDir, "CodeinfraPolicy.yaml")
	...
	policyPackPath = filepath.Join(tmpDir, "CodeinfraPolicy.json")
