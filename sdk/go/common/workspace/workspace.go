...
// W offers functionality for interacting with Codeinfra workspaces.
type W interface {
...
type projectWorkspace struct {
	...
	project  string             // the path to the Codeinfra.[yaml|json] file for this project.
...
// NewFrom creates a new Codeinfra workspace in the given directory. Requires a Codeinfra.yaml file be present in the
// folder hierarchy between dir and the .codeinfra folder.
func NewFrom(dir string) (W, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("no Codeinfra.yaml project file found (searching upwards from %s). If you have not "+
			"created a project yet, use `codeinfra new` to do so", dir)
	}
...
func (pw *projectWorkspace) Save() error {
	...
	// under ~/.codeinfra/workspaces, cleaning them out when possible prevents us from littering a bunch of files in the
	// home directory.
...
func (pw *projectWorkspace) settingsPath() string {
	...
	path, err := GetCodeinfraPath(WorkspaceDir, uniqueFileName)
