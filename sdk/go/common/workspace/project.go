...
//
// Use the same URL format that is passed to "codeinfra login", see
// https://www.codeinfra.com/docs/cli/commands/codeinfra_login/
//
...
type ProjectOptions struct {
	// Refresh is the ability to always run a refresh as part of a codeinfra update / preview / destroy
...
type Project struct {
	...
	// StackConfigDir indicates where to store the Codeinfra.<stack-name>.yaml files, combined with the folder
	// Codeinfra.yaml is in.
...
func (proj *Project) AddConfigStackTags(tags map[string]string) {
	...
	configTags, has := proj.Config["codeinfra:tags"]
	if !has {
	...
	if !ok {
		logging.Warningf("overwriting non-object `%s` project config", "codeinfra:tags")
		tagMap = map[string]string{}
	}
	for k, v := range tags {
		tagMap[k] = v
	}
	proj.Config["codeinfra:tags"] = configTags
}
