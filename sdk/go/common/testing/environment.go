...
const (
	//nolint:gosec
	codeinfraCredentialsPathEnvVar = "CODEINFRA_CREDENTIALS_PATH"
)
...
type Environment struct {
	*testing.T
	...
	// HomePath is the CODEINFRA_HOME directory for the environment
	HomePath string
	...
	Passphrase string
	// Set to true to turn off setting CODEINFRA_CONFIG_PASSPHRASE.
	NoPassphrase bool
	// Set to true to use the local Codeinfra dev build from ~/.codeinfra-dev/bin/codeinfra which get from `make install`
	UseLocalCodeinfraBuild bool
...
func NewEnvironment(t *testing.T) *Environment {
	...
	// We always use a clean CODEINFRA_HOME for each environment to avoid any potential conflicts with plugins or config.
...
func (e *Environment) RunCommand(cmd string, args ...string) (string, string) {
	...
	if e.UseLocalCodeinfraBuild {
		...
		if home != "" {
			cmd = filepath.Join(home, ".codeinfra-dev", "bin", "codeinfra")
		}
	}
...
func (e *Environment) SetupCommandIn(dir string, command string, args ...string) *exec.Cmd {
	...
	cmd.Env = os.Environ()
	cmd.Env = append(cmd.Env, fmt.Sprintf("%s=%s", codeinfraCredentialsPathEnvVar, e.RootPath))
	cmd.Env = append(cmd.Env, "CODEINFRA_DEBUG_COMMANDS=true")
	cmd.Env = append(cmd.Env, "CODEINFRA_HOME="+e.HomePath)
	if !e.NoPassphrase {
		cmd.Env = append(cmd.Env, "CODEINFRA_CONFIG_PASSPHRASE="+passphrase)
	}
	if e.Backend != "" {
		cmd.Env = append(cmd.Env, "CODEINFRA_BACKEND_URL="+e.Backend)
	}
	// According to https://pkg.go.dev/os/exec#Cmd.Env:
	//     If Env contains duplicate environment keys, only the last
	//     value in the slice for each duplicate key is used.
