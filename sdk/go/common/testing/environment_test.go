...
func TestEnvOverrideGetCommandResults(t *testing.T) {
	...
	checkDebug := func(expect string) {
		stdout, stderr, err := e.GetCommandResults("bash", "-c", "echo $CODEINFRA_DEBUG_COMMANDS")
		stdout = strings.TrimSuffix(stdout, "\n")
		assert.NoError(t, err)
		assert.Empty(t, stderr)
		assert.Equal(t, expect, stdout)
	}
	// We default CODEINFRA_DEBUG_COMMANDS to true
	checkDebug("true")
	// We can override the default
	e.SetEnvVars("CODEINFRA_DEBUG_COMMANDS=false")
	checkDebug("false")
}
