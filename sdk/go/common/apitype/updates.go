...
// UpdateProgramRequest is the request type for updating (aka deploying) a Codeinfra program.
...
// UpdateEvent describes an event that happened on the Codeinfra Cloud while processing an update.
...
// UpdateProgram describes the metadata associated with an update's Codeinfra program. Note that this does not
...
type UpdateProgram struct {
	...
	// Main is an optional redirect for the main program location. (e.g. a subfolder under Codeinfra.yaml
...
type RenewUpdateLeaseRequest struct {
	...
	// DEPRECATED as of Codeinfra API version 5+. Codeinfra API will expect the update token
