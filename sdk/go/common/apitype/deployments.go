...
// CodeinfraOperation describes what operation to perform on the
// stack as defined in the Job spec.
type CodeinfraOperation string

// The possible operations we can deploy.
const (
	Update         CodeinfraOperation = "update"
	Preview        CodeinfraOperation = "preview"
	Destroy        CodeinfraOperation = "destroy"
	Refresh        CodeinfraOperation = "refresh"
	DetectDrift    CodeinfraOperation = "detect-drift"
	RemediateDrift CodeinfraOperation = "remediate-drift"
)

func ParseCodeinfraOperation(o string) (CodeinfraOperation, error) {
	switch o {
	case "update":
		return Update, nil
	case "preview":
	...
	default:
		return "", fmt.Errorf("invalid codeinfra operation; %q", o)
	}
}

// DeploymentDuration, wrapper over time.Duration to properly marshall
// time durations according to codeinfra cloud spec.
...
type CreateDeploymentRequest struct {
	// Op
	Op CodeinfraOperation `json:"operation"`
	...
	// Source defines how the source code to the Codeinfra program will be gathered.
	Source *SourceContext `json:"sourceContext,omitempty"`

	// Operation defines the options that the executor will use to run the Codeinfra commands.
...
type ExecutorContext struct {
	...
	// Defines the image that the codeinfra operations should run in.
...
type SourceContextGit struct {
	...
	// where Codeinfra.yaml is located. It is used in case Codeinfra.yaml is not
...
type OperationContext struct {
	...
	// PreRunCommands is an optional list of arbitrary commands to run before Codeinfra
	// is invoked.
	// ref: https://github.com/khulnasoft/codeinfra/issues/9397
	PreRunCommands []string `json:"preRunCommands" yaml:"preRunCommands,omitempty"`

	// Operation is what we plan on doing.
	Operation CodeinfraOperation `json:"operation" yaml:"-"`
