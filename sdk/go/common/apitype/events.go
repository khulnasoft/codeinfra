...
// package. The duplication is intentional to insulate the Codeinfra service from various kinds of
// breaking changes.
//
...
// from the codeinfra CLI itself. Less common that DiagnosticEvent
...
// StepEventMetadata describes a "step" within the Codeinfra engine, which is any concrete action
...
type StepEventMetadata struct {
	...
	// Omitted from the type sent to the Codeinfra Service is "Res", which may be either Old or New.
	...
	// a nil value and an empty map. See https://github.com/khulnasoft/codeinfra/pull/15213 for details.
...
// EngineEvent describes a Codeinfra engine event, such as a change to a resource or diagnostic
...
type EngineEvent struct {
	...
	// Codeinfra Service. Since events may be sent concurrently, and/or delayed via network routing,
