...
const DeploymentSchemaID = "https://github.com/khulnasoft/codeinfra/blob/master/sdk/go/common/apitype/deployments.json"
...
const ResourceSchemaID = "https://github.com/khulnasoft/codeinfra/blob/master/sdk/go/common/apitype/resources.json"
...
const PropertyValueSchemaID = "https://github.com/khulnasoft/codeinfra/blob/master/sdk/go/common/apitype/property-values.json"
...
// and invocation of the Codeinfra engine that caused it.
type SnapshotIntegrityErrorMetadataV1 struct {
	// The version of the Codeinfra engine that caused the integrity error.
	Version string `json:"version,omitempty" yaml:"version,omitempty"`
	// The command/invocation of the Codeinfra engine that caused the integrity error.
...
type UntypedDeployment struct {
	...
	// The opaque Codeinfra deployment. This is conceptually of type `Deployment`, but we use `json.Message` to
...
// ResourceV1 describes a Cloud resource constructed by Codeinfra.
...
//     resources that Codeinfra does not own in the deployment.
...
//     implies that it is owned by Codeinfra. Note that since this is the default value for
...
type ResourceV2 struct {
	...
	// Protect is set to true when this resource is "protected" and may not be deleted.
	Protect bool `json:"protect,omitempty" yaml:"protect,omitempty"`
	// External is set to true when the lifecycle of this resource is not managed by Codeinfra.
...
type ResourceV3 struct {
	...
	// If set to True, the providers Delete method will not be called for this resource. Codeinfra simply stops tracking the deleted resource.
	...
	// Created tracks when the remote resource was first added to state by codeinfra. Checkpoints prior to early 2023 do not include this.
...
type ManifestV1 struct {
	...
	// Version of the Codeinfra engine used to render the checkpoint.
...
const (
	// ProjectNameTag is a tag that represents the name of a project (coresponds to the `name` property of Codeinfra.yaml).
	ProjectNameTag StackTagName = "codeinfra:project"
	// ProjectRuntimeTag is a tag that represents the runtime of a project (the `runtime` property of Codeinfra.yaml).
	ProjectRuntimeTag StackTagName = "codeinfra:runtime"
	// ProjectDescriptionTag is a tag that represents the description of a project (Codeinfra.yaml's `description`).
	ProjectDescriptionTag StackTagName = "codeinfra:description"
	// ProjectTemplateTag is a tag that represents the template that was used to create a project.
	ProjectTemplateTag StackTagName = "codeinfra:template"
	...
	// TODO [codeinfra/codeinfra-service#2306] Once the UI is updated, we would no longer need the GitHub specific keys.
...
const (
	// CodeinfraTagsConfigKey sets additional tags for a stack on a deployment. This is additive to any
	// tags that are already set on the stack.
	CodeinfraTagsConfigKey string = "codeinfra:tags"
)

// Stack describes a Stack running on a Codeinfra Cloud.
type Stack struct {
...
// OperationStatus describes the state of an operation being performed on a Codeinfra stack.
