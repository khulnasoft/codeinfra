...
func TestParseGitRepoURL(t *testing.T) {
	...
	test(exp, "poc/waka-waka", pre+"/")
	pre = "https://gitlab.com/codeinfra/platform/templates.git/templates/javascript"
	exp = "https://gitlab.com/codeinfra/platform/templates.git"
	...
	// No repo.
	testError("https://github.com/codeinfra", "invalid Git URL")
	testError("https://github.com/codeinfra/", "invalid Git URL; no repository")
	testError("git@github.com:codeinfra", "invalid Git URL")
	testError("git@github.com:codeinfra/", "invalid Git URL; no repository")
	testError("ssh://git@github.com/codeinfra", "invalid Git URL")
	testError("ssh://git@github.com/codeinfra/", "invalid Git URL; no repository")
...
func TestParseAuthURL(t *testing.T) {
	...
	t.Run("with passphrase-protected key and environment variable", func(t *testing.T) {
		...
		_, auth, err := parser.Parse("git@github.com:codeinfra/templates.git")
		assert.NoError(t, err)
	...
	t.Run("Don't cache on error", func(t *testing.T) {
		// Regression test for https://github.com/khulnasoft/codeinfra/issues/16637
		t.Setenv(env.GitSSHPassphrase.Var().Name(), "incorrect passphrase")
		t.Setenv("SSH_AUTH_SOCK", "")
		os.Unsetenv("SSH_AUTH_SOCK") // t.Setenv above will restore it after the test
