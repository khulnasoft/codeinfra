...
func TestDetectVars(t *testing.T) {
	...
	systemAndEnvVars := map[SystemName]map[string]string{
		// Since the `codeinfra/codeinfra` repo runs on Travis,
		...
		GenericCI: {
			"TRAVIS":             "",
			"CODEINFRA_CI_SYSTEM":   "generic-ci-system",
			"CODEINFRA_CI_BUILD_ID": buildNumber,
			"GITHUB_ACTIONS":     "",
		},
		GitLab: {
			"TRAVIS":          "",
...
func TestDetectVarsDisableCIDetection(t *testing.T) {
	t.Setenv("CODEINFRA_DISABLE_CI_DETECTION", "nonEmptyString")
