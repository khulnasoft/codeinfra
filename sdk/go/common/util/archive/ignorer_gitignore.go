...
func newGitIgnoreIgnorer(pathToCodeinfraIgnore string) (ignorer, error) {
	gitIgnorer, err := ignore.CompileIgnoreFile(pathToCodeinfraIgnore)
	if err != nil {
		return nil, err
	}

	return &gitIngoreIgnorer{root: path.Dir(pathToCodeinfraIgnore), ignorer: gitIgnorer}, nil
}
