...
// Returns a Codeinfra-formatted tracing endpoint pointing to this
// server.
//
...
func rootSpanTags() []opentracing.Tag {
	...
	// Promote all env vars `codeinfra_tracing_tag_foo=bar` into tags `foo: bar`.
	envPrefix := "codeinfra_tracing_tag_"
...
// `CODEINFRA_TRACING_MEMSTATS_POLL_INTERVAL=1s` or similar. Consider
...
func collectMemStats(spanPrefix string) {
	...
	interval := os.Getenv("CODEINFRA_TRACING_MEMSTATS_POLL_INTERVAL")
