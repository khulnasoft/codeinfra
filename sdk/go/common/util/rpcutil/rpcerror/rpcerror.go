...
func WrapDetailedError(err error) error {
	...
	if errors.As(err, &iperr) {
		...
		errorDetails := codeinfrarpc.InputPropertiesError{}
		for _, e := range iperr.Errors {
			errorDetails.Errors = append(errorDetails.Errors, &codeinfrarpc.InputPropertiesError_PropertyError{
...
func FromError(err error) (*Error, bool) {
	...
	for _, details := range status.Details() {
		if d, ok := details.(*codeinfrarpc.InputPropertiesError); ok {
		...
		if errorCause, ok := details.(*codeinfrarpc.ErrorCause); ok {
...
func serializeErrorCause(err error) *codeinfrarpc.ErrorCause {
	...
	return &codeinfrarpc.ErrorCause{
		Message:    message,
