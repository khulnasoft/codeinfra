...
type bagtag struct {
	String        string                 `codeinfra:"s"`
	StringSkip    string                 `codeinfra:"sc,skip"`
	StringOpt     string                 `codeinfra:"so,optional"`
	StringSkipOpt string                 `codeinfra:"sco,skip,optional"`
	MapOpt        map[string]interface{} `codeinfra:"mo,optional"`
}
...
type bog struct {
	Boggy    bogger     `codeinfra:"boggy"`
	BoggyP   *bogger    `codeinfra:"boggyp"`
	Boggers  []bogger   `codeinfra:"boggers"`
	BoggersP *[]*bogger `codeinfra:"boggersp"`
}

type bogger struct {
	Num float64 `codeinfra:"num"`
}
...
type boggerdybogger struct {
	Bogs  map[string]bog   `codeinfra:"bogs"`
	BogsP *map[string]*bog `codeinfra:"bogsp"`
}
...
type hasmap struct {
	Entries  map[string]mapentry  `codeinfra:"entries"`
	EntriesP map[string]*mapentry `codeinfra:"entriesp"`
}

type mapentry struct {
	Title string `codeinfra:"title"`
}
...
type wrap struct {
	C  customStruct    `codeinfra:"c"`
	CI customInterface `codeinfra:"ci"`
}
...
type customStruct struct {
	X float64 `codeinfra:"x"`
	Y float64 `codeinfra:"y"`
}
...
type outer struct {
	Inners *[]inner `codeinfra:"inners,optional"`
}

type inner struct {
	A string   `codeinfra:"a"`
	B *string  `codeinfra:"b,optional"`
	C *string  `codeinfra:"c,optional"`
	D float64  `codeinfra:"d"`
	E *float64 `codeinfra:"e,optional"`
	F *float64 `codeinfra:"f,optional"`
	G *inner   `codeinfra:"g,optional"`
	H *[]inner `codeinfra:"h,optional"`
}
...
func TestReproduceMapStringPointerTurnaroundIssue(t *testing.T) {
	...
	type X struct {
		Args map[string]*string `codeinfra:"args,optional"`
	}
