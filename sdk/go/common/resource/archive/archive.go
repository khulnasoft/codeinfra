...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource/sig"
...
func (a *Archive) readPath(wd string) (Reader, error) {
	...
	if format == NotArchive {
		...
		if walkerr := filepath.Walk(path, func(filePath string, f os.FileInfo, fileerr error) error {
			...
			// If this is a .codeinfra directory, we will skip this by default.
			// TODO[codeinfra/codeinfra#122]: when we support .codeinfraignore, this will be customizable.
