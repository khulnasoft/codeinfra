...
func TestProviderServer_Configure_variables(t *testing.T) {
	...
	_, err := srv.Configure(ctx, &codeinfrarpc.ConfigureRequest{
...
func TestProviderServer_Read_respects_ID(t *testing.T) {
	...
	srv := NewProviderServer(&provider)
	resp, err := srv.Read(ctx, &codeinfrarpc.ReadRequest{
		Urn: "urn:codeinfra:v2::re::random:index/randomPassword:RandomPassword::newPassword",
		Id:  secret,
	})
	require.NoError(t, err)
