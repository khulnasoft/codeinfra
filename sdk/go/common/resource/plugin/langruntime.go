...
func (info ProgramInfo) Marshal() (*codeinfrarpc.ProgramInfo, error) {
	...
	return &codeinfrarpc.ProgramInfo{
...
// These are the languages dependencies, they are not necessarily Codeinfra packages.
...
// Key is the key as used in runtime.options.<Key> in the Codeinfra.yaml file.
...
func UnmarshallRuntimeOptionPrompt(p *codeinfrarpc.RuntimeOptionPrompt) (RuntimeOptionPrompt, error) {
...
func MakeExecutablePromptChoices(executables ...string) []*codeinfrarpc.RuntimeOptionPrompt_RuntimeOptionValue {
	...
	for _, pm := range pms {
		...
		choices = append(choices, &codeinfrarpc.RuntimeOptionPrompt_RuntimeOptionValue{
