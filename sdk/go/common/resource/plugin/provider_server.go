...
type providerServer struct {
	codeinfrarpc.UnsafeResourceProviderServer // opt out of forward compat
...
func (p *providerServer) marshalDiff(diff DiffResult) (*codeinfrarpc.DiffResponse, error) {
	var changes codeinfrarpc.DiffResponse_DiffChanges
	switch diff.Changes {
	case DiffNone:
		changes = codeinfrarpc.DiffResponse_DIFF_NONE
	case DiffSome:
		changes = codeinfrarpc.DiffResponse_DIFF_SOME
	case DiffUnknown:
		changes = codeinfrarpc.DiffResponse_DIFF_UNKNOWN
	}

	// Infer the result from the detailed diff.
	var diffs, replaces []string
	var detailedDiff map[string]*codeinfrarpc.PropertyDiff
	if len(diff.DetailedDiff) == 0 {
		...
		detailedDiff = make(map[string]*codeinfrarpc.PropertyDiff)
		for path, diff := range diff.DetailedDiff {
			diffs = append(diffs, path)

			var kind codeinfrarpc.PropertyDiff_Kind
			switch diff.Kind {
			case DiffAdd:
				kind = codeinfrarpc.PropertyDiff_ADD
			case DiffAddReplace:
				kind, replaces = codeinfrarpc.PropertyDiff_ADD_REPLACE, append(replaces, path)
			case DiffDelete:
				kind = codeinfrarpc.PropertyDiff_DELETE
			case DiffDeleteReplace:
				kind, replaces = codeinfrarpc.PropertyDiff_DELETE, append(replaces, path)
			case DiffUpdate:
				kind = codeinfrarpc.PropertyDiff_UPDATE
			case DiffUpdateReplace:
				kind, replaces = codeinfrarpc.PropertyDiff_UPDATE_REPLACE, append(replaces, path)
			}

			detailedDiff[path] = &codeinfrarpc.PropertyDiff{
				Kind:      kind,
	...
	return &codeinfrarpc.DiffResponse{
		Replaces:            replaces,
...
func (p *providerServer) Handshake(
	ctx context.Context,
	req *codeinfrarpc.ProviderHandshakeRequest,
) (*codeinfrarpc.ProviderHandshakeResponse, error) {
	...
	return &codeinfrarpc.ProviderHandshakeResponse{}, nil
}

func (p *providerServer) Parameterize(
	ctx context.Context, req *codeinfrarpc.ParameterizeRequest,
) (*codeinfrarpc.ParameterizeResponse, error) {
	var params ParameterizeParameters
	switch p := req.Parameters.(type) {
	case *codeinfrarpc.ParameterizeRequest_Args:
		params = &ParameterizeArgs{Args: p.Args.GetArgs()}
	case *codeinfrarpc.ParameterizeRequest_Value:
	...
	return &codeinfrarpc.ParameterizeResponse{
		Name:    resp.Name,
...
func (p *providerServer) GetSchema(ctx context.Context,
	...
	return &codeinfrarpc.GetSchemaResponse{Schema: string(schema.Schema)}, nil
}

func (p *providerServer) GetPluginInfo(ctx context.Context, req *emptypb.Empty) (*codeinfrarpc.PluginInfo, error) {
	info, err := p.provider.GetPluginInfo(ctx)
	if err != nil {
		return nil, err
	}
	return &codeinfrarpc.PluginInfo{Version: info.Version.String()}, nil
}

func (p *providerServer) Attach(ctx context.Context, req *codeinfrarpc.PluginAttach) (*emptypb.Empty, error) {
...
func (p *providerServer) CheckConfig(ctx context.Context,
	...
	rpcFailures := make([]*codeinfrarpc.CheckFailure, len(resp.Failures))
	for i, f := range resp.Failures {
		rpcFailures[i] = &codeinfrarpc.CheckFailure{Property: string(f.Property), Reason: f.Reason}
	}

	return &codeinfrarpc.CheckResponse{Inputs: rpcInputs, Failures: rpcFailures}, nil
}

func (p *providerServer) DiffConfig(ctx context.Context, req *codeinfrarpc.DiffRequest) (*codeinfrarpc.DiffResponse, error) {
	urn := resource.URN(req.GetUrn())
...
func (p *providerServer) Check(ctx context.Context, req *codeinfrarpc.CheckRequest) (*codeinfrarpc.CheckResponse, error) {
	urn := resource.URN(req.GetUrn())
...
func (p *providerServer) Diff(ctx context.Context, req *codeinfrarpc.DiffRequest) (*codeinfrarpc.DiffResponse, error) {
...
func (p *providerServer) Create(ctx context.Context, req *codeinfrarpc.CreateRequest) (*codeinfrarpc.CreateResponse, error) {
	urn := resource.URN(req.GetUrn())
...
func (p *providerServer) Read(ctx context.Context, req *codeinfrarpc.ReadRequest) (*codeinfrarpc.ReadResponse, error) {
...
func (p *providerServer) Update(ctx context.Context, req *codeinfrarpc.UpdateRequest) (*codeinfrarpc.UpdateResponse, error) {
	urn, id := resource.URN(req.GetUrn()), resource.ID(req.GetId())
	...
	return &codeinfrarpc.UpdateResponse{Properties: rpcState}, nil
}

func (p *providerServer) Delete(ctx context.Context, req *codeinfrarpc.DeleteRequest) (*emptypb.Empty, error) {
	urn, id := resource.URN(req.GetUrn()), resource.ID(req.GetId())
...
func (p *providerServer) Construct(ctx context.Context,
	...
	outputDependencies := map[string]*codeinfrarpc.ConstructResponse_PropertyDependencies{}
	for name, deps := range resp.OutputDependencies {
		urns := make([]string, len(deps))
		for i, urn := range deps {
			urns[i] = string(urn)
		}
		outputDependencies[string(name)] = &codeinfrarpc.ConstructResponse_PropertyDependencies{Urns: urns}
	}

	return &codeinfrarpc.ConstructResponse{
		Urn:               string(resp.URN),
		State:             outputs,
...
func (p *providerServer) Invoke(ctx context.Context, req *codeinfrarpc.InvokeRequest) (*codeinfrarpc.InvokeResponse, error) {
	args, err := UnmarshalProperties(req.GetArgs(), p.unmarshalOptions("args", false /* keepOutputValues */))
	if err != nil {
		return nil, err
	}
...
func (p *providerServer) StreamInvoke(req *codeinfrarpc.InvokeRequest,
	...
	resp, err := p.provider.StreamInvoke(context.TODO(), StreamInvokeRequest{
		...
		OnNext: func(item resource.PropertyMap) error {
			...
			return server.Send(&codeinfrarpc.InvokeResponse{Return: rpcItem})
		},
	})
	if err != nil {
		return err
	...
	return server.Send(&codeinfrarpc.InvokeResponse{Failures: rpcFailures})
}

func (p *providerServer) Call(ctx context.Context, req *codeinfrarpc.CallRequest) (*codeinfrarpc.CallResponse, error) {
	...
	rpcFailures := make([]*codeinfrarpc.CheckFailure, len(result.Failures))
	for i, f := range result.Failures {
		rpcFailures[i] = &codeinfrarpc.CheckFailure{Property: string(f.Property), Reason: f.Reason}
	}

	return &codeinfrarpc.CallResponse{
		Return:             rpcResult,
...
func (p *providerServer) GetMapping(ctx context.Context,
	req *codeinfrarpc.GetMappingRequest,
	...
	return &codeinfrarpc.GetMappingResponse{Data: resp.Data, Provider: resp.Provider}, nil
}

func (p *providerServer) GetMappings(ctx context.Context,
	req *codeinfrarpc.GetMappingsRequest,
) (*codeinfrarpc.GetMappingsResponse, error) {
	...
	return &codeinfrarpc.GetMappingsResponse{Providers: providers.Keys}, nil
}
