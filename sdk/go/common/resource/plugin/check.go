...
import (
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/mapper"
		...
		case mapper.MappingError:
			for _, failure := range e.Failures() {
				...
				case mapper.FieldError:
					failures = append(failures, &codeinfrarpc.CheckFailure{
						Property: f.Field(),
						Reason:   f.Reason(),
					})
				default:
					failures = append(failures, &codeinfrarpc.CheckFailure{Reason: f.Error()})
				}
			}
		default:
			failures = append(failures, &codeinfrarpc.CheckFailure{Reason: e.Error()})
		}
	}
	return &codeinfrarpc.CheckResponse{Failures: failures}
}
