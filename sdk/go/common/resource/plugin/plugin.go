...
// CodeinfraPluginJSON represents additional information about a package's associated Codeinfra plugin.
// For Python, the content is inside a codeinfra-plugin.json file inside the package.
// For Node.js, the content is within the package.json file, under the "codeinfra" node.
// For .NET, the content is inside a codeinfra-plugin.json file inside the NuGet package.
// For Go, the content is inside a codeinfra-plugin.json file inside the module.
...
type plugin struct {
	...
	//
	// unstructuredOutput is only non-nil if Codeinfra launched the process and is hiding
	// unstructured output.
...
// the stack's Codeinfra SDK did not have the required modules. i.e. is too old.
var errRunPolicyModuleNotFound = errors.New("codeinfra SDK does not support policy as code")
...
func dialPlugin[T any](
	...
	// TODO[codeinfra/codeinfra#337]: in theory, this should be unnecessary.  gRPC's default WaitForReady behavior
...
func newPlugin[T any](
	...
	opts := []opentracing.StartSpanOption{
		...
		opentracing.Tag{Key: "codeinfra-decorator", Value: prefix + ":" + bin},
	}
	...
	runtrace := func(t io.Reader, streamID streamID, done chan<- bool) {
		...
		for {
			...
			if strings.TrimSpace(msg) != "" {
				...
				// e.g. the stack's package.json does not contain a recent enough @codeinfra/codeinfra.
				//
				...
				if strings.Contains(msg, "Cannot find module '@codeinfra/codeinfra/cmd/run-policy-pack'") {
...
func execPlugin(ctx *Context, bin, prefix string, kind apitype.PluginKind,
	...
	if _, err := os.Stat(bin); os.IsNotExist(err) {
		// If we don't have the expected binary, see if we have a "CodeinfraPlugin.yaml" or "CodeinfraPolicy.yaml"
		...
		if kind == apitype.ResourcePlugin || kind == apitype.ConverterPlugin {
			proj, err := workspace.LoadPluginProject(filepath.Join(pluginDir, "CodeinfraPlugin.yaml"))
			if err != nil {
				return nil, fmt.Errorf("loading CodeinfraPlugin.yaml: %w", err)
			}
			runtimeInfo = proj.Runtime
		} else if kind == apitype.AnalyzerPlugin {
			proj, err := workspace.LoadPluginProject(filepath.Join(pluginDir, "CodeinfraPolicy.yaml"))
			if err != nil {
				return nil, fmt.Errorf("loading CodeinfraPolicy.yaml: %w", err)
			}
			runtimeInfo = proj.Runtime
		} else {
	...
	if err := cmd.Start(); err != nil {
		...
		// It is a shell script like "codeinfra-analyzer-policy". But during
