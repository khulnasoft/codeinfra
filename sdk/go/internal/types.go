...
// AnyOutputType is the reflected type of codeinfra.AnyOutput.
//
// This type is set by the codeinfra package at init().
...
//
// This map is filled by the codeinfra package at init().
var FullyResolvedTypes = make(map[reflect.Type]struct{})

// Output encodes the relationship between resources in a Codeinfra
// application. See codeinfra.Output for more details.
...
type OutputState struct {
	...
	// This is a []codeinfra.Resource, but we can't use that type here because
...
func NewOutputState(join *WorkGroup, elementType reflect.Type, deps ...Resource) *OutputState {
	if deps == nil && len(deps) != 0 {
		panic(fmt.Sprintf("data race detected - please report to https://github.com/khulnasoft/codeinfra/issues: deps is nil with len %d", len(deps)))
	}
	...
	out := &OutputState{
		...
		// spurious data race here. See note in https://github.com/khulnasoft/codeinfra/pull/10081.
		//
...
//
//	stringOutput := codeinfra.String("hello").ToStringOutput()
...
// It is essentially an attempt to port the logic in the NodeJS SDK's `codeinfra.output` function, which takes a value and
...
// Input is an input value for a Codeinfra resource.
//
