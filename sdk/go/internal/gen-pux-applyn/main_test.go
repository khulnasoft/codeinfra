...
// Verifies that the code in the codeinfrax package is up to date.
// If the parameters in codeinfrax/gen.go ever change,
// this test should be updated.
func TestCodeinfraxIsUpToDate(t *testing.T) {
	t.Parallel()

	outDir := t.TempDir()
	require.NoError(t, run(&params{Dir: outDir}))

	// Compare the generated code to the code in codeinfrax.
	expected := os.DirFS(outDir)
	actual := os.DirFS("../../codeinfrax")
	...
	if t.Failed() {
		t.Errorf("Generated code in codeinfrax is out of date.")
		t.Errorf("To fix this, run `go generate` in the codeinfrax directory.")
	}
}
