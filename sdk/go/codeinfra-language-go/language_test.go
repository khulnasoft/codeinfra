...
type hostEngine struct {
	codeinfrarpc.UnimplementedEngineServer
	t *testing.T
...
func (e *hostEngine) Log(_ context.Context, req *codeinfrarpc.LogRequest) (*pbempty.Empty, error) {
	e.logLock.Lock()
	defer e.logLock.Unlock()

	var sev diag.Severity
	switch req.Severity {
	case codeinfrarpc.LogSeverity_DEBUG:
		sev = diag.Debug
	case codeinfrarpc.LogSeverity_INFO:
		sev = diag.Info
	case codeinfrarpc.LogSeverity_WARNING:
		sev = diag.Warning
	case codeinfrarpc.LogSeverity_ERROR:
		sev = diag.Error
	default:
	...
	message := req.Message
	if os.Getenv("CODEINFRA_LANGUAGE_TEST_SHOW_FULL_OUTPUT") != "true" {
		// Cut down logs so they don't overwhelm the test output
		if len(message) > 1024 {
			message = message[:1024] + "... (truncated, run with CODEINFRA_LANGUAGE_TEST_SHOW_FULL_OUTPUT=true to see full logs))"
		}
	}
...
func runEngine(t *testing.T) string {
	// Run a gRPC server that implements the Codeinfra engine RPC interface. But all we do is forward logs on to T.
	...
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		...
		Init: func(srv *grpc.Server) error {
			codeinfrarpc.RegisterEngineServer(srv, engine)
			return nil
		},
...
func runTestingHost(t *testing.T) (string, testingrpc.LanguageTestClient) {
	// We can't just go run the codeinfra-test-language package because of
	// https://github.com/golang/go/issues/39172, so we build it to a temp file then run that.
	binary := t.TempDir() + "/codeinfra-test-language"
	cmd := exec.Command("go", "build", "-C", "../../../cmd/codeinfra-test-language", "-o", binary)
...
var expectedFailures = map[string]string{
	...
	"l2-component-program-resource-ref":   "codeinfra#18140: cannot use ref.Value (variable of type codeinfra.StringOutput) as string value in return statement", //nolint:lll
	"l2-component-component-resource-ref": "codeinfra#18140: cannot use ref.Value (variable of type codeinfra.StringOutput) as string value in return statement", //nolint:lll
}
