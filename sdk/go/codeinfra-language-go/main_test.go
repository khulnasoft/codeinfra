...
func TestGetPackage(t *testing.T) {
	...
	cases := []struct {
		Name          string
		Mod           *modInfo
		Expected      *codeinfrarpc.PackageDependency
		ExpectedError string
		JSON          *plugin.CodeinfraPluginJSON
		JSONPath      string
	}{
		{
			Name: "valid-codeinfra-mod",
			Mod: &modInfo{
				Path:    "github.com/khulnasoft/codeinfra-aws/sdk",
				Version: "v1.29.0",
			},
			Expected: &codeinfrarpc.PackageDependency{
				Name:    "aws",
		...
		{
			Name: "codeinfra-pseduo-version-plugin",
			Mod: &modInfo{
				Path:    "github.com/khulnasoft/codeinfra-aws/sdk",
		...
		{
			Name: "non-codeinfra-mod",
			Mod: &modInfo{
				Path:    "github.com/moolumi/codeinfra-aws/sdk",
				Version: "v1.29.0",
			},
			ExpectedError: "module is not a codeinfra provider",
		},
		{
		...
		{
			Name: "codeinfra-codeinfra-mod",
			Mod: &modInfo{
				Path:    "github.com/khulnasoft/codeinfra/sdk",
				Version: "v1.14.0",
			},
			ExpectedError: "module is not a codeinfra provider",
		},
		{
			Name: "beta-codeinfra-module",
			Mod: &modInfo{
				Path:    "github.com/khulnasoft/codeinfra-aws/sdk",
		...
		},
		{
			Name: "non-zero-patch-module", Mod: &modInfo{
				Path:    "github.com/khulnasoft/codeinfra-kubernetes/sdk",
				Version: "v1.5.8",
			},
			Expected: &codeinfrarpc.PackageDependency{
				Name:    "kubernetes",
		...
		{
			Name: "codeinfraplugin-go-lookup",
			Mod: &modInfo{
		...
		{
			Name: "codeinfraplugin-go-name-lookup",
			Mod: &modInfo{
				Path:    "github.com/me/myself",
				Version: "v1.2.3",
			},
		...
		{
			Name: "codeinfraplugin-nested-too-deep",
			Mod: &modInfo{
				Path:    "path.com/here",
				Version: "v0.0",
			},
	...
	for _, c := range cases {
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			if c.JSON != nil {
				...
				assert.NoError(t, err, "Failed to setup test codeinfra-plugin.json")
				err = os.WriteFile(filepath.Join(path, "codeinfra-plugin.json"), bytes, 0o600)
				assert.NoError(t, err, "Failed to write codeinfra-plugin.json")
			}
...
// Test for https://github.com/khulnasoft/codeinfra/issues/12526.
// Validates that if a Codeinfra program has vendored its dependencies,
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/12963.
// Verifies that the language host can find plugins and dependencies
// when the Codeinfra program is in a subdirectory of the project root.
...
func testPluginsAndDependencies(t *testing.T, progDir string) {
	...
	t.Run("GetRequiredPackages", func(t *testing.T) {
		...
		res, err := host.GetRequiredPackages(ctx, &codeinfrarpc.GetRequiredPackagesRequest{
	...
	t.Run("GetProgramDependencies", func(t *testing.T) {
		...
		res, err := host.GetProgramDependencies(ctx, &codeinfrarpc.GetProgramDependenciesRequest{
			Project:                "deprecated",
			Pwd:                    progDir,
		...
		assert.Equal(t, map[string]string{
			"github.com/codeinfra/go-dependency-testdata/plugin":          "v1.2.3",
			"github.com/codeinfra/go-dependency-testdata/dep":             "v1.6.0",
			"github.com/codeinfra/go-dependency-testdata/indirect-dep/v2": "v2.1.0",
		}, gotDeps)
	})
}
