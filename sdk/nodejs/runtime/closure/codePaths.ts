 ...
 *
 * During this walk, if a package is encountered that contains a `"codeinfra": {
 * ... }` section then the normal `"dependencies": { ... }` section of that
 * package will not be included.  These are "codeinfra" packages, and those
 * dependencies are only intended for use at deployment time. However, a
 * "codeinfra" package can also specify package that should be available at
...
function addPackageAndDependenciesToSet(
    ...
    if (child.package.codeinfra) {
        // This was a codeinfra deployment-time package.  Check if it had a:
        //
        //    `codeinfra: { runtimeDependencies: ... }`
        //
        ...
        recurse(child.package.codeinfra.runtimeDependencies);
    } else if (pkg.startsWith("@codeinfra")) {
        // exclude it if it's an @codeinfra package.  These packages are intended for deployment
        // time only and will only bloat up the serialized lambda package.  Note: this code can
        // be removed once all codeinfra packages add a "codeinfra" section to their package.json.
        return;
    } else {
