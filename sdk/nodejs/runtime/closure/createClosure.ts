    ...
    async function serializeWorkerAsync(): Promise<FunctionInfo> {
        ...
        if (parseFunctionModule.isNativeFunction(functionString)) {
            try {
                ...
                const boundThis = "__codeinfra_bound_this";
                const boundArgs = boundArgsValues.map((_: any, i: number) => `__codeinfra_bound_arg_${i}`).join(", ");
        ...
        async function processCapturedVariablesAsync(
            ...
            for (const name of capturedVariables.keys()) {
                // We have a special case for __codeinfra_bound_this and __codeinfra_bound_arg_X.
                // We manually inject these variables into the closure environment of a
                // function when we rewrite bound functions.
                if (name.startsWith("__codeinfra_bound_")) {
                    continue;
                }
                let value: any;
                try {
...
async function getOrCreateEntryAsync(
    ...
    async function captureModuleAsync(normalizedModuleName: string): Promise<void> {
        ...
        if (hasTrueBooleanMember(obj, "deploymentOnlyModule") || isLocalModule) {
            ...
            // parts of it may be serializable on the inside (i.e. codeinfra.Config).  So just try to
            ...
            // serializable (like codeinfra.Config)
            //
