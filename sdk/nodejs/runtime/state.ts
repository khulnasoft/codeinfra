...
const nodeEnvKeys = {
    project: "CODEINFRA_NODEJS_PROJECT",
    stack: "CODEINFRA_NODEJS_STACK",
    dryRun: "CODEINFRA_NODEJS_DRY_RUN",
    queryMode: "CODEINFRA_NODEJS_QUERY_MODE",
    parallel: "CODEINFRA_NODEJS_PARALLEL",
    monitorAddr: "CODEINFRA_NODEJS_MONITOR",
    engineAddr: "CODEINFRA_NODEJS_ENGINE",
    syncDir: "CODEINFRA_NODEJS_SYNC",

    // Unlike the values above, this value is not set by the CLI and is
    // controlled via a user-set environment variable.
    cacheDynamicProviders: "CODEINFRA_NODEJS_CACHE_DYNAMIC_PROVIDERS",

    organization: "CODEINFRA_NODEJS_ORGANIZATION",
};

const codeinfraEnvKeys = {
    legacyApply: "CODEINFRA_ENABLE_LEGACY_APPLY",
};
...
export interface Store {
    settings: {
        ...
        rpcDone: Promise<any>;
        // Needed for legacy @codeinfra/codeinfra packages doing async feature checks.
...
export class LocalStore implements Store {
    settings = {
        options: {
            ...
            legacyApply: process.env[codeinfraEnvKeys.legacyApply] === "true",
