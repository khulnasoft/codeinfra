...
import * as query from "@codeinfra/query";
    ...
    debuggablePromise(
        resopAsync
            .then(async (resop) => {
                ...
                req.setTok("codeinfra:codeinfra:getResource");
    ...
    if (remote || !custom) {
        ...
        if (componentOpts.provider !== undefined) {
            if (componentOpts.providers === undefined) {
                ...
                const deprecationd =
                    "This will become an error in a future version. See https://github.com/khulnasoft/codeinfra/issues/8799 for more details";
 ...
 *
 * ```typescript
 * const buckets = codeinfra.runtime.listResourceOutput(aws.s3.Bucket.isInstance);
 * ```
 *
 * @param stackName
 ...
 * all of them.  The issue codeinfra/codeinfra#335 tracks coming up with a long-term
 * solution here.
 */
