 ...
 * of Codeinfra resources allocated by a deployment. This must be kept up to date
 * with `github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource/stack.RootStackType`.
 */
export const rootCodeinfraStackTypeName = "codeinfra:codeinfra:Stack";

/**
 * Creates a new Codeinfra stack resource and executes the callback inside of it.
 * Any outputs returned by the callback will be stored as output properties on
 * this resulting Stack object.
 */
export function runInCodeinfraStack(init: () => Promise<any>): Promise<Inputs | undefined> {
    if (!isQueryMode()) {
...
/**
 * {@link Stack} is the root resource for a Codeinfra stack. Before invoking the
 * `init` callback, it registers itself as the root resource with the Codeinfra
 * engine.
 */
export class Stack extends ComponentResource<Inputs> {
    ...
    constructor(init: () => Promise<Inputs>) {
        ...
        super(rootCodeinfraStackTypeName, `${getProject()}-${getStack()}`, { init });
     ...
     *
     * @param args.init
     *  The callback to run in the context of this Codeinfra stack
     */
...
async function massageComplex(prop: any, objectStack: any[]): Promise<any> {
    ...
    if (Resource.isInstance(prop)) {
        ...
        //
        // In preview only, we mark the POJO with "@isCodeinfraResource" to indicate that it is derived
        ...
        return !isDryRun() ? pojo : { ...pojo, "@isCodeinfraResource": true };
    }
...
/**
 * Add a transformation to all future resources constructed in this Codeinfra
 * stack.
 */
...
export function registerResourceTransform(t: ResourceTransform): void {
    if (!getStore().supportsTransforms) {
        throw new Error("The Codeinfra CLI does not support transforms. Please update the Codeinfra CLI");
    }
...
/**
 * Add a transformation to all future invoke calls in this Codeinfra stack.
 */
