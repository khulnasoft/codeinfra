...
// Write a package.json that installs the local codeinfra package and ensures we
...
async function run(typescriptVersion: string, nodeTypesVersion: string) {
    ...
    const codeinfraPackagePath = path.join(tmpDir.name, packageName);
    await pack(sdkRootBin, codeinfraPackagePath);
    await writePackageJSON(tmpDir.name, codeinfraPackagePath, typescriptVersion, nodeTypesVersion);
