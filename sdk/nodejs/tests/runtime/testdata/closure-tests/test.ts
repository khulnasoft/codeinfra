...
// @ts-ignore: The test installs @codeinfra/codeinfra
import { runtime } from "@codeinfra/codeinfra";
// @ts-ignore: The test installs @codeinfra/codeinfra
import * as pkg from "@codeinfra/codeinfra/runtime/closure/package";
...
// is the same as the one used by the codeinfra package and that we are testing
// what we think we are testing ...
it(`resolve to the correct typescript version within the codeinfra package`,
    async function () {
        const { stdout } = await execa("npm", ["ls", "typescript", "--json"], { cwd: __dirname, reject: false });
        const deps = JSON.parse(stdout);
        const version = deps.dependencies["@codeinfra/codeinfra"].dependencies.typescript.version;
