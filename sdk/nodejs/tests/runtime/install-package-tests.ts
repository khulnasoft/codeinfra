...
// Write a package.json that installs the local codeinfra package and optional dependencies.
async function writePackageJSON(
    dir: string,
    codeinfraPackagePath: string,
    ...
    const packageJSON = {
        ...
        license: "Apache-2.0",
        dependencies: {
            "@codeinfra/codeinfra": codeinfraPackagePath,
            ...dependencies,
        },
    };
...
// A simple TypeScript Codeinfra program to test that we can load and run TypeScript code.
async function writeProgram(dir: string, projectName: string) {
    const indexTS = `import * as codeinfra from "@codeinfra/codeinfra";
codeinfra.runtime.serializeFunction(() => 42);
...
backend:
  url: 'file://~'
`;
    await fs.writeFile(path.join(dir, "Codeinfra.yaml"), project);
}
...
async function main() {
    ...
    const tmpPackageDir = tmp.dirSync({ prefix: "codeinfra-package-", unsafeCleanup: true });
    try {
        // Add a random suffix to the package name to avoid any issues with yarn caching the tgz.
        const packageName = `codeinfra-${randomInt(10000, 99999)}.tgz`;
        const codeinfraPackagePath = path.join(tmpPackageDir.name, packageName);
        await pack(sdkRootBin, codeinfraPackagePath);
        ...
        for (const pm of packageManagers) {
            for (const deps of dependencies) {
                ...
                try {
                    await runTest(tmpDir, codeinfraPackagePath, pm.name, pm.version, deps);
                } finally {
...
async function runTest(
    ...
    await writePackageJSON(tmpDir.name, codeinfraPackagePath, peerDeps);
    ...
    try {
        logs += await exec("codeinfra", ["stack", "init", stackName], {
            cwd: tmpDir.name,
            env,
        });
        logs += await exec("codeinfra", ["up", "--stack", stackName, "--skip-preview"], {
            cwd: tmpDir.name,
            env,
        });
        ...
        await exec("codeinfra", ["destroy", "--stack", stackName, "--yes"], {
            cwd: tmpDir.name,
            env,
        });
        await exec("codeinfra", ["stack", "rm", stackName, "--yes"], {
            cwd: tmpDir.name,
            env,
        });
    }
