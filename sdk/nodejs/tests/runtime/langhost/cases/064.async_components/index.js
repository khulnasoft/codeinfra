...
class CustResource extends codeinfra.CustomResource {
    constructor(name, opts) {
        super("test:index:CustResource", name, {}, opts);
    }
}

class CompResource extends codeinfra.ComponentResource {
    constructor(name, opts) {
        super("test:index:CompResource", name, {}, opts);
        const data = this.getData();
        this.a = codeinfra.output(data.then((d) => d.a));
        this.b = codeinfra.output(data.then((d) => d.b));
        this.cust1 = codeinfra.output(data.then((d) => d.cust1));
        this.cust2 = codeinfra.output(data.then((d) => d.cust2));
    }
