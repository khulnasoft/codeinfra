...
for (let i = 0; i < 10; i++) {
    ...
    let r3 = new MyResource("testResource" + i * 10 + 3, { dependsOn: codeinfra.Output.create(last) });
    let r4 = new MyResource("testResource" + i * 10 + 4, { dependsOn: Promise.resolve(all) });
    let r5 = new MyResource("testResource" + i * 10 + 5, { dependsOn: codeinfra.Output.create(all) });
    ...
    let r7 = new MyResource("testResource" + i * 10 + 7, {
        dependsOn: Promise.resolve(all.map((a) => codeinfra.Output.create(a))),
    });
    let r8 = new MyResource("testResource" + i * 10 + 8, {
        dependsOn: codeinfra.Output.create(all).apply((a) => Promise.resolve(a)),
    });
    let r9 = new MyResource("testResource" + i * 10 + 9, {
        dependsOn: codeinfra.Output.create(all).apply((a) => codeinfra.Output.create(a)),
    });
