    ...
    const cases: { [key: string]: RunCase } = {
        ...
        stack_exports: {
            ...
            registerResource: (ctx, dryrun, t, name, res, deps, custom, protect, parent) => {
                if (t === "codeinfra:codeinfra:Stack") {
            ...
            registerResourceOutputs: (
                ...
                assert.strictEqual(t, "codeinfra:codeinfra:Stack");
        ...
        provider_invokes: {
            ...
            invoke: (ctx: any, dryrun: boolean, tok: string, args: any, version: string, provider: string) => {
                assert.strictEqual(provider, "codeinfra:providers:test::p::1");
        ...
        remote_component_providers: {
            ...
            registerResource: (
                ...
                if (name === "singular" || name === "map" || name === "array") {
                    assert.strictEqual(provider, "codeinfra:providers:test::myprovider::1");
            ...
            log: (ctx: any, severity: number, message: string, urn: URN, streamId: number) => {
                assert.strictEqual(
                    ...
                    "Found a TypeScript project containing an index.js file and no explicit entrypoint" +
                        " in Codeinfra.yaml - Codeinfra will use index.js",
                );
            },
        },
        unusual_alias_names: {
            ...
            for (const dryrun of [true, false]) {
                ...
                const monitor = await createMockEngineAsync(
                    ...
                    (call: any, callback: any) => {
                        ...
                        if (req.getType() !== runtime.rootCodeinfraStackTypeName || opts.showRootResourceRegistration) {
...
function parentDefaultsRegisterResource(
    ...
    if (custom && !t.startsWith("codeinfra:providers:")) {
...
function serveLanguageHostProcess(engineAddr: string): { proc: childProcess.ChildProcess; addr: Promise<string> } {
    ...
    //
    // Normally, `codeinfra-language-nodejs` launches `./node-modules/@codeinfra/codeinfra/cmd/run` which is
    ...
    //
    // `codeinfra-language-nodejs` allows us to set `CODEINFRA_LANGUAGE_NODEJS_RUN_PATH` in the environment, and
    ...
    process.env.CODEINFRA_LANGUAGE_NODEJS_RUN_PATH = path.normalize(path.join(__dirname, "..", "..", "..", "cmd", "run"));
    const proc = childProcess.spawn("codeinfra-language-nodejs", [engineAddr]);
