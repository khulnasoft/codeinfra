...
class TestResource extends codeinfra.CustomResource {
    constructor(name: string, opts?: codeinfra.CustomResourceOptions) {
        super("test:index:TestResource", name, {}, opts);
    }
}

class TestModule implements codeinfra.runtime.ResourceModule {
...
describe("provider", () => {
    ...
    describe("deserializeInputs", () => {
        beforeEach(() => {
            codeinfra.runtime._reset();
            codeinfra.runtime._resetResourcePackages();
            codeinfra.runtime._resetResourceModules();
        });
        ...
        async function assertOutputEqual(
            ...
            assert.ok(codeinfra.Output.isInstance(actual));
            ...
            const actualDeps = new Set<codeinfra.URN>();
            const resources = await actual.allResources!();
            for (const r of resources) {
                const urn = await r.urn.promise();
                actualDeps.add(urn);
            }
            assert.deepStrictEqual(actualDeps, new Set<codeinfra.URN>(deps ?? []));
        }

        function createSecret(value: any) {
            return {
                [codeinfra.runtime.specialSigKey]: codeinfra.runtime.specialSecretSig,
                value,
            };
        }

        function createResourceRef(urn: codeinfra.URN, id?: codeinfra.ID) {
            return {
                [codeinfra.runtime.specialSigKey]: codeinfra.runtime.specialResourceSig,
                urn,
                ...(id && { id }),
            };
        }

        function createOutputValue(value?: any, secret?: boolean, dependencies?: codeinfra.URN[]) {
            return {
                [codeinfra.runtime.specialSigKey]: codeinfra.runtime.specialOutputValueSig,
        ...
        const testURN = "urn:codeinfra:stack::project::test:index:TestResource::name";
        ...
        const tests: {
            ...
            {
                name: "unknown",
                input: codeinfra.runtime.unknownValue,
                deps: ["fakeURN"],
                assert: async (actual) => {
            ...
            },
            {
                ...
                input: { foo: codeinfra.runtime.unknownValue },
                deps: ["fakeURN"],
                assert: async (actual) => {
                    await assertOutputEqual(actual, undefined, false, false, ["fakeURN"]);
                },
        ...
        for (const test of tests) {
            it(`deserializes '${test.name}' correctly`, async () => {
                codeinfra.runtime.setMocks(new TestMocks(), "project", "stack", true);
                codeinfra.runtime.registerResourceModule("test", "index", new TestModule());
                ...
                const result = await codeinfra.provider.deserializeInputs(inputsStruct, inputDependencies);
    ...
    describe("containsOutputs", () => {
        const tests: {
            ...
            {
                name: "Output",
                input: codeinfra.Output.create("hi"),
                expected: true,
            },
            {
                name: "[Output]",
            ...
            {
                name: "{ foo: Output }",
                input: { foo: codeinfra.Output.create("hi") },
                expected: true,
            },
            {
                name: "Resource",
                input: new codeinfra.DependencyResource("fakeURN"),
                expected: false,
            },
            {
                name: "[Resource]",
            ...
            {
                name: "{ foo: Resource }",
                input: { foo: new codeinfra.DependencyResource("fakeURN") },
                expected: false,
            },
        ];
        for (const test of tests) {
            it(`${test.name} should return ${test.expected}`, () => {
                const actual = codeinfra.provider.containsOutputs(test.input);
