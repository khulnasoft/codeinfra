...
import { CodeinfraCommand, parseAndValidateCodeinfraVersion } from "../../automation";

describe("automation/cmd", () => {
    it("calls onOutput when provided to runCodeinfraCmd", async () => {
        let output = "";
        let numCalls = 0;
        const codeinfra = await CodeinfraCommand.get();
        await codeinfra.run(["--help"], ".", {}, (data: string) => {
            output += data;
            numCalls += 1;
        });
    ...
    describe("CLI installation", () => {
        it("installs the requested version", async () => {
            ...
            try {
                const cmd = await CodeinfraCommand.install({ root: tmpDir.name, version: new semver.SemVer("3.97.0") });
                assert.doesNotThrow(() => fs.statSync(upath.join(tmpDir.name, "bin", "codeinfra")));
        ...
        it("does not re-install the version if it already exists", async () => {
            const tmpDir = tmp.dirSync({ prefix: "automation-test-", unsafeCleanup: true });
            try {
                await CodeinfraCommand.install({ root: tmpDir.name, version: new semver.SemVer("3.97.0") });
                const binary1 = fs.statSync(upath.join(tmpDir.name, "bin", "codeinfra"));
                await CodeinfraCommand.install({ root: tmpDir.name, version: new semver.SemVer("3.97.0") });
                const binary2 = fs.statSync(upath.join(tmpDir.name, "bin", "codeinfra"));
        ...
        it("defaults to $HOME/.codeinfra/versions/$VERSION if no root is provided", async () => {
            const version = new semver.SemVer("3.97.0");
            const cmd = await CodeinfraCommand.install({ version });
            assert.doesNotThrow(() =>
                fs.statSync(upath.join(os.homedir(), ".codeinfra", "versions", `${version}`, "bin", "codeinfra")),
            );
            const { stdout } = await cmd.run(["version"], ".", {});
        ...
        it("throws if the found version is not compatible with the requested version", async () => {
            ...
            await CodeinfraCommand.install({ version: installedVersion });
            const requestedVersion = new semver.SemVer("3.98.0");
            assert.rejects(CodeinfraCommand.get({ version: requestedVersion }));
            assert.doesNotReject(CodeinfraCommand.get({ version: installedVersion, skipVersionCheck: true }));
        });
    });
    ...
    describe(`checkVersionIsValid`, () => {
        ...
        versionTests.forEach((test) => {
            it(`validates ${test.name} (${test.currentVersion})`, () => {
                const validate = () => parseAndValidateCodeinfraVersion(minVersion, test.currentVersion, test.optOut);
                if (test.expectError) {
