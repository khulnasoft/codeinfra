...
const userAgent = "codeinfra/codeinfra/test";
...
describe("LocalWorkspace", () => {
        ...
        for (const ext of ["yaml", "yml", "json"]) {
            const ws = await LocalWorkspace.create(
                withTestBackend(
                    ...
                    "testproj",
                    "A minimal Go Codeinfra program",
                    "go",
                ),
            );
            ...
            assert.strictEqual(settings.description, "A minimal Go Codeinfra program");
        }
    });
    ...
    it(`adds/removes/lists plugins successfully`, async () => {
        ...
        // See https://github.com/khulnasoft/codeinfra/issues/11013 for why this is disabled
    ...
    describe("Tag methods: get/set/remove/list", () => {
        ...
        it("lists tag values", async () => {
            if (!process.env.CODEINFRA_ACCESS_TOKEN) {
            ...
            assert.strictEqual(result["codeinfra:project"], projectName);
            assert.strictEqual(result["codeinfra:runtime"], runtime);
        });
        ...
        it("gets a single tag value", async () => {
            ...
            const actualValue = await workspace.getTag(stackName, "codeinfra:project");
    ...
    describe("ListStack Methods", async () => {
        describe("ListStacks", async () => {
            ...
            it(`should handle stacks correctly for listStacks`, async () => {
                ...
                const workspace = await LocalWorkspace.create(
                    withTestBackend({ codeinfraCommand: mockWithReturnedStacks }),
                );
                ...
                assert.strictEqual(stacks[0].url, "https://app.codeinfra.com/testorg1/testproj1/teststack1");
                assert.strictEqual(stacks[1].name, "testorg1/testproj1/teststack2");
                assert.strictEqual(stacks[1].current, false);
                assert.strictEqual(stacks[1].url, "https://app.codeinfra.com/testorg1/testproj1/teststack2");
            });

            it(`should use correct args for listStacks`, async () => {
                let capturedArgs: string[] = [];
                const mockCodeinfraCommand = {
                    command: "codeinfra",
                    version: null,
                    run: async (args: string[], cwd: string, additionalEnv: { [key: string]: string }) => {
                        capturedArgs = args;
                ...
                const workspace = await LocalWorkspace.create(
                    withTestBackend({
                        codeinfraCommand: mockCodeinfraCommand,
                    }),
                );
        ...
        describe("ListStacks with all", async () => {
            ...
            it(`should handle stacks correctly for listStacks when all is set`, async () => {
                ...
                const workspace = await LocalWorkspace.create(
                    withTestBackend({
                        codeinfraCommand: mockWithReturnedStacks,
                    }),
                );
                ...
                assert.strictEqual(stacks[1].current, false);
                assert.strictEqual(stacks[1].url, "https://app.codeinfra.com/testorg1/testproj2/teststack2");
            });
            ...
            it(`should use correct args for listStacks when all is set`, async () => {
                ...
                const workspace = await LocalWorkspace.create(
                    withTestBackend({
                        codeinfraCommand: mockPuluiCommand,
                    }),
                );
    ...
    // TODO[codeinfra/codeinfra#8220] understand why this test was flaky
    ...
    // Regression test for https://github.com/khulnasoft/codeinfra/issues/17613
    ...
    it(`destroys an inline program with excludeProtected`, async () => {
        ...
        // codeinfra destroy to cleanup all resources
    ...
    it(`runs through the stack lifecycle with multiple inline programs in parallel`, async () => {
        ...
        const testStackLifetime = async (stackName: string) => {
            ...
            // codeinfra up
            const upRes = await stack.up({ userAgent }); // codeinfra up
            assert.strictEqual(Object.keys(upRes.outputs).length, 3);
            assert.strictEqual(upRes.outputs["exp_static"].value, "foo");
            assert.strictEqual(upRes.outputs["exp_static"].secret, false);
            assert.strictEqual(upRes.outputs["exp_cfg"].value, "abc");
            ...
            // codeinfra preview
            const preRes = await stack.preview({ userAgent }); // codeinfra preview
            assert.strictEqual(preRes.changeSummary.same, 1);
    ...
    // TODO[codeinfra/codeinfra#8061] flaky test
    ...
    it(`runs successfully after a previous failure`, async () => {
        ...
        // codeinfra up rejects the first time
        await assert.rejects(stack.up());

        // codeinfra up succeeds the 2nd time
        shouldFail = false;
        await assert.doesNotReject(stack.up());
    ...
    it("can import resources into a stack using resource definitions", async () => {
        ...
        const codeinfraRandomVersion = "4.16.3";
        await stack.workspace.installPlugin("random", codeinfraRandomVersion);
        ...
        await stack.destroy();
        await stack.workspace.removeStack(stackName);
        await stack.workspace.removePlugin("random", codeinfraRandomVersion);
    });
    ...
    it(`sets codeinfra version`, async () => {
        const ws = await LocalWorkspace.create(withTestBackend({}));
        assert(ws.codeinfraVersion);
        assert.strictEqual(versionRegex.test(ws.codeinfraVersion), true);
    });
    it("sets codeinfra version when using a custom CLI instance", async () => {
        const tmpDir = tmp.dirSync({ prefix: "automation-test-", unsafeCleanup: true });
        try {
            const cmd = await CodeinfraCommand.get();
            const ws = await LocalWorkspace.create(withTestBackend({ codeinfraCommand: cmd }));
            assert.strictEqual(versionRegex.test(ws.codeinfraVersion), true);
        } finally {
            tmpDir.removeCallback();
        }
    });
    it("throws when attempting to retrieve an invalid codeinfra version", async () => {
        ...
        const ws = await LocalWorkspace.create(
            withTestBackend({
                codeinfraCommand: mockWithNoVersion,
                envVars: {
                    CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK: "true",
                },
            }),
        );
        assert.throws(() => ws.codeinfraVersion);
    });
    it("fails creation if remote operation is not supported", async () => {
        const mockWithNoRemoteSupport = {
            command: "codeinfra",
            version: new semver.SemVer("2.0.0"),
            // We inspect the output of `codeinfra preview --help` to determine
            // if the CLI supports remote operations, see
            // `LocalWorkspace.checkRemoteSupport`.
            run: async () => new CommandResult("some output", "", 0),
        };
        await assert.rejects(
            LocalWorkspace.create(withTestBackend({ codeinfraCommand: mockWithNoRemoteSupport, remote: true })),
        );
    });
    it("bypasses remote support check", async () => {
        ...
        await assert.doesNotReject(
            LocalWorkspace.create(
                withTestBackend({
                    codeinfraCommand: mockWithNoRemoteSupport,
                    remote: true,
                    envVars: {
                        CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK: "true",
                    },
    ...
    it(`correctly sets config on multiple stacks concurrently`, async () => {
        ...
        const ws = await LocalWorkspace.create({
            ...
            envVars: {
                CODEINFRA_CONFIG_PASSPHRASE: "test",
            },
        });
    ...
    it(`runs the install command`, async () => {
        ...
        const ws = await LocalWorkspace.create(withTestBackend({ codeinfraCommand: mockCommand }));
 ...
 * either a file backend or a cloud backend, depending on whether CODEINFRA_ACCESS_TOKEN
 ...
 * environment containing a test `CODEINFRA_CONFIG_PASSPHRASE` variable suitable
