...
export interface CodeinfraCommandOptions {
    /**
     ...
     * installation. Defaults to $HOME/.codeinfra/versions/$VERSION.
     */
    root?: string;
    /**
     * Skips the minimum CLI version check, see `CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK`.
     */
    skipVersionCheck?: boolean;
}

export class CodeinfraCommand {
    private constructor(
    ...
    /**
     * Get a new Codeinfra instance that uses the installation in `opts.root`.
     * Defaults to using the codeinfra binary found in $PATH if no installation
     ...
     * `CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK` or setting
     ...
     */
    static async get(opts?: CodeinfraCommandOptions): Promise<CodeinfraCommand> {
        const command = opts?.root ? path.resolve(path.join(opts.root, "bin/codeinfra")) : "codeinfra";
        ...
        const version = parseAndValidateCodeinfraVersion(min, stdout.trim(), skipVersionCheck);
        return new CodeinfraCommand(command, version);
    }

    /**
     * Installs the Codeinfra CLI.
     */
    static async install(opts?: CodeinfraCommandOptions): Promise<CodeinfraCommand> {
        const optsWithDefaults = withDefaults(opts);
        try {
            return await CodeinfraCommand.get(optsWithDefaults);
        } catch (err) {
            // ignore
        }

        if (process.platform === "win32") {
            await CodeinfraCommand.installWindows(optsWithDefaults);
        } else {
            await CodeinfraCommand.installPosix(optsWithDefaults);
        }

        return await CodeinfraCommand.get(optsWithDefaults);
    }

    private static async installWindows(opts: Required<CodeinfraCommandOptions>): Promise<void> {
        const response = await got("https://get.codeinfra.com/install.ps1");
    ...
    private static async installPosix(opts: Required<CodeinfraCommandOptions>): Promise<void> {
        const response = await got("https://get.codeinfra.com/install.sh");
    ...
    public run(
        ...
        if (path.isAbsolute(this.command)) {
            const codeinfraBin = path.dirname(this.command);
            const sep = os.platform() === "win32" ? ";" : ":";
            const envPath = codeinfraBin + sep + (additionalEnv["PATH"] || process.env.PATH);
...
function withDefaults(opts?: CodeinfraCommandOptions): Required<CodeinfraCommandOptions> {
    ...
    let root = opts?.root;
    if (!root) {
        root = path.join(os.homedir(), ".codeinfra", "versions", `${version}`);
    }
...
/**
 * @internal
 * Throws an error if the Codeinfra CLI version is not valid.
 *
 * @param minVersion The minimum acceptable version of the Codeinfra CLI.
 ...
 */
export function parseAndValidateCodeinfraVersion(
    ...
    if (version == null) {
        throw new Error(
            `Failed to parse Codeinfra CLI version. This is probably an internal error. You can override this by setting "${SKIP_VERSION_CHECK_VAR}" to "true".`,
        );
    }
    if (minVersion.major < version.major) {
        throw new Error(
            `Major version mismatch. You are using Codeinfra CLI version ${currentVersion} with Automation SDK v${minVersion.major}. Please update the SDK.`,
        );
    }
    if (minVersion.compare(version) === 1) {
        throw new Error(
            `Minimum version requirement failed. The minimum CLI version requirement is ${minVersion.toString()}, your current CLI version is ${currentVersion}. Please update the Codeinfra CLI.`,
        );
    }
    return version;
}
