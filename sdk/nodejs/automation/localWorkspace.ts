...
import { CommandResult, CodeinfraCommand } from "./cmd";
...
import { Deployment, PluginInfo, CodeinfraFn, StackSummary, WhoAmIResult, Workspace } from "./workspace";

const SKIP_VERSION_CHECK_VAR = "CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK";

/**
 * {@link LocalWorkspace} is a default implementation of the {@link Workspace} interface.
 *
 * A {@link Workspace} is the execution context containing a single Codeinfra
 ...
 * installation, environment configuration (`$CODEINFRA_HOME`), and creation,
 * deletion, and listing of Stacks.
 *
 * {@link LocalWorkspace} relies on `Codeinfra.yaml` and `Codeinfra.<stack>.yaml` as
 * the intermediate format for Project and Stack settings. Modifying the
 * workspace's {@link ProjectSettings} will alter the workspace's `Codeinfra.yaml`
 * file, and setting config on a Stack will modify the relevant
 * `Codeinfra.<stack>.yaml` file. This is identical to the behavior of Codeinfra CLI
 * driven workspaces.
 *
 * @alpha
 */
export class LocalWorkspace implements Workspace {
    /**
     * The working directory to run Codeinfra CLI commands in.
     */
    readonly workDir: string;

    /**
     * The directory override for CLI metadata if set. This customizes the
     * location of `$CODEINFRA_HOME` where metadata is stored and plugins are
     * installed.
     */
    readonly codeinfraHome?: string;

    /**
     * The secrets provider to use for encryption and decryption of stack secrets.
     * See: https://www.codeinfra.com/docs/intro/concepts/secrets/#available-encryption-providers
     */
    readonly secretsProvider?: string;

    private _codeinfraCommand?: CodeinfraCommand;

    /**
     * The underlying Codeinfra CLI.
     */
    public get codeinfraCommand(): CodeinfraCommand {
        if (this._codeinfraCommand === undefined) {
            throw new Error("Failed to get Codeinfra CLI");
        }
        return this._codeinfraCommand;
    }

    /**
     * The inline program {@link CodeinfraFn} to be used for preview/update
    ...
    /**
     * Environment values scoped to the current workspace. These will be supplied to every Codeinfra command.
     */
    envVars: { [key: string]: string };

    private _codeinfraVersion?: semver.SemVer;

    /**
     * The version of the underlying Codeinfra CLI/engine.
     *
     * @returns A string representation of the version, if available. `null` otherwise.
     */
    public get codeinfraVersion(): string {
        if (this._codeinfraVersion === undefined) {
            throw new Error(`Failed to get Codeinfra version`);
        }
        return this._codeinfraVersion.toString();
    }
    ...
    /**
     * An optional list of arbitrary commands to run before the remote Codeinfra operation is invoked.
     */
    private remotePreRunCommands?: string[];

    /**
     * The environment variables to pass along when running remote Codeinfra operations.
     */
     ...
     * Codeinfra CLI program from the specified working directory. This is a way to
     * create drivers on top of pre-existing Codeinfra programs. This workspace
     * will pick up any available settings files (`Codeinfra.yaml`,
     * `Codeinfra.<stack>.yaml`).
     *
     * @param args
     *  A set of arguments to initialize a stack with a pre-configured Codeinfra
     ...
     * specified inline (in process) Codeinfra program. This program is fully
     ...
     *  {@link CodeinfraFn} program that runs in process.
     * @param opts
     *  Additional customizations to be applied to the Workspace.
     */
     ...
     * the specified inline (in process) Codeinfra program. This program is fully
     * debuggable and runs in process. If no Project option is specified,
     * default project settings will be created on behalf of the user.
     * Similarly, unless a `workDir` option is specified, the working directory
     * will default to a new temporary directory provided by the OS.
     *
     * @param args
     *  A set of arguments to initialize a Stack with and inline `CodeinfraFn`
     ...
     * utilizing the specified inline (in process) Codeinfra CLI program. This
     ...
     * LocalWorkspace} utilizing the specified inline Codeinfra CLI program. This
    ...
    private constructor(opts?: LocalWorkspaceOptions) {
        ...
        if (opts) {
            ...
            this.codeinfraHome = codeinfraHome;
        ...
        const codeinfraCommand = opts?.codeinfraCommand
            ? Promise.resolve(opts.codeinfraCommand)
            : CodeinfraCommand.get({ skipVersionCheck });

        const readinessPromises: Promise<any>[] = [
            codeinfraCommand.then((p) => {
                this._codeinfraCommand = p;
                if (p.version) {
                    this._codeinfraVersion = p.version;
                }
     ...
     * {@link LocalWorkspace} reads settings from the `Codeinfra.yaml`
     ...
     * old. {@link LocalWorkspace} writes this value to a `Codeinfra.yaml` file in
     * `Workspace.WorkDir()`.
     *
     * @param settings
    ...
    async saveProjectSettings(settings: ProjectSettings): Promise<void> {
        ...
        for (const ext of settingsExtensions) {
            const testPath = upath.joinSafe(this.workDir, `Codeinfra${ext}`);
            if (fs.existsSync(testPath)) {
                foundExt = ext;
                break;
            }
        }
        const path = upath.joinSafe(this.workDir, `Codeinfra${foundExt}`);
        let contents;
        if (foundExt === ".json") {
     ...
     * `Codeinfra.<stack>.yaml` file in `Workspace.WorkDir()`.
     *
     * @param stackName
    ...
    async stackSettings(stackName: string): Promise<StackSettings> {
        ...
        for (const ext of settingsExtensions) {
            ...
            const path = upath.joinSafe(this.workDir, `Codeinfra.${stackSettingsName}${ext}`);
     ...
     * name. {@link LocalWorkspace} writes this value to a `Codeinfra.<stack>.yaml`
    ...
    async saveStackSettings(stackName: string, settings: StackSettings): Promise<void> {
        ...
        let foundExt = ".yaml";
        for (const ext of settingsExtensions) {
            const testPath = upath.joinSafe(this.workDir, `Codeinfra.${stackSettingsName}${ext}`);
            if (fs.existsSync(testPath)) {
                foundExt = ext;
                break;
            }
        }
        const path = upath.joinSafe(this.workDir, `Codeinfra.${stackSettingsName}${foundExt}`);
    ...
    async createStack(stackName: string): Promise<void> {
        ...
        await this.runCodeinfraCmd(args);
    }
    ...
    async addEnvironments(stackName: string, ...environments: string[]): Promise<void> {
        let ver = this._codeinfraVersion;
        if (ver === undefined) {
        ...
        if (ver.compare("3.95.0") < 0) {
            throw new Error(`addEnvironments requires Codeinfra version >= 3.95.0`);
        }

        await this.runCodeinfraCmd(["config", "env", "add", ...environments, "--stack", stackName, "--yes"]);
    }
    ...
    async listEnvironments(stackName: string): Promise<string[]> {
        ...
        if (ver.compare("3.99.0") < 0) {
            throw new Error(`listEnvironments requires Codeinfra version >= 3.99.0`);
        }

        const result = await this.runCodeinfraCmd(["config", "env", "ls", "--stack", stackName, "--json"]);
    ...
    async removeEnvironment(stackName: string, environment: string): Promise<void> {
        ...
        // 3.95 added this command (https://github.com/khulnasoft/codeinfra/releases/tag/v3.95.0)
        if (ver.compare("3.95.0") < 0) {
            throw new Error(`removeEnvironments requires Codeinfra version >= 3.95.0`);
        }

        await this.runCodeinfraCmd(["config", "env", "rm", environment, "--stack", stackName, "--yes"]);
    }

    /**
     * Returns the value associated with the specified stack name and key,
     * scoped to the current workspace. {@link LocalWorkspace} reads this config
     * from the matching `Codeinfra.stack.yaml` file.
     *
     * @param stackName
    ...
    async getConfig(stackName: string, key: string, path?: boolean): Promise<ConfigValue> {
        ...
        const result = await this.runCodeinfraCmd(args);
        return JSON.parse(result.stdout);
    }
     ...
     * matching `Codeinfra.stack.yaml` file.
     *
     * @param stackName
     *  The stack to read config from
     */
    async getAllConfig(stackName: string): Promise<ConfigMap> {
        const result = await this.runCodeinfraCmd(["config", "--show-secrets", "--json", "--stack", stackName]);
        return JSON.parse(result.stdout);
    }

    /**
     * Sets the specified key-value pair on the provided stack name. {@link
     * LocalWorkspace} writes this value to the matching `Codeinfra.<stack>.yaml`
     * file in `Workspace.WorkDir()`.
     *
     * @param stackName
     *  The stack to operate on
     ...
     * remove any matching values in the `Codeinfra.<stack>.yaml` file in
     * `Workspace.WorkDir()`.
     *
     * @param stackName
     *  The stack to operate on
     ...
     * Will remove any matching values in the `Codeinfra.<stack>.yaml` file in
     * `Workspace.WorkDir()`.
     *
     * @param stackName
     *  The stack to operate on
    ...
    async refreshConfig(stackName: string): Promise<ConfigMap> {
        await this.runCodeinfraCmd(["config", "refresh", "--force", "--stack", stackName]);
    ...
    async getTag(stackName: string, key: string): Promise<string> {
        const result = await this.runCodeinfraCmd(["stack", "tag", "get", key, "--stack", stackName]);
    ...
    async setTag(stackName: string, key: string, value: string): Promise<void> {
        await this.runCodeinfraCmd(["stack", "tag", "set", key, value, "--stack", stackName]);
    }
    ...
    async removeTag(stackName: string, key: string): Promise<void> {
        await this.runCodeinfraCmd(["stack", "tag", "rm", key, "--stack", stackName]);
    }
    ...
    async listTags(stackName: string): Promise<TagMap> {
        const result = await this.runCodeinfraCmd(["stack", "tag", "ls", "--json", "--stack", stackName]);
        return JSON.parse(result.stdout);
    }
    ...
    async whoAmI(): Promise<WhoAmIResult> {
        ...
        if (ver.compare("3.58.0") >= 0) {
            const result = await this.runCodeinfraCmd(["whoami", "--json"]);
            return JSON.parse(result.stdout);
        } else {
            const result = await this.runCodeinfraCmd(["whoami"]);
     ...
     * present in the workspace as `Codeinfra.<stack>.yaml` files.
     *
     * @param opts
    ...
    async install(opts?: InstallOptions): Promise<void> {
        ...
        if (ver.compare("3.91.0") < 0) {
            // Codeinfra 3.91.0 added the `codeinfra install` command.
            // https://github.com/khulnasoft/codeinfra/releases/tag/v3.91.0
            throw new Error(`codeinfra install requires Codeinfra version >= 3.91.0`);
        }
        ...
        if (opts?.useLanguageVersionTools) {
            if (ver.compare("3.130.0") < 0) {
                ...
                // https://github.com/khulnasoft/codeinfra/releases/tag/v3.130.0
                throw new Error(`useLanguageVersionTools requires Codeinfra version >= 3.130.0`);
            }
        ...
        await this.runCodeinfraCmd(["install", ...args]);
    }
    ...
    async installPlugin(name: string, version: string, kind = "resource"): Promise<void> {
        await this.runCodeinfraCmd(["plugin", "install", kind, name, version]);
    }
    ...
    async installPluginFromServer(name: string, version: string, server: string): Promise<void> {
        await this.runCodeinfraCmd(["plugin", "install", "resource", name, version, "--server", server]);
    }
    ...
    async listPlugins(): Promise<PluginInfo[]> {
        const result = await this.runCodeinfraCmd(["plugin", "ls", "--json"]);
    ...
    async exportStack(stackName: string): Promise<Deployment> {
        const result = await this.runCodeinfraCmd(["stack", "export", "--show-secrets", "--stack", stackName]);
        return JSON.parse(result.stdout);
    }
    ...
    async importStack(stackName: string, state: Deployment): Promise<void> {
        ...
        await this.runCodeinfraCmd(["stack", "import", "--file", filepath, "--stack", stackName]);
    ...
    async stackOutputs(stackName: string): Promise<OutputMap> {
        // TODO: do this in parallel after this is fixed https://github.com/khulnasoft/codeinfra/issues/6050
        const maskedResult = await this.runCodeinfraCmd(["stack", "output", "--json", "--stack", stackName]);
        const plaintextResult = await this.runCodeinfraCmd([
            "stack",
            "output",
            "--json",
            "--show-secrets",
     ...
     * create/modify a `Codeinfra.stack.yaml`) {@link LocalWorkspace} does not
    ...
    private async checkRemoteSupport() {
        ...
        if (!optOut && this.isRemote) {
            ...
            const previewResult = await this.runCodeinfraCmd(["preview", "--help"]);
            const previewOutput = previewResult.stdout.trim();
            if (!previewOutput.includes("--remote")) {
                throw new Error("The Codeinfra CLI does not support remote operations. Please upgrade.");
            }
        }
    }

    private async runCodeinfraCmd(args: string[]): Promise<CommandResult> {
        let envs: { [key: string]: string } = {};
        if (this.codeinfraHome) {
            envs["CODEINFRA_HOME"] = this.codeinfraHome;
        }
        if (this.isRemote) {
            envs["CODEINFRA_EXPERIMENTAL"] = "true";
        }
        envs = { ...envs, ...this.envVars };
        return this.codeinfraCommand.run(args, this.workDir, envs);
    }
...
/**
 * Description of a stack backed by an inline (in process) Codeinfra program.
 */
export interface InlineProgramArgs {
    ...
    /**
     * The inline (in-process) Codeinfra program to use with update and preview operations.
     */
    program: CodeinfraFn;
}

/**
 * Description of a stack backed by pre-existing local Codeinfra CLI program.
 */
...
export interface LocalWorkspaceOptions {
    /**
     * The directory to run Codeinfra commands and read settings (`Codeinfra.yaml` and
     * `Codeinfra.<stack>.yaml`).
     */
    workDir?: string;
    ...
    /**
     * An optional list of arbitrary commands to run before a remote Codeinfra operation is invoked.
     *
     * @internal
     */
...
function loadProjectSettings(workDir: string) {
    for (const ext of settingsExtensions) {
        const isJSON = ext === ".json";
        const path = upath.joinSafe(workDir, `Codeinfra${ext}`);
        if (!fs.existsSync(path)) {
            continue;
        }
        const contents = fs.readFileSync(path).toString();
...
export interface RemoveOptions {
    ...
    /**
     * Do not delete the corresponding Codeinfra.<stack-name>.yaml configuration file for the stack
     */
