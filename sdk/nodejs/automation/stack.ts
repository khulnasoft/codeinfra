...
import { Deployment, CodeinfraFn, Workspace } from "./workspace";

import * as langrpc from "../proto/language_grpc_pb";

/**
 * {@link Stack} is an isolated, independently configurable instance of a Codeinfra
 * program. {@link Stack} exposes methods for the full Codeinfra lifecycle
...
If you're seeing this warning, please comment on https://github.com/khulnasoft/codeinfra/issues/6768 with the event and any
...
Event: ${line}\n${e.toString()}`);
     ...
     *
     * @see https://www.codeinfra.com/docs/cli/commands/codeinfra_up/
     */
    async up(opts?: UpOptions): Promise<UpResult> {
        ...
        if (program) {
            ...
            onExit = (hasError: boolean) => {
                languageServer.onCodeinfraExit(hasError);
        ...
        try {
            upResult = await this.runCodeinfraCmd(args, opts?.onOutput, opts?.signal);
        } catch (e) {
            didError = true;
            throw e;
        } finally {
     ...
     *
     * @see https://www.codeinfra.com/docs/cli/commands/codeinfra_preview/
     */
    async preview(opts?: PreviewOptions): Promise<PreviewResult> {
        ...
        try {
            previewResult = await this.runCodeinfraCmd(args, opts?.onOutput, opts?.signal);
        } catch (e) {
            didError = true;
            throw e;
        } finally {
    ...
    async refresh(opts?: RefreshOptions): Promise<RefreshResult> {
        ...
        try {
            refResult = await this.runCodeinfraCmd(args, opts?.onOutput, opts?.signal);
        } finally {
            await cleanUp(logFile, await logPromise);
        }
    ...
    async destroy(opts?: DestroyOptions): Promise<DestroyResult> {
        ...
        try {
            desResult = await this.runCodeinfraCmd(args, opts?.onOutput, opts?.signal);
        } finally {
            await cleanUp(logFile, await logPromise);
        }
        ...
        // rather than passing `--remove` to `codeinfra destroy` because the latter
    ...
    async import(options: ImportOptions): Promise<ImportResult> {
        ...
        let tempDir: string = "";
        try {
            tempDir = await fs.promises.mkdtemp(pathlib.join(os.tmpdir(), "codeinfra-import-"));
            ...
            if (options.converter) {
                ...
                if (options.converterArgs) {
                    ...
                    // }
                    // would be equivalent to `codeinfra import --from terraform ./tfstate.json`
                    args.push("--");
                    args.push(...options.converterArgs);
                }
            }

            importResult = await this.runCodeinfraCmd(args, options.onOutput);
    ...
    async cancel(): Promise<void> {
        await this.runCodeinfraCmd(["cancel", "--yes"]);
    }
    ...
    private async runCodeinfraCmd(
        ...
        const codeinfraHome = this.workspace.codeinfraHome;
        if (codeinfraHome) {
            envs["CODEINFRA_HOME"] = codeinfraHome;
        }
        ...
        const result = await this.workspace.codeinfraCommand.run(args, this.workspace.workDir, envs, onOutput, signal);
...
/**
 * A set of outputs, keyed by name, that might be returned by a Codeinfra program
 * as part of a stack operation.
 */
export type OutputMap = { [key: string]: OutputValue };

/**
 * An output produced by a Codeinfra program as part of a stack operation.
 */
...
/**
 * The deployment output from running a Codeinfra program update.
 */
...
/**
 * Output from running a Codeinfra program preview.
 */
...
export interface GlobalOpts {
    ...
    /**
     * Save any creates seen during the preview into an import file to use with `codeinfra import`.
     */
    importFile?: string;
}
...
export interface UpOptions extends GlobalOpts {
    ...
    /**
     * An inline (in-process) Codeinfra program to execute the operation against.
     */
    program?: CodeinfraFn;
