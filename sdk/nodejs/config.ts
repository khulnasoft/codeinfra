...
// getProject for use in non-codeinfra engine contexts
...
// This is used to capture and serialize the results of
// getConfig for use in non-codeinfra engine contexts
 ...
 * variables that otherwise share the same key.  For example, a bag whose name is `codeinfra:foo`, with keys `a`, `b`,
 * and `c`, is entirely separate from a bag whose name is `codeinfra:bar` with the same simple key names.  Each key has a
 * fully qualified names, such as `codeinfra:foo:a`, ..., and `codeinfra:bar:a`, respectively.
 */
export class Config {
    /**
    ...
    private getImpl<K extends string = string>(
        ...
        // TODO[codeinfra/codeinfra#7127]: Re-enable the warning.
    ...
    constructor(
        ...
        super(
            `Missing required configuration variable '${key}'\n` +
                `\tplease set a value using the command \`codeinfra config set${
                    secret ? " --secret " : " "
                }${key} <value>\``,
        );
    }
