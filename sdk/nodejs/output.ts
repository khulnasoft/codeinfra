 ...
 * Codeinfra application. Specifically, an {@link Output} holds onto a piece of
...
class OutputImpl<T> implements OutputInstance<T> {
    ...
    public readonly __codeinfraOutput: boolean = true;
     ...
     * Output construction time. It represents the `@codeinfra/codeinfra` API prior to
     * the addition of "async resource" dependencies. Code inside
     * `@codeinfra/codeinfra` should use `.allResources` instead.
     *
     * Only callable on the outside.
     *
     * @internal
     ...
     * a warning. In a future version of `@codeinfra/codeinfra` this will be changed
     ...
     * 2. `codeinfra.interpolate ``prefix${v}suffix`` `
     *
     * This will return an {@link Output} with the inner computed value and all
     * resources still tracked.
     *
     * @see https://www.codeinfra.com/docs/concepts/inputs-outputs
     *
     * @internal
     */
     ...
     * resources still tracked.
     *
     * @see https://www.codeinfra.com/docs/concepts/inputs-outputs for more details
     *
     * @internal
     */
    public toJSON: () => any;
     ...
     * to work even when multiple copies of the Codeinfra SDK have been loaded into
     * the same process.
     */
    public static isInstance<T>(obj: any): obj is Output<T> {
        return utils.isInstance(obj, "__codeinfraOutput");
    }
    ...
    static async getPromisedValue<T>(promise: Promise<T>, withUnknowns?: boolean): Promise<T> {
        ...
        // preserves compatibility with earlier versions of the Codeinfra SDK.
...
Or use ESLint with https://github.com/codeinfra/eslint-plugin-codeinfra to warn or
error lint on using Output<T> in template literals.`;
            if (utils.errorOutputString) {
                throw new Error(message);
            }
            message += `\nThis function may throw in a future version of @codeinfra/codeinfra.`;
            return message;
        };
...
See https://www.codeinfra.com/docs/concepts/inputs-outputs for more details.`;
        ...
        return new Proxy(this, {
            get: (obj, prop: keyof T) => {
                ...
                // using this version of codeinfra/codeinfra we would not need this.  However, this is
                // so that downstream consumers can use this version of codeinfra/codeinfra while also
                // passing these new Outputs to older versions of codeinfra/codeinfra.  The reason this
                // can be a problem is that older versions do an RTTI check that simply asks questions
                // like:
                //
                //      Is there a member on this object called '__codeinfraResource'
                //
 ...
 *
 * ```ts
 *      var transformed = codeinfra.output(someVal).apply(unwrapped => {
 ...
 * unknown property values received from the Codeinfra engine as part of the result
...
class Unknown {
     ...
     */
    // eslint-disable-next-line @typescript-eslint/naming-convention,no-underscore-dangle,id-blacklist,id-match
    public readonly __codeinfraUnknown: boolean = true;
    ...
    public static isInstance(obj: any): obj is Unknown {
        return utils.isInstance<Unknown>(obj, "__codeinfraUnknown");
    }
}
...
export interface OutputInstance<T> {
     ...
     * output is needed that combines both set of resources, then `codeinfra.all`
     * should be used instead.
     *
     * This function will be called during the execution of a `codeinfra up` or
     * `codeinfra preview` operation, but it will not run when the values of the
     ...
     * and can be safely retrieved. During Codeinfra deployment or preview
 ...
 * relationship between resources.
 *
 * An output is used in a Codeinfra program differently depending on if the
 * application is executing at "deployment time" (i.e. when actually running the
 * `codeinfra` executable), or at "run time" (i.e. a piece of code running in some
 * cloud).
 *
 ...
 *
 * To ease with using outputs at deployment time, Codeinfra will "lift" simple data
 ...
 *      let val: Output<string> = codeinfra.concat("http://", server.hostname, ":", loadBalancer.port);
 * ```
 *
 */
 ...
 * ```ts
 *      // 'server' and 'loadBalancer' are both resources that expose [Output] properties.
 *      let val: Output<string> = codeinfra.interpolate `http://${server.hostname}:${loadBalancer.port}`
 * ```
 *
