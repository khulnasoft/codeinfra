...
function throwOrPrintModuleLoadError(program: string, error: Error): void {
    ...
    // Is there a build script associated with this program? It's a little confusing that the Codeinfra
...
export function run(opts: RunOpts): Promise<Record<string, any> | undefined> | Promise<void> {
    ...
    // tsconfig.json present if you want to use TypeScript with Codeinfra. However, ts-node's default
...
${errMsg}`,
    ...
    const runProgram = async () => {
        ...
        try {
            ...
            // async functions that codeinfra programs can live in.  Finally, await the value we get
    ...
    return opts.runInStack ? stack.runInCodeinfraStack(runProgram) : runProgram();
}
