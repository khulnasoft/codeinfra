...
func TestArgumentConstruction(t *testing.T) {
	...
	t.Run("DryRun-NoArguments", func(t *testing.T) {
		...
		rr := &codeinfrarpc.RunRequest{DryRun: true, Info: info}
	...
	t.Run("OptionalArgs-PassedIfSpecified", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		rr := &codeinfrarpc.RunRequest{Project: "foo", Info: info}
	...
	t.Run("OptionalArgs-NotPassedIfNotSpecified", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		rr := &codeinfrarpc.RunRequest{Info: info}
		args := strings.Join(host.constructArguments(rr, "", "", ""), " ")
...
func TestConfig(t *testing.T) {
	...
	t.Run("Config-Empty", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		rr := &codeinfrarpc.RunRequest{Project: "foo"}
...
func TestGetRequiredPackages(t *testing.T) {
	...
	files := []struct {
		...
		{
			filepath.Join(dir, "node_modules", "@codeinfra", "foo", "package.json"),
			`{ "name": "@codeinfra/foo", "version": "1.2.3", "codeinfra": { "resource": true } }`,
		},
		{
			filepath.Join(dir, "node_modules", "@codeinfra", "bar", "package.json"),
			`{ "name": "@codeinfra/bar", "version": "4.5.6", "codeinfra": { "resource": true } }`,
		},
		{
			filepath.Join(dir, "node_modules", "@codeinfra", "baz", "package.json"),
			`{ "name": "@codeinfra/baz", "version": "4.5.6", "codeinfra": { "resource": false } }`,
		},
		{
	...
	host := &nodeLanguageHost{}
	resp, err := host.GetRequiredPackages(context.Background(), &codeinfrarpc.GetRequiredPackagesRequest{
		Info: &codeinfrarpc.ProgramInfo{
			RootDirectory:    dir,
...
func TestGetRequiredPackagesNestedPolicyPack(t *testing.T) {
	...
	files := []struct {
		...
		{
			filepath.Join(dir, "policy", "CodeinfraPolicy.yaml"),
			`name: my-policy`,
		},
		{
			filepath.Join(dir, "policy", "node_modules", "@codeinfra", "baz", "package.json"),
			`{ "name": "@codeinfra/baz", "version": "7.8.9", "codeinfra": { "resource": true } }`,
		},
	}
	for _, file := range files {
		err := os.MkdirAll(filepath.Dir(file.path), 0o755)
...
func TestGetProgramDependencies(t *testing.T) {
	...
	t.Run("With no package.json, no lock files", func(t *testing.T) {
		...
		host := &nodeLanguageHost{}
		_, err := host.GetProgramDependencies(context.Background(), &codeinfrarpc.GetProgramDependenciesRequest{
			Program: testDir,
			Info: &codeinfrarpc.ProgramInfo{
	...
	t.Run("With package.json in project root, no lock files", func(t *testing.T) {
		...
		testDir := setupFiles(t, []filePathAndContents{
			{
				path:    "package.json",
				content: `{ "name": "@codeinfra/baz", "dependencies": { "@codeinfra/codeinfra": "^3.113.0" } }`,
			},
			{
				path:    "Codeinfra.yaml",
				content: `name: test`,
	...
	t.Run("With package.json and yarn.lock in project root", func(t *testing.T) {
		...
		testDir := setupFiles(t, []filePathAndContents{
			...
			{
				path: "yarn.lock",
				content: `"@codeinfra/codeinfra@^3.0.0", "@codeinfra/codeinfra@^3.113.0":
  version "3.131.0"
  resolved "https://registry.yarnpkg.com/@codeinfra/codeinfra/-/codeinfra-3.131.0.tgz#6233e5ee5e72907b99415b32be6a9ebf9041f096"
  ...
  dependencies:
    ...
    "@npmcli/arborist" "^7.3.1"`,
		...
		host := &nodeLanguageHost{}
		resp, err := host.GetProgramDependencies(context.Background(), &codeinfrarpc.GetProgramDependenciesRequest{
			Program: testDir,
			Info: &codeinfrarpc.ProgramInfo{
				RootDirectory:    testDir,
				ProgramDirectory: testDir,
		...
		require.Equal(t, resp.Dependencies[0].Name, "@codeinfra/codeinfra")
	...
	t.Run("With package.json and yarn.lock in parent dir", func(t *testing.T) {
		...
		testDir := setupFiles(t, []filePathAndContents{
			...
			{
				path:    filepath.Join("subdir", "Codeinfra.yaml"),
				content: `name: test`,
			},
			{
				path: "yarn.lock",
...
func TestNonblockingStdout(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/16503
	t.Parallel()
