...
function usage(): void {
    ...
    console.error(`        --query-mode        true to run codeinfra in query mode`);
...
function main(args: string[]): void {
    ...
    // Due to node module loading semantics, multiple copies of @codeinfra/codeinfra could be loaded at runtime. So we need
    ...
    addToEnvIfDefined("CODEINFRA_NODEJS_ORGANIZATION", argv["organization"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_PROJECT", argv["project"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_STACK", argv["stack"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_DRY_RUN", argv["dry-run"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_QUERY_MODE", argv["query-mode"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_PARALLEL", argv["parallel"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_MONITOR", argv["monitor"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_ENGINE", argv["engine"]);
    addToEnvIfDefined("CODEINFRA_NODEJS_SYNC", argv["sync"]);
    ...
    v8Hooks.isInitializedAsync().then(() => {
        ...
        // when the user's program completes successfully, set programRunning back to false.  That way, if the Codeinfra
