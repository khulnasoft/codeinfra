 ...
 * @param programPath the path to the Codeinfra program; this is the project "main" directory,
...
async function npmPackageRootFromProgramPath(programPath: string): Promise<string> {
    ...
    if (pkgDir === undefined) {
        log.warn(
            "Could not find a package.json file for the program. Using the Codeinfra program directory as the project root.",
        );
...
async function throwOrPrintModuleLoadError(program: string, error: Error): Promise<void> {
    ...
    // Codeinfra CLI doesn't run build scripts before running the program so call that out
    // explicitly.
    ...
    if ("build" in scripts) {
        ...
        console.error(
            "Codeinfra does not run build scripts before running your program. " +
    ...
    console.error("  * Codeinfra encountered an unexpected error.");
...
function tracingIsEnabled(tracingUrl: string | boolean): boolean {
    ...
    const experimental = process.env["CODEINFRA_EXPERIMENTAL"] ?? "";
...
export async function run(
    ...
    const typeScript: boolean = process.env["CODEINFRA_NODEJS_TYPESCRIPT"] === "true";

    // We provide reasonable defaults for many ts options, meaning you don't need to have a tsconfig.json present
    // if you want to use TypeScript with Codeinfra. However, ts-node's default behavior is to walk up from the cwd to
    ...
    // This helps with cases like codeinfra/codeinfra#1772.
    const defaultTsConfigPath = "tsconfig.json";
    const tsConfigPath: string = process.env["CODEINFRA_NODEJS_TSCONFIG_PATH"] ?? defaultTsConfigPath;
    ...
    if (typeScript) {
        ...
        //
        // If the `CODEINFRA_NODEJS_TRANSPILE_ONLY `env variable is set, we use
        ...
        const transpileOnlyEnv = process.env["CODEINFRA_NODEJS_TRANSPILE_ONLY"];
        if (transpileOnlyEnv) {
        ...
        tsn.register({
            ...
            transpileOnly,
            // CODEINFRA_NODEJS_TSCONFIG_PATH might be set to a config file such as "tsconfig.codeinfra.yaml" which
...
${defaultErrorMessage(err)}`,
    ...
    const runProgram = async () => {
        ...
        try {
            ...
            // If there is no entrypoint set in Codeinfra.yaml via the main
            ...
            if (await containsTSAndJSModules(program)) {
                log.warn(
                    "Found a TypeScript project containing an index.js file and no explicit entrypoint in Codeinfra.yaml - Codeinfra will use index.js",
                );
            }
            ...
            // exported top level async functions that codeinfra programs can live in.  Finally, await
    ...
    const stackOutputs = await stack.runInCodeinfraStack(runProgram);
