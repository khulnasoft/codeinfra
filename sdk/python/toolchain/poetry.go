...
func newPoetry(directory string) (*poetry, error) {
	...
	if err != nil {
		return nil, errors.New("Could not find `poetry` executable.\n" +
			"Install poetry and make sure is is in your PATH, or set the toolchain option in Codeinfra.yaml to `pip`.")
	}
...
func dependenciesFromRequirementsTxt(r io.Reader) (map[string]string, error) {
	...
	for scanner.Scan() {
		...
		// find the version specififer: "codeinfra>=3.0.0,<4.0.0" -> ">=3.0.0,<4.0.0".
