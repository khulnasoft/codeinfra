...
func TestUvVirtualenvPath(t *testing.T) {
	...
	t.Run("no virtualenv specified, in a subfolder", func(t *testing.T) {
		t.Parallel()
		root := t.TempDir()
		codeinfraRoot := filepath.Join(root, "subfolder")
		require.NoError(t, os.WriteFile(filepath.Join(root, "uv.lock"), []byte{}, 0o600))
		require.NoError(t, os.Mkdir(codeinfraRoot, 0o755))

		uv, err := newUv(codeinfraRoot, "")
		require.NoError(t, err)
	...
	t.Run("no virtualenv specified, in a subfolder", func(t *testing.T) {
		...
		root := t.TempDir()
		codeinfraRoot := filepath.Join(root, "subfolder")
		require.NoError(t, os.Mkdir(codeinfraRoot, 0o700))
		require.NoError(t, os.WriteFile(filepath.Join(root, "uv.lock"), []byte{}, 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(codeinfraRoot, "uv.lock"), []byte{}, 0o600))

		uv, err := newUv(codeinfraRoot, "")
		require.NoError(t, err)
		require.Equal(t, filepath.Join(codeinfraRoot, ".venv"), uv.virtualenvPath,
	...
	t.Run("virtualenv option is provided, in  subfolder", func(t *testing.T) {
		...
		uv, err := newUv(codeinfraRoot, "banana")
		require.NoError(t, err)
		require.Equal(t, filepath.Join(codeinfraRoot, "banana"), uv.virtualenvPath, "virtualenv is in the project root")
	})
}
