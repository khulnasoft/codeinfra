...
const (
	windows             = "windows"
	pythonShimCmdFormat = "codeinfra-%s-shim.cmd"
)

type pip struct {
	// The absolute path to the virtual env. Empty if not using a virtual env.
	virtualenvPath string
	// The virtual option as set in Codeinfra.yaml.
...
func (p *pip) EnsureVenv(ctx context.Context, cwd string, useLanguageVersionTools,
	...
	if err != nil {
		...
		return fmt.Errorf("the 'virtualenv' option in Codeinfra.yaml is set to %q but it is not a directory", p.virtualenvPath)
	}
...
// CommandPath finds the correct path and command for Python. If the `CODEINFRA_PYTHON_CMD`
...
func CommandPath() (string /*pythonPath*/, string /*pythonCmd*/, error) {
	...
	if pythonCmd := os.Getenv("CODEINFRA_PYTHON_CMD"); pythonCmd != "" {
...
func NewVirtualEnvError(dir, fullPath string) error {
	...
	return fmt.Errorf("The 'virtualenv' option in Codeinfra.yaml is set to %q, but %q %s; "+
		"run the following commands to create the virtual environment and install dependencies into it:\n\n%s\n\n"+
		"For more information see: https://www.codeinfra.com/docs/intro/languages/python/#virtual-environments",
