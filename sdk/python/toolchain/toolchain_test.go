...
func TestListPackages(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run("non-empty/"+Name(test.opts.Toolchain), func(t *testing.T) {
			...
			opts := copyOptions(test.opts)
			opts.Root = t.TempDir()
			createVenv(t, opts, "codeinfra-random")
			...
			// codeinfra_random depends on codeinfra, which has pip as a dependency.
			...
			for _, pkg := range append([]string{"codeinfra_random"}, test.expectedPackages...) {
		...
		t.Run("non-empty-with-pip/"+Name(test.opts.Toolchain), func(t *testing.T) {
			...
			opts := copyOptions(test.opts)
			opts.Root = t.TempDir()
			createVenv(t, opts, "codeinfra-random", "pip")
...
[tool.poetry]
name = "test_codeinfra_venv"
version = "0.1.0"
description = ""
authors = []
readme = "README.md"
package-mode = false
packages = [{include = "test_codeinfra_venv"}]
