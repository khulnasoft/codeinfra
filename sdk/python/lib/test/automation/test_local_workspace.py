...
from codeinfra import Config, export
from codeinfra.automation import (
    create_stack,
...
from codeinfra.resource import (
    ComponentResource,
    CustomResource,
    ResourceOptions,
)
...
class TestLocalWorkspace(unittest.TestCase):
    def test_project_settings(self):
        for ext in extensions:
            ...
            self.assertEqual(settings.description, "A minimal Go Codeinfra program")
    ...
    def test_tag_methods(self):
        if os.getenv("CODEINFRA_ACCESS_TOKEN") is None:
            self.skipTest(
        ...
        self.assertEqual(result["codeinfra:project"], project_name)
        self.assertEqual(result["codeinfra:runtime"], runtime)
        ...
        result = ws.get_tag(stack_name, "codeinfra:project")
        self.assertEqual(result, project_name)

        ws.remove_stack(stack_name)

    def test_list_stacks(self):
        mock_with_returned_stacks = CodeinfraCommand()
    ...
    def test_list_all_stacks(self):
        ...
        self.assertEqual(stacks[1].current, False)
        self.assertEqual(
            stacks[1].url, "https://app.codeinfra.com/testorg1/testproj2/teststack2"
        )
    ...
    def test_stack_lifecycle_inline_program(self):
        ...
        stack_name = stack_namer(project_name)
        stack = create_stack(
            stack_name, program=codeinfra_program, project_name=project_name
        )
    ...
    def test_stack_lifecycle_inline_program_remove_without_destroy(self):
        ...
        stack_name = stack_namer(project_name)
        stack = create_stack(
            stack_name, program=codeinfra_program_with_resource, project_name=project_name
        )
    ...
    def test_stack_lifecycle_async_inline_program(self):
        ...
        stack_name = stack_namer(project_name)
        stack = create_stack(
            stack_name, program=async_codeinfra_program, project_name=project_name
        )
    ...
    def test_stack_lifecycle_inline_program_with_exclude_protected(self):
        ...
        try:
            ...
            # codeinfra destroy with exclude protected
            ...
            # codeinfra destroy without exclude protected
            destroy_res = stack.destroy()
            self.assertEqual(destroy_res.summary.kind, "destroy")
            self.assertEqual(destroy_res.summary.result, "succeeded")
    ...
    def test_codeinfra_version(self):
        ws = LocalWorkspace()
        self.assertIsNotNone(ws.codeinfra_version)
        self.assertRegex(ws.codeinfra_version, r"(\d+\.)(\d+\.)(\d+)(-.*)?")
    ...
    def test_codeinfra_command(self):
        p = CodeinfraCommand()
        ws = LocalWorkspace(codeinfra_command=p)
        self.assertIsNotNone(ws.codeinfra_version)
        self.assertRegex(ws.codeinfra_version, r"(\d+\.)(\d+\.)(\d+)(-.*)?")
        self.assertEqual(p.version, ws.codeinfra_command.version)
    ...
    @unittest.skip(
        "Temporarily skipping test until we've re-enabled the warning - codeinfra/codeinfra#7127"
    )
    ...
    def test_install(self):
        class MockCmd(CodeinfraCommand):
        ...
        mock_cmd = MockCmd()
        ws = LocalWorkspace(codeinfra_command=mock_cmd)
        ws.install()
    ...
    def test_pytest_raises_does_not_hang(self):
        ...
        # https://github.com/khulnasoft/codeinfra/issues/17133 where a test failure
...
def codeinfra_program_with_resource():
    class MyComponentResource(ComponentResource):
        def __init__(
            self,
            name: str,
...
async def async_codeinfra_program():
