...
# Regresses [codeinfra/codeinfra#8633]: sequential operations like
...
import codeinfra
from codeinfra import automation


class BadResource(codeinfra.CustomResource):
    def __init__(
        self, resource_name: str, opts: typing.Optional[codeinfra.ResourceOptions] = None
    ):
        if opts is None:
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="TODO[codeinfra/codeinfra#8716] fails on Windows"
)
def test_isolation():
    check_isolation()
