...
from codeinfra.automation._remote_workspace import _is_fully_qualified_stack_name
...
# These tests require the service with access to Codeinfra Deployments.
# Set CODEINFRA_ACCESS_TOKEN to an access token with access to Codeinfra Deployments
# and set CODEINFRA_TEST_DEPLOYMENTS_API to any value to enable the tests.
@pytest.mark.parametrize(
    "factory",
    [
        create_remote_stack_git_source,
...
@pytest.mark.skipif(
    "CODEINFRA_ACCESS_TOKEN" not in os.environ, reason="CODEINFRA_ACCESS_TOKEN not set"
)
@pytest.mark.skipif(
    "CODEINFRA_TEST_DEPLOYMENTS_API" not in os.environ,
    reason="CODEINFRA_TEST_DEPLOYMENTS_API not set",
)
def test_remote_workspace_stack_lifecycle(factory):
    ...
    stack = factory(
        ...
        opts=RemoteWorkspaceOptions(
            pre_run_commands=[
                f"codeinfra config set bar abc --stack {stack_name}",
                f"codeinfra config set --secret buzz secret --stack {stack_name}",
            ],
