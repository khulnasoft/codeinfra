...
from codeinfra.automation import InvalidVersionError, CodeinfraCommand
from codeinfra.automation._cmd import _fixup_path, _parse_and_validate_codeinfra_version


def test_install_default_root():
    requested_version = VersionInfo.parse("3.101.0")

    p = CodeinfraCommand.install(version=requested_version)

    codeinfraBin = (
        Path.home() / ".codeinfra" / "versions" / str(requested_version) / "bin" / "codeinfra"
    )
    try:
        codeinfraBin.stat()
    except Exception as exception:
        pytest.fail(f"did not find codeinfra binary: {exception}")
    assert p.version == "3.101.0"
    out = subprocess.check_output([codeinfraBin, "version"])
...
def test_install_twice():
    with tempfile.TemporaryDirectory(prefix="automation-test-") as root:
        requested_version = VersionInfo.parse("3.101.0")
        codeinfraBin = Path(root) / "bin" / "codeinfra"

        CodeinfraCommand.install(version=requested_version, root=root)
        stat1 = codeinfraBin.stat()

        CodeinfraCommand.install(version=requested_version, root=root)
        stat2 = codeinfraBin.stat()
...
def test_incompatible_version():
    with tempfile.TemporaryDirectory(prefix="automation-test-") as root:
        ...
        CodeinfraCommand.install(version=installed_version, root=root)
        requested_version = VersionInfo.parse("3.101.0")
        # Try getting an incompatible version
        try:
            CodeinfraCommand(root=root, version=requested_version)
        ...
        CodeinfraCommand(root=root, version=requested_version, skip_version_check=True)


def test_fixup_env():
    env = {"PATH": "/usr/bin", "SOME_VAR": "some value"}
    new_env = _fixup_path(env, "/tmp/codeinfra-install/bin")
    if os.name == "nt":
        assert new_env["PATH"] == "/tmp/codeinfra-install/bin;/usr/bin"
    else:
        assert new_env["PATH"] == "/tmp/codeinfra-install/bin:/usr/bin"
...
PARSE = "Could not parse the Codeinfra CLI"
version_tests = [
...
class TestParseAndValidateCodeinfraVersion(unittest.TestCase):
    def test_validate_codeinfra_version(self):
            ...
            with self.subTest():
                if expected_error:
                    with self.assertRaisesRegex(
                        ...
                        _parse_and_validate_codeinfra_version(
