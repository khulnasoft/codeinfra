...
import codeinfra
from codeinfra import UNKNOWN, Input, Output, input_type
...
def codeinfra_test(coro):
    wrapped = codeinfra.runtime.test(coro)
...
class NextSerializationTests(unittest.TestCase):
    ...
    async def test_remote_asset(self):
        asset = RemoteAsset("https://codeinfra.com")
        prop = await rpc.serialize_property(asset, [], None)
        self.assertEqual(rpc._special_asset_sig, prop[rpc._special_sig_key])
        self.assertEqual("https://codeinfra.com", prop["uri"])
    ...
    async def test_remote_archive(self):
        asset = RemoteArchive("https://codeinfra.com")
        prop = await rpc.serialize_property(asset, [], None)
...
@input_type
class FooArgs:
    first_arg: Input[str] = codeinfra.property("firstArg")
    second_arg: Optional[Input[float]] = codeinfra.property("secondArg")
...
class ListDictInputArgs:
    ...
    def __init__(
        ...
        codeinfra.set(self, "a", a)
        codeinfra.set(self, "b", b)
        codeinfra.set(self, "c", c)
        codeinfra.set(self, "d", d)


@input_type
class BarArgs:
    tag_args: Input[dict] = codeinfra.property("tagArgs")

    def __init__(self, tag_args: Input[dict]):
        codeinfra.set(self, "tag_args", tag_args)
...
class SomeFooArgs:
    def __init__(self, the_first: str, the_second: Mapping[str, str]):
        codeinfra.set(self, "the_first", the_first)
        codeinfra.set(self, "the_second", the_second)

    @property
    @codeinfra.getter(name="theFirst")
    def the_first(self) -> str: ...  # type: ignore

    @property
    @codeinfra.getter(name="theSecond")
...
class SerializationArgs:
    def __init__(
        self,
        some_value: codeinfra.Input[str],
        some_foo: codeinfra.Input[codeinfra.InputType[SomeFooArgs]],
        some_bar: codeinfra.Input[
            Mapping[str, codeinfra.Input[codeinfra.InputType[SomeFooArgs]]]
        ],
    ):
        codeinfra.set(self, "some_value", some_value)
        codeinfra.set(self, "some_foo", some_foo)
        codeinfra.set(self, "some_bar", some_bar)

    @property
    @codeinfra.getter(name="someValue")
    def some_value(self) -> codeinfra.Input[str]: ...  # type: ignore

    @property
    @codeinfra.getter(name="someFoo")
    def some_foo(self) -> codeinfra.Input[codeinfra.InputType[SomeFooArgs]]: ...  # type: ignore

    @property
    @codeinfra.getter(name="someBar")
    def some_bar(  # type: ignore
        self,
    ) -> codeinfra.Input[Mapping[str, codeinfra.Input[codeinfra.InputType[SomeFooArgs]]]]: ...
