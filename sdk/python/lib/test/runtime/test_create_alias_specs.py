import pytest
from codeinfra.runtime.resource import create_alias_spec
from codeinfra.resource import Alias
...
async def test_create_alias_spec_with_parent():
    alias = Alias(parent="codeinfra:codeinfra:Stack")
    alias_spec = await create_alias_spec(alias)
    assert alias_spec is not None
    assert alias_spec.noParent is False
    assert alias_spec.parentUrn == "codeinfra:codeinfra:Stack"
