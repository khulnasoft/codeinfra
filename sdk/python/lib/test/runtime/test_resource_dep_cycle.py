...
def test_codeinfra_does_not_hang_on_dependency_cycle(my_mocks):
    c = MockComponentResource(name="c")
    r = MockResource(name="r", input1=c.output1, opts=codeinfra.ResourceOptions(parent=c))
    return codeinfra.Output.all(c.urn, r.urn).apply(print)
