...
import codeinfra
from codeinfra import InvokeOptions, InvokeOutputOptions
from codeinfra.output import UNKNOWN
from codeinfra.resource import Resource


def unknown() -> codeinfra.Output[Any]:
    ...
    value_fut.set_result(codeinfra.UNKNOWN)
    return codeinfra.Output(set(), value_fut, is_known_fut, is_secret_fut)
...
class MockComponentResource(codeinfra.ComponentResource):
    def __init__(self, name: str, opts: Optional[codeinfra.ResourceOptions] = None):
...
def test_invoke_empty_return(tok: str, version: str, empty: bool, expected) -> None:
    codeinfra.runtime.mocks.set_mocks(MyMocks())

    props = {"empty": True} if empty else {}
    opts = codeinfra.InvokeOptions(version=version) if version else None
    assert codeinfra.runtime.invoke(tok, props, opts).value == expected
...
async def test_invoke_depends_on() -> None:
    ...
    dep_output = codeinfra.Output.from_input(dep_future)
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        opts = codeinfra.InvokeOutputOptions(depends_on=[dep_output])
        o = codeinfra.runtime.invoke_output("test::MyFunction", {}, opts)
...
async def test_invoke_depends_on_component() -> None:
    ...
    MockResource(name="r", opts=codeinfra.ResourceOptions(parent=dep))

    opts = codeinfra.InvokeOutputOptions(depends_on=[dep])
    o = codeinfra.runtime.invoke_output("test::MyFunction", {}, opts)
    v = await o.future()
...
async def test_invoke_depends_on_unknown_component_child() -> None:
    ...
    dep = MockResource(name="dep", opts=codeinfra.ResourceOptions(parent=comp))
    dep.__dict__["id"] = unknown()

    opts = codeinfra.InvokeOutputOptions(depends_on=[comp])
    o = codeinfra.runtime.invoke_output("test::MyFunction", {}, opts)
    k = await o.is_known()
    assert k == False
...
async def test_invoke_input_dependency() -> None:
    ...
    arg_with_resource_dependency = codeinfra.Output(
        resources={dep},
        future=arg_value,
        is_known=is_known,
        is_secret=is_secret,
    ...
    o = codeinfra.runtime.invoke_output(
