...
from codeinfra.runtime import rpc, rpc_manager, settings
from codeinfra.runtime._serialization import (
    _deserialize,
    _serialize,
)
...
class OutputFromInputTests(unittest.TestCase):
    ...
    class FooArgs:
        def __init__(
            ...
            *,
            ...
            bar: Optional[codeinfra.Input[Sequence[codeinfra.Input[str]]]] = None,
            baz: Optional[codeinfra.Input[Mapping[str, codeinfra.Input[str]]]] = None,
            nested: Optional[
                codeinfra.Input[codeinfra.InputType["OutputFromInputTests.NestedArgs"]]
            ] = None,
        ):
            if foo is not None:
                codeinfra.set(self, "foo", foo)
            if bar is not None:
                codeinfra.set(self, "bar", bar)
            if baz is not None:
                codeinfra.set(self, "baz", baz)
        ...
        @property
        @codeinfra.getter
        def bar(self) -> Optional[codeinfra.Input[Sequence[codeinfra.Input[str]]]]:
            return codeinfra.get(self, "bar")

        @property
        @codeinfra.getter
        def baz(self) -> Optional[codeinfra.Input[Mapping[str, codeinfra.Input[str]]]]:
    ...
    @codeinfra.input_type
    class NestedArgs:
        def __init__(self, *, hello: Optional[codeinfra.Input[str]] = None):
            if hello is not None:
                codeinfra.set(self, "hello", hello)

        @property
        @codeinfra.getter
        def hello(self) -> Optional[codeinfra.Input[str]]:
...
This function may throw in a future version of Codeinfra.""",
        )
...
class OutputApplyTests(unittest.TestCase):
    async def test_apply_always_sets_is_secret_and_is_known(self):
        ...
        with self.assertRaises(Exception):
            # The overall test will fail with ! because codeinfra_test awaits all outputs
            test()
...
class DeferredOutputTests(unittest.TestCase):
    ...
    async def test_deferred_output(self):
        output, resolve = codeinfra.deferred_output()
