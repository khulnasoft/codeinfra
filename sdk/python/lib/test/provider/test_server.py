...
import codeinfra.output
import pytest
from google.protobuf import struct_pb2
from codeinfra.provider.server import ProviderServicer
from codeinfra.resource import CustomResource, ResourceOptions
from codeinfra.runtime import Mocks, ResourceModule, proto, rpc, rpc_manager
from codeinfra.runtime.proto.provider_pb2 import ConstructRequest
from codeinfra.runtime.settings import Settings, configure
...
async def test_construct_inputs_preserves_unknowns():
    ...
    inputs = await ProviderServicer._construct_inputs(req.inputs, req.inputDependencies)  # pylint: disable=no-member
    assert len(inputs) == 1
    assert isinstance(inputs["echo"], codeinfra.output.Unknown)
...
class TestMocks(Mocks):
    def call(self, args: codeinfra.runtime.MockCallArgs) -> Any:
        raise Exception(f"unknown function {args.token}")

    def new_resource(
        self, args: codeinfra.runtime.MockResourceArgs
...
def assert_output_equal(
    ...
    async def check(actual: Any):
        assert isinstance(actual, codeinfra.Output)
...
test_urn = "urn:codeinfra:stack::project::test:index:MockResource::name"
test_id = "name_id"
...
class UnmarshalOutputTestCase:
    ...
    async def run(self):
        codeinfra.runtime.set_mocks(TestMocks(), "project", "stack", True)
        codeinfra.runtime.register_resource_module("test", "index", TestModule())
        # This registers the resource purely for the purpose of the test.
        codeinfra.runtime.settings.get_monitor().resources[test_urn] = (
            codeinfra.runtime.mocks.MockMonitor.ResourceRegistration(
...
deserialization_tests = [
    ...
    UnmarshalOutputTestCase(
        ...
        deps=["fakeURN"],
        assert_=Assert(
            lambda actual: not isinstance(actual, codeinfra.Output),
            lambda actual: assert_output_equal(None, False, False, ["fakeURN"])(
                actual["foo"]
            ),
        ),
