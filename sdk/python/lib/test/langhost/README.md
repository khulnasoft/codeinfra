...
3. Write your Codeinfra program in `__main__.py`. If you want to do assertions, use the `assert`
keyword to do so.
...
of the top-level Stack resource, `codeinfra:codeinfra:Stack`, because it is annoying to write tests around.
All expected resource counts do not count this resource as a registration and overridden resource monitor
methods will never see a registration for `codeinfra:codeinfra:Stack.`
