...
from codeinfra.runtime import proto, rpc
...
class LanghostMockResourceMonitor(proto.ResourceMonitorServicer):
    ...
    def RegisterResource(self, request, context):
        ...
        if type_ != "codeinfra:codeinfra:Stack":
...
class LanghostTest(unittest.TestCase):
    ...
    Codeinfra Engine gRPC endpoint and running the language host exactly the same
    way that the Codeinfra CLI will. Once the program completes and the language host
    ...
    def _create_language_host(self, port):
        exec_path = path.join(
            ...
            "..",
            "cmd",
            "codeinfra-language-python-exec",
        )
        proc = subprocess.Popen(
            [
                "codeinfra-language-python",
                "--use-executor",
                exec_path,
                "localhost:%d" % port,
            ],
