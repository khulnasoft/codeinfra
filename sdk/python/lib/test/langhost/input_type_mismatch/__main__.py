...
class MyResourceArgs:
    def __init__(self, policy: codeinfra.Input[str]):
        codeinfra.set(self, "policy", policy)

    @property
    @codeinfra.getter
    def policy(self) -> codeinfra.Input[str]:
        return codeinfra.get(self, "policy")
...
class MyResource(codeinfra.CustomResource):
    ...
    @property
    @codeinfra.getter
    def policy(self) -> codeinfra.Output[str]:
        return codeinfra.get(self, "policy")
...
codeinfra.export("r1.policy", r1.policy)
codeinfra.export("r2.policy", r2.policy)
