...
from codeinfra.resource import DependencyProviderResource
from codeinfra.runtime import settings, mocks
from codeinfra.runtime.proto import resource_pb2
from codeinfra import ResourceOptions
from codeinfra.runtime.rpc import ERROR_ON_DEPENDENCY_CYCLES_VAR
import codeinfra
...
class DependencyProviderResourceTests(unittest.TestCase):
    def test_get_package(self):
        res = DependencyProviderResource(
            "urn:codeinfra:stack::project::codeinfra:providers:aws::default_4_13_0"
        )
...
def test_depends_on_accepts_outputs(dep_tracker):
    ...
    res = MockResource(name="res", opts=codeinfra.ResourceOptions(depends_on=[out]))
    ...
    return codeinfra.Output.all(dep1.urn, dep2.urn, res.urn).apply(check)
...
def test_depends_on_outputs_works_in_presence_of_unknowns(dep_tracker_preview):
    ...
    res = MockResource(
        name="res", opts=codeinfra.ResourceOptions(depends_on=[known, unknown])
    )
    ...
    return codeinfra.Output.all(dep1.urn, res.urn).apply(check)
...
def out(x: T) -> codeinfra.Output[T]:
    return codeinfra.Output.from_input(x)


def depends_on_variations(dep: codeinfra.Resource) -> List[codeinfra.ResourceOptions]:
    return [
        codeinfra.ResourceOptions(depends_on=None),
        codeinfra.ResourceOptions(depends_on=dep),
        codeinfra.ResourceOptions(depends_on=[dep]),
        codeinfra.ResourceOptions(depends_on=promise(dep)),
        codeinfra.ResourceOptions(depends_on=out(dep)),
        codeinfra.ResourceOptions(depends_on=promise([dep])),
        codeinfra.ResourceOptions(depends_on=out([dep])),
        codeinfra.ResourceOptions(depends_on=promise([promise(dep)])),
        codeinfra.ResourceOptions(depends_on=promise([out(dep)])),
        codeinfra.ResourceOptions(depends_on=out([promise(dep)])),
        codeinfra.ResourceOptions(depends_on=out([out(dep)])),
    ]


@codeinfra.runtime.test
def test_depends_on_typing_variations(dep_tracker) -> None:
    dep: codeinfra.Resource = MockResource(name="dep1")
    ...
    def check_opts(i, name, opts):
        ...
        return codeinfra.Output.all(dep.urn, res.urn).apply(lambda urns: check(i, urns))
...
def test_depends_on_typechecks_sync():
    # https://github.com/khulnasoft/codeinfra/issues/13917
    try:
        res = MockResource(
            name="res", opts=codeinfra.ResourceOptions(depends_on=["hello"])
        )
...
def test_depends_on_typechecks_async():
    ...
    def test():
        ...
        res = MockResource(name="res", opts=codeinfra.ResourceOptions(depends_on=[dep]))
...
def test_component_resource_propagates_provider() -> None:
    ...
    provider = codeinfra.ProviderResource("test", "prov", {})
    component = codeinfra.ComponentResource(
        "custom:foo:Component",
        "comp",
        opts=codeinfra.ResourceOptions(provider=provider),
    )
    custom = codeinfra.CustomResource(
        "test:index:Resource",
        "res",
        opts=codeinfra.ResourceOptions(parent=component),
    )
...
def test_component_resource_propagates_providers_list() -> None:
    ...
    component = codeinfra.ComponentResource(
        "custom:foo:Component",
        "comp",
        opts=codeinfra.ResourceOptions(providers=[provider]),
    )
    custom = codeinfra.CustomResource(
        "test:index:Resource",
        "res",
...
def output_depending_on_resource(
    r: codeinfra.Resource, isKnown: bool
) -> codeinfra.Output[None]:
    """Returns an output that depends on the given resource."""
    o = codeinfra.Output.from_input(None)
    is_known_fut: asyncio.Future[bool] = asyncio.Future()
    is_known_fut.set_result(isKnown)

    return codeinfra.Output(resources=set([r]), is_known=is_known_fut, future=o.future())
...
class MinimalMocks(codeinfra.runtime.Mocks):
...
class MockResource(codeinfra.CustomResource):
...
# Regression test for https://github.com/khulnasoft/codeinfra/issues/12032
@codeinfra.runtime.test
def test_parent_and_depends_on_are_the_same_12032():
    os.environ[ERROR_ON_DEPENDENCY_CYCLES_VAR] = "false"
    mocks.set_mocks(MinimalMocks())

    parent = codeinfra.ComponentResource("pkg:index:first", "first")
    child = codeinfra.ComponentResource(
        "pkg:index:second",
        "second",
        opts=codeinfra.ResourceOptions(parent=parent, depends_on=[parent]),
    )
    ...
    codeinfra.CustomResource(
        "foo:bar:baz",
        "myresource",
        opts=codeinfra.ResourceOptions(parent=child),
    )


# Regression test for https://github.com/khulnasoft/codeinfra/issues/12736
@codeinfra.runtime.test
def test_complex_parent_child_dependencies():
    ...
    class B(codeinfra.ComponentResource):
        def __init__(self, name: str, opts=None):
    ...
    class C(codeinfra.ComponentResource):
        def __init__(self, name: str, opts=None):
    ...
    class D(codeinfra.ComponentResource):
        def __init__(self, name: str, opts=None):
...
# Regression test for https://github.com/khulnasoft/codeinfra/issues/13997
