...
class CodeinfraCommand:
    """
    CodeinfraCommand manages the Codeinfra CLI. It can be used to to install the CLI and run commands.
    """
    ...
    def __init__(
        ...
        """
        Creates a new CodeinfraCommand.

        :param root: The directory where to look for the Codeinfra installation. Defaults to running `codeinfra` from $PATH.
        :param version: The minimum version of the Codeinfra CLI to use and validates that it is compatbile with this version.
        :param skip_version_check: If true the version validation will be skipped. The env variable
                `CODEINFRA_AUTOMATION_API_SKIP_VERSION_CHECK` also disable this check, and takes precendence. If it is set it
                is not possible to re-enable the validation even if `skip_version_check` is `True`.
        """
        self.command = os.path.join(root, "bin", "codeinfra") if root else "codeinfra"
        ...
        self.version = _parse_and_validate_codeinfra_version(
    ...
    def install(
        ...
        """
        Downloads and installs the Codeinfra CLI.  By default the CLI version
        matching the current SDK release is installed in
        $HOME/.codeinfra/versions/$VERSION. Set `root` to specify a
        different directory, and `version` to install a custom version.

        :param root: The root directory to install the CLI to. Defaults to `~/.codeinfra/versions/<version>`
        ...
        if not root:
            root = os.path.join(
                os.path.expanduser("~"), ".codeinfra", "versions", str(version)
            )
    ...
    def _install_windows(cls, root: str, version: VersionInfo):
        ...
        try:
            _download_to_file("https://get.codeinfra.com/install.ps1", script.name)
    ...
    def _install_posix(cls, root: str, version: VersionInfo):
        ...
        try:
            _download_to_file("https://get.codeinfra.com/install.sh", script.name)
    ...
    def run(
        ...
        """
        Runs a Codeinfra command, returning a CommandResult. If the command fails, a CommandError is raised.

        :param args: The arguments to pass to the Codeinfra CLI, for example `["stack", "ls"]`.
...
def _fixup_path(env: Dict[str, str], codeinfraBin: str) -> Dict[str, str]:
    """
    Fixup path so that we prioritize up the bundled plugins next to the codeinfra binary.
    """
    new_env = dict(env)
    new_env["PATH"] = os.pathsep.join([codeinfraBin, env["PATH"]])
    return new_env


def _parse_and_validate_codeinfra_version(
    ...
    *None* is returned.
    ...
    if version is None:
        raise InvalidVersionError(
            f"Could not parse the Codeinfra CLI version. This is probably an internal error. "
            f"If you are sure you have the correct version, set {_SKIP_VERSION_CHECK_VAR}=true."
        )
    if min_version.major < version.major:
        raise InvalidVersionError(
            f"Major version mismatch. You are using Codeinfra CLI version {version} with "
    ...
    if min_version.compare(version) == 1:
        raise InvalidVersionError(
            ...
            f"Please update the Codeinfra CLI."
        )
    return version
