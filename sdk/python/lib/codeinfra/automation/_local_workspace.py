...
from ._cmd import CommandResult, OnOutput, CodeinfraCommand
...
if TYPE_CHECKING:
    from codeinfra.automation._remote_workspace import RemoteGitAuth
...
class LocalWorkspaceOptions:
    ...
    codeinfra_home: Optional[str] = None
    program: Optional[CodeinfraFn] = None
    ...
    codeinfra_command: Optional[CodeinfraCommand] = None
    ...
    def __init__(
        ...
        self.codeinfra_home = codeinfra_home
        self.program = program
        self.env_vars = env_vars
        self.secrets_provider = secrets_provider
        self.project_settings = project_settings
        self.stack_settings = stack_settings
        self.codeinfra_command = codeinfra_command
...
class LocalWorkspace(Workspace):
    ...
    def __init__(
        ...
        self.codeinfra_command = codeinfra_command or CodeinfraCommand(
    ...
    def codeinfra_version(self) -> str:  # type: ignore
        v = self.codeinfra_command.version
        if v:
            return str(v)
        raise InvalidVersionError("Could not get Codeinfra CLI version")

    def __repr__(self):
        return (
            f"{self.__class__.__name__}(work_dir={self.work_dir!r}, "
            f"program={self.program.__name__ if self.program else None}, "
            f"codeinfra_home={self.codeinfra_home!r}, env_vars={self.env_vars!r}, "
    ...
    def save_project_settings(self, settings: ProjectSettings) -> None:
        ...
        for ext in _setting_extensions:
            test_path = os.path.join(self.work_dir, f"Codeinfra{ext}")
            if os.path.exists(test_path):
                found_ext = ext
                break
        path = os.path.join(self.work_dir, f"Codeinfra{found_ext}")
    ...
    def stack_settings(self, stack_name: str) -> StackSettings:
        ...
        for ext in _setting_extensions:
            path = os.path.join(self.work_dir, f"Codeinfra.{stack_settings_name}{ext}")
    ...
    def save_stack_settings(self, stack_name: str, settings: StackSettings) -> None:
        ...
        for ext in _setting_extensions:
            test_path = os.path.join(
                self.work_dir, f"Codeinfra.{stack_settings_name}{ext}"
            )
            if os.path.exists(test_path):
                found_ext = ext
                break
        path = os.path.join(self.work_dir, f"Codeinfra.{stack_settings_name}{found_ext}")
        with open(path, "w", encoding="utf-8") as file:
            if found_ext == ".json":
    ...
    def add_environments(self, stack_name: str, *environment_names: str) -> None:
        ...
        ver = VersionInfo(3)
        if self.codeinfra_command.version is not None:
            ver = self.codeinfra_command.version
        ...
        if ver >= VersionInfo(3, 95):
            ...
            self._run_codeinfra_cmd_sync(args)
        else:
    ...
    def list_environments(self, stack_name: str) -> List[str]:
        ...
        if ver >= VersionInfo(3, 99):
            result = self._run_codeinfra_cmd_sync(
    ...
    def get_config(
        ...
        result = self._run_codeinfra_cmd_sync(args)
    ...
    def remove_tag(self, stack_name: str, key: str) -> None:
        self._run_codeinfra_cmd_sync(["stack", "tag", "rm", key, "--stack", stack_name])
    ...
    def who_am_i(self) -> WhoAmIResult:
        ...
        if ver >= VersionInfo(3, 58):
            result = self._run_codeinfra_cmd_sync(["whoami", "--json"])
        ...
        result = self._run_codeinfra_cmd_sync(["whoami"])
    ...
    def install(
        ...
        if ver >= VersionInfo(3, 91):
            ...
            self._run_codeinfra_cmd_sync(["install", *args], on_output=on_output)
            return
    ...
    def install_plugin(self, name: str, version: str, kind: str = "resource") -> None:
        self._run_codeinfra_cmd_sync(["plugin", "install", kind, name, version])
    ...
    def list_plugins(self) -> List[PluginInfo]:
        result = self._run_codeinfra_cmd_sync(["plugin", "ls", "--json"])
        json_list = json.loads(result.stdout)
    ...
    def stack_outputs(self, stack_name: str) -> OutputMap:
        masked_result = self._run_codeinfra_cmd_sync(
            ["stack", "output", "--json", "--stack", stack_name]
        )
        plaintext_result = self._run_codeinfra_cmd_sync(
    ...
    def _remote_supported(self) -> bool:
        ...
        result = self._run_codeinfra_cmd_sync(["preview", "--help"])
        help_string = result.stdout.strip()
        return "--remote" in help_string

    def _remote_inherit_settings_supported(self) -> bool:
        # See if `--remote-inherit-settings` is present in `codeinfra preview --help`'s output.
        result = self._run_codeinfra_cmd_sync(["preview", "--help"])
        help_string = result.stdout.strip()
        return "--remote-inherit-settings" in help_string

    def _run_codeinfra_cmd_sync(
        self, args: List[str], on_output: Optional[OnOutput] = None
    ) -> CommandResult:
        envs = {"CODEINFRA_HOME": self.codeinfra_home} if self.codeinfra_home else {}
        if self._remote:
            envs["CODEINFRA_EXPERIMENTAL"] = "true"
        envs = {**envs, **self.env_vars}
        return self.codeinfra_command.run(args, self.work_dir, envs, on_output)
...
def create_stack(
    ...
    """
    Creates a Stack with a LocalWorkspace utilizing the specified inline (in process) Codeinfra program or the local
    Codeinfra CLI program from the specified working dir.
    ...
    **Local Programs**
    ...
    This is a way to create drivers on top of pre-existing Codeinfra programs. This Workspace will pick up any
    available Settings files (Codeinfra.yaml, Codeinfra.[stack].yaml).
...
def select_stack(
    ...
    """
    Selects a Stack with a LocalWorkspace utilizing the specified inline (in process) Codeinfra program or the local
    Codeinfra CLI program from the specified working dir.
...
def create_or_select_stack(
    ...
    """
    Creates or selects an existing Stack with a LocalWorkspace utilizing the specified inline (in process) Codeinfra
    program or the local Codeinfra CLI program from the specified working dir.
...
def _load_project_settings(work_dir: str) -> ProjectSettings:
    for ext in _setting_extensions:
        project_path = os.path.join(work_dir, f"Codeinfra{ext}")
