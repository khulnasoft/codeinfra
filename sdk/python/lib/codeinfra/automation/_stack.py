...
from ._workspace import Workspace, CodeinfraFn, Deployment
...
class ImportResource(TypedDict, total=False):
    ...
    ImportResource represents a resource to import into a stack.
      ...
      - type: The type token of the Codeinfra resource
...
class Stack:
    ...
    def up(
        ...
        :param on_event: A function to process structured events from the Codeinfra event stream.
        ...
        try:
            up_result = self._run_codeinfra_cmd_sync(args, on_output)
    ...
    def preview(
        ...
        :param suppress_outputs: Suppress display of stack outputs (in case they contain sensitive values)
        :param suppress_progress: Suppress display of periodic progress dots
        :param import_file: Save any creates seen during the preview into an import file to use with codeinfra import
        :param attach_debugger: Run the process under a debugger, and pause until a debugger is attached
        ...
        try:
            preview_result = self._run_codeinfra_cmd_sync(args, on_output)
        finally:
            _cleanup(temp_dir, log_watcher_thread, on_exit)
    ...
    def refresh(
        ...
        try:
            refresh_result = self._run_codeinfra_cmd_sync(args, on_output)
        finally:
    ...
    def destroy(
        ...
        try:
            destroy_result = self._run_codeinfra_cmd_sync(args, on_output)
        finally:
            _cleanup(temp_dir, log_watcher_thread)
    ...
    def import_resources(
        ...
        with tempfile.TemporaryDirectory(prefix="codeinfra-import-") as temp_dir:
            ...
            import_result = self._run_codeinfra_cmd_sync(args, on_output)
            summary = self.info(show_secrets and not self._remote)
            generated_code = ""
    ...
    def cancel(self) -> None:
        ...
        """
        self._run_codeinfra_cmd_sync(["cancel", "--yes"])
    ...
    def _run_codeinfra_cmd_sync(
        ...
        envs = {"CODEINFRA_DEBUG_COMMANDS": "true"}
        if self._remote:
            envs = {**envs, "CODEINFRA_EXPERIMENTAL": "true"}
        if self.workspace.codeinfra_home is not None:
            envs = {**envs, "CODEINFRA_HOME": self.workspace.codeinfra_home}
        ...
        result = self.workspace.codeinfra_command.run(
    ...
    def _remote(self) -> bool:
        from codeinfra.automation._local_workspace import LocalWorkspace
