...
class Output(Generic[T_co]):
    """
    Output helps encode the relationship between Resources in a Codeinfra application. Specifically an
    ...
    def future(self, with_unknowns: Optional[bool] = None) -> Awaitable[Optional[T_co]]:
        ...
        # return None. This preserves compatibility with earlier versios of the Codeinfra SDK.
    ...
    def get(self) -> T_co:
        ...
        values will be known and can be safely retrieved. During codeinfra deployment or preview
    ...
    def apply(
        ...
        This function will be called during execution of a `codeinfra up` or `codeinfra preview` request.
    ...
    def __getattr__(self, item: str) -> "Output[Any]":  # type: ignore
        ...
        This is generally fine for most Codeinfra use cases, but it can cause
...
See https://www.codeinfra.com/docs/concepts/inputs-outputs for more details."""
        if os.getenv("CODEINFRA_ERROR_OUTPUT_STRING", "").lower() in ["1", "true"]:
            raise TypeError(msg)
