...
#
#   class FooResource(codeinfra.CustomResource):
#       nested_value: codeinfra.Output[Nested] = codeinfra.property("nestedValue")
#
#       def __init__(self, resource_name, nested_value: codeinfra.InputType[NestedArgs]):
#           super().__init__("my:module:FooResource", resource_name, {"nestedValue": nested_value})
#
#
# The resource declares a single output `nested_value` of type `codeinfra.Output[Nested]` and uses
# `codeinfra.property()` to indicate the Codeinfra property name.
#
# The resource's `__init__()` method accepts a `nested_value` argument typed as
# `codeinfra.InputType[NestedArgs]`, which is an alias for accepting either an input type (in this
...
#       def nested_value(self) -> codeinfra.Output[Nested]:
#           ...
#
#
# Note the `nested_value` property getter function is empty. The `@codeinfra.getter` decorator replaces
...
#       @codeinfra.getter(name="nestedValue")
#       def nested_value(self) -> codeinfra.Output[Nested]:
#           codeinfra.get(self, "nested_value")
#
#
...
#   class NestedArgs:
#       first_arg: codeinfra.Input[str] = codeinfra.property("firstArg")
#       second_arg: Optional[codeinfra.Input[float]] = codeinfra.property("secondArg", default=None)
#
#
# The class is decorated with the `@codeinfra.input_type` decorator, which indicates the class is an
# input type and does some processing of the class (explained below). `NestedArgs` declares two
# inputs (`first_arg` and `second_arg`) and uses type annotations and `codeinfra.property()` to
# specify the types and Codeinfra input property names. An `__init__()` method is automatically added
...
#   @codeinfra.input_type
#   class NestedArgs:
#       def __init__(self, *, first_arg: codeinfra.Input[str], second_arg: Optional[codeinfra.Input[float]] = None):
#           codeinfra.set(self, "first_arg", first_arg)
#           if second_arg is not None:
#               codeinfra.set(self, "second_arg", second_arg)
#
#       @property
#       @codeinfra.getter(name="firstArg")
#       def first_arg(self) -> codeinfra.Input[str]:
#           ...
#
#       @first_arg.setter
#       def first_arg(self, value: codeinfra.Input[str]):
#           ...
#
#       @property
#       @codeinfra.getter(name="secondArg")
#       def second_arg(self) -> Optional[codeinfra.Input[float]]:
#           ...
#
#       @second_arg.setter
#       def second_arg(self, value: Optional[codeinfra.Input[float]]):
#           ...
#
# This latter (more verbose) declaration is equivalent to the former (simpler) declaration;
# the `@codeinfra.input_type` processes the class and transforms the former declaration into the
# latter declaration.
#
...
# Empty getter functions are automatically replaced by the `@codeinfra.getter` decorator with an
# actual implementation, and the `@codeinfra.input_type` decorator will automatically replace any
# empty setter functions associated with a getter decorated with `@codeinfra.getter` with an actual
...
#       @codeinfra.getter(name="firstArg")
#       def first_arg(self) -> codeinfra.Input[str]:
#           return codeinfra.get(self, "first_arg")
#
#       @first_arg.setter
#       def first_arg(self, value: codeinfra.Input[str]):
#           codeinfra.set(self, "first_arg", value)
#
#       @property
#       @codeinfra.getter(name="secondArg")
#       def second_arg(self) -> Optional[codeinfra.Input[float]]:
#           return codeinfra.get(self, "second_arg")
#
#       @second_arg.setter
#       def second_arg(self, value: Optional[codeinfra.Input[float]]):
#           codeinfra.set(self, "second_arg", value)
#
#
...
#   class Nested:
#       first_arg: str = codeinfra.property("firstArg")
#       second_arg: Optional[float] = codeinfra.property("secondArg")
#
#
...
#           ...
#
# An `__init__()` method is added to the class by the `@codeinfra.output_type` decorator (if an
...
# replaced with implementations by the `@codeinfra.getter` decorator.
#
...
# `codeinfra.get` and `codeinfra.set` will call to translate the Codeinfra property name to a translated
...
_CODEINFRA_NAME = "_codeinfra_name"
_CODEINFRA_INPUT_TYPE = "_codeinfra_input_type"
_CODEINFRA_OUTPUT_TYPE = "_codeinfra_output_type"
_CODEINFRA_PYTHON_TO_CODEINFRA_TABLE = "_codeinfra_python_to_codeinfra_table"
_CODEINFRA_DEPRECATED_CALLABLE = "_codeinfra_deprecated_callable"
_TRANSLATE_PROPERTY = "_translate_property"


def is_input_type(cls: type) -> bool:
    return hasattr(cls, _CODEINFRA_INPUT_TYPE)


def is_output_type(cls: type) -> bool:
    return hasattr(cls, _CODEINFRA_OUTPUT_TYPE)
...
class _Property:
    """
    Represents a Codeinfra property. It is not meant to be created outside this module,
...
def property(name: str, *, default: Any = MISSING) -> Any:  # noqa: A001 shadowing builtin
    """
    Return an object to identify Codeinfra properties.

    name is the Codeinfra property name.
    """
...
def _create_py_property(a_name: str, codeinfra_name: str, typ: Any, setter: bool = False):
    """
    ...
    setattr(getter_fn, _CODEINFRA_NAME, codeinfra_name)
    ...
    for base in reversed(cls.__mro__):
        for python_name, v in base.__dict__.items():
            if isinstance(v, builtins.property):
                ...
                codeinfra_name = getattr(prop.fget, _CODEINFRA_NAME, MISSING)
                if codeinfra_name is not MISSING:
                    yield (python_name, cast(str, codeinfra_name), prop)
...
def input_type(cls: Type[T]) -> Type[T]:
    ...
    _process_class(cls, _CODEINFRA_INPUT_TYPE, is_input=True, setter=True)
...
def input_type_py_to_codeinfra_names(input_type_cls: Type) -> Dict[str, str]:
    """
    Returns a dict of Python names to Codeinfra names for the input type.
    """
    assert is_input_type(input_type_cls)
    return {
        python_name: codeinfra_name
        for python_name, codeinfra_name, _ in _py_properties(input_type_cls)
    }


def input_type_types(input_type_cls: type) -> Dict[str, type]:
    """
    Returns a dict of Codeinfra names to types for the input type.
    """
    assert is_input_type(input_type_cls)
...
def input_type_to_dict(obj: Any) -> Dict[str, Any]:
    ...
    The keys of the dict are Codeinfra names that should not be translated.
    """
    cls = type(obj)
    ...
    result: Dict[str, Any] = {}
    for _, codeinfra_name, prop in _py_properties(cls):
        fget = prop.fget

        # If the property has a _codeinfra_deprecated_callable attribute, use that
        ...
        deprecated_callable = prop.fget.__dict__.get(_CODEINFRA_DEPRECATED_CALLABLE)
...
def output_type(cls: Type[T]) -> Type[T]:
    ...
    Python property getters are created for each Codeinfra output property
    ...
    _process_class(cls, _CODEINFRA_OUTPUT_TYPE)

    # If the class has a _translate_property() method, build a mapping table of Python names to
    # Codeinfra names. Calls to codeinfra.get() will then convert the name passed to codeinfra.get() from
    # the Python name to the Codeinfra name, and then pass the Codeinfra name to _translate_property() to
    # convert the Codeinfra name to whatever name _translate_property() returns (which, for our
    # provider codegen, will be the translated name from _tables.CAMEL_TO_SNAKE_CASE_TABLE).
    if hasattr(cls, _TRANSLATE_PROPERTY):
        python_to_codeinfra_table = None
        for python_name, codeinfra_name, _ in _py_properties(cls):
            if python_name != codeinfra_name:
                python_to_codeinfra_table = python_to_codeinfra_table or {}
                python_to_codeinfra_table[python_name] = codeinfra_name
        if python_to_codeinfra_table is not None:
            setattr(cls, _CODEINFRA_PYTHON_TO_CODEINFRA_TABLE, python_to_codeinfra_table)
...
def output_type_from_dict(cls: Type[T], output: Dict[str, Any]) -> T:
    ...
    args = {}
    for python_name, codeinfra_name, _ in _py_properties(cls):
        args[python_name] = output.get(codeinfra_name)
...
def getter(_fn=None, *, name: Optional[str] = None):
    """
    Decorator to indicate a function is a Codeinfra property getter.

    name is the Codeinfra property name. If not set, the name of the function is used.
    """
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        codeinfra_name = name if name is not None else fn.__name__
        if _utils.is_empty_function(fn):
            ...
            fn = get_fn
        setattr(fn, _CODEINFRA_NAME, codeinfra_name)
        return fn
...
def _translate_name(obj: Any, name: str) -> str:
    cls = type(obj)
    if hasattr(cls, _CODEINFRA_OUTPUT_TYPE):
        # If the class has a _translate_property() method we need to do two translations:
        #   1. Translate Python => Codeinfra name.
        #   2. Translate Codeinfra name => result of _translate_property().
        translate = getattr(cls, _TRANSLATE_PROPERTY, None)
        if callable(translate):
            table = getattr(cls, _CODEINFRA_PYTHON_TO_CODEINFRA_TABLE, None)
...
def get(self, name: str) -> Any:
    ...
    if hasattr(cls, _CODEINFRA_INPUT_TYPE) or hasattr(cls, _CODEINFRA_OUTPUT_TYPE):
        if hasattr(cls, _CODEINFRA_OUTPUT_TYPE):
...
def _types_from_py_properties(cls: type) -> Dict[str, type]:
    """
    Returns a dict of Codeinfra names to types for a type.
    """
    from . import Output
    ...
    # Build-up a dictionary of Codeinfra property names to types by looping through all the
    # Python properties on the class that have a getter marked as a Codeinfra property getter,
    ...
    for _, codeinfra_name, prop in _py_properties(cls):
        ...
        if return_hint is not None:
            ...
            if typ is Output:
                continue
            result[codeinfra_name] = typ
    return result


def _codeinfra_to_py_names_from_py_properties(cls: type) -> Dict[str, str]:
    return {
        codeinfra_name: python_name for python_name, codeinfra_name, _ in _py_properties(cls)
    }


def _py_to_codeinfra_names_from_py_properties(cls: type) -> Dict[str, str]:
    return {
        python_name: codeinfra_name for python_name, codeinfra_name, _ in _py_properties(cls)
    }
...
def _types_from_annotations(cls: type) -> Dict[str, type]:
    ...
    Returns a dict of Codeinfra names to types for a type.
    """
    # Get the "Codeinfra properties" from the class's type annotations.
    props = _properties_from_annotations(cls)
    if not props:
        return {}
    ...
    # Return a dictionary of Codeinfra property names to types. Types that are Output[T] and
...
def _names_from_annotations(cls: type) -> Iterator[Tuple[str, str]]:
    ...
    def get_codeinfra_name(a_name: str) -> str:
        default = getattr(cls, a_name, MISSING)
        return default.name if isinstance(default, _Property) else a_name

    for python_name in cls_annotations.keys():
        yield (python_name, get_codeinfra_name(python_name))


def _codeinfra_to_py_names_from_annotations(cls: type) -> Dict[str, str]:
    return {
        codeinfra_name: python_name
        for python_name, codeinfra_name in _names_from_annotations(cls)
    }


def _py_to_codeinfra_names_from_annotations(cls: type) -> Dict[str, str]:
    return dict(_names_from_annotations(cls))


def output_type_types(output_type_cls: type) -> Dict[str, type]:
    """
    Returns a dict of Codeinfra names to types for the output type.
    """
...
def resource_types(resource_cls: type) -> Dict[str, type]:
    """
    Returns a dict of Codeinfra names to types for the resource.
    """
...
def resource_codeinfra_to_py_names(resource_cls: type) -> Dict[str, str]:
    """
    Returns a dict of Codeinfra names to types for the resource.
    """
    # First, get the names from the class's type annotations.
    names_from_annotations = _codeinfra_to_py_names_from_annotations(resource_cls)

    # Next, get the names from the class's Python properties.
    names_from_py_properties = _codeinfra_to_py_names_from_py_properties(resource_cls)

    # Return the merged dictionaries.
    return {**names_from_annotations, **names_from_py_properties}


def resource_py_to_codeinfra_names(resource_cls: type) -> Dict[str, str]:
    """
    Returns a dict of Codeinfra names to types for the resource.
    """
    # First, get the names from the class's type annotations.
    names_from_annotations = _py_to_codeinfra_names_from_annotations(resource_cls)

    # Next, get the names from the class's Python properties.
    names_from_py_properties = _py_to_codeinfra_names_from_py_properties(resource_cls)
