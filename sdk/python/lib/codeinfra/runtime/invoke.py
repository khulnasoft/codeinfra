...
def _requires_legacy_none_return_for_empty_struct(
    ...
    Codeinfra Kubernetes SDK that can't handle empty dicts.
    See https://github.com/khulnasoft/codeinfra/issues/14508.
    """
    ...
    # If we have a version and it's less than or equal to codeinfra-kubernetes 4.5.4, we need the legacy behavior;
...
def _invoke(
    ...
    async def do_invoke() -> Tuple[InvokeResult, Optional[Exception]]:
        ...
        # of an empty dict for empty results if this invoke is for a version of the Codeinfra
