...
async def run_codeinfra_func(func: Callable[[], None]):
    try:
        func()
    finally:
        await wait_for_rpcs()
        ...
        log.debug("run_codeinfra_func completed")
...
class Stack(ComponentResource):
    ...
    def __init__(self, func: Callable[[], Optional[Awaitable[None]]]) -> None:
        ...
        if get_root_resource() is not None:
            raise Exception("Only one root Codeinfra Stack may be active at once")

        # Now invoke the registration to begin creating this resource.
        name = f"{get_project()}-{get_stack()}"
        super().__init__("codeinfra:codeinfra:Stack", name, None, None)
        ...
        try:
            awaitable = func()
            # This _should_ be an awaitable but old codeinfra executors returned modules here, so we need to handle that
...
def massage_complex(attr: Any, seen: List[Any]) -> Any:
    ...
    if isinstance(attr, Resource):
        ...
        # In preview only, we mark the result with "@isCodeinfraResource" to indicate that it is derived
        ...
        return (
            serialized_attr
            if not is_dry_run()
            else {**serialized_attr, "@isCodeinfraResource": True}
        )
...
def register_stack_transformation(t: ResourceTransformation):
    """
    Add a transformation to all future resources constructed in this Codeinfra stack.
    """
...
def register_resource_transform(t: ResourceTransform) -> None:
    """
    Add a transform to all future resources constructed in this Codeinfra stack.
    """
    if not _sync_monitor_supports_transforms():
        raise Exception(
            "The Codeinfra CLI does not support transforms. Please update the Codeinfra CLI."
        )
...
def register_invoke_transform(t: InvokeTransform) -> None:
    """
    Add a transforms to all future invokes called in this Codeinfra stack.
    """

    if not _sync_monitor_supports_invoke_transforms():
        raise Exception(
            "The Codeinfra CLI does not support invoke transforms. Please update the Codeinfra CLI."
        )
