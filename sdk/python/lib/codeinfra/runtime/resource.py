...
async def prepare_resource(
    ...
    elif ty != "codeinfra:codeinfra:Stack":
...
def create_urn(
    ...
    if parent is not None:
        ...
        parent_prefix = Output.from_input("urn:codeinfra:" + stack + "::" + project + "::")
    ...
    async def do_get():
        try:
            ...
            accept_resources = os.getenv(
                "CODEINFRA_DISABLE_RESOURCE_REFERENCES", ""
            ).upper() not in {"TRUE", "1"}
...
def _translate_ignore_changes(
    ...
    if ignore_changes is not None:
        if typ is not None:
            ...
            input_names = _types.input_type_py_to_codeinfra_names(typ)
            ignore_changes = list(
...
def _translate_additional_secret_outputs(
    ...
    if additional_secret_outputs is not None:
        if typ is not None:
            ...
            output_names = _types.resource_py_to_codeinfra_names(type(res))
    ...
    async def do_register() -> None:
        try:
            ...
            if (
                ...
                raise Exception(
                    "The Codeinfra CLI does not support the DeletedWith option. Please update the Codeinfra CLI."
                )
        ...
        # throw an exception sometimes. This was causing Codeinfra
