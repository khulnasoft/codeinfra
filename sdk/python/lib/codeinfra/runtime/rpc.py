...
ERROR_ON_DEPENDENCY_CYCLES_VAR = "CODEINFRA_ERROR_ON_DEPENDENCY_CYCLES"
...
# [2] https://github.com/khulnasoft/codeinfra-kubernetes/issues/984
#
...
async def serialize_properties(
    ...
    names to Codeinfra camelCase names, rather than using the `input_transformer`.
    ...
    if typ is not None:
        py_name_to_codeinfra_name = _types.input_type_py_to_codeinfra_names(typ)
        types = _types.input_type_types(typ)
        translate = lambda k: py_name_to_codeinfra_name.get(k) or k
        ...
        if typ is not None:
            if _types.is_input_type(typ):
                ...
                if typ is dict or origin in [dict, Dict, Mapping, abc.Mapping]:
                    ...
                    # is currently typed as `codeinfra.Input[str]`, but we've allowed a dict to be passed, which will
...
def deserialize_properties(
    ...
    for k, v in list(props_struct.items()):
        # Unilaterally skip properties considered internal by the Codeinfra engine.
...
def deserialize_resource(
    ...
    is_provider = pkg_name == "codeinfra" and mod_name == "providers"
    if is_provider:
...
def transfer_properties(
    ...
    for name in props:
        ...
        # from the Codeinfra name to the Python name and then use *that* name to index into the resolvers table.
...
def resolve_outputs(
    ...
    if transform_using_type_metadata:
        codeinfra_to_py_names = _types.resource_codeinfra_to_py_names(resource_cls)

        # pylint: disable=C3001
        translate = lambda prop: codeinfra_to_py_names.get(prop) or prop
        # pylint: disable=C3001
    ...
    if not settings.is_dry_run() or settings.is_legacy_apply_enabled():
        for key, value in list(serialized_props.items()):
            ...
            if translated_key not in all_properties:
                ...
                # input prop). This allows `codeinfra up` to work without erroring when there is an input and output prop
...
def resolve_properties(
    ...
    for key, value in all_properties.items():
        ...
        if not settings.is_dry_run():
            # normal 'codeinfra up'.  resolve the output with the value we got back
