...
"""
Runtime support for the Codeinfra configuration system.  Please use codeinfra.Config instead.
"""
...
def get_config_env() -> Dict[str, Any]:
    ...
    """
    if "CODEINFRA_CONFIG" in os.environ:
        env_config = os.environ["CODEINFRA_CONFIG"]
...
def get_config_env_key(k: str) -> str:
    """
    Returns a scrubbed environment variable key, CODEINFRA_CONFIG_<k>, that can be used for
    setting explicit variables.  This is unlike CODEINFRA_CONFIG which is just a JSON-serialized bag.
    """
    env_key = ""
    for c in k:
        if c == "_" or "A" <= c <= "Z" or "0" <= c <= "9":
            ...
            env_key += "_"
    return f"CODEINFRA_CONFIG_{env_key}"
...
def get_config_secret_keys_env() -> List[str]:
    ...
    """
    if "CODEINFRA_CONFIG_SECRET_KEYS" in os.environ:
        keys = os.environ["CODEINFRA_CONFIG_SECRET_KEYS"]
...
def get_config(k: str) -> Any:
    ...
    # If there is a specific CODEINFRA_CONFIG_<k> environment variable, use it.
    env_key = get_config_env_key(k)
    if env_key in os.environ:
        return os.environ[env_key]

    # If the config hasn't been set, but there is a process-wide CODEINFRA_CONFIG environment variable, use it.
