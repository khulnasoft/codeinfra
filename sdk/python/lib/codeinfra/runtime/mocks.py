...
from ..runtime.stack import Stack, run_codeinfra_func
...
class Mocks(ABC):
    """
    Mocks is an abstract class that allows subclasses to replace operations normally implemented by the Codeinfra engine with
...
class MockMonitor:
    ...
    def make_urn(self, parent: str, type_: str, name: str) -> str:
        ...
        return "urn:codeinfra:" + "::".join([get_stack(), get_project(), type_, name])
    ...
    def Invoke(self, request):
        ...
        if request.tok == "codeinfra:codeinfra:getResource":
    ...
    def RegisterResource(self, request):
        ...
        if request.type == "codeinfra:codeinfra:Stack":
...
def set_mocks(
    ...
    """
    set_mocks configures the Codeinfra runtime to use the given mocks for testing.
    """
