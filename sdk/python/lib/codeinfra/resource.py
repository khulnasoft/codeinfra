...
"""
Constant to represent the 'root stack' resource for a Codeinfra application.  The purpose of this is
solely to make it easy to write an [Alias] like so:

`aliases=[Alias(parent=codeinfra.ROOT_STACK_RESOURCE)]`.
...
class Alias:
    ...
    To specify no original parent, use `Alias(parent=codeinfra.ROOT_STACK_RESOURCE)`.
    """

    stack: Optional["Input[str]"]
    """
    The name of the previous stack of the resource.  If not provided, defaults to `codeinfra.getStack()`.
    """

    project: Optional["Input[str]"]
    """
    The previous project of the resource. If not provided, defaults to `codeinfra.getProject()`.
    """
...
class ResourceOptions:
    ...
    that the resource provider and codeinfra engine already determine based on inputs to your resource.
...
class Resource:
    ...
    def __init__(
        ...
        :param Optional[ResourceOptions] opts: Optional set of :class:`codeinfra.ResourceOptions` to use for this
               resource.
    ...
    def _get_providers(
        ...
        if opts.provider:
            ...
            if provider_pkg in opts_providers:
                ...
                deprecation = "This will become an error in a future version. See https://github.com/khulnasoft/codeinfra/issues/8799 for more details"
    ...
    def translate_input_property(self, prop: str) -> str:
        ...
        a format of their choosing before sending those properties to the Codeinfra engine.
...
class ProviderResource(CustomResource):
    ...
    def __init__(
        ...
        # Provider resources are given a well-known type, prefixed with "codeinfra:providers".
        CustomResource.__init__(
            self, f"codeinfra:providers:{pkg}", name, props, opts, dependency, package_ref
        )
        self.package = pkg
...
class DependencyProviderResource(ProviderResource):
    ...
    def __init__(self, ref: str) -> None:
        ...
        # `typ` will be `codeinfra:providers:<package>` and we want the
