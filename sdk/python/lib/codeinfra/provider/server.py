...
instance as a gRPC server so that it can be used as a Codeinfra plugin.
...
from codeinfra.provider.provider import InvokeResult, Provider, CallResult, ConstructResult
...
from codeinfra.runtime import known_types, proto, rpc
from codeinfra.runtime.proto import (
    provider_pb2_grpc,
    ResourceProviderServicer,
    status_pb2,
    errors_pb2,
)
from codeinfra.runtime.stack import wait_for_rpcs
import codeinfra
import codeinfra.resource
import codeinfra.runtime.config
import codeinfra.runtime.settings
from codeinfra.errors import (
...
class ProviderServicer(ResourceProviderServicer):
    ...
    async def _construct(
        ...
        codeinfra.runtime.settings.reset_options(
        ...
        codeinfra.runtime.config.set_all_config(
    ...
    @staticmethod
    def _construct_options(request: proto.ConstructRequest) -> codeinfra.ResourceOptions:
        parent = None
    ...
    async def _construct_response(
        ...
        urn = await codeinfra.Output.from_input(result.urn).future()
        assert urn is not None

        # Note: property_deps is populated by rpc.serialize_properties.
        property_deps: Dict[str, List[codeinfra.resource.Resource]] = {}
    ...
    @staticmethod
    async def _call_args(request: proto.CallRequest) -> Dict[str, codeinfra.Input[Any]]:
        def deps(key: str) -> Set[str]:
            return set(
                urn
    ...
    async def _call_response(self, result: CallResult) -> proto.CallResponse:
        ...
        ret_deps: Dict[str, List[codeinfra.resource.Resource]] = {}
...
def main(provider: Provider, args: List[str]) -> None:  # args not in use?
    ...
    implementation into a Codeinfra-compatible gRPC server.
    ...
    argp = argparse.ArgumentParser(description="Codeinfra provider plugin (gRPC server)")
    argp.add_argument("engine", help="Codeinfra engine address")
...
def _create_provider_resource(ref: str) -> ProviderResource:
    ...
    urn_parts = codeinfra.urn._parse_urn(urn)
