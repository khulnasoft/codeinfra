...
class Resource(CustomResource):
    """
    Resource represents a Codeinfra Resource that incorporates an inline implementation of the Resource's CRUD operations.
    """
    ...
    def __init__(
        ...
        if serialize_as_secret_always or contains_secrets:
            serialized_provider = codeinfra.Output.secret(serialized_provider)

        props[PROVIDER_KEY] = serialized_provider

        super().__init__(f"codeinfra-python:{self._resource_type_name}", name, props, opts)
