...
from codeinfra.metadata import get_project
from codeinfra.runtime._serialization import _deserialize
from codeinfra.runtime import configure, proto, rpc, Settings
from codeinfra.runtime.proto import provider_pb2_grpc, ResourceProviderServicer
from codeinfra.dynamic import ResourceProvider, ConfigureRequest, Config
...
def get_provider(props: Dict[str, Any], config: Dict[str, Any]) -> ResourceProvider:
    ...
    configure(
        Settings(
            project=os.environ.get("CODEINFRA_PROJECT", "project"),
    ...
    if provider is None:
        ...
        # like https://github.com/khulnasoft/codeinfra/issues/14159 there may be resource contention in dill.loads,
