...
// codeinfraWheel searches for the built codeinfra wheel in the sdk/python/dist directory
// and returns its path.
func codeinfraWheel(t *testing.T) string {
...
func TestDetermineCodeinfraPackages(t *testing.T) {
	t.Parallel()
	...
	for _, toolchainName := range []string{"pip", "poetry", "uv"} {
		...
		t.Run(toolchainName+"/empty", func(t *testing.T) {
			...
			// We need `pip` installed. This is a dependency of `codeinfra`, so it will always be
			// available Codeinfra virtual environments.
			createVenv(t, cwd, toolchainName, opts, "pip")

			packages, err := determineCodeinfraPackages(context.Background(), opts)
		...
		t.Run(toolchainName+"/non-empty", func(t *testing.T) {
			...
			createVenv(t, cwd, toolchainName, opts, codeinfraWheel(t), "codeinfra-random", "pip-install-test")
			...
			random := packages[0]
			require.Equal(t, "codeinfra_random", random.Name)
			require.NotEmpty(t, random.Location)
		})

		t.Run(toolchainName+"/codeinfraplugin", func(t *testing.T) {
			t.Parallel()
			...
			path := filepath.Join(sitePackages, "pip_install_test", "codeinfra-plugin.json")
			bytes := []byte(`{ "name": "thing1", "version": "thing2", "server": "thing3", "resource": true }` + "\n")
			err = os.WriteFile(path, bytes, 0o600)
			require.NoError(t, err)
			t.Logf("Wrote codeinfra-plugin.json file: %s", path)
		...
		t.Run(toolchainName+"/codeinfraplugin-resource-false", func(t *testing.T) {
			t.Parallel()

			cwd := t.TempDir()
			opts := getOptions(t, toolchainName, cwd)
			createVenv(t, cwd, toolchainName, opts, codeinfraWheel(t), "pip")

			// Install a local codeinfra SDK that has a codeinfra-plugin.json file with `{ "resource": false }`.
			...
			// The package should be considered a Codeinfra package since its name is prefixed with "codeinfra_".
			packages, err := determineCodeinfraPackages(context.Background(), opts)
			require.NoError(t, err)
			assert.Equal(t, 1, len(packages))
			assert.Equal(t, "codeinfra_foo", packages[0].Name)
		...
		t.Run(toolchainName+"/no-codeinfraplugin.json-file", func(t *testing.T) {
			t.Parallel()

			cwd := t.TempDir()
			opts := getOptions(t, toolchainName, cwd)
			createVenv(t, cwd, toolchainName, opts, codeinfraWheel(t))

			// Install a local old provider SDK that does not have a codeinfra-plugin.json file.
			...
			assert.Equal(t, 1, len(packages))
			old := packages[0]
			assert.Equal(t, "codeinfra_old", old.Name)
			assert.NotEmpty(t, old.Location)
		})

		t.Run(toolchainName+"/codeinfra-policy", func(t *testing.T) {
			t.Parallel()

			cwd := t.TempDir()
			opts := getOptions(t, toolchainName, cwd)
			createVenv(t, cwd, toolchainName, opts, codeinfraWheel(t), "codeinfra-policy")

			// The package should not be considered a Codeinfra package since it is hardcoded not to be,
