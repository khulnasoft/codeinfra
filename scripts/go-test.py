...
dryrun = os.environ.get("CODEINFRA_TEST_DRYRUN", None) == "true"

def options(options_and_packages: List[str]):
    return [o for o in options_and_packages if not o.startswith('github.com/khulnasoft/codeinfra')]


def packages(options_and_packages: List[str]):
    return [o for o in options_and_packages if o.startswith('github.com/khulnasoft/codeinfra')]

root = pathlib.Path(__file__).absolute().parent.parent
integration_test_subset = os.environ.get('CODEINFRA_INTEGRATION_TESTS', None)
args = sys.argv[1:]

covdir = os.environ.get('CODEINFRA_TEST_COVERAGE_PATH', None)
covprofile = None
