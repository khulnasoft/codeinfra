...
func NewAboutCmd() *cobra.Command {
	...
	var stack string
	short := "Print information about the Codeinfra environment."
	cmd := &cobra.Command{
		Use:   "about",
		Short: short,
		Long: short + "\n" +
			"\n" +
			"Prints out information helpful for debugging the Codeinfra CLI." +
			"\n" +
			"This includes information about:\n" +
			" - the CLI and how it was built\n" +
			" - which OS Codeinfra was run from\n" +
			" - the current project\n" +
			" - the current stack\n" +
			" - the current backend\n",
	...
	if proj, pwd, err = ws.ReadProject(); err != nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				if err != nil {
					addError(err, "Failed to get information about the Codeinfra program's dependencies")
				} else {
...
func formatLogAbout() string {
	...
	if logDir != nil && logDir.Value.String() != "" {
		return fmt.Sprintf("Codeinfra locates its logs in %s", logDir)
	}
	return fmt.Sprintf("Codeinfra locates its logs in %s by default", os.TempDir())
}
