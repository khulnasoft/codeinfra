...
func locateStackResource(opts display.Options, snap *deploy.Snapshot, urn resource.URN) (*resource.State, error) {
	...
	// Note: this is done to adhere to the same color scheme as the `codeinfra new` picker, which also does this.
	surveycore.DisableColor = true
...
func missingNonInteractiveArg(args ...string) error {
	...
	case 1:
		return fmt.Errorf("Must supply <%s> unless codeinfra is run interactively", args[0])
	default:
		for i, s := range args {
			args[i] = "<" + s + ">"
		}
		return fmt.Errorf("Must supply %s and %s unless codeinfra is run interactively",
