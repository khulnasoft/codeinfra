...
func TestRenameProvider(t *testing.T) {
	...
	provURN := resource.URN("urn:codeinfra:dev::prog-aws-typescript::codeinfra:providers:random::my-provider")
	prov := resource.State{
		URN:  provURN,
		ID:   "81cd12dd-2a90-4f21-a521-f4c71c1f11eb",
		Type: "codeinfra:providers:random",
	}

	providerRefString := string(prov.URN) + "::" + string(prov.ID)

	res1 := resource.State{
		URN:      resource.URN("urn:codeinfra:dev::prog-aws-typescript::random:index/randomPet:RandomPet::pet-0"),
...
func TestStateRename_invalidName(t *testing.T) {
	...
	prov := resource.URN("urn:codeinfra:dev::xxx-dev::kubernetes::provider")
	res := resource.URN("urn:codeinfra:dev::xxx-dev::kubernetes:core/v1:Namespace::amazon_cloudwatchNamespace")
		...
		Resources: []*resource.State{
			{
				URN:  prov,
				ID:   "provider-id",
				Type: "codeinfra:provider:kubernetes",
			},
			{
				URN:  res,
				ID:   "res-id",
	...
	assert.Panicsf(t, func() {
		_ = stateRenameOperation(
			res,
			"urn:codeinfra:dev::xxx-dev::eks:index:Cluster$kubernetes:core/v1:Namespace::amazon_cloudwatchNamespace",
			display.Options{},
			&snap,
		)
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/13179.
//
...
func TestStateRename_updatesChildren(t *testing.T) {
	...
	provider := resource.URN("urn:codeinfra:dev::pets::random::provider")
	parent := resource.URN("urn:codeinfra:dev::pets::random:index/randomPet:RandomPet::parent")
	child := resource.URN("urn:codeinfra:dev::pets::random:index/randomPet:RandomPet$random:index/randomPet:RandomPet::child")
		...
		Resources: []*resource.State{
			{
				URN:  provider,
				ID:   "provider-id",
				Type: "codeinfra:provider:random",
			},
			{
				URN:  parent,
				ID:   "parent-id",
