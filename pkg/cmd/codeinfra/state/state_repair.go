...
func (cmd *stateRepairCmd) manualRepairError(initialErr error, err error) string {
	...
	stateExportCommand := fmt.Sprintf("codeinfra stack%s export > %s", cmd.FlagsString, stateFile)
	...
	stateImportCommand := fmt.Sprintf("codeinfra stack%s import < %s", cmd.FlagsString, stateFile)

	return fmt.Sprintf(`Failed to repair the snapshot automatically: %[1]v

Codeinfra is unable to automatically repair the snapshot. This can happen if the
...
================================================================================
We would appreciate a report: https://github.com/khulnasoft/codeinfra/issues/

Please provide all of the text below in your report.
================================================================================
Codeinfra Version:    %[6]s
Go Version:        %[7]s
Go Compiler:       %[8]s
Architecture:      %[9]s
