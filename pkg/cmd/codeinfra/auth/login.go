...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/backend"
...
func NewLoginCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "login [<url>]",
		Short: "Log in to the Codeinfra Cloud",
		Long: "Log in to the Codeinfra Cloud.\n" +
			"\n" +
			"The Codeinfra Cloud manages your stack's state reliably. Simply run\n" +
			"\n" +
			"    $ codeinfra login\n" +
			"\n" +
			"and this command will prompt you for an access token, including a way to launch your web browser to\n" +
			"easily obtain one. You can script by using `CODEINFRA_ACCESS_TOKEN` environment variable.\n" +
			"\n" +
			"By default, this will log in to the managed Codeinfra Cloud backend.\n" +
			"If you prefer to log in to a self-hosted Codeinfra Cloud backend, specify a URL. For example, run\n" +
			"\n" +
			"    $ codeinfra login https://api.codeinfra.acmecorp.com\n" +
			"\n" +
			"to log in to a self-hosted Codeinfra Cloud running at the api.codeinfra.acmecorp.com domain.\n" +
			"\n" +
			"For `https://` URLs, the CLI will speak REST to a Codeinfra Cloud that manages state and concurrency control.\n" +
			"You can specify a default org to use when logging into the Codeinfra Cloud backend or a " +
			"self-hosted Codeinfra Cloud.\n" +
			"\n" +
			"[PREVIEW] If you prefer to operate Codeinfra independently of a Codeinfra Cloud, and entirely local to your computer,\n" +
			"pass `file://<path>`, where `<path>` will be where state checkpoints will be stored. For instance,\n" +
			"\n" +
			"    $ codeinfra login file://~\n" +
			"\n" +
			"will store your state information on your computer underneath `~/.codeinfra`. It is then up to you to\n" +
			...
			"\n" +
			"    $ codeinfra login --local\n" +
			"\n" +
			"[PREVIEW] Additionally, you may leverage supported object storage backends from one of the cloud providers " +
			"to manage the state independent of the Codeinfra Cloud. For instance,\n" +
			"\n" +
			"AWS S3:\n" +
			"\n" +
			"    $ codeinfra login s3://my-codeinfra-state-bucket\n" +
			"\n" +
			"GCP GCS:\n" +
			"\n" +
			"    $ codeinfra login gs://my-codeinfra-state-bucket\n" +
			"\n" +
			"Azure Blob:\n" +
			"\n" +
			"    $ codeinfra login azblob://my-codeinfra-state-bucket\n",
		...
		Run: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			if cloudURL == "" {
				...
				cloudURL, "https://"), "http://"); strings.HasPrefix(url, "app.codeinfra.com/") ||
				strings.HasPrefix(url, "codeinfra.com") {
				return fmt.Errorf("%s is not a valid self-hosted backend, "+
					"use `codeinfra login` without arguments to log into the Codeinfra Cloud backend", cloudURL)
			} else {
	...
	cmd.PersistentFlags().BoolVarP(&localMode, "local", "l", false, "Use Codeinfra in local-only mode")
...
func chooseAccount(accounts map[string]workspace.Account, opts display.Options) (string, error) {
	...
	if err := survey.AskOne(&survey.Select{
		...
		return "", errors.New("no account selected; please use `codeinfra login --interactive` to choose one")
	}
