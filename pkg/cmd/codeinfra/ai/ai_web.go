...
const (
	TypeScript CodeinfraAILanguage = "TypeScript"
	Python     CodeinfraAILanguage = "Python"
	Go         CodeinfraAILanguage = "Go"
	CSharp     CodeinfraAILanguage = "C#"
	Java       CodeinfraAILanguage = "Java"
	YAML       CodeinfraAILanguage = "YAML"
)

func (l *CodeinfraAILanguage) String() string {
	return string(*l)
}

func (l *CodeinfraAILanguage) Set(v string) error {
...
func (l *CodeinfraAILanguage) Type() string {
	return "string"
}
...
func (cmd *aiWebCmd) Run(ctx context.Context, args []string) error {
	...
	if err = browser.OpenURL(requestURL.String()); err != nil {
		fmt.Printf("We couldn't launch your web browser for some reason. Please visit:\n\n%s\n\n"+
			"to continue your Codeinfra AI session.\n", requestURL)
...
func newAIWebCommand() *cobra.Command {
	...
	if aiwebcmd.appURL == "" {
		aiwebcmd.appURL = "https://www.codeinfra.com/ai"
	}
	cmd := &cobra.Command{
		Use:   "web",
		Short: "Opens Codeinfra AI in your local browser",
		Long: `Opens Codeinfra AI in your local browser

This command opens the Codeinfra AI web app in your local default browser.
It can be further initialized by providing a prompt to pre-fill in the app,
with the default behavior then automatically submitting that prompt to Codeinfra AI.

If no prompt is provided, the app will be opened with no prompt pre-filled.

If you do not want to submit the prompt to Codeinfra AI, you can opt-out of this
...
`,
	...
	cmd.PersistentFlags().BoolVar(
		...
		"Opt-out of automatically submitting the prompt to Codeinfra AI",
	)
