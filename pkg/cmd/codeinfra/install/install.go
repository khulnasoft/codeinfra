...
func shouldInstallPolicyPackDependencies() (bool, error) {
	...
	if policyPackPath != "" {
		// There's a CodeinfraPolicy.yaml in cwd or a parent folder. The policy pack might be nested
		// within a project, or vice-vera, so we need to check if there's a Codeinfra.yaml in a parent
		// folder.
