...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/about"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/ai"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/auth"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/cancel"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/cmd"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/completion"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/config"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/console"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/convert"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/deployment"
	cmdEnv "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/env"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/events"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/install"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/logs"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/markdown"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/newcmd"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/operations"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/org"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/packagecmd"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/plugin"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/policy"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/query"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/schema"
	cmdStack "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/stack"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/state"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/trace"
	cmdVersion "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/version"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/whoami"
	"github.com/khulnasoft/codeinfra/pkg/v3/util/tracing"
...
func setCommandGroups(cmd *cobra.Command, rootCgs []commandGroup) {
	...
	cmd.SetHelpFunc(func(c *cobra.Command, args []string) {
		...
		fmt.Println("Use `codeinfra [command] --help` for more information about a command.")
	})
}
...
// NewCodeinfraCmd creates a new Codeinfra Cmd instance.
func NewCodeinfraCmd() *cobra.Command {
	var cwd string
	var logFlow bool
	...
	cmd := &cobra.Command{
		Use:   "codeinfra",
		Short: "Codeinfra command line",
		Long: "Codeinfra - Modern Infrastructure as Code\n" +
			"\n" +
			"To begin working with Codeinfra, run the `codeinfra new` command:\n" +
			"\n" +
			"    $ codeinfra new\n" +
			"\n" +
			...
			"\n" +
			"    - codeinfra up       : Deploy code and/or resource changes\n" +
			"    - codeinfra stack    : Manage instances of your project\n" +
			"    - codeinfra config   : Alter your stack's configuration or secrets\n" +
			"    - codeinfra destroy  : Tear down your stack's resources entirely\n" +
			"\n" +
			"For more information, please visit the project page: https://www.codeinfra.com/docs/",
		PersistentPreRun: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			cmdutil.InitTracing("codeinfra-cli", "codeinfra", tracingFlag)
			...
			if cmdutil.IsTracingEnabled() {
				...
				tracingHeader := os.Getenv("CODEINFRA_TRACING_HEADER")
	...
	cmd.PersistentFlags().StringVarP(&cwd, "cwd", "C", "",
		"Run codeinfra as if it had been started in another directory")
	...
	setCommandGroups(cmd, []commandGroup{
		...
		//     hidden unless CODEINFRA_EXPERIMENTAL is set to true.
		{
		...
		// We have a set of options that are useful for developers of codeinfra
		//    hidden unless CODEINFRA_DEBUG_COMMANDS is set to true.
		{
		...
		// AI Commands relating to specifically the Codeinfra AI service
...
func getUpgradeMessage(latest semver.Version, current semver.Version, isDevVersion bool) string {
	...
	msg := fmt.Sprintf("A new version of Codeinfra is available. To upgrade from version '%s' to '%s', ", current, latest)
	if cmd != "" {
		msg += "run \n   " + cmd + "\nor "
	}

	msg += "visit https://codeinfra.com/docs/install/ for manual instructions and release notes."
	return msg
}
...
func getUpgradeCommand(isDevVersion bool) string {
	...
	if isBrew {
		return "$ brew update && brew upgrade codeinfra"
	}
	...
	if runtime.GOOS != "windows" {
		command := "$ curl -sSL https://get.codeinfra.com | sh"
		if isDevVersion {
	...
	powershellCmd = "> " + powershellCmd + ` -NoProfile -InputFormat None -ExecutionPolicy Bypass -Command "iex ` +
		`((New-Object System.Net.WebClient).DownloadString('https://get.codeinfra.com/install.ps1'))"`
	if isDevVersion {
...
func isBrewInstall(exe string) (bool, error) {
	...
	brewPrefixCmd := exec.Command(brewBin, "--prefix", "codeinfra")
	...
	if err = brewPrefixCmd.Run(); err != nil {
		...
		return false, fmt.Errorf("'brew --prefix codeinfra' failed: %w", err)
	}

	brewPrefixCmdOutput := strings.TrimSpace(stdout.String())
	if brewPrefixCmdOutput == "" {
		return false, errors.New("trimmed output from 'brew --prefix codeinfra' is empty")
	}
	...
	brewPrefixExePath := filepath.Join(brewPrefixPath, "bin", "codeinfra")
...
func getLatestBrewFormulaVersion() (semver.Version, bool, error) {
	...
	const formulaJSON = "https://formulae.brew.sh/api/formula/codeinfra.json"
