...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/env"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/workspace"
	"github.com/stretchr/testify/assert"
)

// Test for https://github.com/khulnasoft/codeinfra/issues/11703, check we give an error when trying to install a
// bundled plugin
func TestBundledError(t *testing.T) {
	...
	cmd := &pluginInstallCmd{
		...
		env: env.NewEnv(
			env.MapStore{"CODEINFRA_DEV": "false"},
		),
	}

	err := cmd.Run(context.Background(), []string{"language", "nodejs"})
	assert.EqualError(t, err,
		"the nodejs language plugin is bundled with Codeinfra, "+
			"and cannot be directly installed with this command. "+
			"If you need to reinstall this plugin, reinstall Codeinfra via your package manager or install script.")
}

// Test for https://github.com/khulnasoft/codeinfra/issues/11703, check we still try to install bundled plugins if
// CODEINFRA_DEV is set.
func TestBundledDev(t *testing.T) {
	...
	cmd := &pluginInstallCmd{
		diag: diagtest.LogSink(t),
		env: env.NewEnv(
			env.MapStore{"CODEINFRA_DEV": "true"},
		),
