...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/util"
...
func newPluginInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Install one or more plugins.\n" +
			...
			"these arguments and letting Codeinfra compute the set of plugins required by the\n" +
			"current project. When Codeinfra computes the download set automatically, it may\n" +
			...
			"If VERSION is unspecified, Codeinfra will attempt to look up the latest version of\n" +
...
func (cmd *pluginInstallCmd) Run(ctx context.Context, args []string) error {
	...
	if len(args) > 0 {
		...
		// distributed with Codeinfra itself. But we turn this check off if CODEINFRA_DEV is set so we can
		...
		if !cmd.env.GetBool(env.Dev) && workspace.IsPluginBundled(pluginSpec.Kind, pluginSpec.Name) {
			return fmt.Errorf(
				"the %v %v plugin is bundled with Codeinfra, and cannot be directly installed"+
					" with this command. If you need to reinstall this plugin, reinstall"+
					" Codeinfra via your package manager or install script.",
				pluginSpec.Name,
				pluginSpec.Kind,
			)
		}
		...
		for _, plugin := range plugins {
			...
			// TODO[codeinfra/codeinfra#956]: eventually we will want to honor and install these in the usual way.
