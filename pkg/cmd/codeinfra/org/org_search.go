...
func (cmd *orgSearchCmd) Run(ctx context.Context, args []string) error {
	...
	if !isCloud {
		return errors.New("Codeinfra AI search is only supported for the Codeinfra Cloud")
	}
...
func newSearchCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "search",
		Short: "Search for resources in Codeinfra Cloud",
		Long:  "Search for resources in Codeinfra Cloud.",
		Args:  cmdutil.NoArgs,
	...
	cmd.PersistentFlags().StringArrayVarP(
		...
		"A Codeinfra Query to send to Codeinfra Cloud for resource search."+
			...
			"See https://www.codeinfra.com/docs/codeinfra-cloud/insights/search/#query-syntax for syntax reference.",
	)
...
func renderSearchTable(w io.Writer, results *apitype.ResourceSearchResponse) error {
	urlInfo := "Results are also visible in Codeinfra Cloud:"
	...
	_, err = w.Write(
		[]byte(fmt.Sprintf("\nResources displayed are the result of the following Codeinfra query:\n%s\n", results.Query)),
	)
	return err
}
