...
func TestSearch_cmd(t *testing.T) {
	...
	searchURL := "https://app.codeinfra.com/codeinfra/resources?foo=bar"
	total := int64(132)
	orgName := "org1"
	cmd := orgSearchCmd{
		searchCmd: searchCmd{
			...
			currentBackend: func(
				...
				return &stubHTTPBackend{
					SearchF: func(context.Context, string, *apitype.CodeinfraQueryRequest) (*apitype.ResourceSearchResponse, error) {
	...
	assert.Contains(t, buff.String(), "Results are also visible in Codeinfra Cloud:\n"+searchURL)
...
func TestSearchNoOrgName_cmd(t *testing.T) {
	...
	mod := "mod1"
	modified := "2023-01-01T00:00:00.000Z"
	searchURL := "https://app.codeinfra.com/user/resources?foo=bar"
	total := int64(132)
	cmd := orgSearchCmd{
		searchCmd: searchCmd{
			Stdout: &buff,
...
type stubHTTPBackend struct {
	...
	SearchF func(
		context.Context, string, *apitype.CodeinfraQueryRequest,
