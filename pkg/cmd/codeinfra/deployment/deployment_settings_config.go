...
func newDeploymentSettingsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Manage stack deployment settings\n" +
			...
			"updated settings to Codeinfra Cloud.",
...
func initializeDeploymentSettingsCmd(
	...
	if !be.SupportsDeployments() {
		unsupportedBackendMsg := fmt.Sprintf("Backends of type %q do not support managed deployments.\n\n"+
			"Create a Codeinfra Cloud account to get started, learn more about codeinfra deployments here: "+
			"https://www.codeinfra.com/docs/codeinfra-cloud/deployments/",
			be.Name())
		...
		unsupportedBackendMsg = colors.Highlight(unsupportedBackendMsg, "Codeinfra Cloud", colors.BrightCyan+colors.Bold)
		unsupportedBackendMsg = colors.Highlight(unsupportedBackendMsg,
			"https://www.codeinfra.com/docs/codeinfra-cloud/deployments/", colors.BrightBlue+colors.Underline+colors.Bold)
...
func initStackDeploymentCmd(
	...
	oidcExplanationMsg := "\nCodeinfra supports OpenID Connect (OIDC) integration across various services by " +
	...
	oidcExplanationMsg = colors.Highlight(oidcExplanationMsg, "Codeinfra", colors.SpecHeadline)
...
func newDeploymentSettingsConfigureCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			if d.Deployment == nil {
				return errors.New("Deployment file not initialized, please run `codeinfra deployment settings init` instead")
			}
...
func configureGit(d *deploymentSettingsCommandDependencies, gitSSHPrivateKeyPath string, gitSSHPrivateKeyValue string,
	...
	if useGitHub {
		...
		if !integration.Installed {
			...
			ghAppExplanationTitle := "\nCodeinfra’s GitHub app is not installed\n\n"
			ghAppExplanationMsg := "Codeinfra’s GitHub app displays the results of Codeinfra stack update previews in " +
				"pull requests and enables automatic stack deployments via Codeinfra Deployments. " +
				...
				"https://www.codeinfra.com/docs/iac/packages-and-automation/continuous-delivery/github-app/\n"

			ghAppExplanationTitle = colors.Highlight(ghAppExplanationTitle,
				"Codeinfra’s GitHub app is not installed", colors.SpecWarning)

			ghAppExplanationMsg = colors.Highlight(ghAppExplanationMsg, "Codeinfra’s GitHub app", colors.SpecHeadline)
			ghAppExplanationMsg = colors.Highlight(ghAppExplanationMsg, "Codeinfra Deployments", colors.SpecHeadline)

			d.Prompts.Print(d.DisplayOptions.Color.Colorize(ghAppExplanationTitle + ghAppExplanationMsg))

			confirm := d.Prompts.AskForConfirmation("Do you want to continue without using the Codeinfra's GitHub app?",
	...
	if useGitHub {
		ghAppExplanationMsg := "\nCodeinfra’s GitHub app displays the results of Codeinfra stack update previews in " +
			"pull requests and enables automatic stack deployments via Codeinfra Deployments. " +
			"Once installed and configured, it will show you any potential infrastructure " +
			"changes on Pull Requests and commit checks. You can also configure git push to " +
			"deploy workflows that update your stacks whenever a pull request is merged.\n"
		...
		useGitHub = d.Prompts.AskForConfirmation(
			"Do you want to use the Codeinfra GitHub App?",
...
func configureGitSSH(
	...
	if gitSSHPrivateKeyPath == "" && gitSSHPrivateKeyValue == "" {
		configureMsg := "\nNo SSH private key was provided, run `codeinfra deployment settings " +
		...
		configureMsg = colors.Highlight(configureMsg, "codeinfra deployment settings configure", colors.BrightBlue+colors.Bold)
