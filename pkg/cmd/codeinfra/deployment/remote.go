...
func (r *RemoteArgs) ApplyFlagsForDeploymentCommand(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVar(
		...
		"The directory to work from in the project's source repository "+
			"where Codeinfra.yaml is located; used when Codeinfra.yaml is not in the project source root")
	cmd.PersistentFlags().StringVar(
	...
	cmd.PersistentFlags().StringVar(
		...
		"The agent pool to use to run the deployment job. When empty, the Codeinfra Cloud shared queue "+
			"will be used.")
}
...
func (r *RemoteArgs) ApplyFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVar(
		...
		"[EXPERIMENTAL] The agent pool to use to run the deployment job. When empty, the Codeinfra Cloud shared queue "+
			"will be used.")
}
...
func RunDeployment(ctx context.Context, ws pkgWorkspace.Context, cmd *cobra.Command, opts display.Options,
	operation apitype.CodeinfraOperation, stack, url string, args RemoteArgs,
) error {
	...
	if !isCloud {
		return errors.New("the Codeinfra Cloud backend must be used for remote operations; " +
			"use `codeinfra login` without arguments to log into the Codeinfra Cloud backend")
	}
