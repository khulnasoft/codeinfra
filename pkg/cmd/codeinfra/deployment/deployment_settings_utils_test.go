...
func TestValidateRelativeDirectory(t *testing.T) {
	...
	pathsToTest := []relativeDirectoryValidationCase{
		...
		{false, filepath.Join(".", "goproj", "Codeinfra.yaml")},
	}
...
func TestDSFileParsing(t *testing.T) {
	...
	stackDeploymentConfigFile := "Codeinfra.stack.deploy.yaml"
  ...
  agentPoolID: 51035bee-a4d6-4b63-9ff6-418775c5da8d`), 0o600)
	...
	require.Equal(t, "git@github.com:codeinfra/test-repo.git", deploymentFile.DeploymentSettings.SourceContext.Git.RepoURL)
...
func setUpGitWorkspace(ctx context.Context, t *testing.T) string {
	workDir, err := os.MkdirTemp("", "codeinfra_deployment_settings")
	assert.NoError(t, err)
