...
import (
	...
	aferoUtil "github.com/khulnasoft/codeinfra/pkg/v3/util/afero"
...
func NewConvertCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "convert",
		Short: "Convert Codeinfra programs from a supported source program into other supported languages",
		Long: "Convert Codeinfra programs from a supported source program into other supported languages.\n" +
			"\n" +
			...
			"\n" +
			"    codeinfra convert --from yaml --language java --out . \n",
	...
	cmd.PersistentFlags().StringVar(
		&language, "language", "", "Which language plugin to use to generate the Codeinfra project")
...
func runConvert(
	...
	pclDirectory, err := os.MkdirTemp("", "codeinfra-convert")
	if err != nil {
	...
	if diagnostics.HasErrors() {
		...
		if !strict {
			...
			fmt.Fprintln(os.Stderr, "The Codeinfra CLI encountered a code generation error. This is a bug!")
			fmt.Fprintln(os.Stderr, "We would appreciate a report: https://github.com/khulnasoft/codeinfra/issues/")
			fmt.Fprintln(os.Stderr, "Please provide all of the below text in your report.")
			fmt.Fprintln(os.Stderr, "================================================================================")
			fmt.Fprintf(os.Stderr, "Codeinfra Version:   %s\n", version.Version)
		}
	...
	// If we've got code generation warnings only print them if we've got CODEINFRA_DEV set or emitting pcl
...
func generateAndLinkSdksForPackages(
	...
	for _, pkg := range pkgs {
		...
		if err != nil {
			return fmt.Errorf("generated root is not a valid codeinfra workspace %q: %w", convertOutputDirectory, err)
		}
