...
// TestYamlConvert is an entrypoint for debugging `codeinfra convert`. To use this with an editor such as
// VS Code, drop a Codeinfra.yaml in the testdata folder and with the VS Code Go extension, the
...
func TestYamlConvert(t *testing.T) {
	...
	if info, err := os.Stat("testdata/Codeinfra.yaml"); err != nil && os.IsNotExist(err) {
		t.Skip("skipping test, no Codeinfra.yaml found")
	} else if err != nil {
		t.Fatalf("failed to stat Codeinfra.yaml: %v", err)
	} else if info.IsDir() {
		t.Fatalf("Codeinfra.yaml is a directory, not a file")
	}
...
func TestProjectNameDefaults(t *testing.T) {
	...
	// Assert.
	yamlBytes, err := os.ReadFile(filepath.Join(outDir, "Codeinfra.yaml"))
	assert.NoError(t, err)
