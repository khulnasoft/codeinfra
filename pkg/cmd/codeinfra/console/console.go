...
func NewConsoleCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "console",
		Short: "Opens the current stack in the Codeinfra Console",
		Args:  cmdutil.NoArgs,
		Run: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
				...
				if stackName != "" {
					...
					if stack == nil {
						fmt.Println("No stack is currently selected. " +
							"Run `codeinfra stack select` to select a stack.")
						return nil
					}
				}

				// Open the stack specific URL (e.g. app.codeinfra.com/{org}/{project}/{stack}) for this
				// stack if a stack is selected and is a cloud stack, else open the cloud backend URL
				// home page, e.g. app.codeinfra.com.
			...
			fmt.Println("This command is not available for your backend. " +
				"To migrate to the Codeinfra Cloud backend, " +
				"please see https://www.codeinfra.com/docs/intro/concepts/state/#codeinfra-cloud-backend")
			return nil
		}),
	}
