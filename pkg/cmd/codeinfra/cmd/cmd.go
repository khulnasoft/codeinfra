// Copyright 2018-2024, KhulnaSoft Ltd.
//
...
// handling specific to the Codeinfra CLI. This includes e.g. specific and more
...
func printDecryptError(e engine.DecryptError) {
	...
	ui.Fprintf(writer, ""+
		...
		"\n"+
		"You can re-encrypt your configuration by running `codeinfra config set %s [value] --secret` with your "+
		"new stack selected.\n"+
		"\n"+
...
// Codeinfra engine. This function is a type-specific handler for these errors that
...
func printSnapshotIntegrityError(err error, sie deploy.SnapshotIntegrityError) {
	...
	message.WriteString(`The Codeinfra CLI encountered a snapshot integrity error. This is a bug!

You can try to repair your snapshot as follows:

` + "* Using the `codeinfra state repair` command." + ` This is the recommended approach.
  Run ` + "`codeinfra state repair --help`" + ` for more information.
` + "* Passing the `--disable-integrity-checking` flag." + ` This will disable integrity
  checking on many Codeinfra commands. This is riskier, but may allow you to
  execute fine-grained repairs using e.g. ` + "`codeinfra state delete`" + `.
