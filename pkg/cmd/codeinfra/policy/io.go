...
// ReadPolicyProject attempts to detect and read a Codeinfra PolicyPack project for the current
// workspace. If the project is successfully detected and read, it is returned along with the path
// to its containing directory, which will be used as the root of the project's Codeinfra program.
func ReadPolicyProject(pwd string) (*workspace.PolicyPackProject, string, string, error) {
	...
	if err != nil {
		return nil, "", "", fmt.Errorf("failed to find current Codeinfra project because of "+
			"an error when searching for the CodeinfraPolicy.yaml file (searching upwards from %s)"+": %w", pwd, err)
	} else if path == "" {
		return nil, "", "", fmt.Errorf("no CodeinfraPolicy.yaml project file found (searching upwards from %s)", pwd)
	}
	proj, err := workspace.LoadPolicyPack(path)
	if err != nil {
		return nil, "", "", fmt.Errorf("failed to load Codeinfra policy project located at %q: %w", path, err)
	}
