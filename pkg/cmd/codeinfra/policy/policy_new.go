...
func newPolicyNewCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:      "Create a new Codeinfra Policy Pack",
		Long: "Create a new Codeinfra Policy Pack from a template.\n" +
			"\n" +
...
func printPolicyPackNextSteps(proj *workspace.PolicyPackProject, root string, generateOnly bool, opts display.Options) {
	...
	usageCommandPreambles := []string{
		"run the Policy Pack against a Codeinfra program, in the directory of the Codeinfra program run",
	}
	usageCommands := []string{"codeinfra up --policy-pack " + root}
	...
	if strings.EqualFold(proj.Runtime.Name(), "nodejs") || strings.EqualFold(proj.Runtime.Name(), "python") {
		...
		usageCommands = append(usageCommands, "codeinfra policy publish [org-name]")
	}
...
func choosePolicyPackTemplate(templates []workspace.PolicyPackTemplate,
	...
	const chooseTemplateErr = "no template selected; please use `codeinfra policy new` to choose one"
