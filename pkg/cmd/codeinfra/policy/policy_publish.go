...
func newPolicyPublishCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Publish a Policy Pack to the Codeinfra Cloud",
		Long: "Publish a Policy Pack to the Codeinfra Cloud\n" +
			"\n" +
...
func (cmd *policyPublishCmd) Run(ctx context.Context, lm cmdBackend.LoginManager, args []string) error {
	...
	//
	// Obtain current PolicyPack, tied to the Codeinfra Cloud backend.
	//
...
func requirePolicyPack(
	...
	if err != nil {
		return nil, fmt.Errorf("`codeinfra policy` command requires the user to be logged into the Codeinfra Cloud: %w", err)
	}
