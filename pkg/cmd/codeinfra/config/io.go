...
// primarily for use in commands like `codeinfra destroy`, where it is useful to be
	...
	if err != nil || workspaceStack == nil {
		...
		if err != nil {
			return backend.StackConfiguration{}, nil, fmt.Errorf(
				"stack configuration could not be loaded from either Codeinfra.yaml or the backend: %w", err)
		}
