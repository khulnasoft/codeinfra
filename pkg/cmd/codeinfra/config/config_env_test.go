	...
	return &configEnvCmd{
		...
		ws: &pkgWorkspace.MockContext{
			ReadProjectF: func() (*workspace.Project, string, error) {
				p, err := workspace.LoadProjectBytes([]byte(projectYAML), "Codeinfra.yaml", encoding.YAML)
				if err != nil {
					return nil, "", err
				}
				return p, "", nil
		...
		loadProjectStack: func(p *workspace.Project, _ backend.Stack) (*workspace.ProjectStack, error) {
			return workspace.LoadProjectStackBytes(p, []byte(projectStackYAML), "Codeinfra.stack.yaml", encoding.YAML)
		},
