...
import (
	...
	cmdConfig "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/config"
	...
	newcmd "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/newcmd"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/plan"
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/resource/autonaming"
...
// See https://github.com/khulnasoft/codeinfra/issues/14989 for context around the cpu * 4 choice.
...
func NewUpCmd() *cobra.Command {
	...
	// up implementation used when the source of the Codeinfra program is in the current working directory.
	...
	// up implementation used when the source of the Codeinfra program is a template name or a URL to a template.
	upTemplateNameOrURL := func(
		...
		repo, err := workspace.RetrieveTemplates(ctx, templateNameOrURL, false, workspace.TemplateKindCodeinfraProject)
		if err != nil {
			return err
		}
		defer func() {
		...
		temp, err := os.MkdirTemp("", "codeinfra-up-")
		if err != nil {
			return err
		}
		defer func() {
	...
	cmd := &cobra.Command{
		...
		Long: "Create or update the resources in a stack.\n" +
			...
			"is computed by running the current Codeinfra program and observing all resource allocations to produce a\n" +
		...
		Run: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			logging.V(7).Infof("CODEINFRA_SUPPRESS_COPILOT_LINK=%v", env.SuppressCopilotLink.Value())
	...
	cmd.PersistentFlags().BoolVar(
		...
		"Continue updating resources even if an error is encountered "+
			"(can also be set with CODEINFRA_CONTINUE_ON_ERROR environment variable)")
	cmd.PersistentFlags().BoolVar(
