...
func TestBuildImportFile_ExistingProvider(t *testing.T) {
	...
	providerState := makeStateMetadata(t, "prov", "codeinfra:providers:pkg", true, stateOptions{
		Inputs: resource.NewPropertyMapFromMap(map[string]interface{}{
			"version": "3.2.1",
		}),
	})
...
func TestBuildImportFile_NewProvider(t *testing.T) {
	...
	assert.ErrorContains(
		t, err,
		"cannot import resource \"urn:codeinfra:stack::project::pkg:mod:typ::res\" "+
			"with a new explicit provider \"urn:codeinfra:stack::project::codeinfra:providers:pkg::prov\"")
}
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/15002
...
func TestBuildImportFile_regress_15002(t *testing.T) {
	...
	events <- engine.NewEvent(engine.ResourcePreEventPayload{
		Metadata: engine.StepEventMetadata{
			URN: "urn:codeinfra:test::test::pkgA:m:typA::resA",
		},
	})
	events <- engine.NewEvent(engine.ResourcePreEventPayload{
		Metadata: engine.StepEventMetadata{
			URN: "urn:codeinfra:test::test::pkgA:m:a-different-type::resA",
		},
	})
	// Finally, close the events channel to signal that we're done
	close(events)
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/15068
