...
func NewDestroyCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Destroy all existing resources in the stack, but not the stack itself\n" +
			...
			"\n" +
			"The stack itself is not deleted. Use `codeinfra stack rm` or the \n" +
		...
		Run: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			if err != nil && errors.Is(err, workspace.ErrProjectNotFound) {
				logging.Warningf("failed to find current Codeinfra project, continuing with an empty project"+
			...
			if stackName != "" {
				// `codeinfra destroy --stack <stack>` can be run outside of the project directory.
			...
			if destroyErr == nil && protectedCount > 0 && !jsonDisplay {
				...
				if !jsonDisplay && !remove && !previewOnly {
					fmt.Printf("The resources in the stack have been deleted, but the history and configuration "+
						...
						"completely, run `codeinfra stack rm %s`.\n", s.Ref())
				} else if remove {
	...
	cmd.PersistentFlags().BoolVar(
		...
		"Continue to perform the destroy operation despite the occurrence of errors "+
			"(can also be set with CODEINFRA_CONTINUE_ON_ERROR env var)")
