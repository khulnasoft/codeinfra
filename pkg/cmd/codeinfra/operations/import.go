...
import (
	...
	cmdConvert "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/convert"
	cmdDiag "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/diag"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/metadata"
	cmdStack "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/stack"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/convert"
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/importer"
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/dotnet"
)
...
type importSpec struct {
	...
	// LogicalName is the resources Codeinfra name (i.e. the first argument to `new Resource`).
...
func writeImportFileToTemp(v importFile) (string, error) {
	...
	f, err := os.CreateTemp(wd, "codeinfra-import-*.json")
	if err != nil {
...
func getCurrentDeploymentForStack(
	...
	if err != nil {
		...
		case stack.ErrDeploymentSchemaVersionTooOld:
			return nil, fmt.Errorf("the stack '%s' is too old to be used by this version of the Codeinfra CLI",
				s.Ref().Name())
		case stack.ErrDeploymentSchemaVersionTooNew:
			return nil, fmt.Errorf("the stack '%s' is newer than what this version of the Codeinfra CLI understands. "+
				"Please update your version of the Codeinfra CLI", s.Ref().Name())
		}
...
func generateImportedDefinitions(ctx *plugin.Context,
	...
	defer func() {
		...
		if v != nil {
			...
			errMsg.WriteString("Your resource has been imported into Codeinfra state, but there was an error generating the import code.\n") //nolint:lll
			errMsg.WriteString("\n")
			if strings.Contains(fmt.Sprintf("%v", v), "invalid Go source code:") {
				errMsg.WriteString("You will need to copy and paste the generated code into your Codeinfra application and manually edit it to correct any errors.\n\n") //nolint:lll
			}
...
func NewImportCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Import resources into an existing stack.\n" +
			"\n" +
			"Resources that are not managed by Codeinfra can be imported into a Codeinfra stack\n" +
			...
			"should be added to the Codeinfra program. The resources are protected from deletion\n" +
			"by default.\n" +
			"\n" +
			...
			"If a logical name is given, it will be used to name the resource in the Codeinfra state.\n" +
			"\n" +
			"A resource can also be declared as a \"component\" (and optionally as \"remote\"). These resources\n" +
			"don't have an id set and instead just create an empty placeholder component resource in the Codeinfra state.\n" +
			"\n" +
			...
			"\n" +
			"You can use `codeinfra preview` with the `--import-file` option to emit an import file\n" +
			...
			if generateCode {
				...
				if validImports {
					...
					if outputFilePath == "" && !jsonDisplay {
						fmt.Print("Please copy the following code into your Codeinfra application. Not doing so\n" +
							"will cause Codeinfra to report that an update will happen on the next update command.\n\n")
						if protectResources {
							fmt.Print(("Please note that the imported resources are marked as protected. " +
								"To destroy them\n" +
								"you will need to remove the `protect` option and run `codeinfra update` *before*\n" +
