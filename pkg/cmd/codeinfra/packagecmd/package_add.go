...
// Constructs the `codeinfra package add` command.
func newPackageAddCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "add <provider|schema> [provider-parameter...]",
		Args:  cobra.MinimumNArgs(1),
		Short: "Add a package to your Codeinfra project",
		Long: `Add a package to your Codeinfra project.

This command locally generates an SDK in the currently selected Codeinfra language
and prints instructions on how to link it into your project. The SDK is based on
a Codeinfra package schema extracted from a given resource plugin or provided
directly.

When <provider> is specified as a PLUGIN[@VERSION] reference, Codeinfra attempts to
resolve a resource plugin first, installing it on-demand, similarly to:

  codeinfra plugin install resource PLUGIN [VERSION]

When <provider> is specified as a local path, Codeinfra executes the provider
binary to extract its package schema:

  codeinfra package add ./my-provider
...
from the parameters, as in:
  ...
  codeinfra package add <provider> -- --provider-parameter-flag value

When <schema> is a path to a local file with a '.json', '.yml' or '.yaml'
extension, Codeinfra package schema is read from it directly:

  codeinfra package add ./my/schema.json`,
		Run: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			tempOut, err := os.MkdirTemp("", "codeinfra-package-add-")
			if err != nil {
