...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/deepcopy"
...
func CreateSecretsManagerForExistingStack(
	...
	// for the Codeinfra Cloud backend secrets provider.
...
// Codeinfra.<stack>.yaml file themselves, prior to stack initialisation), try to respect the settings therein. Otherwise,
...
// we only want to log a secrets decryption for a Codeinfra Cloud backend project
// we will allow any secrets provider to be used (Codeinfra Cloud or passphrase/cloud/etc)
...
func Log3rdPartySecretsProviderDecryptionEvent(ctx context.Context, backend backend.Stack,
	...
	if stack, ok := backend.(httpstate.Stack); ok {
		// we only want to do something if this is a Codeinfra Cloud backend
