...
func ChooseStack(ctx context.Context, ws pkgWorkspace.Context,
	...
	if lopt.OfferNew() {
		chooseStackErr = "no stack selected; please use `codeinfra stack select` or `codeinfra stack init` to choose one"
	} else {
		chooseStackErr = "no stack selected; please use `codeinfra stack select` to choose one"
	}
...
func checkDeploymentVersionError(err error, stackName string) error {
	...
	case stack.ErrDeploymentSchemaVersionTooOld:
		return fmt.Errorf("the stack '%s' is too old to be used by this version of the Codeinfra CLI",
			stackName)
	case stack.ErrDeploymentSchemaVersionTooNew:
		return fmt.Errorf("the stack '%s' is newer than what this version of the Codeinfra CLI understands. "+
			"Please update your version of the Codeinfra CLI", stackName)
	}
