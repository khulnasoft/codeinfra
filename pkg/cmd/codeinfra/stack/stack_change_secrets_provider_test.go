...
`), 0o600)
	...
	project, err := workspace.LoadProject("Codeinfra.yaml")
	require.NoError(t, err)
	projectStack, err := workspace.LoadProjectStack(project, "Codeinfra.testStack.yaml")
	require.NoError(t, err)
...
`), 0o600)
	...
	err = cfg.Save("Codeinfra.testStack.yaml")
	require.NoError(t, err)
