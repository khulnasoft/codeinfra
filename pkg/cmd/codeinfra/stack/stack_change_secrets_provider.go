...
func newStackChangeSecretsProviderCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Change the secrets provider for a stack. " +
			...
			"To change to using the Codeinfra Default Secrets Provider, use the following:\n" +
			"\n" +
			"codeinfra stack change-secrets-provider default" +
			"\n" +
			"\n" +
			"To change the stack to use a cloud secrets backend, use one of the following:\n" +
			"\n" +
			"* `codeinfra stack change-secrets-provider \"awskms://alias/ExampleAlias?region=us-east-1\"" +
			"`\n" +
			"* `codeinfra stack change-secrets-provider " +
			"\"awskms://1234abcd-12ab-34cd-56ef-1234567890ab?region=us-east-1\"`\n" +
			"* `codeinfra stack change-secrets-provider " +
			"\"azurekeyvault://mykeyvaultname.vault.azure.net/keys/mykeyname\"`\n" +
			"* `codeinfra stack change-secrets-provider " +
			"\"gcpkms://projects/<p>/locations/<l>/keyRings/<r>/cryptoKeys/<k>\"`\n" +
			"* `codeinfra stack change-secrets-provider \"hashivault://mykey\"`",
