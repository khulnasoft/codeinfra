...
func newStackRenameCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Rename an existing stack.\n" +
			"\n" +
			"Note: Because renaming a stack will change the value of `getStack()` inside a Codeinfra program, if this\n" +
			"name is used as part of a resource's name, the next `codeinfra up` will want to delete the old resource and\n" +
			...
			"to update the name field of Codeinfra.yaml, so the project names match.",
		Run: cmd.RunCmdFunc(func(cmd *cobra.Command, args []string) error {
			...
			// Move the configuration data stored in Codeinfra.<stack-name>.yaml.
