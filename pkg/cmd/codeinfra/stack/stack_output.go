...
func (cmd *stackOutputCmd) Run(ctx context.Context, args []string) error {
	...
	// If there is an argument, just print that property.  Else, print them all (similar to `codeinfra stack`).
	if len(args) > 0 {
		name := args[0]
		v, has := outputs[name]
		if has {
	...
	if cmd.showSecrets {
		Log3rdPartySecretsProviderDecryptionEvent(ctx, s, "", "codeinfra stack output")
	}
...
// on behalf of 'codeinfra stack output'.
