...
func NewStackCmd() *cobra.Command {
	...
	cmd.Flags().BoolVarP(
		&args.showURNs, "show-urns", "u", false, "Display each resource's Codeinfra-assigned globally unique URN")
	cmd.Flags().BoolVar(
...
func runStack(ctx context.Context, s backend.Stack, out io.Writer, args stackArgs) error {
	...
	if !isCloud || cloudBe.CloudURL() != httpstate.CodeinfraCloudURL {
	...
	if snap != nil {
		...
		if snap.Manifest.Version != "" {
			fmt.Fprintf(out, "    Codeinfra version used: %s\n", snap.Manifest.Version)
		}
		...
		fmt.Fprintf(out, "    No updates yet; run `codeinfra up`\n")
	}
	...
	if resourceCount == 0 {
		...
		if args.showSecrets {
			Log3rdPartySecretsProviderDecryptionEvent(ctx, s, "", "codeinfra stack")
		}
	}
	...
	fmt.Fprintf(out, "Use `codeinfra stack select` to change stack; `codeinfra stack ls` lists known ones\n")
