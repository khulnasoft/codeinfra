...
func HandleConfig(
	...
	// Handle config.
	// If this is an initial preconfigured empty stack (i.e. configured in the Codeinfra Console),
	...
	if isPreconfiguredEmptyStack(templateNameOrURL, template.Config, stackConfig, snap) {
		c = stackConfig
		// TODO[codeinfra/codeinfra#1894] consider warning if templateNameOrURL is different from
		// the stack's `codeinfra:template` config value.
	} else {
...
// isPreconfiguredEmptyStack returns true if the url matches the value of `codeinfra:template` in stackConfig,
...
func isPreconfiguredEmptyStack(
	...
	// Does stackConfig have a `codeinfra:template` value and does it match url?
	if stackConfig == nil {
		return false
	}
...
var templateKey = config.MustMakeKey("codeinfra", "template")
