...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/backend/state"
	cmdBackend "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/backend"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/cmd"
	cmdStack "github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/stack"
	"github.com/khulnasoft/codeinfra/pkg/v3/cmd/codeinfra/ui"
...
type newArgs struct {
	...
	listTemplates        bool
	aiPrompt             string
	aiLanguage           httpstate.CodeinfraAILanguage
	templateMode         bool
...
func runNew(ctx context.Context, args newArgs) error {
		...
		if aiOrTemplate == "ai" {
			...
			if !ok {
				if args.aiLanguage != "" && args.aiPrompt == "" {
					return errors.New(
						"--language is used to generate a template with Codeinfra AI. " +
							"Please log in to Codeinfra Cloud to use Codeinfra AI.\n" +
							"Use --template to create a project from a template, " +
							"or no flags to choose one interactively.")
				}
				return errors.New("please log in to Codeinfra Cloud to use Codeinfra AI")
			}
	...
	repo, err = workspace.RetrieveTemplates(
		ctx, args.templateNameOrURL, args.offline, workspace.TemplateKindCodeinfraProject)
	if err != nil {
		// Bail on all errors unless its a 401 from a Codeinfra Cloud backend...
		if !errors.Is(err, workspace.ErrCodeinfraCloudUnauthorized) {
			return err
		}

		// ...If the request has 401'd AND we've identified the backend as being a Codeinfra Cloud instance, we can
		// attempt to retrieve the template using the user's Codeinfra Cloud credentials.
		repo, err = retrievePrivateCodeinfraCloudTemplate(args.templateNameOrURL)
		if err != nil {
			return fmt.Errorf("retrieving private codeinfra cloud template: %w", err)
		}
	}
	...
	if !args.yes && hasAtLeastOnePrompt {
		fmt.Println("This command will walk you through creating a new Codeinfra project.")
		fmt.Println()
		fmt.Println(
			opts.Color.Colorize(
	...
	if args.name == "" {
		...
		if err != nil {
			...
			// See https://github.com/khulnasoft/codeinfra/issues/8747.
			if args.yes {
	...
	// Set the codeinfra:template tag to the template name or URL.
	...
	appendFileName := "Codeinfra.yaml.append"
...
// Retrieve a Private template from the given Codeinfra Cloud URL **including an auth token for Codeinfra Cloud**.
//
// workspace.GetAccount ensures the user has a valid session with the Codeinfra Cloud backend.
//   - If the user is not logged in, the login flow will be initiated.
//   - If the user is not logged in and codeinfra does not recognize the backend as a known workspace then
//     the user will see an authentication error.
func retrievePrivateCodeinfraCloudTemplate(templateURL string) (workspace.TemplateRepository, error) {
	...
	// e.g. `codeinfra login https://api.codeinfra.com` or `codeinfra login https://api.acme.org`
	templateCodeinfraCloudHost := "https://" + u.Host

	account, err := workspace.GetAccount(templateCodeinfraCloudHost)
	if err != nil {
		return workspace.TemplateRepository{}, fmt.Errorf(
			"looking up codeinfra cloud backend %s: %w",
			templateCodeinfraCloudHost,
			err,
		)
	}

	if account.AccessToken == "" {
		return workspace.TemplateRepository{}, fmt.Errorf("no access token found for %s", templateCodeinfraCloudHost)
	}
...
// isInteractive lets us force interactive mode for testing by setting CODEINFRA_TEST_INTERACTIVE.
func isInteractive() bool {
	test, ok := os.LookupEnv("CODEINFRA_TEST_INTERACTIVE")
...
func NewNewCmd() *cobra.Command {
	...
	getTemplates := func(ctx context.Context) ([]workspace.Template, error) {
		...
		repo, err := workspace.RetrieveTemplates(ctx, "", false /*offline*/, workspace.TemplateKindCodeinfraProject)
		if err != nil {
	...
	cmd := &cobra.Command{
		...
		Short:      "Create a new Codeinfra project",
		Long: "Create a new Codeinfra project and stack from a template.\n" +
			"\n" +
			...
			"\n" +
			"By default, a stack created using the codeinfra.com backend will use the codeinfra.com secrets\n" +
			"provider and a stack created using the local or cloud object storage backend will use the\n" +
			"`passphrase` secrets provider.  A different secrets provider can be selected by passing the\n" +
			"`--secrets-provider` flag.\n" +
			"\n" +
			"To use the `passphrase` secrets provider with the codeinfra.com backend, use:\n" +
			"* `codeinfra new --secrets-provider=passphrase`\n" +
			"\n" +
			"To use a cloud secrets provider with any backend, use one of the following:\n" +
			"* `codeinfra new --secrets-provider=\"awskms://alias/ExampleAlias?region=us-east-1\"`\n" +
			"* `codeinfra new --secrets-provider=\"awskms://1234abcd-12ab-34cd-56ef-1234567890ab?region=us-east-1\"`\n" +
			"* `codeinfra new --secrets-provider=\"azurekeyvault://mykeyvaultname.vault.azure.net/keys/mykeyname\"`\n" +
			"* `codeinfra new --secrets-provider=\"gcpkms://projects/p/locations/l/keyRings/r/cryptoKeys/k\"`\n" +
			"* `codeinfra new --secrets-provider=\"hashivault://mykey\"`" +
			"\n\n" +
			"To create a project from a specific source control location, pass the url as follows e.g.\n" +
			"* `codeinfra new https://gitlab.com/<user>/<repo>`\n" +
			"* `codeinfra new https://bitbucket.org/<user>/<repo>`\n" +
			"* `codeinfra new https://github.com/<user>/<repo>`\n" +
			"\n" +
			"To create the project from a branch of a specific source control location, pass the url to the branch, e.g.\n" +
			"* `codeinfra new https://gitlab.com/<user>/<repo>/tree/<branch>`\n" +
			"* `codeinfra new https://bitbucket.org/<user>/<repo>/tree/<branch>`\n" +
			"* `codeinfra new https://github.com/<user>/<repo>/tree/<branch>`\n" +
			"\n" +
			...
			"* `codeinfra new git@github.com:<user>/<private-repo>`\n" +
			"* `codeinfra new https://<user>:<password>@<hostname>/<project>/<repo>`\n" +
			"* `codeinfra new <user>@<hostname>:<project>/<repo>`\n" +
			"* `CODEINFRA_GITSSH_PASSPHRASE=<passphrase> codeinfra new ssh://<user>@<hostname>/<project>/<repo>`\n" +
			"To create a project using Codeinfra AI, either select `ai` from the first selection, " +
			"or provide any of the following:\n" +
			"* `codeinfra new --ai \"<prompt>\"`\n" +
			"* `codeinfra new --language <language>`\n" +
			"* `codeinfra new --ai \"<prompt>\" --language <language>`\n" +
	...
	cmd.PersistentFlags().StringVar(
		&args.aiPrompt, "ai", "", "Prompt to use for Codeinfra AI",
	)
	cmd.PersistentFlags().Var(
		&args.aiLanguage, "language", "Language to use for Codeinfra AI "+
			fmt.Sprintf("(must be one of %s)", httpstate.CodeinfraAILanguagesClause),
	)
	cmd.PersistentFlags().BoolVarP(
...
func printNextSteps(proj *workspace.Project, originalCwd, cwd string, generateOnly bool, opts display.Options) {
	...
	if generateOnly {
		...
		commands = append(commands, "codeinfra install")
		// We didn't create a stack so show that as a command to run before `codeinfra up`.
		commands = append(commands, "codeinfra stack init")
	}

	if len(commands) == 0 { // No additional commands need to be run.
		deployMsg := "To perform an initial deployment, run `codeinfra up`"
		deployMsg = colors.Highlight(deployMsg, "codeinfra up", colors.BrightBlue+colors.Bold)
	...
	if len(commands) == 1 { // Only one additional command need to be run.
		deployMsg := fmt.Sprintf("To perform an initial deployment, run '%s', then, run `codeinfra up`", commands[0])
	...
	upMsg := colors.Highlight("Then, run `codeinfra up`", "codeinfra up", colors.BrightBlue+colors.Bold)
