...
// TestRegress13774 checks that you can run `codeinfra new` on an existing project as described in the
// Codeinfra Cloud new project instructions.
...
func TestRegress13774(t *testing.T) {
	...
	err = runNew(context.Background(), args)
	defer removeStack(t, tempdir, args.stack)
	assert.NoError(t, err, "should be able to run `codeinfra new` successfully on an existing project")
}
...
func skipIfShortOrNoCodeinfraAccessToken(t *testing.T) {
	_, ok := os.LookupEnv("CODEINFRA_ACCESS_TOKEN")
	if !ok {
