...
func TestFailInInteractiveWithoutYes(t *testing.T) {
	skipIfShortOrNoCodeinfraAccessToken(t)
...
func TestCreatingProjectWithEmptyConfig(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/4081
	skipIfShortOrNoCodeinfraAccessToken(t)
	...
	proj := loadProject(t, tempdir)
	projStack, err := workspace.LoadProjectStack(proj, filepath.Join(tempdir, "Codeinfra."+stackName+".yaml"))
	require.NoError(t, err)
...
func TestGenerateOnlyProjectCheck(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/13527, make sure various combinations of
...
func TestCodeinfraNewConflictingProject(t *testing.T) {
	t.Parallel()
...
//nolint:paralleltest // changes directory for process
func TestCodeinfraNewSetsTemplateTag(t *testing.T) {
	tests := []struct {
		argument string
		prompted string
		expected string
	...
	for _, tt := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			proj := loadProject(t, tempdir)
			require.NoError(t, err)
			tagsValue, has := proj.Config[apitype.CodeinfraTagsConfigKey]
			assert.True(t, has)
...
//nolint:paralleltest // changes directory for process
func TestCodeinfraPromptRuntimeOptions(t *testing.T) {
	tempdir := tempProjectDir(t)
	chdir(t, tempdir)
