...
// Iteratively prompt the user for input, sending their input as a prompt tp Codeinfra AI
...
func runAINew(
	...
	languageOptions := make([]string, 0, len(httpstate.CodeinfraAILanguageOptions))
	for _, language := range httpstate.CodeinfraAILanguageOptions {
...
func sendPromptToCodeinfraAI(
	ctx context.Context,
	backend httpstate.Backend,
	promptMessage string,
	conversationID string,
	connectionID string,
	language httpstate.CodeinfraAILanguage,
) (string, string, string, error) {
	codeinfraAIURL := env.AIServiceEndpoint.Value()
	if codeinfraAIURL == "" {
		codeinfraAIURL = "https://www.codeinfra.com/ai"
	}
	parsedURL, err := url.Parse(codeinfraAIURL)
	if err != nil {
		return "", "", "", err
	}
	...
	fmt.Println("Sending prompt to Codeinfra AI...")
	...
	fmt.Println("Codeinfra AI response:")
	for {
		...
		if err != nil && err.Error() == "EOF" {
			// Add a newline to ensure we don't overwrite the last line of the Codeinfra AI response
			fmt.Println()
			break
		}
	}
...
const refinePromptPrompt = "Tell Codeinfra AI how to refine the previous program:"
...
func runAINewPromptStep(
	...
	conversationURLReturn, connectionIDReturn, conversationIDReturn, err = sendPromptToCodeinfraAI(
		ctx,
		backend,
		promptMessage,
		conversationID,
...
func chooseWithAIOrTemplate(opts display.Options) (string, error) {
	...
	optionsDescriptionMap := map[string]string{
		"template": "Create a new Codeinfra project using a template",
		"ai":       "Create a new Codeinfra project using Codeinfra AI",
	}

	var ai string
	if err := survey.AskOne(&survey.Select{
		Message: "Would you like to create a project from a template or using a Codeinfra AI prompt?",
		Options: options,
		Description: func(opt string, _ int) string {
