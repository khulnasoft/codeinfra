...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/ciutil"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/cmdutil"
	declared "github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/env"
...
func GetUpdateMetadata(
	...
	addCodeinfraCLIMetadataToEnvironment(m.Environment, flags, os.Environ)
...
// addCodeinfraCLIMetadataToEnvironment enriches updates with metadata to provide context about
// the system, environment, and options that an update is being performed with.
func addCodeinfraCLIMetadataToEnvironment(env map[string]string, flags *pflag.FlagSet, getEnviron func() []string) {
	// Codeinfra Environment Variables (Name and Set/Truthiness Only)
	...
	// Add all environment variables that start with "CODEINFRA_" and their set-ness or truthiness.
	for _, envvar := range getEnviron() {
		if !strings.HasPrefix(envvar, "CODEINFRA_") {
			// Not a Codeinfra environment variable. Skip.
			continue
		}
		...
		env["codeinfra.env."+name] = flag
	}

	// System information
	env["codeinfra.version"] = version.Version
	env["codeinfra.os"] = runtime.GOOS
	env["codeinfra.arch"] = runtime.GOARCH
	...
	// Codeinfra CLI Flags (Name Only)
	flags.Visit(func(f *pflag.Flag) {
		env["codeinfra.flag."+f.Name] = func() string {
