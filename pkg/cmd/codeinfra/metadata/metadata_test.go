...
func TestReadingGitRepo(t *testing.T) {
	...
	t.Setenv("CODEINFRA_DISABLE_CI_DETECTION", "1")
	...
	os.Unsetenv("CODEINFRA_DISABLE_CI_DETECTION") // Restore our CI/CD detection logic.
	t.Setenv("TRAVIS", "1")
...

	{
		...
		// assert.Equal(t, "branch-from-ci", name) # see https://github.com/khulnasoft/codeinfra/issues/5303
	}
}
...
// TestCodeinfraCLIMetadata tests that the update metadata is correctly populated
// when running a Codeinfra program.
func TestCodeinfraCLIMetadata(t *testing.T) {
	t.Parallel()
	...
	getEnv := func() []string { // This is the function that returns the environment variables.
		return []string{
			...
			"CODEINFRA_STRING_FLAG=true",
			// Checks that an old and forgotten CLI flag is detected.
			"CODEINFRA_DEPRECATED_FLAG=1.2.3",
		}
	}
	...
	subCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			addCodeinfraCLIMetadataToEnvironment(actualEnv, cmd.Flags(), getEnv)
		},
	}

	cmd.AddCommand(subCmd)
	cmd.SetArgs([]string{"subcommand", "--name", "codeinfrapus", "--age", "100", "--human"})
	...
	assert.Equal(t, map[string]string{
		...
		"codeinfra.version": "SKIP",
		"codeinfra.arch":    "SKIP",
		"codeinfra.os":      "SKIP",

		// CLI flags
		"codeinfra.flag.name":  "set",
		"codeinfra.flag.age":   "set",
		"codeinfra.flag.human": "true",

		// Environment variables

		// Env Vars are considered truthy if they are "1" or "true" case-insensitive.
		"codeinfra.env.CODEINFRA_SKIP_UPDATE_CHECK": "true",
		// The following is a boolean flag that is set to "0", which is falsey.
		"codeinfra.env.CODEINFRA_EXPERIMENTAL": "false",
		// The following is truthy, but NOT a boolean flag.
		"codeinfra.env.CODEINFRA_STRING_FLAG": "set",
		// The following is falsey, but NOT a boolean flag.
		"codeinfra.env.CODEINFRA_DEPRECATED_FLAG": "set",
	}, actualEnv)
}
