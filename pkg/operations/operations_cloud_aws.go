...
// TODO[codeinfra/codeinfra#54] This should be factored out behind an OperationsProvider RPC interface and versioned with the
// `codeinfra-cloud` repo instead of statically linked into the engine.

// CloudOperationsProvider creates an OperationsProvider capable of answering operational queries based on the
// underlying resources of the `@codeinfra/cloud-aws` implementation.
...
const (
	// Codeinfra Framework component types
			...
			if extractedLog != nil {
				...
				if len(match) != 2 {
					logging.V(5).Infof("Skipping invalid log name found in log collector %s. "+
						"Possibly mismatched versions of codeinfra and codeinfra-cloud.", state.URN)
					continue
				}
				logName := match[1]
...
var (
	...
	// Used prior to codeinfra-terraform@1307256eeeefdd87ffd76581cd3ab73c3d7cfd4a
