...
// `codeinfra-aws` repo instead of statically linked into the engine.

// AWSOperationsProvider creates an OperationsProvider capable of answering operational queries based on the
// underlying resources of the `@codeinfra/aws` implementation.
func AWSOperationsProvider(
	...
	//
	// [codeinfra/codeinfra#608]: We are only approximating the actual logic that the AWS provider (via
