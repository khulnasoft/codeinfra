...
func (d deleteProtectedError) Error() string {
	return fmt.Sprintf("resource %[1]q cannot be deleted\n"+
		...
		"either remove the `protect` flag from the resource in your Codeinfra "+
		"program and run `codeinfra up`, or use the command:\n"+
		"`codeinfra state unprotect %[2]s`", d.urn, d.urn.Quote())
}
...
func (s *DeleteStep) Apply() (resource.Status, StepCompleteFunc, error) {
	...
	if s.deployment.opts.DryRun {
		...
		// Deleting an External resource is a no-op, since Codeinfra does not own the lifecycle.
	...
	// two kinds of replacements Codeinfra currently supports:
...
// ReadStep is a step indicating that an existing resources will be "read" and projected into the Codeinfra object
...
func (s *RefreshStep) ResultOp() display.StepOp {
	...
	// refresh:
	//
	// * The resource is external, that is, it is not managed by Codeinfra.
...
func (s *RefreshStep) Apply() (resource.Status, StepCompleteFunc, error) {
	...
	if err != nil {
		...
		if initErr, isInitErr := err.(*plugin.InitError); isInitErr {
			...
			//    `codeinfra up` will surface them to the user.
			err = nil
...
type ImportStep struct {
	...
	replacing     bool                           // true if we are replacing a Codeinfra-managed resource.
...
func (s *ImportStep) Apply() (resource.Status, StepCompleteFunc, error) {
	...
	// Import takes a resource that Codeinfra did not create and imports it into codeinfra state.
	now := time.Now().UTC()
	s.new.Modified = &now
