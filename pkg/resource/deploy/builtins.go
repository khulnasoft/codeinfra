...
// The built-in provider provides resources and functions in the `codeinfra` package, such as stack references and the
...
const stackReferenceType = "codeinfra:codeinfra:StackReference"
...
func (p *builtinProvider) Check(_ context.Context, req plugin.CheckRequest) (plugin.CheckResponse, error) {
	...
	// call Check first.
	msg := "The \"codeinfra:codeinfra:StackReference\" resource type is deprecated. " +
		"Update your SDK or if already up to date raise an issue at https://github.com/khulnasoft/codeinfra/issues."
...
const (
	readStackOutputs         = "codeinfra:codeinfra:readStackOutputs"
	readStackResourceOutputs = "codeinfra:codeinfra:readStackResourceOutputs" //nolint:gosec // not a credential
	getResource              = "codeinfra:codeinfra:getResource"
)
