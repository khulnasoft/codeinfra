...
type Source interface {
	...
	// Project returns the package name of the Codeinfra project we are obtaining resources from.
...
type SourceResourceMonitor interface {
	// NOTE: This interface does not implement codeinfrarpc.ResourceMonitorClient because the eval and
	// query implementations of `Source` do not implement precisely the same signatures.

	Address() string
	Cancel() error
	AbortChan() <-chan bool
	Invoke(ctx context.Context, req *codeinfrarpc.ResourceInvokeRequest) (*codeinfrarpc.InvokeResponse, error)
	Call(ctx context.Context, req *codeinfrarpc.ResourceCallRequest) (*codeinfrarpc.CallResponse, error)
