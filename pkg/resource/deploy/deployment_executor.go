...
func (ex *deploymentExecutor) printPendingOperationsWarning() {
	...
	resolutionMessage := "" +
		"These resources are in an unknown state because the Codeinfra CLI was interrupted while " +
		"waiting for changes to these resources to complete. You should confirm whether or not the " +
		"operations listed completed successfully by checking the state of the appropriate provider. " +
		"For example, if you are using AWS, you can confirm using the AWS Console.\n" +
		"\n" +
		"Once you have confirmed the status of the interrupted operations, you can repair your stack " +
		"using `codeinfra refresh` which will refresh the state from the provider you are using and " +
		"clear the pending operations if there are any.\n" +
		"\n" +
		"Note that `codeinfra refresh` will need to be run interactively to clear pending CREATE operations."
...
func (ex *deploymentExecutor) Execute(callerCtx context.Context) (*Plan, error) {
	...
	if ex.deployment.opts.Refresh {
		...
		// Explain that these operations can be cleared using codeinfra refresh (except for CREATE operations)
