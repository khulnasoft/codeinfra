...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/resource/graph"
...
func (sg *stepGenerator) checkParent(parent resource.URN, resourceType tokens.Type) (resource.URN, error) {
	...
	if parent != "" {
	...
	} else { //nolint:staticcheck // https://github.com/khulnasoft/codeinfra/issues/10950
		// Else try and set it to the root stack

		// TODO: It looks like this currently has some issues with state ordering (see
		// https://github.com/khulnasoft/codeinfra/issues/10950). Best I can guess is the stack resource is
		...
		// else is causing a codeinfra:providers:codeinfra default provider to be created, this picks up the
...
func (sg *stepGenerator) generateURN(
	...
	if sg.urns[urn] {
		// TODO[codeinfra/codeinfra-framework#19]: improve this error message!
...
func (sg *stepGenerator) GenerateReadSteps(event ReadResourceEvent) ([]Step, error) {
	...
	// release of a resource from the management of Codeinfra.
...
func (sg *stepGenerator) inheritedChildAlias(
	...
	// * parentAlias: "urn:codeinfra:stackname::projectname::awsx:ec2:Vpc::app"
	// * parentAliasName: "app"
	// * aliasName: "app-function"
	// * childAlias: "urn:codeinfra:stackname::projectname::aws:s3/bucket:Bucket::app-function"
...
func (sg *stepGenerator) generateSteps(event RegisterResourceEvent) ([]Step, error) {
	...
	// Internally managed resources are under Codeinfra's control and changes or creations should be invisible to
	// the user, we also implicitly target providers (both default and explicit, see
	// https://github.com/khulnasoft/codeinfra/issues/13557 and https://github.com/khulnasoft/codeinfra/issues/13591 for
	// context on why).
	...
	if prov != nil {
		...
		// don't consider those inputs since Codeinfra does not own them. Finally, if the resource has been
	...
	//  was "external" - Codeinfra does not own its lifecycle. Conceptually, this operation is
		...
		if !isTargeted {
			...
			if old.Provider != "" {
				...
				if !has {
					...
					// provider version in a targeted update. See https://github.com/khulnasoft/codeinfra/issues/15704
			...
			//   Codeinfra can retry the deletion in a subsequent operation. Setting
		...
		if diff.Replace() || old.PendingReplacement {
			...
			if new.Protect && old.Protect {
				message := fmt.Sprintf("unable to replace resource %q\n"+
					...
					"remove the `protect` flag from the resource in your Codeinfra "+
					"program and run `codeinfra up`", urn)
					...
					if dependentResource.Delete {
						...
						if dependentResource.Protect {
							message := fmt.Sprintf("unable to replace resource %q as part of replacing %q "+
								"as it is currently marked for protection. To unprotect the resource, "+
								"remove the `protect` flag from the resource in your Codeinfra "+
								"program and run `codeinfra up`, or use the command:\n"+
								"`codeinfra state unprotect %q`",
...
func (sg *stepGenerator) GenerateDeletes(targetsOpt UrnTargets) ([]Step, error) {
	...
	// If -target was provided to either `codeinfra update` or `codeinfra destroy` then only delete
...
func (sg *stepGenerator) providerChanged(urn resource.URN, old, new *resource.State) (bool, error) {
	...
	// codeinfra/codeinfra#2753 for more details.
	//
	// Recent versions of Codeinfra allow for language hosts to pass a plugin version to the engine. The purpose of this is
...
func diffResource(urn resource.URN, id resource.ID, oldInputs, oldOutputs,
	...
	// Grab the diff from the provider. At this point we know that there were changes to the Codeinfra inputs, so if the
