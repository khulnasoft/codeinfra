...
func TestLoadProvider_missingError(t *testing.T) {
	...
	t.Run("CODEINFRA_DISABLE_AUTOMATIC_PLUGIN_ACQUISITION=true", func(t *testing.T) {
		t.Setenv("CODEINFRA_DISABLE_AUTOMATIC_PLUGIN_ACQUISITION", "true")
		...
		assert.ErrorContains(t, err,
			"no resource plugin 'codeinfra-resource-myplugin' found in the workspace at version v1.2.3")
		assert.Equal(t, 0, count)
	})

	t.Run("CODEINFRA_DISABLE_AUTOMATIC_PLUGIN_ACQUISITION=false", func(t *testing.T) {
		...
		_, err := loadProvider(
			...
			nil, host, nil /* builtins */)
		assert.ErrorContains(t, err,
			"Could not automatically download and install resource plugin 'codeinfra-resource-myplugin' at version v1.2.3")
		assert.ErrorContains(t, err,
			fmt.Sprintf("install the plugin using `codeinfra plugin install resource myplugin v1.2.3 --server %s`", srv.URL))
		assert.Equal(t, 5, count)
	})
}

func TestConcurrentRegistryUsage(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/13491, make sure we can use registry in
	// parallel.
...
func TestRegistry(t *testing.T) {
	...
	t.Run("Pkg", func(t *testing.T) {
		t.Parallel()
		r := &Registry{}
		assert.Equal(t, tokens.Package("codeinfra"), r.Pkg())
	})
