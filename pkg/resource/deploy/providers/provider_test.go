...
func TestProviderRequestNameNoPreURL(t *testing.T) {
	...
	ver := semver.MustParse("0.18.1")
	req := NewProviderRequest("pkg", &ver, "codeinfra.com/pkg", nil, nil)
	assert.Equal(t, "default_0_18_1_codeinfra.com/pkg", req.DefaultName())
	assert.Equal(t, "pkg-0.18.1-codeinfra.com/pkg", req.String())
}
