...
// string representation is <URN> "::" <ID>. The URN's type portion must be of the form "codeinfra:providers:<pkg>".
...
// IsProviderType returns true if the supplied type token refers to a Codeinfra provider.
func IsProviderType(typ tokens.Type) bool {
	...
	return typ.Module() == "codeinfra:providers" && typ.Name() != ""
}

// IsDefaultProvider returns true if this URN refers to a default Codeinfra provider.
...
func validateURN(urn resource.URN) error {
	...
	typ := urn.Type()
	if typ.Module() != "codeinfra:providers" {
		return fmt.Errorf("invalid module in type: expected 'codeinfra:providers', got '%v'", typ.Module())
	}
	if typ.Name() == "" {
...
func NewDenyDefaultProvider(name string) Reference {
	return mustNewReference(
		resource.NewURN("denied", "denied", "denied", "codeinfra:providers:denied", name),
...
// "urn:codeinfra:stack::project::codeinfra:providers:aws::default_4_35_0"
