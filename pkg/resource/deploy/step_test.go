	...
	t.Run("Apply", func(t *testing.T) {
		...
		t.Run("bad provider state for resource", func(t *testing.T) {
			...
			s := &SameStep{
				...
				old: &resource.State{
					URN: "urn:codeinfra:stack::project::type::foo",
				},
				new: &resource.State{
					URN:  "urn:codeinfra:stack::project::type::foo",
					Type: "codeinfra:providers:some-provider",
				},
			}
			_, _, err := s.Apply()
			...
			t.Run("error getting provider", func(t *testing.T) {
				...
				s := &CreateStep{
					...
					new: &resource.State{
						...
						Provider: "urn:codeinfra:stack::project::codeinfra:providers:aws::default_5_42_0::denydefaultprovider",
					},
				}
	...
	t.Run("Apply", func(t *testing.T) {
		...
		t.Run("missing parent", func(t *testing.T) {
			...
			s := &ImportStep{
				planned: true,
				new: &resource.State{
					Parent: "urn:codeinfra:stack::project::foo:bar:Bar::name",
				},
				deployment: &Deployment{
					opts: &Options{
						DryRun: true,
		...
		t.Run("getProvider error", func(t *testing.T) {
			...
			s := &ImportStep{
				...
				new: &resource.State{
					URN:    "urn:codeinfra:stack::project::foo:bar:Bar::name",
					ID:     "some-id",
					Custom: true,
				},
			}
			...
			t.Run("failures", func(t *testing.T) {
				...
				s := &ImportStep{
					...
					new: &resource.State{
						...
						Type:     "foo:bar:Bar",
						Custom:   true,
						Parent:   "urn:codeinfra:stack::project::codeinfra:codeinfra:Stack::name",
