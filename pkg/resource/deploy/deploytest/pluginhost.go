...
func wrapProviderWithGrpc(provider plugin.Provider) (plugin.Provider, io.Closer, error) {
	...
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		...
		Init: func(srv *grpc.Server) error {
			codeinfrarpc.RegisterResourceProviderServer(srv, plugin.NewProviderServer(provider))
			return nil
		},
	...
	wrapped := plugin.NewProviderWithClient(nil, provider.Pkg(), codeinfrarpc.NewResourceProviderClient(conn), false)
	return wrapped, wrapper, nil
}

type hostEngine struct {
	codeinfrarpc.UnsafeEngineServer // opt out of forward compat
...
func (e *hostEngine) Log(_ context.Context, req *codeinfrarpc.LogRequest) (*emptypb.Empty, error) {
