...
type ResourceMonitor struct {
	...
	resmon codeinfrarpc.ResourceMonitorClient
...
func supportsFeature(ctx context.Context, resmon codeinfrarpc.ResourceMonitorClient, id string) (bool, error) {
	resp, err := resmon.SupportsFeature(ctx, &codeinfrarpc.SupportsFeatureRequest{Id: id})
	if err != nil {
		return false, err
	}
...
func parseSourcePosition(raw string) (*codeinfrarpc.SourcePosition, error) {
	u, err := url.Parse(raw)
	if err != nil {
		return nil, err
	}

	pos := codeinfrarpc.SourcePosition{
...
func NewResourceMonitor(resmon codeinfrarpc.ResourceMonitorClient) *ResourceMonitor {
...
func (rm *ResourceMonitor) RegisterResource(t tokens.Type, name string, custom bool,
	...
	inputDeps := make(map[string]*codeinfrarpc.RegisterResourceRequest_PropertyDependencies)
	for pk, pd := range opts.PropertyDeps {
		pdeps := []string{}
		for _, d := range pd {
			pdeps = append(pdeps, string(d))
		}
		inputDeps[string(pk)] = &codeinfrarpc.RegisterResourceRequest_PropertyDependencies{
			Urns: pdeps,
		}
	}
	...
	if opts.CustomTimeouts != nil {
		timeouts = &codeinfrarpc.RegisterResourceRequest_CustomTimeouts{
	...
	requestInput := &codeinfrarpc.RegisterResourceRequest{
		Type:                       string(t),
		Name:                       name,
		Custom:                     custom,
...
func (rm *ResourceMonitor) RegisterResourceOutputs(urn resource.URN, outputs resource.PropertyMap) error {
	...
	// submit request
	_, err = rm.resmon.RegisterResourceOutputs(context.Background(), &codeinfrarpc.RegisterResourceOutputsRequest{
		Urn:     string(urn),
		Outputs: outs,
	})
	return err
...
func (rm *ResourceMonitor) ReadResource(t tokens.Type, name string, id resource.ID, parent resource.URN,
	...
	var sourcePos *codeinfrarpc.SourcePosition
	if sourcePosition != "" {
	...
	// submit request
	resp, err := rm.resmon.ReadResource(context.Background(), &codeinfrarpc.ReadResourceRequest{
		Type:           string(t),
		Name:           name,
		Id:             string(id),
...
func (rm *ResourceMonitor) Invoke(tok tokens.ModuleMember, inputs resource.PropertyMap,
	...
	// submit request
	resp, err := rm.resmon.Invoke(context.Background(), &codeinfrarpc.ResourceInvokeRequest{
		Tok:        string(tok),
		Provider:   provider,
		Args:       ins,
		Version:    version,
...
func (rm *ResourceMonitor) Call(
	...
	mArgDependencies := make(map[string]*codeinfrarpc.ResourceCallRequest_ArgumentDependencies)
	for k, p := range argDependencies {
		urns := make([]string, len(p))
		for i, urn := range p {
			urns[i] = string(urn)
		}
		mArgDependencies[string(k)] = &codeinfrarpc.ResourceCallRequest_ArgumentDependencies{
			Urns: urns,
		}
	}

	// submit request
	resp, err := rm.resmon.Call(context.Background(), &codeinfrarpc.ResourceCallRequest{
		Tok:             string(tok),
		Provider:        provider,
		Args:            mArgs,
...
func (rm *ResourceMonitor) RegisterPackage(pkg, version, downloadURL string, checksums map[string][]byte,
	...
	resp, err := rm.resmon.RegisterPackage(context.Background(), &codeinfrarpc.RegisterPackageRequest{
		Name:             pkg,
		Version:          version,
