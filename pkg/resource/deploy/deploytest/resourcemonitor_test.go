...
func TestResourceMonitor_Call_deps(t *testing.T) {
	...
	client := stubResourceMonitorClient{
		...
		CallFunc: func(req *codeinfrarpc.ResourceCallRequest) (*codeinfrarpc.CallResponse, error) {
...
func TestResourceMonitor_RegisterResource_customTimeouts(t *testing.T) {
	...
	tests := []struct {
		...
		want *codeinfrarpc.RegisterResourceRequest_CustomTimeouts
	}{
		{desc: "nil", give: nil, want: nil},
		{
			desc: "create",
			give: &resource.CustomTimeouts{Create: 1},
			want: &codeinfrarpc.RegisterResourceRequest_CustomTimeouts{Create: "1s"},
		},
		{
			desc: "update",
			give: &resource.CustomTimeouts{Update: 1},
			want: &codeinfrarpc.RegisterResourceRequest_CustomTimeouts{Update: "1s"},
		},
		{
			desc: "delete",
			give: &resource.CustomTimeouts{Delete: 1},
			want: &codeinfrarpc.RegisterResourceRequest_CustomTimeouts{Delete: "1s"},
		},
		{
			desc: "all",
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			client := stubResourceMonitorClient{
				RegisterResourceFunc: func(
					...
					return &codeinfrarpc.RegisterResourceResponse{}, nil
				},
			}
...
type stubResourceMonitorClient struct {
	...
	CallFunc             func(req *codeinfrarpc.ResourceCallRequest) (*codeinfrarpc.CallResponse, error)
	RegisterResourceFunc func(req *codeinfrarpc.RegisterResourceRequest) (*codeinfrarpc.RegisterResourceResponse, error)
}
