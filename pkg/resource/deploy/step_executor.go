...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/util/gsync"
...
// Chains are a simplification of the full dependency graph DAG within Codeinfra programs. Since
// Codeinfra language hosts can only invoke the resource monitor once all of their dependencies have
...
func (se *stepExecutor) executeStep(workerID int, step Step) error {
	...
	// https://github.com/khulnasoft/codeinfra/issues/14994).
	if step.New() != nil && step.Op() != OpReplace {
		...
		for _, k := range newState.AdditionalSecretOutputs {
			if k == "id" {
				se.deployment.Diag().Warningf(&diag.Diag{
					URN:     step.URN(),
					Message: "The 'id' property cannot be made secret. See codeinfra/codeinfra#2717 for more details.",
				})
			} else {
				if v, has := newState.Outputs[k]; has && !v.IsSecret() {
					newState.Outputs[k] = resource.MakeSecret(v)
				} else if !has { //nolint:staticcheck // https://github.com/khulnasoft/codeinfra/issues/9926
					// TODO (https://github.com/khulnasoft/codeinfra/issues/9926): We want to re-enable this warning
		...
		// references using codeinfra:codeinfra:getResource. If it's a resource managed by Codeinfra (i.e. it's the result of a
		// resource registration with a goal state), we'll record it in news. If it's not managed by Codeinfra (i.e. it's the
