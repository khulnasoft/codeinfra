...
import (
	...
	resource_testing "github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource/testing"
...
// references were produced by Codeinfra v2.18.0.
func TestDeserializeResourceReferencePropertyValueID(t *testing.T) {
	t.Parallel()

	// Serialize replicates Codeinfra 2.18.0's buggy resource reference serializer. We round-trip the value through JSON
...
func TestDeserializeDeploymentSecretCache(t *testing.T) {
	...
	urn := "urn:codeinfra:prod::acme::acme:erp:Backend$aws:ebs/volume:Volume::PlatformBackendDb"
	ctx := context.Background()
...
func TestDeserializeMissingSecretsManager(t *testing.T) {
	...
	urn := "urn:codeinfra:urn:codeinfra:test_stack::test_project::pkg:index:type::name"
	ctx := context.Background()
	deployment, err := DeserializeDeploymentV3(ctx, apitype.DeploymentV3{
		Resources: []apitype.ResourceV3{
			{
