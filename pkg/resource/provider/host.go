...
func NewHostClient(addr string) (*HostClient, error) {
	...
	return &HostClient{
		conn:   conn,
		client: codeinfrarpc.NewEngineClient(conn),
	}, nil
}
...
func (host *HostClient) log(
	...
	var rpcsev codeinfrarpc.LogSeverity
	switch sev {
	case diag.Debug:
		rpcsev = codeinfrarpc.LogSeverity_DEBUG
	case diag.Info, diag.Infoerr:
		rpcsev = codeinfrarpc.LogSeverity_INFO
	case diag.Warning:
		rpcsev = codeinfrarpc.LogSeverity_WARNING
	case diag.Error:
		rpcsev = codeinfrarpc.LogSeverity_ERROR
	default:
		contract.Failf("Unrecognized log severity type: %v", sev)
	}
	_, err := host.client.Log(context, &codeinfrarpc.LogRequest{
		Severity:  rpcsev,
