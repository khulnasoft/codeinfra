...
import (
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/testing"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/workspace"
	"github.com/stretchr/testify/assert"
)

// CreateBasicCodeinfraRepo will initialize the environment with a basic Codeinfra repository and
// project file definition. Returns the repo owner and name used.
func CreateBasicCodeinfraRepo(e *testing.Environment) {
	e.RunCommand("git", "init")

	contents := "name: codeinfra-test\ndescription: a test\nruntime: nodejs\n"
...
// CreateCodeinfraRepo will initialize the environment with a basic Codeinfra repository and
// project file definition based on the project file content.
// Returns the repo owner and name used.
func CreateCodeinfraRepo(e *testing.Environment, projectFileContent string) {
	e.RunCommand("git", "init")
...
// GetStacks returns the list of stacks and current stack by scraping `codeinfra stack ls`.
// Assumes .codeinfra is in the current working directory. Fails the test on IO errors.
func GetStacks(e *testing.Environment) ([]string, *string) {
	out, err := e.RunCommand("codeinfra", "stack", "ls")
	...
	// TODO(codeinfra/codeinfra/issues/496): Provide structured output for codeinfra commands. e.g., so we can avoid this
