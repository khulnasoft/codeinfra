...
func RunCommand(t *testing.T, name string, args []string, wd string, opts *ProgramTestOptions) error {
	return RunCommandCodeinfraHome(t, name, args, wd, opts, "")
}

// RunCommandCodeinfraHome executes the specified command and additional arguments, wrapping any output in the
// specialized test output streams that list the location the test is running in, and sets the CODEINFRA_HOME
// environment variable.
func RunCommandCodeinfraHome(
	t *testing.T,
	name string,
	args []string,
	wd string,
	...
	env = append(env, "CODEINFRA_DEBUG_COMMANDS=true")
	env = append(env, "CODEINFRA_RETAIN_CHECKPOINTS=true")
	env = append(env, "CODEINFRA_CONFIG_PASSPHRASE=correct horse battery staple")
	if codeinfraHome != "" {
		env = append(env, "CODEINFRA_HOME="+codeinfraHome)
	}
	...
	var runout []byte
	var runerr error
	if opts.Verbose || os.Getenv("CODEINFRA_VERBOSE_TEST") != "" {
