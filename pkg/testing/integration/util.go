...
func CheckRuntimeOptions(t *testing.T, root string, expected map[string]interface{}) {
	...
	yamlFile, err := os.ReadFile(filepath.Join(root, "Codeinfra.yaml"))
	if err != nil {
		t.Logf("could not read Codeinfra.yaml in %s", root)
		t.FailNow()
	}
	if err := yaml.Unmarshal(yamlFile, &config); err != nil {
		t.Logf("could not parse Codeinfra.yaml in %s", root)
		t.FailNow()
	}
