...
func newCodeinfraPackage() *Package {
	spec := PackageSpec{
		Name:        "codeinfra",
		DisplayName: "Codeinfra",
		Version:     "1.0.0",
		Description: "mock codeinfra package",
		Resources: map[string]ResourceSpec{
			"codeinfra:codeinfra:StackReference": {
				ObjectTypeSpec: ObjectTypeSpec{
					Properties: map[string]PropertySpec{
						"outputs": {TypeSpec: TypeSpec{
							Type: "object",
							AdditionalProperties: &TypeSpec{
								Ref: "codeinfra.json#/Any",
							},
						}},
					},
					Required: []string{
	...
	contract.AssertNoErrorf(err, "failed to bind mock codeinfra package")
	return pkg
}
...
var DefaultCodeinfraPackage = newCodeinfraPackage()
