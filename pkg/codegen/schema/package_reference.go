...
// A PackageReference represents a references Codeinfra Package. Applications that do not need access to the entire
// definition of a Codeinfra Package should use PackageReference rather than Package, as the former uses memory more
...
func (p *PartialPackage) Provider() (*Resource, error) {
	...
	provider, diags, err := p.types.bindResourceDef("codeinfra:providers:" + p.spec.Name)
	if err != nil {
		return nil, err
	}
	if diags.HasErrors() {
...
func (p *PartialPackage) Snapshot() (*Package, error) {
	...
	config := p.config
	provider := p.types.resourceDefs["codeinfra:providers:"+p.spec.Name]
