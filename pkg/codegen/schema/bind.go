...
// input against the Codeinfra package metaschema. ImportSpec should only be used to load packages that are assumed to be
...
func (s packageSpecSource) GetResourceSpec(token string) (ResourceSpec, bool, error) {
	if token == "codeinfra:providers:"+s.spec.Name {
...
func (t *types) parseTypeSpecRef(refPath, ref string) (typeSpecRef, hcl.Diagnostics) {
	...
	case "provider":
		...
		token = "codeinfra:providers:" + pkgName
...
func (t *types) bindTypeSpecRef(path string, spec TypeSpec, inputShape bool) (Type, hcl.Diagnostics, error) {
	...
	switch spec.Ref {
	case "codeinfra.json#/Archive":
		return ArchiveType, nil, nil
	case "codeinfra.json#/Asset":
		return AssetType, nil, nil
	case "codeinfra.json#/Json":
		return JSONType, nil, nil
	case "codeinfra.json#/Any":
...
func (t *types) bindResourceDef(token string) (res *Resource, diags hcl.Diagnostics, err error) {
	...
	if token == "codeinfra:providers:"+t.pkg.Name {
...
func (t *types) bindProvider(decl *Resource) (hcl.Diagnostics, error) {
	spec, ok, err := t.spec.GetResourceSpec("codeinfra:providers:" + t.pkg.Name)
	if err != nil {
		return nil, err
	}
	contract.Assertf(ok, "provider resource %q not found", t.pkg.Name)

	diags, err := t.bindResourceDetails("#/provider", "codeinfra:providers:"+t.pkg.Name, spec, decl)
	if err != nil {
		return diags, err
	}
...
func (t *types) finishResources(tokens []string) (*Resource, []*Resource, hcl.Diagnostics, error) {
	...
	provider, provDiags, err := t.bindResourceTypeDef("codeinfra:providers:" + t.pkg.Name)
	if err != nil {
