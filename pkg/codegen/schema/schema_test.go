...
func TestRoundtripRemoteTypeRef(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/13000
	t.Parallel()
...
func TestRoundtripLocalTypeRef(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/13671
	t.Parallel()
...
func TestRoundtripEnum(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/13921
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			want: typeSpecRef{
				...
				Version: toVersionPtr("1.2.3"),
				Kind:    "provider",
				Token:   "codeinfra:providers:test",
			},
		},
		{
...
func TestLoaderRespectsDebugProviders(t *testing.T) {
	...
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		...
		Init: func(srv *grpc.Server) error {
			codeinfrarpc.RegisterResourceProviderServer(srv, &debugProvidersHelperServer{})
			return nil
		},
	})
	require.NoError(t, err)
	...
	t.Setenv("CODEINFRA_DEBUG_PROVIDERS", fmt.Sprintf("imaginary:%d", handle.Port))
...
func (*debugProvidersHelperServer) GetSchema(
	ctx context.Context, req *codeinfrarpc.GetSchemaRequest,
	...
	return &codeinfrarpc.GetSchemaResponse{Schema: string(bytes)}, nil
}

func (*debugProvidersHelperServer) GetPluginInfo(
	context.Context, *emptypb.Empty,
) (*codeinfrarpc.PluginInfo, error) {
	return &codeinfrarpc.PluginInfo{Version: "0.0.1"}, nil
}

func (*debugProvidersHelperServer) Attach(
	context.Context, *codeinfrarpc.PluginAttach,
) (*emptypb.Empty, error) {
	return &emptypb.Empty{}, nil
}

// This is the host that codeinfra-yaml is using. Somehow the test does not work with utils.NewHost,
// perhaps that does not support CODEINFRA_DEBUG_PROVIDERS yet.
...
func TestProviderVersionIsAnError(t *testing.T) {
	// c.f. https://github.com/khulnasoft/codeinfra/issues/16757
	t.Parallel()
