(pcl)=
# Codeinfra Configuration Language (PCL)

*Codeinfra Configuration Language* (PCL) is an internal representation of Codeinfra
programs which supports all core concepts of the Codeinfra programming model in a
...
and to various supported Codeinfra languages.

Codeinfra supports a number of operations pertaining to PCL programs:
...
* [*Type checking*](pcl-type-checking) and [*binding*](pcl-binding), in which a
  ...
  on), associated with ("bound to") a set of Codeinfra [*schema*](schema), and type
...
* [*Code generation*](programgen) ("programgen"), in which a PCL program is
  ...
  underpins the core of tools such as `codeinfra convert`.
