...
func BindProgram(files []*syntax.File, opts ...BindOption) (*Program, hcl.Diagnostics, error) {
	...
	for name, fn := range codeinfraBuiltins(options) {
...
// Evaluate a constant string attribute that's internal to Codeinfra, e.g.: the Logical Name on a resource or output.
