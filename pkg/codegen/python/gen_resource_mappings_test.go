...
// The schema is taken from `codeinfra-aws` and minified to the smallest
...
func TestGenResourceMappingsIsDeterministic(t *testing.T) {
	...
	generateInitHash := func() string {
		...
		file, haveFile := files["codeinfra_aws/__init__.py"]
		if !haveFile {
			t.Error("Cannot find codeinfra_aws/__init__.py in the generated files")
			return ""
		}
