...
from codeinfra.runtime.sync_await import _sync_await
...
def _get_semver_version():
    ...
    # Unfortunately, PEP440 and semver differ slightly in incompatible ways. The Codeinfra engine expects
...
def get_resource_opts_defaults() -> codeinfra.ResourceOptions:
    return codeinfra.ResourceOptions(
        version=get_version(),
        plugin_download_url=get_plugin_download_url(),
    )

def get_invoke_opts_defaults() -> codeinfra.InvokeOptions:
    return codeinfra.InvokeOptions(
        version=get_version(),
        plugin_download_url=get_plugin_download_url(),
    )
...
# Temporary: just use codeinfra._utils.lazy_import once everyone upgrades.
def lazy_import(fullname):

    import codeinfra._utils as u
...
# Copied from codeinfra._utils.lazy_import, see comments there.
...
class Module(codeinfra.runtime.ResourceModule):
    ...
    def construct(self, name: str, typ: str, urn: str) -> codeinfra.Resource:
        ...
        return TheClass(name, codeinfra.ResourceOptions(urn=urn))
...
def register(resource_modules, resource_packages):
    ...
    for pkg_info in resource_packages:
        codeinfra.runtime.register_resource_package(pkg_info['pkg'], Package(pkg_info))

    for mod_info in resource_modules:
        codeinfra.runtime.register_resource_module(
            mod_info['pkg'],
            mod_info['mod'],
            Module(mod_info))
...
def lift_output_func(func: typing.Any) -> typing.Callable[[_F], _F]:
    ...
    def lifted_func(*args, opts=None, **kwargs):
        ...
        # Convert tuple to list, see codeinfra/codeinfra#8172
        args_list = list(bound_args.args)
        return codeinfra.Output.from_input({
            'args': args_list,
...
def call_plain(
    tok: str,
    props: codeinfra.Inputs,
    res: typing.Optional[codeinfra.Resource] = None,
    typ: typing.Optional[type] = None,
) -> typing.Any:
    """
    Wraps codeinfra.runtime.plain to force the output and return it plainly.
    """

    output = codeinfra.runtime.call(tok, props, res, typ)
...
async def _await_output(o: codeinfra.Output[typing.Any]) -> typing.Tuple[object, bool, bool, set]:
    return (
        await o._future,
        await o._is_known,
        await o._is_secret,
...
# of the Codeinfra SDK (<3.121.0) that does not include the `deprecated` decorator.
def deprecated(message: str) -> typing.Callable[[C], C]:
    ...
    so that Codeinfra code can detect that it is deprecated and react appropriately
    ...
    def decorator(fn: C) -> C:
        ...
        def deprecated_fn(*args, **kwargs):
            ...
            codeinfra.warn(f"{fn.__name__} is deprecated: {message}")

            return fn(*args, **kwargs)

        deprecated_fn.__dict__["_codeinfra_deprecated_callable"] = fn
