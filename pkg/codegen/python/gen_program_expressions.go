...
var functionImports = map[string][]string{
	"fileArchive":      {"codeinfra"},
	"remoteArchive":    {"codeinfra"},
	"assetArchive":     {"codeinfra"},
	"fileAsset":        {"codeinfra"},
	"stringAsset":      {"codeinfra"},
	"remoteAsset":      {"codeinfra"},
	"filebase64":       {"base64"},
...
func (g *generator) GenFunctionCallExpression(w io.Writer, expr *model.FunctionCallExpression) {
	...
	case "fileArchive":
		g.Fgenf(w, "codeinfra.FileArchive(%.v)", expr.Args[0])
	case "remoteArchive":
		g.Fgenf(w, "codeinfra.RemoteArchive(%.v)", expr.Args[0])
	case "assetArchive":
		g.Fgenf(w, "codeinfra.AssetArchive(%.v)", expr.Args[0])
	case "fileAsset":
		g.Fgenf(w, "codeinfra.FileAsset(%.v)", expr.Args[0])
	case "stringAsset":
		g.Fgenf(w, "codeinfra.StringAsset(%.v)", expr.Args[0])
	case "remoteAsset":
		g.Fgenf(w, "codeinfra.RemoteAsset(%.v)", expr.Args[0])
	case "filebase64":
	...
	case pcl.Invoke:
		...
		if len(expr.Args) == 3 {
			...
			if invokeOptions, ok := expr.Args[2].(*model.ObjectConsExpression); ok {
				if isOut {
					g.Fgen(&buf, ", opts=codeinfra.InvokeOutputOptions(")
				} else {
					g.Fgen(&buf, ", opts=codeinfra.InvokeOptions(")
				}
	...
	case "secret":
		g.Fgenf(w, "codeinfra.Output.secret(%v)", expr.Args[0])
	case "unsecret":
		g.Fgenf(w, "codeinfra.Output.unsecret(%v)", expr.Args[0])
	case "split":
	...
	case "toJSON":
		if model.ContainsOutputs(expr.Args[0].Type()) {
			g.Fgenf(w, "codeinfra.Output.json_dumps(%.v)", expr.Args[0])
		} else {
	...
	case "project":
		g.Fgen(w, "codeinfra.get_project()")
	case "stack":
		g.Fgen(w, "codeinfra.get_stack()")
	case "organization":
		g.Fgen(w, "codeinfra.get_organization()")
	case "cwd":
		g.Fgen(w, "os.getcwd()")
	case "getOutput":
