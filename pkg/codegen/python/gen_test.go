...
func buildVirtualEnv(ctx context.Context) error {
	...
	// install Codeinfra Python SDK from the current source tree, -e means no-copy, ref directly
	...
	if err != nil {
		contract.Failf("failed to link venv against in-source codeinfra: %v\nstdout/stderr:\n%s",
			err, output)
	}
...
func TestCalculateDeps(t *testing.T) {
	...
	type TestCase struct {
		...
		expected [][2]string
		// calculateDeps can error if the Codeinfra version provided is
	...
	cases := []TestCase{{
		...
		expected: [][2]string{
			...
			// with semver and parver formatted differently from Codeinfra.
			// Codeinfra should not have a version.
			{"parver>=0.2.1", ""},
			{"codeinfra", ">=3.142.0,<4.0.0"},
			{"semver>=2.8.1"},
		},
	}, {
		// Test 2: If you only one dep, we expect Codeinfra to have a narrower
		...
		// Test 3: If you provide codeinfra, we expect the constraint to
		// be respected.
		inputDeps: map[string]string{
			"codeinfra": ">=3.0.0,<3.50.0",
		},
		expected: [][2]string{
			// We expect three alphabetized deps,
			// with semver and parver formatted differently from Codeinfra.
		...
		// Test 4: If you provide an illegal codeinfra version, we expect an error.
		inputDeps: map[string]string{
			"codeinfra": ">=0.16.0,<4.0.0",
		},
		expectedErr: fmt.Errorf("lower version bound must be at least %v", oldestAllowedCodeinfra),
	}}
