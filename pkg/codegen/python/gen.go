...
// https://www.codeinfra.com/docs/languages-sdks/python/
...
type modContext struct {
	...
	modNameOverrides map[string]string // Optional overrides for Codeinfra module names
...
func (mod *modContext) objectType(t *schema.ObjectType, input bool, forDict bool) string {
	...
	if !codegen.PkgEquals(t.PackageReference, mod.pkg) {
		...
		if forDict {
			...
			// TODO[https://github.com/khulnasoft/codeinfra/issues/16702]
...
func (mod *modContext) resourceType(r *schema.ResourceType) string {
	...
	if strings.HasPrefix(r.Token, "codeinfra:providers:") {
		pkgName := strings.TrimPrefix(r.Token, "codeinfra:providers:")
		return fmt.Sprintf("codeinfra_%s.Provider", pkgName)
	}
...
func (mod *modContext) tokenToResource(tok string) string {
	...
	if components[0] == "codeinfra" && components[1] == "providers" {
		return fmt.Sprintf("codeinfra_%s.Provider", components[2])
	}
...
func (mod *modContext) generateCommonImports(w io.Writer, imports imports, typingImports []string) {
	...
	fmt.Fprintf(w, "import codeinfra\n")
	fmt.Fprintf(w, "import codeinfra.runtime\n")
...
func (mod *modContext) genHeader(w io.Writer, needsSDK bool, imports imports) {
	...
	// If needed, emit the standard Codeinfra SDK import statement.
	if needsSDK {
...
async def get_package():
	global _package_ref
	if _package_ref is ...:
		if codeinfra.runtime.settings._sync_monitor_supports_parameterization():
			async with _package_lock:
				if _package_ref is ...:
					monitor = codeinfra.runtime.settings.get_monitor()
	...
	if _package_ref is None or _package_ref is ...:
		raise Exception("The Codeinfra CLI does not support parameterization. Please update the Codeinfra CLI.")
	return _package_ref
	`,
	...
	if mod.submodulesExist() {
		...
		for _, submod := range children {
			if !submod.isEmpty() {
				...
				// better.
				//
				// See https://github.com/khulnasoft/codeinfra/issues/7367
...
func (mod *modContext) importResourceType(r *schema.ResourceType) string {
	...
	if parts[0] == "codeinfra" && parts[1] == "providers" {
		return "import codeinfra_" + parts[2]
	}
...
func (mod *modContext) genConfig(variables []*schema.Property) (string, error) {
	...
	fmt.Fprintf(w, "__config__ = codeinfra.Config('%s')\n", mod.pkg.Name())
	fmt.Fprintf(w, "\n\n")
...
func genConfigVarType(configVar *schema.Property) string {
	...
	// To return the rich objects would be a breaking change, tracked in https://github.com/khulnasoft/codeinfra/issues/7493
	typeString := "str"
...
func (mod *modContext) genAwaitableType(w io.Writer, obj *schema.ObjectType) string {
	...
	fmt.Fprint(w, "@codeinfra.output_type\n")
	...
	for _, prop := range obj.Properties {
		...
		fmt.Fprintf(w, "        codeinfra.set(__self__, \"%[1]s\", %[1]s)\n", pname)
	}
	fmt.Fprintf(w, "\n")
...
func (mod *modContext) genResource(res *schema.Resource) (string, error) {
	...
	switch {
	case res.IsProvider:
		baseType = "codeinfra.ProviderResource"
	case res.IsComponent:
		baseType = "codeinfra.ComponentResource"
	default:
		baseType = "codeinfra.CustomResource"
	}
	...
	emitInitMethodSignature := func(methodName string) {
		...
		fmt.Fprintf(w, "                 opts: Optional[codeinfra.ResourceOptions] = None")
	...
	fmt.Fprintf(w, "        resource_args, opts = _utilities.get_resource_args_opts(%s, codeinfra.ResourceOptions, *args, **kwargs)\n", resourceArgsName)
	...
	if res.DeprecationMessage != "" && mod.compatibility != kubernetes20 {
		fmt.Fprintf(w, "        codeinfra.log.warn(\"\"\"%s is deprecated: %s\"\"\")\n", name, res.DeprecationMessage)
	}
	fmt.Fprintf(w, "        opts = codeinfra.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)\n")
	fmt.Fprintf(w, "        if not isinstance(opts, codeinfra.ResourceOptions):\n")
	...
	for _, prop := range res.InputProperties {
		...
		if res.IsProvider && !isStringType(prop.Type) {
			if prop.Secret {
				arg = fmt.Sprintf("codeinfra.Output.secret(%s).apply(codeinfra.runtime.to_json) if %s is not None else None", arg, arg)
				handledSecret = true
			} else {
				arg = fmt.Sprintf("codeinfra.Output.from_input(%s).apply(codeinfra.runtime.to_json) if %s is not None else None", arg, arg)
			}
		}
		name := PyName(prop.Name)
		if prop.Secret && !handledSecret {
			fmt.Fprintf(w, "            __props__.__dict__[%[1]q] = None if %[2]s is None else codeinfra.Output.secret(%[2]s)\n", name, arg)
		} else {
	...
	if len(res.Aliases) > 0 {
		fmt.Fprintf(w, `        alias_opts = codeinfra.ResourceOptions(aliases=[`)

		for i, alias := range res.Aliases {
			if alias.Type != nil {
				fmt.Fprintf(w, "codeinfra.Alias(type_=\"%v\")", *alias.Type)
		...
		fmt.Fprintf(w, "])\n")
		fmt.Fprintf(w, "        opts = codeinfra.ResourceOptions.merge(opts, alias_opts)\n")
	}

	if len(secretProps) > 0 {
		fmt.Fprintf(w, `        secret_opts = codeinfra.ResourceOptions(additional_secret_outputs=["%s"])`, strings.Join(secretProps, `", "`))
	...
	if len(replaceOnChangesProps) > 0 {
		...
		fmt.Fprintf(w, `        replace_on_changes = codeinfra.ResourceOptions(replace_on_changes=["%s"])`, strings.Join(replaceOnChangesStrings, `", "`))
	...
	if !res.IsProvider && !res.IsComponent {
		...
		fmt.Fprintf(w, "            id: codeinfra.Input[str],\n")
		fmt.Fprintf(w, "            opts: Optional[codeinfra.ResourceOptions] = None")
	...
	mod.genProperties(w, res.Properties, false /*setters*/, "", func(prop *schema.Property) string {
		...
		return fmt.Sprintf("codeinfra.Output[%s]", ty)
	})
...
func (mod *modContext) genProperties(w io.Writer, properties []*schema.Property, setters bool, indent string,
	...
	emitProp := func(pname string, prop *schema.Property) {
		...
		fmt.Fprintf(w, "%s    @property\n", indent)
		if pname == prop.Name {
			fmt.Fprintf(w, "%s    @codeinfra.getter\n", indent)
		} else {
			fmt.Fprintf(w, "%s    @codeinfra.getter(name=%q)\n", indent, prop.Name)
		}
		...
		fmt.Fprintf(w, "%s        return codeinfra.get(self, %q)\n\n", indent, pname)

		if setters {
			fmt.Fprintf(w, "%s    @%s.setter\n", indent, pname)
			fmt.Fprintf(w, "%s    def %s(self, value: %s):\n", indent, pname, ty)
			fmt.Fprintf(w, "%s        codeinfra.set(self, %q, value)\n\n", indent, pname)
		}
	}
...
func (mod *modContext) genMethodReturnType(w io.Writer, method *schema.Method) string {
	...
	// Produce a class definition with optional """ comment.
	fmt.Fprintf(w, "    @codeinfra.output_type\n")
	fmt.Fprintf(w, "    class %s:\n", name)
...
func (mod *modContext) genMethods(w io.Writer, res *schema.Resource) {
	genMethod := func(method *schema.Method) {
		...
		if returnType != nil || fun.ReturnTypePlain {
			...
			retTypeNameQualifiedOutput = fmt.Sprintf("codeinfra.Output['%s']", retTypeNameQualified)
			if shouldLiftReturn {
				methodRetType = fmt.Sprintf("codeinfra.Output['%s']", mod.pyType(returnType.Properties[0].Type))
...
func (mod *modContext) genFunction(fun *schema.Function) (string, error) {
	...
	if returnType != nil {
		...
		retTypeNameOutput = fmt.Sprintf("codeinfra.Output[%s]", originalOutputTypeName)
		fmt.Fprintf(w, "\n\n")
	} else {
		retTypeName = "Awaitable[None]"
		retTypeNameOutput = "codeinfra.Output[None]"
	}
	...
	genFunctionDef := func(returnTypeName string, plain bool) error {
		...
		fmt.Fprintf(w, "    opts = codeinfra.%s.merge(_utilities.get_invoke_opts_defaults(), opts)\n", optionsClass)
		...
		fmt.Fprintf(w, "    __ret__ = codeinfra.runtime.%s('%s', __args__, opts=opts%s)",
			runtimeFunction,
			fun.Token,
			trailingArgs)
		...
		if returnType != nil {
			...
			for i, ret := range rets {
				...
				// Use the `codeinfra.get()` utility instead of calling `__ret__.field` directly.
				...
				fmt.Fprintf(w, "\n        %[1]s=codeinfra.get(%[2]s, '%[1]s')", PyName(ret.Name), getter)
			}
...
func (mod *modContext) genFunDef(w io.Writer, name, retTypeName string, args []*schema.Property, wrapInput, plain bool) {
	...
	if plain {
		fmt.Fprintf(w, "%sopts: Optional[codeinfra.InvokeOptions] = None", indent)
	} else {
		fmt.Fprintf(w, "%sopts: Optional[Union[codeinfra.InvokeOptions, codeinfra.InvokeOutputOptions]] = None", indent)
	}
	if retTypeName != "" {
		fmt.Fprintf(w, ") -> %s:\n", retTypeName)
	} else {
...
var oldestAllowedCodeinfra = semver.Version{
	Major: 0,
	Minor: 17,
	Patch: 28,
}
...
func genCodeinfraPluginFile(pkg *schema.Package) ([]byte, error) {
	...
	return codeinfraPlugin.JSON()
}

// genPackageMetadata generates all the non-code metadata required by a Codeinfra package.
func genPackageMetadata(
	...
	fmt.Fprintf(w, "        return \"%s Codeinfra Package - Development Version\"\n", pkg.Name)
	fmt.Fprintf(w, "\n\n")
	...
	fmt.Fprintf(w, "              'codeinfra-plugin.json',\n")
...
func (mod *modContext) genInitDocstring(w io.Writer, res *schema.Resource, resourceArgsName string, argOverload bool) {
	...
	fmt.Fprintln(b, ":param codeinfra.ResourceOptions opts: Options for the resource.")
	if !argOverload {
		for _, prop := range res.InputProperties {
...
func (mod *modContext) genGetDocstring(w io.Writer, res *schema.Resource) {
	...
	fmt.Fprintln(b, ":param codeinfra.Input[str] id: The unique provider ID of the resource to lookup.")
	fmt.Fprintln(b, ":param codeinfra.ResourceOptions opts: Options for the resource.")
	if res.StateInputs != nil {
		for _, prop := range res.StateInputs.Properties {
			mod.genPropDocstring(b, InitParamName(prop.Name), prop, true /*acceptMapping*/)
...
func (mod *modContext) typeString(t schema.Type, input, acceptMapping bool, forDict bool) string {
	...
	case *schema.InputType:
		...
		return fmt.Sprintf("codeinfra.Input[%s]", typ)
	case *schema.EnumType:
	...
	case *schema.ObjectType:
		...
		return fmt.Sprintf("codeinfra.InputType[%s]", typ)
	case *schema.ResourceType:
	...
	default:
		...
		case schema.AssetType:
			return "Union[codeinfra.Asset, codeinfra.Archive]"
...
func pyPack(s string) string {
	return "codeinfra_" + strings.ReplaceAll(s, "-", "_")
}
...
func (mod *modContext) genType(w io.Writer, name, comment string, properties []*schema.Property, input, resourceOutput bool) error {
	...
	decorator := "@codeinfra.output_type"
	if input {
		decorator = "@codeinfra.input_type"
	}
	...
	if resourceOutput {
		...
		if needsCaseWarning {
			...
			fmt.Fprintf(w, "            codeinfra.log.warn(f\"Key '{key}' not found in %s. Access the value via the '{suggest}' property getter instead.\")\n", name)
			fmt.Fprintf(w, "\n")
	...
	for _, prop := range props {
		...
		fmt.Fprintf(w, "%s        codeinfra.set(__self__, \"%s\", %s)\n", indent, pname, arg)
	}
	fmt.Fprintf(w, "\n")
...
func (mod *modContext) genDictType(w io.Writer, name, comment string, properties []*schema.Property) error {
	...
	// TODO[codeinfra/codeinfra/issues/16408]
...
func generateModuleContextMap(tool string, pkg *schema.Package, info PackageInfo, extraFiles map[string][]byte) (map[string]*modContext, error) {
	...
	if pyPkgName == "" {
		pyPkgName = "codeinfra_" + strings.ReplaceAll(pkg.Name, "-", "_")
	}
...
type LanguageResource struct {
	*schema.Resource
	...
	Package string // The package name (e.g. codeinfra_kubernetes.apps.v1)
}
...
func GeneratePackage(tool string, pkg *schema.Package, extraFiles map[string][]byte) (map[string][]byte, error) {
	...
	plugin, err := genCodeinfraPluginFile(pkg)
	if err != nil {
		return nil, err
	}
	files.Add(filepath.Join(pkgName, "codeinfra-plugin.json"), plugin)
...
func genPyprojectTOML(tool string,
	...
	// an invalid Codeinfra package version as a dep.
	...
	// codeinfra_kubernetes = ["py.typed", "codeinfra-plugin.json"]
	//
	// This ensures that `python -m build` can proceed without `setup.py` present, while still
	// including the required files `py.typed` and `codeinfra-plugin.json` in the distro.
...
// It falls back to the default version supported by Codeinfra if the user hasn't provided
// one in the schema.
...
// ensureValidCodeinfraVersion ensures that the Codeinfra SDK has an entry.
// It accepts a list of dependencies
// as provided in the package schema, and validates whether
// this list correctly includes the Codeinfra Python package.
// It returns a map that correctly specifies the dependency.
// This function does not modify the argument. Instead, it returns
// a copy of the original map, except that the `codeinfra` key is guaranteed to have
// a valid value.
// This function returns an error if the provided Codeinfra version fails to
// validate.
func ensureValidCodeinfraVersion(parameterized bool, requires map[string]string) (map[string]string, error) {
	deps := map[string]string{}
	// Special case: if the map is empty, we return just codeinfra with the minimum version constraint.

	if len(requires) == 0 {
		result := map[string]string{
			"codeinfra": MinimumValidSDKVersion,
		}
		return result, nil
	}
	// If the codeinfra dep is missing, we require it to fall within
	// our major version constraint.
	if codeinfraDep, ok := requires["codeinfra"]; !ok {
		deps["codeinfra"] = MinimumValidSDKVersion
	} else {
		...
		matches := requirementRegex.FindStringSubmatch(codeinfraDep)
		if len(matches) != 2 {
			return nil, fmt.Errorf("invalid requirement specifier \"%s\"; expected \">=version1,<version2\"", codeinfraDep)
		}
		...
		if lowerBound.LT(oldestAllowedCodeinfra) {
			return nil, fmt.Errorf("lower version bound must be at least %v", oldestAllowedCodeinfra)
		}
		// The provided Codeinfra version is valid, so we're copy it into
		// the new map.
		deps["codeinfra"] = codeinfraDep
	}
...
// This function returns an error if the version of Codeinfra listed as a
...
func calculateDeps(parameterized bool, requires map[string]string) ([][2]string, error) {
	...
	if requires, err = ensureValidCodeinfraVersion(parameterized, requires); err != nil {
		return nil, err
	}
