...
const stackRefQualifiedName = "codeinfra.StackReference"
...
func (g *generator) genComponentDefinition(w io.Writer, component *pcl.Component, componentName string) {
	...
	g.Fgenf(w, "class %s(codeinfra.ComponentResource):\n", componentName)
	g.Indented(func() {
		if hasAnyInputVariables {
			g.Fgenf(w, "%sdef __init__(self, name: str, args: %s, opts:Optional[codeinfra.ResourceOptions] = None):\n",
				g.Indent,
				componentName+"Args")
			...
			g.Fgenf(w, "%sdef __init__(self, name: str, opts: Optional[codeinfra.ResourceOptions] = None):\n", g.Indent)
...
func GenerateProject(
	...
	if _, ok := localDependencies["codeinfra"]; ok {
	...
	// Set the runtime to "python" then marshal to Codeinfra.yaml
	...
	if path, ok := localDependencies["codeinfra"]; ok {
		...
		requirementsTxtLines = append(requirementsTxtLines, "codeinfra>=3.0.0,<4.0.0")
	}
...
func (g *generator) genPreamble(w io.Writer, program *pcl.Program, preambleHelperMethods codegen.StringSet) {
	// Print the codeinfra import at the top.
	g.Fprintln(w, "import codeinfra")
	...
	for _, n := range program.Nodes {
		if r, isResource := n.(*pcl.Resource); isResource {
			...
			packageName := "codeinfra_" + makeValidIdentifier(pkg)
			...
			if call, ok := n.(*model.FunctionCallExpression); ok {
				if i := g.getFunctionImports(call); len(i) > 0 && i[0] != "" {
					for _, importPackage := range i {
						importAs := strings.HasPrefix(importPackage, "codeinfra_")
						var maybePkg string
						if importAs {
							maybePkg = importPackage[len("codeinfra_"):]
						}
	...
	if g.isComponent {
		...
		imports = append(imports, "from codeinfra import Input")
	}
...
func (g *generator) genResourceOptions(w io.Writer, block *model.Block, hasInputs bool) {
	...
	g.Fprintf(w, ",%sopts = codeinfra.ResourceOptions(", prefix)
	g.Indented(func() {
	...
	instantiate := func(resName string) {
		...
		indenter(func() {
			for _, attr := range r.Inputs {
				...
				// special case: codeinfra.StackReference requires `stack_name` instead of `name`
	...
	if r.Options != nil && r.Options.Range != nil {
		...
		if model.ContainsOutputs(rangeType) {
			...
			case *model.FunctionCallExpression:
				if expr.Name == pcl.IntrinsicApply {
					...
					if len(applyArgs) == 1 {
						...
						// Otherwise, generate a call to `codeinfra.Output.all([]).apply()`.
						g.Fgen(w, "codeinfra.Output.all(\n")
						g.Indented(func() {
			...
			case *model.ForExpression:
				...
				g.Fgenf(w, "codeinfra.Output.all(%v).apply(lambda resolved_outputs: %s(resolved_outputs[0]))\n",
					rangeExpr,
					localFuncName)
				return
...
func (g *generator) genComponent(w io.Writer, r *pcl.Component) {
	...
	declareDeferredOutputVariables := func() {
		for _, output := range componentDeferredOutputVariables {
			g.Fgenf(w, "%s", g.Indent)
			g.Fgenf(w, "%s, resolve_%s = codeinfra.deferred_output()\n",
				PyName(output.Name),
				PyName(output.Name))
		}
	}
	...
	for _, output := range g.deferredOutputVariables {
		if output.SourceComponent.Name() == r.Name() {
			...
			if _, ok := output.Expr.(*model.ScopeTraversalExpression); ok {
				...
				g.Fgenf(w, "resolve_%s(codeinfra.Output.from_input(%v))\n", PyName(output.Name), expr)
			}
		}
	}
...
func (g *generator) genConfigVariable(w io.Writer, v *pcl.ConfigVariable) {
	...
	if !g.configCreated {
		g.Fprintf(w, "%sconfig = codeinfra.Config()\n", g.Indent)
...
func (g *generator) genOutputVariable(w io.Writer, v *pcl.OutputVariable) {
	...
	// TODO(pdg): trivia
	g.Fgenf(w, "%scodeinfra.export(\"%s\", %.v)\n", g.Indent, v.LogicalName(), value)
}
