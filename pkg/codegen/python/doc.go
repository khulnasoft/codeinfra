...
func (d DocLanguageHelper) GetDocLinkForResourceType(pkg *schema.Package, modName, typeName string) string {
	...
	case pkg.Name != "" && modName != "":
		path = fmt.Sprintf("codeinfra_%s/%s", pkg.Name, modName)
		fqdnTypeName = fmt.Sprintf("codeinfra_%s.%s.%s", pkg.Name, modName, typeName)
	case pkg.Name == "" && modName != "":
		path = modName
		fqdnTypeName = fmt.Sprintf("%s.%s", modName, typeName)
	case pkg.Name != "" && modName == "":
		path = "codeinfra_" + pkg.Name
		fqdnTypeName = fmt.Sprintf("codeinfra_%s.%s", pkg.Name, typeName)
	}
