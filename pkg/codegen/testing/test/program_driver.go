// Copyright 2021-2024, KhulnaSoft Ltd.
//
...
func ProgramTestBatch(k, n int) []ProgramTest {
	start := ((k - 1) * len(CodeinfraCodeinfraProgramTests)) / n
	end := ((k) * len(CodeinfraCodeinfraProgramTests)) / n
	return CodeinfraCodeinfraProgramTests[start:end]
}

// Useful when debugging a single test case.
func SingleTestCase(directoryName string) []ProgramTest {
	output := make([]ProgramTest, 0)
	for _, t := range CodeinfraCodeinfraProgramTests {
...
var CodeinfraCodeinfraProgramTests = []ProgramTest{
	...
	},
	{
		...
		// Blocked on go:
		//   TODO[codeinfra/codeinfra#8064]
		//   TODO[codeinfra/codeinfra#8065]
	},
	{
		Directory:   "aws-eks",
		Description: "AWS EKS",
	...
	},
	{
		...
		SkipCompile: codegen.NewStringSet(TestGo),
		// Blocked on nodejs: TODO[codeinfra/codeinfra#8068]
		// Flaky in go: TODO[codeinfra/codeinfra#8123]
	},
	{
	...
	},
	{
		...
		// Blocked on go:
		//   TODO[codeinfra/codeinfra#8073]
		//   TODO[codeinfra/codeinfra#8074]
	},
	{
	...
	},
	{
		...
		// and instead generates codeinfra.StringArray
		SkipCompile: codegen.NewStringSet(TestGo),
	},
	{
	...
	},
	{
		...
		// Todo: https://github.com/khulnasoft/codeinfra/issues/12606
	...
	},
	{
		...
		// TODO[https://github.com/khulnasoft/codeinfra/issues/14957] - object config variables are broken here
		SkipCompile: codegen.NewStringSet(TestGo, TestDotnet),
	},
	{
...
var CodeinfraCodeinfraYAMLProgramTests = []ProgramTest{
	// PCL files from codeinfra/yaml transpiled examples
	{
...
// can be generated by running `CODEINFRA_ACCEPT=true go test ./..." from the `pkg/codegen` directory.
//
//nolint:revive
func TestProgramCodegen(
	...
	codeinfraAccept := cmdutil.IsTruthy(os.Getenv("CODEINFRA_ACCEPT"))
	skipCompile := cmdutil.IsTruthy(os.Getenv("CODEINFRA_SKIP_COMPILE_TEST"))
	...
	for _, tt := range testcase.TestCases {
		...
		t.Run(tt.Directory, func(t *testing.T) {
			...
			if !skipCompile && testcase.Check != nil && !tt.SkipCompile.Has(testcase.Language) {
				extraCodeinfraPackages := codegen.NewStringSet()
				collectExtraCodeinfraPackages(program, extraCodeinfraPackages)
				testcase.Check(t, expectedFile, extraCodeinfraPackages)
			}
		})
	}
}

func collectExtraCodeinfraPackages(program *pcl.Program, extraCodeinfraPackages codegen.StringSet) {
	for _, n := range program.Nodes {
		if r, isResource := n.(*pcl.Resource); isResource {
			pkg, _, _, _ := r.DecomposeToken()
			if pkg != "codeinfra" {
				extraCodeinfraPackages.Add(pkg)
			}
		}

		if component, isComponent := n.(*pcl.Component); isComponent {
			collectExtraCodeinfraPackages(component.Program, extraCodeinfraPackages)
		}
	}
}
