...
var CodeinfraCodeinfraSDKTests = []*SDKTest{
	...
	},
	{
		...
		// For golang skip check, see https://github.com/khulnasoft/codeinfra/issues/11567
...
func init() {
	noChecks := false
	if env, ok := os.LookupEnv("CODEINFRA_TEST_SDK_NO_CHECKS"); ok {
...
// intentional, it can be accepted again via `CODEINFRA_ACCEPT=true`.
//
...
func TestSDKCodegen(t *testing.T, opts *SDKCodegenOptions) { // revive:disable-line
	...
	for _, tt := range opts.TestCases {
		...
		t.Run(tt.Directory, func(t *testing.T) {
			...
			if !RewriteFilesWhenCodeinfraAccept(t, dirPath, opts.Language, files) {
