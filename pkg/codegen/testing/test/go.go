...
func GenerateGoProgramTest(
	...
	expectedVersion := map[string]PkgVersionInfo{
		"aws-resource-options-4.26": {
			Pkg:          "github.com/khulnasoft/codeinfra-aws/sdk/v4",
			OpAndVersion: "v4.26.0",
		},
		"aws-resource-options-5.16.2": {
			Pkg:          "github.com/khulnasoft/codeinfra-aws/sdk/v5",
...
func checkGo(t *testing.T, path string, deps codegen.StringSet, codeinfraSDKPath string) {
	...
	typeCheckGo(t, path, deps, codeinfraSDKPath)
}

func typeCheckGo(t *testing.T, path string, deps codegen.StringSet, codeinfraSDKPath string) {
	dir := filepath.Dir(path)
	ex, err := executable.FindExecutable("go")
	require.NoError(t, err)
