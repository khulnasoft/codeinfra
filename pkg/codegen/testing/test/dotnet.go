...
func GenerateDotnetProgramTest(
	...
	expectedVersion := map[string]PkgVersionInfo{
		"aws-resource-options-4.26": {
			Pkg:          "<PackageReference Include=\"Codeinfra.Aws\"",
...
func GenerateDotnetYAMLBatchTest(t *testing.T, rootDir string, genProgram GenProgram) {
	...
	TestProgramCodegen(t,
		ProgramCodegenOptions{
			...
			GenProgram: genProgram,
			TestCases:  CodeinfraCodeinfraYAMLProgramTests,
		},
	)
}

func checkDotnet(t *testing.T, path string, dependencies codegen.StringSet, codeinfraSDKPath string) {
	var err error
	...
	if len(pkgs) != 0 {
		...
		dep{"Codeinfra", CodeinfraDotnetSDKVersion}.install(t, ex, dir)
	} else {
		...
		if codeinfraSDKPath != "" {
			err = integration.RunCommand(t, "add sdk ref",
				[]string{ex, "add", "reference", codeinfraSDKPath},
	...
	typeCheckDotnet(t, path, dependencies, codeinfraSDKPath)
}

func typeCheckDotnet(t *testing.T, path string, dependencies codegen.StringSet, codeinfraSDKPath string) {
	var err error
	dir := filepath.Dir(path)
...
// Example:
//
//	"aws" => {"Codeinfra.Aws", 4.21.1}
//	"azure" => {"Codeinfra.Azure", 4.21.1}
func dotnetDependencies(deps codegen.StringSet) []dep {
	result := make([]dep, len(deps))
	for i, d := range deps.SortedValues() {
		switch d {
		case "aws":
			result[i] = dep{"Codeinfra.Aws", AwsSchema}
		case "azure-native":
			result[i] = dep{"Codeinfra.AzureNative", AzureNativeSchema}
		case "azure":
			// TODO: update constant in test.AzureSchema to v5.x
			// because it has output-versioned function invokes
			result[i] = dep{"Codeinfra.Azure", "5.12.0"}
		case "kubernetes":
			result[i] = dep{"Codeinfra.Kubernetes", KubernetesSchema}
		case "random":
			result[i] = dep{"Codeinfra.Random", RandomSchema}
		case "aws-static-website":
			result[i] = dep{"Codeinfra.AwsStaticWebsite", AwsStaticWebsiteSchema}
		case "aws-native":
			result[i] = dep{"Codeinfra.AwsNative", AwsNativeSchema}
		default:
			runes := []rune(d)
			titlecase := append([]rune{unicode.ToUpper(runes[0])}, runes[1:]...)
			result[i] = dep{"Codeinfra." + string(titlecase), ""}
		}
	}
	return result
}
