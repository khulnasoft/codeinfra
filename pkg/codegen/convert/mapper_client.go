...
func (m *mapperClient) GetMapping(ctx context.Context, provider string, codeinfraProvider string) ([]byte, error) {
	label := "GetMapping"
	logging.V(7).Infof("%s executing: provider=%s, codeinfra=%s", label, provider, codeinfraProvider)

	resp, err := m.clientRaw.GetMapping(ctx, &codegenrpc.GetMappingRequest{
		Provider:       provider,
		CodeinfraProvider: codeinfraProvider,
	})
	if err != nil {
