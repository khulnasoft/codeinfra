...
func TestPluginMapper_MappedNameDiffersFromCodeinfraName(t *testing.T) {
	t.Parallel()

	ws := &testWorkspace{
		infos: []workspace.PluginInfo{
			{
				Name:    "codeinfraProvider",
				Kind:    apitype.ResourcePlugin,
				Version: semverMustParse("1.0.0"),
			},
		},
	...
	testProvider := &testProvider{
		pkg: tokens.Package("codeinfraProvider"),
		mapping: func(key, provider string) ([]byte, string, error) {
			assert.Equal(t, "key", key)
			assert.Equal(t, "", provider)
	...
	installPlugin := func(pkg tokens.Package) *semver.Version {
		...
		// "codeinfraProvider" is already installed and will return a mapping for "otherProvider".
...
func TestPluginMapper_MappedNamesDifferFromCodeinfraName(t *testing.T) {
	t.Parallel()

	ws := &testWorkspace{
		infos: []workspace.PluginInfo{
			{
				Name:    "codeinfraProviderAws",
				Kind:    apitype.ResourcePlugin,
				Version: semverMustParse("1.0.0"),
			},
			{
				Name:    "codeinfraProviderGcp",
				Kind:    apitype.ResourcePlugin,
				Version: semverMustParse("1.0.0"),
			},
		},
	...
	testProviderAws := &testProvider{
		pkg: tokens.Package("codeinfraProviderAws"),
		mapping: func(key, provider string) ([]byte, string, error) {
			assert.Equal(t, "key", key)
			assert.Equal(t, "", provider)
	...
	testProviderGcp := &testProvider{
		pkg: tokens.Package("codeinfraProviderGcp"),
		mapping: func(key, provider string) ([]byte, string, error) {
			assert.Equal(t, "key", key)
			assert.Equal(t, "", provider)
	...
	installPlugin := func(pkg tokens.Package) *semver.Version {
		// This will want to install the "gcp" package, but we're calling the codeinfra name "codeinfraProviderGcp"
		// for this test.
...
func TestPluginMapper_MappedNamesDifferFromCodeinfraNameWithHint(t *testing.T) {
	t.Parallel()
	...
	// Get the mapping for the GCP provider, telling the mapper that it's codeinfra name is "codeinfraProviderGcp".
	data, err := mapper.GetMapping(ctx, "gcp", "codeinfraProviderGcp")
	assert.NoError(t, err)
	assert.Equal(t, []byte("datagcp"), data)
}

// Regression test for https://github.com/khulnasoft/codeinfra/issues/13105
func TestPluginMapper_MissingProviderOnlyTriesToInstallOnce(t *testing.T) {
	...
	installPlugin := func(pkg tokens.Package) *semver.Version {
		called++
		assert.Equal(t, "codeinfraProviderGcp", string(pkg))
		return nil
	}
	...
	// Try to get the mapping for the GCP provider, telling the mapper that it's codeinfra name is "codeinfraProviderGcp".
	data, err := mapper.GetMapping(ctx, "gcp", "codeinfraProviderGcp")
	assert.NoError(t, err)
	assert.Equal(t, []byte{}, data)
...
func TestPluginMapper_GetMappingsIsUsed(t *testing.T) {
	...
	testProvider := &testProvider{
		pkg: tokens.Package("codeinfraProviderK8s"),
		mapping: func(key, provider string) ([]byte, string, error) {
