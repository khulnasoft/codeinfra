...
func NewPluginMapper(ws Workspace,
	...
	// convert aws terraform code for example by just having 'codeinfra-aws' plugin locally, without needing to
	// specify it anywhere on the command line, and without tf2codeinfra needing to know about every possible
	// plugin.
...
func (l *pluginMapper) GetMapping(ctx context.Context, provider string, codeinfraProvider string) ([]byte, error) {
	// See https://github.com/khulnasoft/codeinfra/issues/14718 for why we need this lock. It may be possible to be
	...
	// Converters might not set codeinfraProvider so default it to the same name as the foreign provider.
	if codeinfraProvider == "" {
		codeinfraProvider = provider
	}
	codeinfraProviderPkg := tokens.Package(codeinfraProvider)

	// Optimization:
	// If there's a plugin with a name that matches the expected codeinfra provider name, move it to the front of
	// the list.
	// This is a common case, so we can avoid an expensive linear search through the rest of the plugins.
	foundCodeinfraProvider := false
	for i := 0; i < len(l.plugins); i++ {
		pluginSpec := l.plugins[i]
		if pluginSpec.name == codeinfraProviderPkg {
			l.plugins[0], l.plugins[i] = l.plugins[i], l.plugins[0]
			foundCodeinfraProvider = true
			break
		}
	}

	// If we didn't find the codeinfra provider in the list of plugins, then we want to try to install it. Note
	// that we don't want to hard fail here because it might turn out the provider hint was bad.
	if !foundCodeinfraProvider {
		version := l.installProvider(codeinfraProviderPkg)
		if version != nil {
	...
	if len(l.plugins) > 0 && l.plugins[0].name == codeinfraProviderPkg {
	...
	// one codeinfra provider will provide a mapping for each source mapping. This _might_ change in the future
