...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/nodejs/tstypes"
...
const (
	// The minimum version of @codeinfra/codeinfra compatible with the generated SDK.
...
func (mod *modContext) typeAst(t schema.Type, input bool, constValue interface{}) tstypes.TypeAst {
	...
	case *schema.InputType:
		...
		return tstypes.Identifier(fmt.Sprintf("codeinfra.Input<%s>", typ))
	...
	default:
		...
		case schema.ArchiveType:
			return tstypes.Identifier("codeinfra.asset.Archive")
		case schema.AssetType:
			return tstypes.Union(
				tstypes.Identifier("codeinfra.asset.Asset"),
				tstypes.Identifier("codeinfra.asset.Archive"),
			)
...
func (mod *modContext) genPlainObjectDefaultFunc(w io.Writer, name string,
	...
	for _, p := range properties {
		if p.DefaultValue != nil {
			...
			if codegen.IsNOptionalInput(p.Type) {
				compositeObject = fmt.Sprintf("codeinfra.output(val.%s).apply(%s)", p.Name, funcName)
			} else {
	...
	// export function %sProvideDefaults(val: codeinfra.Input<%s> | undefined): codeinfra.Output<%s> | undefined {
...
func (mod *modContext) genResource(w io.Writer, r *schema.Resource) (resourceFileInfo, error) {
	...
	fmt.Fprintf(w, "export class %s extends codeinfra.%s {\n", info.resourceClassName, baseType)
	...
	if !r.IsProvider && !r.IsComponent {
		...
		// TODO: Document id format: https://github.com/khulnasoft/codeinfra/issues/4754
		...
		fmt.Fprintf(w, "    public static get(name: string, id: codeinfra.Input<codeinfra.ID>, %sopts?: codeinfra.%s): %s {\n",
	...
	codeinfraType := r.Token
	if r.IsProvider {
		codeinfraType = mod.pkg.Name()
	}

	fmt.Fprintf(w, "    /** @internal */\n")
	fmt.Fprintf(w, "    public static readonly __codeinfraType = '%s';\n", codeinfraType)
	fmt.Fprintf(w, "\n")
	fmt.Fprintf(w, "    /**\n")
	fmt.Fprintf(w, "     * Returns true if the given object is an instance of %s.  This is designed to work even\n", name)
	fmt.Fprintf(w, "     * when multiple copies of the Codeinfra SDK have been loaded into the same process.\n")
	fmt.Fprintf(w, "     */\n")
	...
	typeExpression := name + ".__codeinfraType"
	if r.IsProvider {
		// We pass __codeinfraType to the ProviderResource constructor as the "type" for this provider, the
		// ProviderResource constructor in the SDK then prefixes "codeinfra:providers:" to that token and passes that
		// down to the CustomResource constructor, which then assigns that type token to the newly constructed
		// objects __codeinfraType field. As such we also need to prefix "codeinfra:providers:" when doing the equality
		// check here.
		typeExpression = "\"codeinfra:providers:\" + " + typeExpression
	}
	fmt.Fprintf(w, "        return obj['__codeinfraType'] === %s;\n", typeExpression)
	fmt.Fprintf(w, "    }\n")
	fmt.Fprintf(w, "\n")

	// Emit all properties (using their output types).
	// TODO[codeinfra/codeinfra#397]: represent sensitive types using a Secret<T> type.
	...
	fmt.Fprintf(w, "    constructor(name: string, args%s: %s, opts?: codeinfra.%s)%s\n", argsFlags, argsType,
	...
	genInputProps := func() error {
		...
		for _, prop := range r.InputProperties {
			...
			applyDefaults := func(arg string) string {
				if name := mod.provideDefaultsFuncName(prop.Type, true /*input*/); name != "" {
					...
					if codegen.IsNOptionalInput(prop.Type) {
						body = fmt.Sprintf("codeinfra.output(%[2]s).apply(%[1]s)", name, arg)
					} else {
			...
			if prop.Secret {
				arg = fmt.Sprintf("args?.%[1]s ? codeinfra.secret(%[2]s) : undefined", prop.Name, argValue)
			} else {
			...
			if prop.ConstValue != nil {
				...
				if r.IsProvider && !isStringType(prop.Type) {
					arg = fmt.Sprintf("codeinfra.output(%s).apply(JSON.stringify)", arg)
				}
			}
	...
	if !r.IsProvider {
		if r.StateInputs != nil {
			...
			fmt.Fprintf(w, "    constructor(name: string, argsOrState?: %s | %s, opts?: codeinfra.%s) {\n",
				argsType, stateType, optionsType)
		}
		if r.DeprecationMessage != "" && mod.compatibility != kubernetes20 {
			fmt.Fprintf(w, "        codeinfra.log.warn(\"%s is deprecated: %s\")\n", name, escape(r.DeprecationMessage))
		}
		fmt.Fprintf(w, "        let resourceInputs: codeinfra.Inputs = {};\n")
	...
	fmt.Fprint(w, "        opts = codeinfra.mergeOptions(utilities.resourceOptsDefaults(), opts);\n")
	...
	if len(r.Aliases) > 0 {
		...
		fmt.Fprintf(w, "] };\n")
		fmt.Fprintf(w, "        opts = codeinfra.mergeOptions(opts, aliasOpts);\n")
	}
	...
	if r.IsComponent {
		fmt.Fprintf(w, "        super(%s.__codeinfraType, name, resourceInputs, opts, true /*remote*/", name)
	} else {
		fmt.Fprintf(w, "        super(%s.__codeinfraType, name, resourceInputs, opts", name)
	...
	genMethod := func(method *schema.Method) {
		...
		if fun.ReturnType == nil {
			...
			retty = fmt.Sprintf("codeinfra.Output<%s>", mod.typeString(objectReturnType.Properties[0].Type, false, nil))
		} else {
			retty = fmt.Sprintf("codeinfra.Output<%s.%sResult>", name, title(method.Name))
		}
		fmt.Fprintf(w, "    %s(%s): %s {\n", methodName, argsig, retty)
		if fun.DeprecationMessage != "" {
			fmt.Fprintf(w, "        codeinfra.log.warn(\"%s.%s is deprecated: %s\")\n", name, methodName,
		...
		if fun.ReturnType != nil {
			if liftReturn {
				ret = fmt.Sprintf("const result: codeinfra.Output<%s.%sResult> = ", name, title(method.Name))
			} else {
				ret = "return "
			}
		}
		...
		if fun.ReturnTypePlain {
			...
			fmt.Fprintf(w, "        %scodeinfra.runtime.call(\"%s\", {\n", ret, fun.Token)
		}
...
func (mod *modContext) genFunctionDefinition(w io.Writer, fun *schema.Function, plain bool) (functionFileInfo, error) {
	...
	if !plain {
		returnType = fmt.Sprintf("codeinfra.Output<%s>", funReturnType)
	}

	invokeOptionsType := "codeinfra.InvokeOptions"
	if !plain {
		invokeOptionsType = "codeinfra.InvokeOutputOptions"
	}
	...
	// If the caller didn't request a specific version, supply one using the version of this library.
	fmt.Fprintf(w, "    opts = codeinfra.mergeOptions(utilities.resourceOptsDefaults(), opts || {});\n")
	...
	fmt.Fprintf(w, "    return codeinfra.runtime.%s(\"%s\", {\n", invokeCall, fun.Token)
	if fun.Inputs != nil {
		for _, p := range fun.Inputs.Properties {
			...
			if name := mod.provideDefaultsFuncName(p.Type, true /*input*/); name != "" {
				if codegen.IsNOptionalInput(p.Type) || !plain {
					body = fmt.Sprintf("codeinfra.output(%s).apply(%s)", body, name)
				} else {
...
func (mod *modContext) getTypeImportsForResource(t schema.Type, recurse bool, externalImports codegen.StringSet, imports map[string]codegen.StringSet, seen codegen.Set, res *schema.Resource) bool {
	...
	writeImports := func(pkg string) {
		if imp, ok := nodePackageInfo.ProviderNameToModuleName[pkg]; ok {
			...
			externalImports.Add(fmt.Sprintf("import * as %s from \"@codeinfra/%s\";", externalModuleName(pkg), pkg))
		}
	}
...
func (mod *modContext) genConfig(w io.Writer, variables []*schema.Property) error {
	...
	fmt.Fprintf(w, "const __config = new codeinfra.Config(\"%v\");\n", mod.pkg.Name())
	fmt.Fprintf(w, "\n")
	...
	for _, p := range variables {
		...
		// TODO: handle ConstValues https://github.com/khulnasoft/codeinfra/issues/4755
		if p.DefaultValue != nil {
...
func (mod *modContext) sdkImports(nested, utilities bool) []string {
	imports := []string{"import * as codeinfra from \"@codeinfra/codeinfra\";"}
...
func (mod *modContext) genResourceModule(w io.Writer) {
	...
	if providerOnly := len(mod.resources) == 1 && mod.resources[0].IsProvider; providerOnly {
		...
		fmt.Fprintf(w, "    construct: (name: string, type: string, urn: string): codeinfra.Resource => {\n")
		...
		for _, name := range registrations.SortedValues() {
			fmt.Fprintf(w, "codeinfra.runtime.registerResourceModule(\"%v\", \"%v\", _module)\n", mod.pkg.Name(), name)
		}
	}

	if provider != nil {
		fmt.Fprintf(w, "codeinfra.runtime.registerResourcePackage(\"%v\", {\n", mod.pkg.Name())
		fmt.Fprintf(w, "    version: utilities.getVersion(),\n")
		fmt.Fprintf(w, "    constructProvider: (name: string, type: string, urn: string): codeinfra.ProviderResource => {\n")
...
func genPackageMetadata(pkg *schema.Package, info NodePackageInfo, fs codegen.Fs, localDependencies map[string]string, localSDK bool) error {
	// The generator already emitted Codeinfra.yaml, so that leaves three more files to write out:
...
type npmPackage struct {
	...
	Codeinfra           plugin.CodeinfraPluginJSON `json:"codeinfra,omitempty"`
}
...
func genNPMPackageMetadata(pkg *schema.Package, info NodePackageInfo, localDependencies map[string]string, localSDK bool) (string, error) {
	...
	if packageName == "" {
		packageName = "@codeinfra/" + pkg.Name
	}
	...
	var codeinfraPlugin plugin.CodeinfraPluginJSON
	if pkg.Parameterization != nil {
		codeinfraPlugin = plugin.CodeinfraPluginJSON{
			...
			Parameterization: &plugin.CodeinfraParameterizationJSON{
				Name:    pkg.Name,
				Version: pkg.Version.String(),
	...
	// If there is no @codeinfra/codeinfra, add "latest" as a peer dependency (for npm linking style usage).
	sdkPack := "@codeinfra/codeinfra"
	if npminfo.Dependencies[sdkPack] == "" &&
		...
		if path, ok := localDependencies["codeinfra"]; ok {
...
func (mod *modContext) genUtilitiesFile(w io.Writer) error {
	...
	if def.Parameterization != nil {
		fmt.Fprintf(w, `import * as resproto from "@codeinfra/codeinfra/proto/resource_pb";
...
export async function getPackage() : Promise<string | undefined> {
	if (_packageRef === undefined) {
		if (!runtime.supportsParameterization()) {
			throw new Error("The Codeinfra CLI does not support parameterization. Please update the Codeinfra CLI");
		}
