...
var functionImports = map[string][]string{
	intrinsicInterpolate: {"@codeinfra/codeinfra"},
	"fileArchive":        {"@codeinfra/codeinfra"},
	"remoteArchive":      {"@codeinfra/codeinfra"},
	"assetArchive":       {"@codeinfra/codeinfra"},
	"fileAsset":          {"@codeinfra/codeinfra"},
	"stringAsset":        {"@codeinfra/codeinfra"},
	"remoteAsset":        {"@codeinfra/codeinfra"},
	"filebase64":         {"fs"},
...
func (g *generator) GenFunctionCallExpression(w io.Writer, expr *model.FunctionCallExpression) {
	...
	case intrinsicInterpolate:
		g.Fgen(w, "codeinfra.interpolate`")
	...
	case "entries":
		g.genEntries(w, expr)
	case "fileArchive":
		g.Fgenf(w, "new codeinfra.asset.FileArchive(%.v)", expr.Args[0])
	case "remoteArchive":
		g.Fgenf(w, "new codeinfra.asset.RemoteArchive(%.v)", expr.Args[0])
	case "assetArchive":
		g.Fgenf(w, "new codeinfra.asset.AssetArchive(%.v)", expr.Args[0])
	case "fileAsset":
		g.Fgenf(w, "new codeinfra.asset.FileAsset(%.v)", expr.Args[0])
	case "stringAsset":
		g.Fgenf(w, "new codeinfra.asset.StringAsset(%.v)", expr.Args[0])
	case "remoteAsset":
		g.Fgenf(w, "new codeinfra.asset.RemoteAsset(%.v)", expr.Args[0])
	case "filebase64":
	...
	case "secret":
		g.Fgenf(w, "codeinfra.secret(%v)", expr.Args[0])
	case "unsecret":
		g.Fgenf(w, "codeinfra.unsecret(%v)", expr.Args[0])
	case "split":
	...
	case "toJSON":
		if model.ContainsOutputs(expr.Args[0].Type()) {
			g.Fgenf(w, "codeinfra.jsonStringify(%v)", expr.Args[0])
		} else {
	...
	case "stack":
		g.Fgenf(w, "codeinfra.getStack()")
	case "project":
		g.Fgenf(w, "codeinfra.getProject()")
	case "organization":
		g.Fgenf(w, "codeinfra.getOrganization()")
	case "cwd":
