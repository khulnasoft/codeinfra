...
func GenerateProject(
	...
	// Set the runtime to "nodejs" then marshal to Codeinfra.yaml
	...
	err = os.WriteFile(path.Join(rootDirectory, "Codeinfra.yaml"), projectBytes, 0o600)
	if err != nil {
		return fmt.Errorf("write Codeinfra.yaml: %w", err)
	}
	...
	// Check if codeinfra is a local dependency, else add it as a normal range dependency
	if codeinfraArtifact, has := localDependencies[CodeinfraToken]; has {
		fmt.Fprintf(&packageJSON, `"@codeinfra/codeinfra": "%s"`, codeinfraArtifact)
	} else {
		fmt.Fprintf(&packageJSON, `"@codeinfra/codeinfra": "^3.0.0"`)
	}
	...
	// @codeinfra/codeinfra dependencies are already added above and not sorted.
...
func (g *generator) collectProgramImports(program *pcl.Program) programImports {
	importSet := codegen.NewStringSet("@codeinfra/codeinfra")
	...
	for _, n := range program.Nodes {
		...
		case *pcl.Resource:
			...
			if pkg == CodeinfraToken {
				continue
			}
			pkgName := "@codeinfra/" + pkg
...
func (g *generator) genPreamble(w io.Writer, program *pcl.Program) error {
	// Print the @codeinfra/codeinfra import at the top.
	g.Fprintln(w, `import * as codeinfra from "@codeinfra/codeinfra";`)
...
func componentElementType(pclType model.Type) string {
	...
	default:
		...
		case *model.MapType:
			elementType := componentElementType(pclType.ElementType)
			return fmt.Sprintf("Record<string, codeinfra.Input<%s>>", elementType)
...
func componentInputType(pclType model.Type) string {
	...
	return fmt.Sprintf("codeinfra.Input<%s>", elementType)
}

func componentOutputType(pclType model.Type) string {
	elementType := componentElementType(pclType)
	return fmt.Sprintf("codeinfra.Output<%s>", elementType)
}
...
func (g *generator) genComponentResourceDefinition(w io.Writer, componentName string, component *pcl.Component) {
	...
	g.Fgenf(w, "export class %s extends codeinfra.ComponentResource {\n", componentName)
	g.Indented(func() {
		for _, output := range outputs {
			...
			case *model.ScopeTraversalExpression:
				...
				if ok && len(expr.Parts) == 1 {
					...
					outputType = fmt.Sprintf("codeinfra.Output<%s>", qualifiedMemberName)
				} else {
		...
		if len(configVars) == 0 {
			g.Fgenf(w, "%s", g.Indent)
			g.Fgen(w, "constructor(name: string, opts?: codeinfra.ComponentResourceOptions) {\n")
			g.Indented(func() {
				g.Fgenf(w, "%s", g.Indent)
			...
			g.Fgenf(w, "constructor(name: string, args: %s, opts?: codeinfra.ComponentResourceOptions) {\n",
				argsTypeName)
			g.Indented(func() {
				g.Fgenf(w, "%s", g.Indent)
		...
		g.Indented(func() {
			...
			for _, output := range outputs {
				...
				case *model.ScopeTraversalExpression:
					...
					if ok && len(expr.Parts) == 1 {
						// special case: the output is a Resource type
						g.Fgenf(w, "%sthis.%s = codeinfra.output(%v);\n",
	...
	if r.Options != nil && r.Options.Range != nil {
		...
		if model.ContainsOutputs(r.Options.Range.Type()) {
			...
			case *model.FunctionCallExpression:
				if expr.Name == pcl.IntrinsicApply {
					...
					if len(applyArgs) == 1 {
						...
						// Otherwise, generate a call to `codeinfra.all([]).apply()`.
						g.Fgen(w, "codeinfra.all([")
			...
			case *model.TupleConsExpression, *model.ForExpression:
				...
				// so we call codeinfra.all to lift the elements of the list, then call apply
				g.Fgenf(w, "codeinfra.all(%.20v).apply(rangeBody => {\n", rangeExpr)
				g.Indented(func() {
					r.Options.Range = model.VariableReference(&model.Variable{
						Name:         "rangeBody",
						VariableType: model.ResolveOutputs(rangeExpr.Type()),
...
func (g *generator) genComponent(w io.Writer, component *pcl.Component) {
	...
	declareDeferredOutputVariables := func() {
		for _, output := range componentDeferredOutputVariables {
			...
			g.Fgenf(w, "%s", g.Indent)
			g.Fgenf(w, "const [%s, resolve%s] = codeinfra.deferredOutput<%s>();\n",
				output.Name,
				title(output.Name),
				typeParameter)
		}
	...
	for _, output := range g.deferredOutputVariables {
		if output.SourceComponent.Name() == component.Name() {
			...
			if _, ok := output.Expr.(*model.ScopeTraversalExpression); ok {
				...
				g.Fgenf(w, "resolve%s(codeinfra.output(%v));\n", title(output.Name), expr)
			}
		}
	}
...
func (g *generator) genConfigVariable(w io.Writer, v *pcl.ConfigVariable) {
	if !g.configCreated {
		g.Fprintf(w, "%sconst config = new codeinfra.Config();\n", g.Indent)
