...
func TestEnumReferencesCorrectIdentifier(t *testing.T) {
	t.Parallel()
	s := &schema.Package{
		Name: "codeinfraservice",
		Language: map[string]interface{}{
			"nodejs": NodePackageInfo{
				PackageName: "@codeinfra/bar",
			},
		},
	}
	result, err := enumNameWithPackage("codeinfraservice:index:WebhookFilters", s.Reference())
	assert.NoError(t, err)
	assert.Equal(t, "codeinfraservice.WebhookFilters", result)
	...
	assert.NotEqual(t, "@codeinfra/bar.WebhookFilters", result)
}
