...
//
// Each of these patterns matches an apply that can be handled by `codeinfra.Output`'s property access proxy.
...
// codeinfra.interpolate function. Concretely, this boils down to rewriting the following shapes
//
...
// that uses `codeinfra.interpolate`.
...
// Note that this depends on the fact that invokes are the only way to introduce promises in to a Codeinfra program; if
