...
func GenerateProject(
	...
	// Set the runtime to "dotnet" then marshal to Codeinfra.yaml
...
`)
	...
	if _, hasLocalCodeinfraReference := localDependencies[codeinfraPackage]; !hasLocalCodeinfraReference {
		csproj.WriteString("		<PackageReference Include=\"Codeinfra\" Version=\"3.*\" />\n")
	}
	...
	for _, p := range packages {
		...
		if p.Name == codeinfraPackage {
			continue
		}

		packageName := "Codeinfra." + namespaceName(map[string]string{}, p.Name)
...
type programUsings struct {
	...
	codeinfraUsings        codegen.StringSet
	codeinfraHelperMethods codegen.StringSet
}
...
func (g *generator) usingStatements(program *pcl.Program) programUsings {
	...
	codeinfraUsings := codegen.NewStringSet()
	preambleHelperMethods := codegen.NewStringSet()
	for _, n := range program.Nodes {
		if r, isResource := n.(*pcl.Resource); isResource {
			pcl.FixupCodeinfraPackageTokens(r)
			pkg, _, _, _ := r.DecomposeToken()
			if pkg != codeinfraPackage {
				...
				codeinfraUsings.Add(fmt.Sprintf("%s = %[2]s.%[1]s", namespace, info.GetRootNamespace()))
			}
		}
	...
	return programUsings{
		...
		codeinfraUsings:        codeinfraUsings,
		codeinfraHelperMethods: preambleHelperMethods,
	}
}
...
func (g *generator) genComponentPreamble(w io.Writer, componentName string, component *pcl.Component) {
	...
	codeinfraUsings := programUsings.codeinfraUsings
	for _, pkg := range systemUsings.SortedValues() {
		g.Fprintf(w, "using %v;\n", pkg)
	}
	g.Fprintln(w, `using Codeinfra;`)
	for _, pkg := range codeinfraUsings.SortedValues() {
		g.Fprintf(w, "using %v;\n", pkg)
	}
	...
	g.Indented(func() {
		if len(configVars) > 0 {
			g.Fprintf(w, "%spublic class %sArgs : global::Codeinfra.ResourceArgs\n", g.Indent, componentName)
			g.Fprintf(w, "%s{\n", g.Indent)
			g.Indented(func() {
				...
				for _, variableName := range variableNames {
					...
					g.Fprintf(w, "%spublic class %s : global::Codeinfra.ResourceArgs\n", g.Indent, objectTypeName)
					g.Fprintf(w, "%s{\n", g.Indent)
					g.Indented(func() {
		...
		g.Fprintf(w, "%spublic class %s : global::Codeinfra.ComponentResource\n", g.Indent, componentName)
		g.Fprintf(w, "%s{\n", g.Indent)
		g.Indented(func() {
			...
			for _, preambleHelperMethodBody := range programUsings.codeinfraHelperMethods.SortedValues() {
...
func (g *generator) genPreamble(w io.Writer, program *pcl.Program) {
	...
	systemUsings := programUsings.systemUsings
	codeinfraUsings := programUsings.codeinfraUsings
	preambleHelperMethods := programUsings.codeinfraHelperMethods
...
func (g *generator) genComponent(w io.Writer, r *pcl.Component) {
	...
	declareDeferredOutputVariables := func() {
		for _, output := range componentDeferredOutputVariables {
			...
			g.Fgenf(w, "var %s = new Codeinfra.DeferredOutput<%s>();\n",
				output.Name,
				typeParameter)
		}
	}
