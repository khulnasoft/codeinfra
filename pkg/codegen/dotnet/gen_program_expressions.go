...
// in to a Codeinfra program; if this changes in the future, this transform will need to be applied in a more general way
...
// nastiness of working with raw `Task` and wrap it into Codeinfra's Output immediately to be able to `Apply` on it.
...
func (g *generator) genFunctionUsings(x *model.FunctionCallExpression) []string {
	...
	return []string{fmt.Sprintf("%s = Codeinfra.%[1]s", pkg)}
}
