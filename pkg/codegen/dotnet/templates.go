...
namespace {{.Namespace}}
{
    static class {{.ClassName}}
    {
        ...
        public static global::Codeinfra.InvokeOptions WithVersion(this global::Codeinfra.InvokeOptions? options)
        {
            var dst = options ?? new global::Codeinfra.InvokeOptions{};
            dst.Version = options?.Version ?? Version;
            return dst;
        }

        public static global::Codeinfra.InvokeOptions WithDefaults(this global::Codeinfra.InvokeOptions? src)
        {
            var dst = src ?? new global::Codeinfra.InvokeOptions{};
            dst.Version = src?.Version ?? Version;{{if ne .PluginDownloadURL "" }}
            dst.PluginDownloadURL = src?.PluginDownloadURL ?? "{{.PluginDownloadURL}}";{{end}}
            return dst;
        }

        public static global::Codeinfra.InvokeOutputOptions WithDefaults(this global::Codeinfra.InvokeOutputOptions? src)
        {
            var dst = src ?? new global::Codeinfra.InvokeOutputOptions{};
            dst.Version = src?.Version ?? Version;{{if ne .PluginDownloadURL "" }}
            dst.PluginDownloadURL = src?.PluginDownloadURL ?? "{{.PluginDownloadURL}}";{{end}}
            return dst;
        }
{{if .HasParameterization }}
        public static global::Codeinfra.RegisterPackageRequest PackageParameterization()
        {
            return new global::Codeinfra.RegisterPackageRequest(
                name: "{{.BaseProviderName}}",
                version: "{{.BaseProviderVersion}}",
                downloadUrl: "{{.BaseProviderPluginDownloadURL}}",
                parameterization: new global::Codeinfra.RegisterPackageRequest.PackageParameterization(
                    name: "{{.PackageName}}",
...
{{end}}
    ...
    internal sealed class {{.Name}}ResourceTypeAttribute : global::Codeinfra.ResourceTypeAttribute
    {
...
const csharpProjectFileTemplateText = `<Project Sdk="Microsoft.NET.Sdk">
  ...
  <PropertyGroup>
    ...
    <Authors>{{or .Package.Publisher "Codeinfra Corp."}}</Authors>
    <Company>{{or .Package.Publisher "Codeinfra Corp."}}</Company>
...
{{ end }}
  ...
  <ItemGroup>
    <EmbeddedResource Include="codeinfra-plugin.json" />
    <None Include="codeinfra-plugin.json" Pack="True" PackagePath="content" />
  </ItemGroup>

  <ItemGroup>
    {{- range $package, $version := .PackageReferences}}
    <PackageReference Include="{{$package}}" Version="{{$version}}"{{if iscodeinfrapkg $package}} ExcludeAssets="contentFiles"{{end}} />
    {{- end}}
  </ItemGroup>
...
var csharpProjectFileTemplate = template.Must(template.New("CSharpProject").Funcs(template.FuncMap{
	// iscodeinfrapkg is used in the template to conditionally emit `ExcludeAssets="contentFiles"`
	// for `<PackageReference>`s that start with "Codeinfra.", to prevent the references's contentFiles
	...
	"iscodeinfrapkg": func(s string) bool {
		return strings.HasPrefix(s, "Codeinfra.")
	},
