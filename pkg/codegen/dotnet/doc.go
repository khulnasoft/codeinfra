...
func (d DocLanguageHelper) GetDocLinkForCodeinfraType(pkg *schema.Package, typeName string) string {
	...
	// which in the case of the Codeinfra.Input class is 1.
	case "Codeinfra.Input":
		filename = "Codeinfra.Input-1"
	default:
		filename = typeName
	}
	return fmt.Sprintf("/docs/reference/pkg/dotnet/Codeinfra/%s.html", filename)
}
...
func (d DocLanguageHelper) GetDocLinkForResourceType(pkg *schema.Package, _, typeName string) string {
	...
	return fmt.Sprintf("/docs/reference/pkg/dotnet/Codeinfra%s/%s.html", packageNamespace, typeName)
}
...
func (d DocLanguageHelper) GetModuleDocLink(pkg *schema.Package, modName string) (string, string) {
	...
	var link string
	if modName == "" {
		displayName = "Codeinfra." + namespaceName(d.Namespaces, pkg.Name)
	} else {
		displayName = fmt.Sprintf("Codeinfra.%s.%s", namespaceName(d.Namespaces, pkg.Name), modName)
	}
