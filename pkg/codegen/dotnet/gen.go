...
func (mod *modContext) typeString(t schema.Type, qualifier string, input, state, requireInitializers bool) string {
	...
	case *schema.ResourceType:
		if strings.HasPrefix(t.Token, "codeinfra:providers:") {
			pkgName := strings.TrimPrefix(t.Token, "codeinfra:providers:")
...
func (pt *plainType) genInputTypeWithFlags(w io.Writer, level int, generateInputAttributes bool) error {
	...
	var suffix string
	if pt.baseClass != "" {
		suffix = " : global::Codeinfra." + pt.baseClass
	}
...
func (mod *modContext) genResource(w io.Writer, r *schema.Resource) error {
	...
	switch {
	case r.IsProvider:
		baseType = "global::Codeinfra.ProviderResource"
	case mod.isK8sCompatMode() && !r.IsComponent:
		baseType = "KubernetesResource"
	case r.IsComponent:
		baseType = "global::Codeinfra.ComponentResource"
		optionsType = "ComponentResourceOptions"
	default:
		baseType = "global::Codeinfra.CustomResource"
	}
	...
	if len(r.Aliases) > 0 {
		...
		for _, alias := range r.Aliases {
			fmt.Fprintf(w, "                    ")
			if alias.Type != nil {
				fmt.Fprintf(w, "new global::Codeinfra.Alias { Type = \"%v\" },\n", *alias.Type)
			}
		}
		fmt.Fprintf(w, "                },\n")
	}
	...
	genMethod := func(method *schema.Method) {
		...
		if fun.ReturnType != nil {
			...
			if liftReturn {
				returnType = fmt.Sprintf("global::Codeinfra.Output<%s>",
				...
				returnType = "global::Codeinfra.Output" + typeParameter
			}
		}
		...
		if mod.parameterization != nil {
			...
			fmt.Fprintf(w, "            => global::Codeinfra.Deployment.Instance.Call%s(\"%s\", %s, this, null, %s)%s;\n",
				typeParameter, fun.Token, argsParamRef, "Utilities.PackageParameterization()", lift)
		} else {
			fmt.Fprintf(w, "            => global::Codeinfra.Deployment.Instance.Call%s(\"%s\", %s, this)%s;\n",
...
func (mod *modContext) genFunctionFileCode(f *schema.Function) (string, error) {
	buffer := &bytes.Buffer{}
	importStrings := mod.codeinfraImports()
...
func (mod *modContext) genFunction(w io.Writer, fun *schema.Function) error {
	...
	if !fun.MultiArgumentInputs {
		...
		// new line and indent
		fmt.Fprint(w, "            ")
		fmt.Fprintf(w, "=> global::Codeinfra.Deployment.Instance.%sAsync%s", invokeCall, typeParamOrEmpty(typeParameter))
		if mod.parameterization != nil {
		...
		funcBodyIndent()
		fmt.Fprint(w, "var args = new global::Codeinfra.DictionaryInvokeArgs(builder.ToImmutableDictionary());\n")
		funcBodyIndent()
		// full invoke call
		fmt.Fprint(w, "return await global::Codeinfra.Deployment.Instance.")
...
func (mod *modContext) genFunctionOutputVersion(w io.Writer, fun *schema.Function, outputOptions bool) error {
	...
	if !fun.MultiArgumentInputs {
		...
		fmt.Fprintf(w, "            => global::Codeinfra.Deployment.Instance.%s%s(\"%s\", %s, options.WithDefaults());\n",
		...
		fmt.Fprint(w, "            var args = new global::Codeinfra.DictionaryInvokeArgs(builder.ToImmutableDictionary());\n")
		fmt.Fprintf(w, "            return global::Codeinfra.Deployment.Instance.%s%s(\"%s\", args, invokeOptions.WithDefaults());\n",
...
func (mod *modContext) genEnum(w io.Writer, enum *schema.EnumType) error {
	...
	default:
		// Issue to implement boolean-based enums: https://github.com/khulnasoft/codeinfra/issues/5652
...
// codeinfraImports is a slice of common imports that are used with the genHeader method.
func (mod *modContext) codeinfraImports() []string {
	codeinfraImports := []string{
		"System",
		"System.Collections.Generic",
		"System.Collections.Immutable",
		"System.Threading.Tasks",
		"Codeinfra.Serialization",
	}
	if mod.RootNamespace() != "Codeinfra" {
		codeinfraImports = append(codeinfraImports, "Codeinfra")
	}
	return codeinfraImports
}
...
func (mod *modContext) genConfig(variables []*schema.Property) (string, error) {
	...
	// Use the root namespace to avoid `Codeinfra.Provider.Config.Config.VarName` usage.
	fmt.Fprintf(w, "namespace %s\n", mod.namespaceName)
	fmt.Fprintf(w, "{\n")
	...
	fmt.Fprintf(w, "        private static readonly global::Codeinfra.Config __config = new global::Codeinfra.Config(\"%v\");\n", mod.pkg.Name())
	fmt.Fprintf(w, "\n")
...
func (mod *modContext) gen(fs codegen.Fs) error {
	...
	for _, t := range mod.types {
		...
		if mod.details(t).inputType {
			buffer := &bytes.Buffer{}
			mod.genHeader(buffer, mod.codeinfraImports())
	...
	if len(mod.enums) > 0 {
		buffer := &bytes.Buffer{}
		mod.genHeader(buffer, []string{"System", "System.ComponentModel", "Codeinfra"})
...
func genPackageMetadata(pkg *schema.Package,
	...
	if codeinfra, ok := localDependencies["codeinfra"]; ok {
		localPackages["codeinfra"] = codeinfra
	}
	...
	plugin, err := (codeinfraPlugin).JSON()
	if err != nil {
		return err
	}
	...
	files.Add("codeinfra-plugin.json", plugin)
	return nil
}
...
func genProjectFile(pkg *schema.Package,
	...
	// if we don't have a package reference to Codeinfra SDK from nuget
	// we need to add it, unless we are referencing a local Codeinfra SDK project via a project reference
	if _, ok := packageReferences["Codeinfra"]; !ok {
		referencedLocalCodeinfraProject := false
		for _, projectReference := range projectReferences {
			if strings.HasSuffix(projectReference, "Codeinfra.csproj") {
				referencedLocalCodeinfraProject = true
				break
			}
		}

		// only add a package reference to Codeinfra if we're not referencing a local Codeinfra project
		// which we usually do when testing schemas locally
		if !referencedLocalCodeinfraProject {
			packageReferences["Codeinfra"] = "[3.71.1.0,4)"
		}
	}
...
func getLogo(pkg *schema.Package) ([]byte, error) {
	url := pkg.LogoURL
	if url == "" {
		// Default to a generic Codeinfra logo from the parent repository.
		url = "https://raw.githubusercontent.com/khulnasoft/codeinfra/dbc96206bec722b7791a22ff50e895ab7c0abdc0/sdk/dotnet/codeinfra_logo_64x64.png"
	}
