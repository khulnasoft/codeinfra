...
func TestGeneratePackage(t *testing.T) {
	...
	test.TestSDKCodegen(t, &test.SDKCodegenOptions{
		...
		TestCases: test.CodeinfraCodeinfraSDKTests,
	})
}
...
func typeCheckGeneratedPackage(t *testing.T, codeDir string) {
	...
	if alreadyHaveGoMod {
		...
		replacement := "github.com/khulnasoft/codeinfra/sdk/v3=" + sdk
...
func TestLanguageResources(t *testing.T) {
	...
	for _, test := range test.CodeinfraCodeinfraSDKTests {
		test := test
...
func TestPackageNaming(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			importBasePath: "github.com/khulnasoft/codeinfra-azure-quickstart-acr-geo-replication/sdk/go/acr",
			expectedRoot:   "acr",
		},
		{
...
func TestTokenToType(t *testing.T) {
	...
	const awsImportBasePath = "github.com/khulnasoft/codeinfra-aws/sdk/v4/go/aws"
	...
	const googleNativeImportBasePath = "github.com/khulnasoft/codeinfra-google-native/sdk/go/google"
	...
	tests := []struct {
		...
		},
		{
			pkg: &pkgContext{
				...
				pkgImportAliases: map[string]string{
					"github.com/khulnasoft/codeinfra-aws/sdk/v4/go/aws/s3": "awss3",
				},
			},
			token:    "aws:s3/BucketWebsite:BucketWebsite",
		...
		},
		{
			pkg: &pkgContext{
				...
				pkgImportAliases: map[string]string{
					"github.com/khulnasoft/codeinfra-google-native/sdk/go/google/dns/v1": "dns",
				},
			},
...
func TestRegressTypeDuplicatesInChunking(t *testing.T) {
	...
	assert.Contains(t, fs, "test/codeinfraTypes.go")
	assert.Contains(t, fs, "test/codeinfraTypes1.go")
	assert.NotContains(t, fs, "test/codeinfraTypes2.go")
	...
	typedefs1 := typedefs(string(fs["test/codeinfraTypes.go"]))
	typedefs2 := typedefs(string(fs["test/codeinfraTypes1.go"]))
