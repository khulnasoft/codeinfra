...
func (g *generator) genComponentType(w io.Writer, componentName string, component *pcl.Component) {
	...
	g.Indented(func() {
		g.Fgen(w, g.Indent)
		g.Fgen(w, "codeinfra.ResourceState\n")
		for _, output := range outputs {
			g.Fgen(w, g.Indent)
			fieldName := Title(output.LogicalName())
			fieldType := "codeinfra.AnyOutput" // TODO: update this
			g.Fgenf(w, "%s %s\n", fieldName, fieldType)
			g.Fgen(w, "")
		}
	})
...
func (g *generator) genComponentDefinition(w io.Writer, componentName string, component *pcl.Component) {
	...
	g.Indented(func() {
		g.Fgenf(w, "%sctx *codeinfra.Context,\n", g.Indent)
		g.Fgenf(w, "%sname string,\n", g.Indent)
		g.Fgenf(w, "%sargs *%s,\n", g.Indent, argsTypeName)
		g.Fgenf(w, "%sopts ...codeinfra.ResourceOption,\n", g.Indent)
	})
	...
	g.Indented(func() {
		...
		if len(outputs) == 0 {
			g.Fgenf(w, "err = %sctx.RegisterResourceOutputs(&componentResource, codeinfra.Map{})\n", g.Indent)
			g.Fgenf(w, "%sif err != nil {\n", g.Indent)
			g.Indented(func() {
				g.Fgenf(w, "%sreturn nil, err\n", g.Indent)
			})
...
func GenerateProgramWithOptions(program *pcl.Program, opts GenerateProgramOptions) (
	...
	g.Fprintf(&index, "codeinfra.Run(func(ctx *codeinfra.Context) error {\n")
...
func GenerateProjectFiles(project workspace.Project, program *pcl.Program,
	...
	// Set the runtime to "go" then marshal to Codeinfra.yaml
	project.Runtime = workspace.NewProjectRuntimeInfo("go", nil)
	projectBytes, err := encoding.YAML.Marshal(project)
	if err != nil {
		return nil, diagnostics, err
	}
	files["Codeinfra.yaml"] = projectBytes
	...
	packagePaths["codeinfra"] = "github.com/khulnasoft/codeinfra/sdk/v3"
	err = gomod.AddRequire("github.com/khulnasoft/codeinfra/sdk/v3", "v3.30.0")
	contract.AssertNoErrorf(err, "could not add require statement for github.com/khulnasoft/codeinfra/sdk/v3 to go.mod")
	...
	for _, p := range packages {
		...
		if p.Version != nil && p.Version.Major <= 0 {
			...
			//
			// * go get github.com/khulnasoft/codeinfra-aws/sdk/v5/go/aws@v5.3.0
			//          ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ module path
			//                                           ~~ major version
			...
			// * go get github.com/khulnasoft/codeinfra-aws-native/sdk/go/aws@v0.16.0
			//          ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ module path
			//                                                  ~~~~~~ package path - can be any number of path parts
			//                                                         ~~~~~~~ version
			//
		...
		// Relatively safe default, this works for Codeinfra provider packages:
		vPath := ""
	...
	// https://github.com/khulnasoft/codeinfra/issues/16859).
...
func GenerateProject(
	...
	for filename, data := range files {
		dir := directory
		// Codeinfra.yaml needs to be written to root, everything else goes to the main directory.
		if filename == "Codeinfra.yaml" {
			dir = rootDirectory
		}
...
func (g *generator) genPreamble(w io.Writer, program *pcl.Program, preambleHelperMethods codegen.StringSet) {
	...
	g.importer.Import("github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra", "codeinfra")
	if hasDeferredOutputs(program) {
		g.importer.Import("github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfrax", "codeinfrax")
	}
...
func (g *generator) collectTypeImports(program *pcl.Program, t schema.Type) {
	...
	case *schema.TokenType:
		...
		g.addCodeinfraImport(pkg, vPath, mod, name)
...
// collect Imports returns two sets of packages imported by the program, std lib packages and codeinfra packages
func (g *generator) collectImports(program *pcl.Program) (helpers codegen.StringSet) {
	...
	for _, n := range program.Nodes {
		if r, isResource := n.(*pcl.Resource); isResource {
			pcl.FixupCodeinfraPackageTokens(r)
		...
		if _, isConfigVar := n.(*pcl.ConfigVariable); isConfigVar {
			g.importer.Import("github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra/config", "config")
		}
			...
			if call, ok := n.(*model.FunctionCallExpression); ok {
				if call.Name == pcl.Invoke {
					...
					if call.Type() == model.DynamicType {
						...
						dummyVersionPath := ""
						g.addCodeinfraImport(pkg, dummyVersionPath, mod, name)
						return call, nil
					}
...
func (g *generator) addCodeinfraImport(pkg, versionPath, mod, name string) {
	...
	importPath := func(mod string) string {
		importBasePath := fmt.Sprintf("github.com/khulnasoft/codeinfra-%s/sdk%s/go/%s", pkg, versionPath, pkg)
...
var resourceType = model.NewOpaqueType("codeinfra.Resource")
...
func (g *generator) genResourceOptions(w io.Writer, block *model.Block) {
	...
	for _, item := range block.Body.Items {
		...
		g.Fgenf(w, ", codeinfra.%s(%v)", attr.Name, attr.Value)
	}
}
...
func (g *generator) genResource(w io.Writer, r *pcl.Resource) {
	...
	originalMod := mod
	if pkg == "codeinfra" && mod == "providers" {
		pkg = typ
		mod = ""
		typ = "Provider"
	}
...
func deferredOutputCastTypeParameter(outputType model.Type) string {
	...
	default:
		...
		case *model.ListType:
			...
			case model.IntType:
				return "codeinfra.IntArrayOutput"
			case model.BoolType:
				return "codeinfra.BoolArrayOutput"
			case model.NumberType:
				return "codeinfra.Float64ArrayOutput"
			case model.StringType:
				return "codeinfra.StringArrayOutput"
			}
			return "codeinfra.ArrayOutput"
		case *model.MapType:
			switch exprType.ElementType {
			case model.IntType:
				return "codeinfra.IntMapOutput"
			case model.BoolType:
				return "codeinfra.BoolMapOutput"
			case model.NumberType:
				return "codeinfra.Float64MapOutput"
			case model.StringType:
				return "codeinfra.StringMapOutput"
			}
...
func (g *generator) genComponent(w io.Writer, r *pcl.Component) {
	...
	declareDeferredOutputVariables := func() {
		for _, output := range componentDeferredOutputVariables {
			...
			g.Fgenf(w, "%s, resolve%s := codeinfra.DeferredOutput[%s](ctx)\n",
				output.Name,
				Title(output.Name),
				typeParameter)
		}
...
// GetVPC resource getter: https://github.com/khulnasoft/codeinfra-aws/blob/7835df354694e2f9f23371602a9febebc6b45be8/sdk/go/aws/ec2/getVpc.go#L15
// LookupVPC function: https://github.com/khulnasoft/codeinfra-aws/blob/7835df354694e2f9f23371602a9febebc6b45be8/sdk/go/aws/ec2/getVpc.go#L15
...
func (fi *fileImporter) Import(importPath string, name string) (actualName string) {
	...
	//
	// For example, if "github.com/khulnasoft/codeinfra-aws/sdk/go/awsx/s3"
