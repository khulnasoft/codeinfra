...
// GetDocLinkForCodeinfraType returns the doc link for a Codeinfra type.
func (d DocLanguageHelper) GetDocLinkForCodeinfraType(pkg *schema.Package, typeName string) string {
	version := codeinfraSDKVersion
	if info, ok := pkg.Language["go"].(GoPackageInfo); ok {
		if info.CodeinfraSDKVersion == 1 {
			return "https://pkg.go.dev/github.com/khulnasoft/codeinfra/sdk/go/codeinfra?tab=doc#" + typeName
		}
		if info.CodeinfraSDKVersion != 0 {
			version = fmt.Sprintf("v%d", info.CodeinfraSDKVersion)
		}
	}
	return fmt.Sprintf("https://pkg.go.dev/github.com/khulnasoft/codeinfra/sdk/%s/go/codeinfra?tab=doc#%s", version, typeName)
}
...
func (d DocLanguageHelper) GetDocLinkForResourceType(pkg *schema.Package, moduleName string, typeName string) string {
	...
	return fmt.Sprintf("https://pkg.go.dev/github.com/khulnasoft/codeinfra-%s/sdk/%sgo/%s?tab=doc#%s", pkg.Name, moduleVersion, path, typeName)
}
...
// GetLanguageTypeString returns the Go-specific type given a Codeinfra schema type.
