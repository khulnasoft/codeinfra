...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/cgstrings"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/schema"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/diag"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource/plugin"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/slice"
...
func (pkg *pkgContext) tokenToResource(tok string) string {
	...
	if components[0] == "codeinfra" && components[1] == "providers" {
...
func (pkg *pkgContext) inputType(t schema.Type) (result string) {
	...
	default:
		...
		case schema.BoolType:
			return "codeinfra.BoolInput"
		case schema.IntType:
			return "codeinfra.IntInput"
		case schema.NumberType:
			return "codeinfra.Float64Input"
		case schema.StringType:
			return "codeinfra.StringInput"
		case schema.ArchiveType:
			return "codeinfra.ArchiveInput"
		case schema.AssetType:
			return "codeinfra.AssetOrArchiveInput"
...
func (pkg *pkgContext) genericInputType(t schema.Type) string {
	...
	return fmt.Sprintf("codeinfrax.Input[%s%s]", optionalPointer, inputType)
}
...
func (pkg *pkgContext) argsTypeImpl(t schema.Type) (result string) {
	...
	case *schema.UnionType:
		...
		return "codeinfra.Any"
	default:
		switch t {
		case schema.BoolType:
			return "codeinfra.Bool"
		case schema.IntType:
			return "codeinfra.Int"
		case schema.NumberType:
			return "codeinfra.Float64"
		case schema.StringType:
			return "codeinfra.String"
		case schema.ArchiveType:
			return "codeinfra.Archive"
		case schema.AssetType:
			return "codeinfra.AssetOrArchive"
		case schema.JSONType:
			fallthrough
		case schema.AnyType:
			return "codeinfra.Any"
...
func (pkg *pkgContext) typeStringImpl(t schema.Type, argsType bool) string {
	...
	case *schema.OptionalType:
		if input, isInputType := t.ElementType.(*schema.InputType); isInputType {
			...
			if isNilType(input.ElementType) || elem == "codeinfra.Input" {
				return elem
			}
...
func (pkg *pkgContext) outputTypeImpl(t schema.Type) string {
	...
	case *schema.OptionalType:
		...
		if isNilType(t.ElementType) || elem == "codeinfra.AnyOutput" {
			return elem
		}
	...
	case *schema.ArrayType:
		...
		if en == "codeinfra.Any" {
			return "codeinfra.ArrayOutput"
		}
		return en + "ArrayOutput"
	case *schema.MapType:
		en := strings.TrimSuffix(pkg.outputTypeImpl(t.ElementType), "Output")
		if en == "codeinfra.Any" {
			return "codeinfra.MapOutput"
		}
		return en + "MapOutput"
	case *schema.ObjectType:
	...
	default:
		switch t {
		case schema.BoolType:
			return "codeinfra.BoolOutput"
		case schema.IntType:
			return "codeinfra.IntOutput"
		case schema.NumberType:
			return "codeinfra.Float64Output"
		case schema.StringType:
			return "codeinfra.StringOutput"
		case schema.ArchiveType:
			return "codeinfra.ArchiveOutput"
		case schema.AssetType:
			return "codeinfra.AssetOrArchiveOutput"
		case schema.JSONType:
			fallthrough
		case schema.AnyType:
			return "codeinfra.AnyOutput"
...
func (pkg *pkgContext) genericElementType(schemaType schema.Type) (string, bool) {
	...
	case schema.NumberType:
		return "float64", true
	case schema.ArchiveType:
		return "codeinfra.Archive", true
	case schema.AssetType:
		return "codeinfra.AssetOrArchive", true
	default:
...
// for example instead of codeinfra.StringOutput, it generates codeinfrax.Output[string]
func (pkg *pkgContext) genericOutputTypeImpl(t schema.Type) string {
	...
	case *schema.OptionalType:
		...
		if elementType == "any" {
			return fmt.Sprintf("codeinfrax.Output[%s]", elementType)
		}

		if isPrimitive {
			// for example OptionalType{StringType} becomes codeinfrax.Output[*string]
			return fmt.Sprintf("codeinfrax.Output[*%s]", elementType)
		}
	...
	case *schema.ArrayType:
		elementType, isPrimitive := pkg.genericElementType(t.ElementType)
		if isPrimitive {
			return fmt.Sprintf("codeinfrax.ArrayOutput[%s]", elementType)
		}

		// for non-primitive types such as objects and resources
		// use GArrayOutput[Type, TypeOutput]
		return fmt.Sprintf("codeinfrax.GArrayOutput[%s, %sOutput]", elementType, elementType)
	case *schema.MapType:
		elementType, isPrimitive := pkg.genericElementType(t.ElementType)
		if isPrimitive {
			return fmt.Sprintf("codeinfrax.MapOutput[%s]", elementType)
		}

		// for non-primitive types such as objects and resources
		// use GMapOutput[Type, TypeOutput]
		return fmt.Sprintf("codeinfrax.GMapOutput[%s, %sOutput]", elementType, elementType)
	case *schema.ObjectType:
		objectTypeName, _ := pkg.genericElementType(t)
		return fmt.Sprintf("codeinfrax.GPtrOutput[%s, %sOutput]", objectTypeName, objectTypeName)
	case *schema.ResourceType:
		resourceTypeName, _ := pkg.genericElementType(t)
		// element type of a ResourceOutput is Resource
		return fmt.Sprintf("codeinfrax.GPtrOutput[%s, %sOutput]", resourceTypeName, resourceTypeName)
	case *schema.TokenType:
		// Use the underlying type for now.
		if t.UnderlyingType != nil {
		...
		return fmt.Sprintf("codeinfrax.Output[%s]", tokenType)
	case *schema.UnionType:
		// If the union is actually a relaxed enum type, use the underlying
		// type for the enum instead
		for _, e := range t.ElementTypes {
		...
		// TODO(pdg): union types
		return "codeinfrax.Output[interface{}]"
	case *schema.InputType:
		// We can't make output types for input types. We instead strip the input and try again.
		return pkg.genericOutputTypeImpl(t.ElementType)
	default:
...
// a schema.String, outputType returns "codeinfra.String", and given a *schema.ObjectType with the token pkg:mod:Name,
...
// For example, given a schema.StringType, genericOutputType returns "codeinfrax.Output[string]",
...
func genInputImplementationWithArgs(w io.Writer, genArgs genInputImplementationArgs) {
	...
	if genArgs.toOutputMethods {
		...
		fmt.Fprintf(w, "\treturn codeinfra.ToOutputWithContext(ctx, i).(%sOutput)\n", name)
		fmt.Fprintf(w, "}\n\n")

		if !genArgs.usingGenericTypes {
			// Generate 'ToOuput(context.Context) codeinfrax.Output[T]' method
			// to satisfy codeinfrax.Input[T].
			if genArgs.goPackageinfo.Generics == GenericsSettingSideBySide {
				fmt.Fprintf(w, "func (i %s) ToOutput(ctx context.Context) codeinfrax.Output[%s] {\n", receiverType, elementType)
				fmt.Fprintf(w, "\treturn codeinfrax.Output[%s]{\n", elementType)
			...
			// Generate 'ToOuput(context.Context) codeinfrax.Output[T]' method which lifts the receiver type *T
			// to satisfy codeinfrax.Input[*T].
			fmt.Fprintf(w, "func (i *%s) ToOutput(ctx context.Context) codeinfrax.Output[*%s] {\n", receiverType, receiverType)
			fmt.Fprint(w, "\treturn codeinfrax.Val(i)\n")
			fmt.Fprint(w, "}\n\n")
		}
	}
	...
	if genArgs.ptrMethods && !genArgs.usingGenericTypes {
		...
		if strings.HasSuffix(receiverType, "Args") {
			fmt.Fprintf(w, "\treturn codeinfra.ToOutputWithContext(ctx, i).(%[1]sOutput).To%[1]sPtrOutputWithContext(ctx)\n", name)
		} else {
			fmt.Fprintf(w, "\treturn codeinfra.ToOutputWithContext(ctx, i).(%sPtrOutput)\n", name)
		}
		fmt.Fprintf(w, "}\n\n")
...
func (pkg *pkgContext) genOutputType(w io.Writer, baseName, elementType string, ptrMethods, usingGenericTypes bool) {
	fmt.Fprintf(w, "type %sOutput struct { *codeinfra.OutputState }\n\n", baseName)
	...
	if goPackageInfo.Generics == GenericsSettingSideBySide || goPackageInfo.Generics == GenericsSettingGenericsOnly {
		fmt.Fprintf(w, "func (o %sOutput) ToOutput(ctx context.Context) codeinfrax.Output[%s] {\n", baseName, elementType)
		fmt.Fprintf(w, "\treturn codeinfrax.Output[%s]{\n", elementType)
...
func (pkg *pkgContext) genArrayOutput(w io.Writer, baseName, elementType string) {
	...
	fmt.Fprintf(w, "func (o %[1]sArrayOutput) Index(i codeinfra.IntInput) %[1]sOutput {\n", baseName)
	fmt.Fprintf(w, "\treturn codeinfra.All(o, i).ApplyT(func (vs []interface{}) %s {\n", elementType)
...
func (pkg *pkgContext) genMapOutput(w io.Writer, baseName, elementType string) {
	...
	fmt.Fprintf(w, "func (o %[1]sMapOutput) MapIndex(k codeinfra.StringInput) %[1]sOutput {\n", baseName)
	fmt.Fprintf(w, "\treturn codeinfra.All(o, k).ApplyT(func (vs []interface{}) %s{\n", elementType)
...
func (pkg *pkgContext) genEnum(w io.Writer, enumType *schema.EnumType, usingGenericTypes bool) error {
	...
	asFuncName := strings.TrimPrefix(elementArgsType, "codeinfra.")
...
func (pkg *pkgContext) genEnumInputTypes(w io.Writer, name string, enumType *schema.EnumType, elementGoType string) {
	...
	fmt.Fprintf(w, "return codeinfra.ToOutput(in).(%sPtrOutput)\n", name)
	fmt.Fprintf(w, "}\n")
	fmt.Fprintln(w)

	fmt.Fprintf(w, "func (in *%[1]sPtr) To%[2]sPtrOutputWithContext(ctx context.Context) %[2]sPtrOutput {\n", cgstrings.Camel(name), name)
	fmt.Fprintf(w, "return codeinfra.ToOutputWithContext(ctx, in).(%sPtrOutput)\n", name)
	fmt.Fprintf(w, "}\n")
	fmt.Fprintln(w)

	if goPkgInfo.Generics != GenericsSettingNone {
		// ToOutput implementation for codeinfrax.Input.
		fmt.Fprintf(w, "func (in *%sPtr) ToOutput(ctx context.Context) codeinfrax.Output[*%s] {\n", typeName, name)
		fmt.Fprintf(w, "\treturn codeinfrax.Output[*%s]{\n", name)
...
func (pkg *pkgContext) genEnumInputFuncs(w io.Writer, typeName string, enum *schema.EnumType, elementArgsType, inputType, asFuncName string) {
	...
	fmt.Fprintf(w, "return codeinfra.ToOutput(e).(%sOutput)\n", typeName)
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)

	fmt.Fprintf(w, "func (e %[1]s) To%[1]sOutputWithContext(ctx context.Context) %[1]sOutput {\n", typeName)
	fmt.Fprintf(w, "return codeinfra.ToOutputWithContext(ctx, e).(%sOutput)\n", typeName)
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)
	...
	fmt.Fprintf(w, "return codeinfra.ToOutput(%[1]s(e)).(%[1]sOutput)\n", elementArgsType)
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)

	fmt.Fprintf(w, "func (e %[1]s) To%[2]sOutputWithContext(ctx context.Context) %[3]sOutput {\n", typeName, asFuncName, elementArgsType)
	fmt.Fprintf(w, "return codeinfra.ToOutputWithContext(ctx, %[1]s(e)).(%[1]sOutput)\n", elementArgsType)
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)
	...
	if details.output || genArgs.output {
		...
		for _, p := range t.Properties {
			...
			if !genArgs.usingGenericTypes {
				...
				if !needsCast {
					fmt.Fprintf(w, "\treturn codeinfrax.Apply[%s](o, func (v %s) %s { return v.%s })\n",
						name, name, pkg.plainGenericInputType(p.Type), pkg.fieldName(nil, p))
				} else {
					fmt.Fprintf(w, "\tvalue := codeinfrax.Apply[%s](o, func (v %s) %s { return v.%s })\n",
						name, name, pkg.plainGenericInputType(p.Type), pkg.fieldName(nil, p))
...
func (pkg *pkgContext) setDefaultValue(
	...
	case *schema.ArrayType:
		parser, typ = pkg.internalModuleName+".ParseEnvStringArray", "codeinfra.StringArray"
	}
	switch t {
	case schema.BoolType:
...
func (pkg *pkgContext) genResource(
	...
	switch {
	case r.IsProvider:
		fmt.Fprintf(w, "\tcodeinfra.ProviderResourceState\n\n")
	case r.IsComponent:
		fmt.Fprintf(w, "\tcodeinfra.ResourceState\n\n")
	default:
		fmt.Fprintf(w, "\tcodeinfra.CustomResourceState\n\n")
	}
	...
	fmt.Fprintf(w, "func New%s(ctx *codeinfra.Context,\n", name)
	fmt.Fprintf(w, "\tname string, args *%[1]sArgs, opts ...codeinfra.ResourceOption) (*%[1]s, error) {\n", name)
	...
	for _, p := range r.InputProperties {
		if p.ConstValue != nil {
			...
			if useGenericVariant {
				...
				fmt.Fprintf(w, "args.%s = codeinfrax.Apply(args.%s, func(o %s%s) %s%s { return o.Defaults() })\n",
					pkg.fieldName(r, p),
					pkg.fieldName(r, p),
					optionalPointer,
					inputType,
	...
	if len(r.Aliases) > 0 {
		fmt.Fprintf(w, "\taliases := codeinfra.Aliases([]codeinfra.Alias{\n")
		for _, alias := range r.Aliases {
			s := "\t\t{\n"
			if alias.Type != nil {
				s += fmt.Sprintf("\t\t\tType: codeinfra.String(%q),\n", *alias.Type)
			}
			s += "\t\t},\n"
			fmt.Fprint(w, s)
		}
	...
	for _, p := range secretInputProps {
		...
		if !useGenericVariant {
			fmt.Fprintf(w, "\t\targs.%[1]s = codeinfra.ToSecret(args.%[1]s).(%[2]s)\n",
				pkg.fieldName(r, p),
				pkg.typeString(p.Type))
		} else {
			fmt.Fprintf(w, "\t\tuntypedSecretValue := codeinfra.ToSecret(args.%s.ToOutput(ctx.Context()).Untyped())\n",
				pkg.fieldName(r, p))
			...
			fmt.Fprintf(w, "\t\targs.%s = codeinfrax.MustConvertTyped[%s%s](untypedSecretValue)\n",
				pkg.fieldName(r, p),
				optionalPointer,
				inputType)
		}
	...
	if len(secretProps) > 0 {
		fmt.Fprintf(w, "\tsecrets := codeinfra.AdditionalSecretOutputs([]string{\n")
	...
	if len(replaceOnChangesProps) > 0 {
		fmt.Fprint(w, "\treplaceOnChanges := codeinfra.ReplaceOnChanges([]string{\n")
	...
	if !r.IsProvider && !r.IsComponent {
		...
		fmt.Fprintf(w, "func Get%s(ctx *codeinfra.Context,\n", name)
		fmt.Fprintf(w, "\tname string, id codeinfra.IDInput, state *%[1]sState, opts ...codeinfra.ResourceOption) (*%[1]s, error) {\n", name)
		fmt.Fprintf(w, "\tvar resource %s\n", name)
	...
	for _, method := range r.Methods {
		...
		outputsType := "codeinfra.AnyOutput"
		...
		if objectReturnType != nil || f.ReturnTypePlain {
			...
			fmt.Fprintf(w, "\n")
			fmt.Fprintf(w, "type %s%sResultOutput struct{ *codeinfra.OutputState }\n\n", outputStructName, methodName)
			...
			for _, p := range properties {
				...
				if !useGenericVariant {
					...
					fmt.Fprintf(w, "\treturn codeinfrax.Apply(o, func(v %s%sResult) %s { return v.%s })\n", outputStructName, methodName,
	...
	for _, p := range r.Properties {
		...
		if !useGenericVariant {
			...
			if strings.HasPrefix(outputType, "codeinfrax.GPtrOutput") && !strings.HasPrefix(elementType, "*") {
				elementType = "*" + elementType
			}
			...
			if needsCast {
				...
				fmt.Fprintf(w, "\tunwrapped := codeinfrax.Flatten[%s, %s](value)\n",
				...
				fmt.Fprintf(w, "\tvalue := codeinfrax.Apply[%s](o, func (v %s) %s { return v.%s })\n",
					name, name, outputType, pkg.fieldName(r, p))
				if !p.Plain {
					fmt.Fprintf(w, "\treturn codeinfrax.Flatten[%s, %s](value)\n",
						elementType, outputType)
				} else {
...
func (pkg *pkgContext) genFunctionCodeFile(f *schema.Function) (string, error) {
	...
	importsAndAliases["github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra"] = ""
	...
	if f.ReturnType != nil {
		...
		if goInfo.Generics == GenericsSettingSideBySide {
			importsAndAliases["github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfrax"] = ""
		}
	}
...
func (pkg *pkgContext) genFunction(w io.Writer, f *schema.Function, useGenericTypes bool) error {
	...
	fmt.Fprintf(w, "func %s(%s, opts ...codeinfra.InvokeOption) %s {\n", name, argsig, retty)
...
func genericTypeNeedsExplicitCasting(outputType string) bool {
	return strings.HasPrefix(outputType, "codeinfrax.ArrayOutput") ||
		strings.HasPrefix(outputType, "codeinfrax.MapOutput") ||
		strings.HasPrefix(outputType, "codeinfrax.GPtrOutput") ||
		strings.HasPrefix(outputType, "codeinfrax.GArrayOutput") ||
		strings.HasPrefix(outputType, "codeinfrax.GMapOutput")
}
...
func (pkg *pkgContext) genFunctionOutputGenericVersion(w io.Writer, f *schema.Function) {
	...
	if f.Inputs != nil {
		code = `
func ${fn}Output(ctx *codeinfra.Context, args ${fn}OutputArgs, opts ...codeinfra.InvokeOption) ${outputType} {
	outputResult := codeinfrax.ApplyErr[*${fn}Args](args.ToOutput(), func(plainArgs *${fn}Args) (*${fn}Result, error) {
		return ${fn}(ctx, plainArgs, opts...)
	})

	return codeinfrax.Cast[${outputType}, *${fn}Result](outputResult)
}
`
	} else {
		code = `
func ${fn}Output(ctx *codeinfra.Context, opts ...codeinfra.InvokeOption) ${outputType} {
	outputResult := codeinfrax.ApplyErr[int](codeinfrax.Val(0), func(_ int) (*${fn}Result, error) {
		return ${fn}(ctx, opts...)
	})
...
`
	...
	if f.Inputs != nil {
		...
		fmt.Fprintf(w, "func (args %s) ToOutput() codeinfrax.Output[*%s] {\n", receiverType, plainType)
		fmt.Fprint(w, "\tallArgs := codeinfrax.All(\n")
		...
		fmt.Fprintf(w, "\treturn codeinfrax.Apply[[]any](allArgs, func(resolvedArgs []interface{}) *%s {\n", plainType)
	...
	if objectReturnType != nil {
		...
		fmt.Fprintf(w, "func (o %sOutput) ToOutput(context.Context) codeinfrax.Output[*%s] {\n",
			originalResultTypeName,
			originalResultTypeName)
		fmt.Fprintf(w, "\treturn codeinfrax.Output[*%s]{\n", originalResultTypeName)
...
func (pkg *pkgContext) genFunctionOutputVersion(w io.Writer, f *schema.Function, useGenericTypes bool) error {
	...
	if f.Inputs != nil {
		fmt.Fprintf(w, "func %[1]sOutput(ctx *codeinfra.Context, args %[1]sOutputArgs, opts ...codeinfra.InvokeOption) %[2]s {\n",
			originalName, resultTypeName)
		fmt.Fprint(w, "	return codeinfra.ToOutputWithContext(ctx.Context(), args).\n")
		fmt.Fprintf(w, "		ApplyT(func(v interface{}) (%s, error) {\n", resultTypeName)
		fmt.Fprintf(w, "			args := v.(%sArgs)\n", originalName)
		fmt.Fprintf(w, "			options := codeinfra.InvokeOutputOptions{InvokeOptions: %s.PkgInvokeDefaultOpts(opts)}\n", pkg.internalModuleName)
		if def.Parameterization != nil {
		...
		fmt.Fprintf(w, "func %sOutput(ctx *codeinfra.Context, opts ...codeinfra.InvokeOption) %s {\n",
			originalName, resultTypeName)
		fmt.Fprintf(w, "	return codeinfra.ToOutput(0).ApplyT(func(int) (%s, error) {\n", resultTypeName)
		fmt.Fprintf(w, "		options := codeinfra.InvokeOutputOptions{InvokeOptions: %s.PkgInvokeDefaultOpts(opts)}\n", pkg.internalModuleName)
		if def.Parameterization != nil {
			err = pkg.GenPkgGetPackageRefCall(w, resultTypeName+"{}")
			if err != nil {
	...
	initCode := `
func init() {
        codeinfra.RegisterOutputType(${outputType}{})
}
...
func (pkg *pkgContext) genTypeRegistrations(
	...
	if !pkg.disableInputTypeRegistrations && !usingGenericTypes {
		for _, obj := range objTypes {
			...
			if details.input {
				fmt.Fprintf(w,
					"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sInput)(nil)).Elem(), %[1]sArgs{})\n", name)
			}
			if details.ptrInput {
				fmt.Fprintf(w,
					"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sPtrInput)(nil)).Elem(), %[1]sArgs{})\n", name)
			}
			if details.arrayInput && !pkg.names.Has(name+"Array") {
				fmt.Fprintf(w,
					"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sArrayInput)(nil)).Elem(), %[1]sArray{})\n", name)
			}
			if details.mapInput && !pkg.names.Has(name+"Map") {
				fmt.Fprintf(w,
					"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sMapInput)(nil)).Elem(), %[1]sMap{})\n", name)
			}
		}
		for _, t := range types {
			if strings.HasSuffix(t, "Input") {
				fmt.Fprintf(w, "\tcodeinfra.RegisterInputType(reflect.TypeOf((*%s)(nil)).Elem(), %s{})\n", t, strings.TrimSuffix(t, "Input"))
			}
		}
	}
	...
	for _, obj := range objTypes {
		...
		name, details := pkg.tokenToType(obj.Token), pkg.detailsForType(obj)
		if details.output {
			fmt.Fprintf(w, "\tcodeinfra.RegisterOutputType(%sOutput{})\n", name)
		}
		if details.ptrOutput && !usingGenericTypes {
			fmt.Fprintf(w, "\tcodeinfra.RegisterOutputType(%sPtrOutput{})\n", name)
		}
		if details.arrayOutput && !usingGenericTypes {
			fmt.Fprintf(w, "\tcodeinfra.RegisterOutputType(%sArrayOutput{})\n", name)
		}
		if details.mapOutput && !usingGenericTypes {
			fmt.Fprintf(w, "\tcodeinfra.RegisterOutputType(%sMapOutput{})\n", name)
		}
	}
	for _, t := range types {
		if strings.HasSuffix(t, "Output") {
			fmt.Fprintf(w, "\tcodeinfra.RegisterOutputType(%s{})\n", t)
		}
	}
...
func (pkg *pkgContext) genEnumRegistrations(w io.Writer) {
	...
	if !pkg.disableInputTypeRegistrations {
		for _, e := range pkg.enums {
			...
			if details.input || details.ptrInput {
				fmt.Fprintf(w,
					"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sInput)(nil)).Elem(), %[1]s(%[2]s))\n",
					name, instance)
				fmt.Fprintf(w,
					"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sPtrInput)(nil)).Elem(), %[1]s(%[2]s))\n",
					name, instance)
			}
			if details.arrayInput {
				fmt.Fprintf(w,
...
func (pkg *pkgContext) genResourceRegistrations(
	...
	if !pkg.disableInputTypeRegistrations && !usingGenericTypes {
		fmt.Fprintf(w,
			"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sInput)(nil)).Elem(), &%[1]s{})\n",
			name)
		if generateResourceContainerTypes && !r.IsProvider {
			fmt.Fprintf(w,
				"\tcodeinfra.RegisterInputType(reflect.TypeOf((*%[1]sArrayInput)(nil)).Elem(), %[1]sArray{})\n",
	...
	for _, method := range r.Methods {
		...
		if objectReturnType != nil {
			if pkg.liftSingleValueMethodReturns && len(objectReturnType.Properties) == 1 {
				fmt.Fprintf(w, "\tcodeinfra.RegisterOutputType(%s%sResultOutput{})\n", cgstrings.Camel(name), Title(method.Name))
			} else {
...
func extractModulePath(extPkg schema.PackageReference) string {
	...
	// Default to example.com/codeinfra-pkg if we have no other information.
	root := "example.com/codeinfra-" + name
	// But if we have a publisher use that instead, assuming it's from github
	if extPkg.Publisher() != "" {
		root = fmt.Sprintf("github.com/%s/codeinfra-%s", extPkg.Publisher(), name)
	}
...
func (pkg *pkgContext) genHeader(w io.Writer, goImports []string, importsAndAliases map[string]string, isUtil bool) {
	...
	if pkg.mod == "" {
		if isUtil {
			// we place codeinfraVersion and codeinfraUtilities in an ./internal folder
...
func (pkg *pkgContext) genConfig(w io.Writer, variables []*schema.Property) error {
	...
	for _, p := range variables {
		...
		fmt.Fprintf(w, "func Get%s(ctx *codeinfra.Context) %s {\n", Title(p.Name), getType)
		if p.DefaultValue != nil {
...
// Codeinfra runtime. The generated ResourceModule supports the deserialization of resource references into fully-
...
func (pkg *pkgContext) genResourceModule(w io.Writer) error {
	...
	if providerOnly := len(pkg.resources) == 1 && pkg.resources[0].IsProvider; providerOnly {
		...
		fmt.Fprintf(w, "func (m *module) Construct(ctx *codeinfra.Context, name, typ, urn string) (r codeinfra.Resource, err error) {\n")
		...
		fmt.Fprintf(w, "\t}\n\n")
		fmt.Fprintf(w, "\terr = ctx.RegisterResource(typ, name, nil, r, codeinfra.URN_(urn))\n")
		fmt.Fprintf(w, "\treturn\n")
		fmt.Fprintf(w, "}\n\n")
	}
	...
	if provider != nil {
		...
		fmt.Fprintf(w, "func (p *pkg) ConstructProvider(ctx *codeinfra.Context, name, typ, urn string) (codeinfra.ProviderResource, error) {\n")
		fmt.Fprintf(w, "\tif typ != \"codeinfra:providers:%s\" {\n", pkg.pkg.Name())
	...
	if len(registrations) > 0 {
		for _, mod := range registrations.SortedValues() {
			fmt.Fprintf(w, "\tcodeinfra.RegisterResourceModule(\n")
			fmt.Fprintf(w, "\t\t%q,\n", pkg.pkg.Name())
			fmt.Fprintf(w, "\t\t%q,\n", mod)
	...
	if provider != nil {
		fmt.Fprintf(w, "\tcodeinfra.RegisterResourcePackage(\n")
		fmt.Fprintf(w, "\t\t%q,\n", pkg.pkg.Name())
...
type LanguageResource struct {
	*schema.Resource
	...
	Package string // The package name (e.g. github.com/khulnasoft/codeinfra-kubernetes/sdk/v2/go/kubernetes/apps/v1)
}
...
func GeneratePackage(tool string,
	...
	// Generate codeinfra-plugin.json
	codeinfraPlugin := &plugin.CodeinfraPluginJSON{
		Resource: true,
		Name:     pkg.Name,
		Server:   pkg.PluginDownloadURL,
	}
	if goPkgInfo.RespectSchemaVersion && pkg.Version != nil {
		codeinfraPlugin.Version = pkg.Version.String()
	...
	if pkg.Parameterization != nil {
		...
		codeinfraPlugin.Parameterization = &plugin.CodeinfraParameterizationJSON{
			Name:    codeinfraPlugin.Name,
			Version: codeinfraPlugin.Version,
			Value:   pkg.Parameterization.Parameter,
		}
		codeinfraPlugin.Name = pkg.Parameterization.BaseProvider.Name
		codeinfraPlugin.Version = pkg.Parameterization.BaseProvider.Version.String()
	}

	codeinfraPluginJSON, err := codeinfraPlugin.JSON()
	if err != nil {
		return nil, fmt.Errorf("Failed to format codeinfra-plugin.json: %w", err)
	}
	files.Add(path.Join(pathPrefix, "codeinfra-plugin.json"), codeinfraPluginJSON)
	...
	for _, mod := range pkgMods {
		...
		case "":
			...
			versionFilePath := pkg.internalModuleName + "/codeinfraVersion.go"
		...
		if len(pkg.enums) > 0 {
			...
			if hasOutputs {
				...
				imports["github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra"] = ""
				if goPkgInfo.Generics != GenericsSettingNone {
					imports["github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfrax"] = ""
				}
			}
			...
			setFile(path.Join(mod, "codeinfraEnums.go"), buffer.String())
			setGenericVariantFile(path.Join(mod, "codeinfraEnums.go"), genericVariantBuffer.String())
		}
		...
		if len(pkg.types) == 0 && len(pkg.enums) > 0 {
			...
			if len(collectionTypes) > 0 {
				...
				typeFilePath := path.Join(mod, "codeinfraTypes.go")
		...
		if len(mod) == 0 {
			...
			if def.Parameterization != nil {
				...
				importsAndAliases["github.com/khulnasoft/codeinfra/sdk/v3/proto/go"] = "codeinfrarpc"
			}

			pkg.genHeader(buffer, imports, importsAndAliases, true /* isUtil */)

			packageRegex := fmt.Sprintf("^.*/codeinfra-%s/sdk(/v\\d+)?", pkg.pkg.Name())
			...
			utilFilePath := pkg.internalModuleName + "/codeinfraUtilities.go"
	...
	if pkg.SupportPack {
		...
		codeinfraPackagePath := "github.com/khulnasoft/codeinfra/sdk/v3"
		codeinfraVersion := "v3.30.0"
		if pkg.Parameterization != nil {
			codeinfraVersion = "v3.133.0"
		}
		err = gomod.AddRequire(codeinfraPackagePath, codeinfraVersion)
		contract.AssertNoErrorf(err, "could not add require statement to go.mod")
		if replacementPath, hasReplacement := localDependencies["codeinfra"]; hasReplacement {
			err = gomod.AddReplace(codeinfraPackagePath, "", replacementPath, "")
...
// Pkg%[1]sDefaultOpts provides package level defaults to codeinfra.Option%[1]s.
func Pkg%[1]sDefaultOpts(opts []codeinfra.%[1]sOption) []codeinfra.%[1]sOption {
	defaults := []codeinfra.%[1]sOption{}
		%[2]s
		version := %[3]s
		if !version.Equals(semver.Version{}){
			defaults = append(defaults, codeinfra.Version(version.String()))
		}
...
`
	var pluginDownloadURL string
	if url != "" {
		pluginDownloadURL = fmt.Sprintf(`defaults = append(defaults, codeinfra.PluginDownloadURL("%s"))`, url)
	}
...
func PkgGetPackageRef(ctx *codeinfra.Context) (string, error) {
	if packageRef == nil {
		...
		resp, err := ctx.RegisterPackage(&codeinfrarpc.RegisterPackageRequest{
			Name: %q,
			Version: %q,
			DownloadUrl: %q,
