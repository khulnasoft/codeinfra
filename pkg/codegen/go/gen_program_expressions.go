...
func (g *generator) genAnonymousFunctionExpression(
	...
	if b, ok := body.(*model.FunctionCallExpression); ok && b.Name == fromBase64Fn {
		g.Fgenf(w, "value, _ := %v\n", b)
		g.Fgenf(w, "return codeinfra.String(value), nil")
	} else if strings.HasPrefix(retTypeName, "codeinfra") {
		...
		if union, isUnion := dest.(*model.UnionType); isUnion && len(union.Annotations) > 0 {
			if input, ok := union.Annotations[0].(schema.Type); ok {
				if _, ok := codegen.ResolvedType(input).(*schema.UnionType); ok {
					g.Fgenf(w, "codeinfra.String(%s.%s)", mod, memberTag)
					return
				}
			}
		}
...
func (g *generator) GenFunctionCallExpression(w io.Writer, expr *model.FunctionCallExpression) {
	...
	case pcl.IntrinsicConvert:
		...
		default:
			...
			if originalTo.AssignableFrom(from.Type()) && (isOutput == isFromOutput) {
				...
				if typeName == "" {
					g.Fgenf(w, "%.v", from)
				} else if typeName == "codeinfra.String" && isID {
					g.Fgenf(w, "%.v", from)
				} else {
	...
	case pcl.IntrinsicApply:
		g.genApply(w, expr)
	case "fileArchive":
		g.Fgenf(w, "codeinfra.NewFileArchive(%.v)", expr.Args[0])
	case "remoteArchive":
		g.Fgenf(w, "codeinfra.NewRemoteArchive(%.v)", expr.Args[0])
	case "assetArchive":
		g.Fgenf(w, "codeinfra.NewAssetArchive(%.v)", expr.Args[0])
	case "fileAsset":
		g.Fgenf(w, "codeinfra.NewFileAsset(%.v)", expr.Args[0])
	case "stringAsset":
		g.Fgenf(w, "codeinfra.NewStringAsset(%.v)", expr.Args[0])
	case "remoteAsset":
		g.Fgenf(w, "codeinfra.NewRemoteAsset(%.v)", expr.Args[0])
	case "filebase64":
	...
	case "castDeferredOutput":
		...
		g.Fgenf(w, "codeinfrax.Cast[%s](%v)", typeParameter, expr.Args[0])
	case pcl.Invoke:
		...
		if len(expr.Args) == 3 {
			if invokeOptions, ok := expr.Args[2].(*model.ObjectConsExpression); ok {
				...
				for i, item := range invokeOptions.Items {
					...
					case "provider":
						g.Fgenf(&buf, "codeinfra.Provider(%v)", item.Value)
					case "parent":
						g.Fgenf(&buf, "codeinfra.Parent(%v)", item.Value)
					case "version":
						g.Fgenf(&buf, "codeinfra.Version(%v)", item.Value)
					case "pluginDownloadUrl":
						g.Fgenf(&buf, "codeinfra.PluginDownloadURL(%v)", item.Value)
					case "dependsOn":
						...
						if isInputty(value.Type()) {
							g.Fgenf(&buf, "codeinfra.DependsOnInputs(%v)", value)
						} else {
							g.Fgenf(&buf, "codeinfra.DependsOn(%v)", value)
						}
					}
	...
	case "secret":
		outputTypeName := "codeinfra.Any"
		if model.ResolveOutputs(expr.Type()) != model.DynamicType {
			outputTypeName = g.argumentTypeName(expr.Type(), false)
		}
		g.Fgenf(w, "codeinfra.ToSecret(%v).(%sOutput)", expr.Args[0], outputTypeName)
	case "unsecret":
		outputTypeName := "codeinfra.Any"
		if model.ResolveOutputs(expr.Type()) != model.DynamicType {
			outputTypeName = g.argumentTypeName(expr.Type(), false)
		}
		g.Fgenf(w, "codeinfra.Unsecret(%v).(%sOutput)", expr.Args[0], outputTypeName)
	case "toBase64":
	...
	case "goOptionalFloat64":
		g.Fgenf(w, "codeinfra.Float64Ref(%.v)", expr.Args[0])
	case "goOptionalBool":
		g.Fgenf(w, "codeinfra.BoolRef(%.v)", expr.Args[0])
	case "goOptionalInt":
		g.Fgenf(w, "codeinfra.IntRef(%.v)", expr.Args[0])
	case "goOptionalString":
		g.Fgenf(w, "codeinfra.StringRef(%.v)", expr.Args[0])
	case "stack":
		g.Fgen(w, "ctx.Stack()")
	case "project":
	...
	case "getOutput":
		g.Fgenf(w, "%v.GetOutput(codeinfra.String(%v))", expr.Args[0], expr.Args[1])
	default:
...
func (g *generator) genLiteralValueExpression(w io.Writer, expr *model.LiteralValueExpression, destType model.Type) {
	...
	isCodeinfraType := strings.HasPrefix(argTypeName, "codeinfra.")
...
func (g *generator) genScopeTraversalExpression(
	...
	case *pcl.ConfigVariable:
		if g.isComponent {
			...
			// these shouldn't be wrapped in a codeinfra.String(...), codeinfra.Int(...) etc. functions
	...
	// TODO if it's an array type, we need a lowering step to turn []string -> codeinfra.StringArray
	if isInput {
...
func (g *generator) genTemplateExpression(w io.Writer, expr *model.TemplateExpression, destType model.Type) {
	...
	argTypeName := g.argumentTypeName(destType, false)
	isCodeinfraType := strings.HasPrefix(argTypeName, "codeinfra.")
	isCodeinfraStr := argTypeName == "codeinfra.String"
	if isCodeinfraType && !isCodeinfraStr {
		g.Fgenf(w, "%s(", argTypeName)
		defer g.Fgenf(w, ")")
	}
...
func (g *generator) argumentTypeName(destType model.Type, isInput bool) (result string) {
	...
	case *model.ObjectType:
		...
		if isInput {
			...
			return "codeinfra.Map"
		}
		return "map[string]interface{}"
	case *model.MapType:
		valType := g.argumentTypeName(destType.ElementType, isInput)
		if isInput {
			trimmedType := strings.TrimPrefix(valType, "codeinfra.")
			return fmt.Sprintf("codeinfra.%sMap", Title(trimmedType))
		}
		return "map[string]" + valType
	case *model.ListType:
		argTypeName := g.argumentTypeName(destType.ElementType, isInput)
		if strings.HasPrefix(argTypeName, "codeinfra.") && argTypeName != "codeinfra.Resource" {
			if argTypeName == "codeinfra.Any" {
				return "codeinfra.Array"
			}
...
func (g *generator) argumentTypeNamePtr(destType model.Type, isInput bool) (result string) {
	...
	if !strings.HasPrefix(res, "codeinfra.") {
		return "*" + res
	}
	return res
}
...
func (g *generator) genApply(w io.Writer, expr *model.FunctionCallExpression) {
	...
	// TODO[codeinfra/codeinfra#8453] incomplete pattern code below.
	var typeAssertion string
	if retType == "[]string" {
		typeAssertion = ".(codeinfra.StringArrayOutput)"
	} else {
		...
		if !strings.Contains(retType, ".") {
			typeAssertion = fmt.Sprintf(".(codeinfra.%sOutput)", Title(retType))
		}
	}
	...
	if len(applyArgs) == 1 {
		...
		g.Fgenf(w, "codeinfra.All(%.v", applyArgs[0])
...
func isInputty(destType model.Type) bool {
	...
	// https://github.com/khulnasoft/codeinfra/blob/5330c97684cad78bcc60d8867f1b28704bd8a555/pkg/codegen/hcl2/model/type_eventuals.go#L244
	switch destType := destType.(type) {
	case *model.UnionType:
...
func (g *generator) functionName(tokenArg model.Expression) (string, string, string, hcl.Diagnostics) {
	...
	// Compute the resource type from the Codeinfra type token.
