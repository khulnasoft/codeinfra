...
func (p *promptToInputArrayHelper) generateHelperMethod(w io.Writer) {
	...
	fmt.Fprintf(w, "var codeinfraArr %s\n", p.destType)
	fmt.Fprintf(w, "for _, v := range arr {\n")
	fmt.Fprintf(w, "codeinfraArr = append(codeinfraArr, %s(v))\n", inputType)
	fmt.Fprintf(w, "}\n")
	fmt.Fprintf(w, "return codeinfraArr\n")
	fmt.Fprintf(w, "}\n")
}
...
func getHelperMethodIfNeeded(functionName string, indent string) (string, bool) {
	switch functionName {
	case "readFile":
		return `func readFileOrPanic(path string) codeinfra.StringPtrInput {
	...
	case "notImplemented":
		return fmt.Sprintf(`
%sfunc notImplemented(message string) codeinfra.AnyOutput {
%s  panic(message)
