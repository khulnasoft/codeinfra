...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/hcl2/model"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/hcl2/model/format"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/hcl2/syntax"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/pcl"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/testing/test"
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/testing/utils"
)
...
func TestFileImporter(t *testing.T) {
	...
	tests := []struct {
		...
		{
			desc: "single import/codeinfra",
			imports: []importCall{
				{importPath: "github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra", name: "codeinfra", want: "codeinfra"},
			},
			wantGroups: [][]string{
		...
		{
			desc: "std and codeinfra/no conflict",
			imports: []importCall{
				{importPath: "fmt", name: "fmt", want: "fmt"},
				{importPath: "github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra", name: "codeinfra", want: "codeinfra"},
			},
		...
		{
			desc: "std and codeinfra many imports, no conflict",
			imports: []importCall{
				{importPath: "fmt", name: "fmt", want: "fmt"},
				{importPath: "github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra", name: "codeinfra", want: "codeinfra"},
				{importPath: "github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra/config", name: "config", want: "config"},
				{importPath: "encoding/json", name: "json", want: "json"},
				{importPath: "github.com/khulnasoft/codeinfra-aws/sdk/v5/go/aws/s3", name: "s3", want: "s3"},
				{importPath: "io", name: "io", want: "io"},
				{importPath: "github.com/khulnasoft/codeinfra-awsx/sdk/v5/go/awsx", name: "awsx", want: "awsx"},
			},
			wantGroups: [][]string{
				{
					`"encoding/json"`,
				...
				{
					`"github.com/khulnasoft/codeinfra-aws/sdk/v5/go/aws/s3"`,
					`"github.com/khulnasoft/codeinfra-awsx/sdk/v5/go/awsx"`,
					`"github.com/khulnasoft/codeinfra/sdk/v3/go/codeinfra"`,
		...
		{
			desc: "std and codeinfra/conflict",
			imports: []importCall{
				{importPath: "encoding/json", name: "json", want: "json"},
				{
					// This doesn't actually exist yet,
					// but it's conceivable that it might.
					importPath: "github.com/khulnasoft/codeinfra-std/sdk/go/std/encoding/json",
					name:       "json",
					want:       "encodingjson",
				},
			},
			wantGroups: [][]string{
				{`"encoding/json"`},
				{`encodingjson "github.com/khulnasoft/codeinfra-std/sdk/go/std/encoding/json"`},
			},
		},
		{
			desc: "std and codeinfra/conflict repeated",
			imports: []importCall{
				{importPath: "encoding/json", name: "json", want: "json"},
				{
					importPath: "github.com/khulnasoft/codeinfra-std/sdk/go/std/encoding/json",
				...
				{
					importPath: "github.com/khulnasoft/codeinfra-std/sdk/v2/go/std/encoding/json",
					name:       "json",
					want:       "json2",
				},
			},
			wantGroups: [][]string{
				...
				{
					`encodingjson "github.com/khulnasoft/codeinfra-std/sdk/go/std/encoding/json"`,
					`json2 "github.com/khulnasoft/codeinfra-std/sdk/v2/go/std/encoding/json"`,
				},
			},
		},
		{
			desc: "std and codeinfra/conflict reverse",
			imports: []importCall{
				{
					// This doesn't actually exist yet,
					// but it's conceivable that it might.
			...
			wantGroups: [][]string{
				{`json2 "encoding/json"`},
				{`"github.com/khulnasoft/codeinfra-std/sdk/go/std/encoding/json"`},
			},
		},
		{
			desc: "codeinfra aws awsx conflict",
			imports: []importCall{
				{importPath: "github.com/khulnasoft/codeinfra-aws/sdk/v5/go/aws/ecs", name: "ecs", want: "ecs"},
				{importPath: "github.com/khulnasoft/codeinfra-awsx/sdk/go/awsx/ecs", name: "ecs", want: "awsxecs"},
			},
			wantGroups: [][]string{
				{
		...
		{
			desc: "basename mismatch/codeinfra",
			imports: []importCall{
				{importPath: "github.com/khulnasoft/codeinfra-kubernetes/sdk/v3/go/kubernetes/core/v1", name: "corev1", want: "corev1"},
			},
			wantGroups: [][]string{
...
func TestDeferredOutputCastTypeParameter(t *testing.T) {
	t.Parallel()
	assert.Equal(t, "codeinfra.AnyOutput", deferredOutputCastTypeParameter(model.DynamicType))
	assert.Equal(t, "codeinfra.StringOutput", deferredOutputCastTypeParameter(model.StringType))
	assert.Equal(t, "codeinfra.BoolOutput", deferredOutputCastTypeParameter(model.BoolType))
	assert.Equal(t, "codeinfra.IntOutput", deferredOutputCastTypeParameter(model.IntType))
	assert.Equal(t, "codeinfra.Float64Output", deferredOutputCastTypeParameter(model.NumberType))
	assert.Equal(t, "codeinfra.StringArrayOutput", deferredOutputCastTypeParameter(model.NewListType(model.StringType)))
	assert.Equal(t, "codeinfra.IntArrayOutput", deferredOutputCastTypeParameter(model.NewListType(model.IntType)))
	assert.Equal(t, "codeinfra.BoolArrayOutput", deferredOutputCastTypeParameter(model.NewListType(model.BoolType)))
	assert.Equal(t, "codeinfra.Float64ArrayOutput", deferredOutputCastTypeParameter(model.NewListType(model.NumberType)))
	assert.Equal(t, "codeinfra.ArrayOutput", deferredOutputCastTypeParameter(model.NewListType(model.DynamicType)))
	assert.Equal(t, "codeinfra.StringMapOutput", deferredOutputCastTypeParameter(model.NewMapType(model.StringType)))
	assert.Equal(t, "codeinfra.IntMapOutput", deferredOutputCastTypeParameter(model.NewMapType(model.IntType)))
	assert.Equal(t, "codeinfra.BoolMapOutput", deferredOutputCastTypeParameter(model.NewMapType(model.BoolType)))
	assert.Equal(t, "codeinfra.Float64MapOutput", deferredOutputCastTypeParameter(model.NewMapType(model.NumberType)))
	assert.Equal(t, "codeinfra.MapOutput", deferredOutputCastTypeParameter(model.NewMapType(model.DynamicType)))
}
