...
func TestInstallPluginErrorText(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			ExpectedError: "Could not automatically download and install resource plugin 'codeinfra-resource-myplugin'," +
				" install the plugin using `codeinfra plugin install resource myplugin`: some error",
		},
		{
			Name: "Different kind",
			Err: InstallPluginError{
			...
			ExpectedError: "Could not automatically download and install converter plugin 'codeinfra-converter-myplugin'," +
				" install the plugin using `codeinfra plugin install converter myplugin`: some error",
		},
		{
			...
			ExpectedError: "Could not automatically download and install resource plugin 'codeinfra-resource-myplugin'" +
				" at version v0.1.0, install the plugin using `codeinfra plugin install resource myplugin v0.1.0`: some error",
		},
		{
			...
			ExpectedError: "Could not automatically download and install resource plugin 'codeinfra-resource-myplugin'" +
				" at version v0.1.0, install the plugin using `codeinfra plugin install resource myplugin v0.1.0" +
		...
		},
		{
			...
			ExpectedError: "Could not automatically download and install resource plugin 'codeinfra-resource-myplugin'," +
				" install the plugin using `codeinfra plugin install resource myplugin" +
				" --server github://owner/repo`: some error",
		},
	}
