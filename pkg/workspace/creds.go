...
// have not logged in. Note if CODEINFRA_BACKEND_URL is set, the corresponding value is returned
...
func GetCurrentCloudURL(ws Context, e env.Env, project *workspace.Project) (string, error) {
	// Allow CODEINFRA_BACKEND_URL to override the current cloud URL selection
...
func GetBackendConfigDefaultOrg(project *workspace.Project) (string, error) {
	config, err := workspace.GetCodeinfraConfig()
