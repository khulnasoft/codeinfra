...
func ValidateProjectName(s string) error {
	...
	if strings.ToLower(s) == "codeinfra" || strings.HasPrefix(strings.ToLower(s), "codeinfra.") {
		return errors.New("project name must not be `Codeinfra` and must not start with the prefix `Codeinfra.` " +
