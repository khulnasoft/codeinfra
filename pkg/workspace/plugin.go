...
func (err *InstallPluginError) Error() string {
	...
	if err.Spec.Version != nil {
		return fmt.Sprintf("Could not automatically download and install %[1]s plugin 'codeinfra-%[1]s-%[2]s'"+
			" at version v%[3]s"+
			", install the plugin using `codeinfra plugin install %[1]s %[2]s v%[3]s%[4]s`: %[5]v",
			err.Spec.Kind, err.Spec.Name, err.Spec.Version, server, err.Err)
	}

	return fmt.Sprintf("Could not automatically download and install %[1]s plugin 'codeinfra-%[1]s-%[2]s'"+
		", install the plugin using `codeinfra plugin install %[1]s %[2]s%[3]s`: %[4]v",
