...
// openKeeper opens the keeper, handling codeinfra-specifc cases in the URL.
...
func NewCloudSecretsManagerFromState(state json.RawMessage) (secrets.Manager, error) {
	...
	if strings.HasPrefix(s.URL, "azurekeyvault://") {
		...
		if err != nil {
			// https://github.com/khulnasoft/codeinfra/issues/15329 resulted in some non-encoded keys being written
...
func NewCloudSecretsManager(info *workspace.ProjectStack,
	...
	if override := os.Getenv("CODEINFRA_CLOUD_SECRET_OVERRIDE"); override != "" {
