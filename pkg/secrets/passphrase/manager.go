...
func symmetricCrypterFromPhraseAndState(phrase string, state string) (config.Crypter, error) {
	...
	if err != nil || decrypted != "codeinfra" {
...
func NewPassphraseSecretsManager(phrase string) (string, secrets.Manager, error) {
	...
	ignoredCtx := context.Background()
	msg, err := crypter.EncryptValue(ignoredCtx, "codeinfra")
...
// given state. Will use the passphrase found in CODEINFRA_CONFIG_PASSPHRASE, the file specified by
// CODEINFRA_CONFIG_PASSPHRASE_FILE, or otherwise will prompt for the passphrase if interactive.
func newPromptingPassphraseSecretsManagerFromState(state, stackName string) (secrets.Manager, error) {
	...
	prompt += "    (set CODEINFRA_CONFIG_PASSPHRASE or CODEINFRA_CONFIG_PASSPHRASE_FILE to remember)"
	for {
...
func readPassphrase(prompt string, useEnv bool) (phrase string, interactive bool, err error) {
	if useEnv {
		if phrase, ok := os.LookupEnv("CODEINFRA_CONFIG_PASSPHRASE"); ok {
			return phrase, false, nil
		}
		if phraseFile, ok := os.LookupEnv("CODEINFRA_CONFIG_PASSPHRASE_FILE"); ok && phraseFile != "" {
			phraseFilePath, err := filepath.Abs(phraseFile)
			if err != nil {
				return "", false, fmt.Errorf("unable to construct a path the CODEINFRA_CONFIG_PASSPHRASE_FILE: %w", err)
			}
			phraseDetails, err := os.ReadFile(phraseFilePath)
			if err != nil {
				return "", false, fmt.Errorf("unable to read CODEINFRA_CONFIG_PASSPHRASE_FILE: %w", err)
			}
			return strings.TrimSpace(string(phraseDetails)), false, nil
		}
		if !isInteractive() {
			return "", false, errors.New("passphrase must be set with CODEINFRA_CONFIG_PASSPHRASE or " +
				"CODEINFRA_CONFIG_PASSPHRASE_FILE environment variables")
		}
	}
...
func isInteractive() bool {
	test, ok := os.LookupEnv("CODEINFRA_TEST_PASSPHRASE")
...
func (ec *errorCrypter) EncryptValue(ctx context.Context, _ string) (string, error) {
	return "", errors.New("failed to encrypt: incorrect passphrase, please set CODEINFRA_CONFIG_PASSPHRASE to the " +
		"correct passphrase or set CODEINFRA_CONFIG_PASSPHRASE_FILE to a file containing the passphrase")
}

func (ec *errorCrypter) DecryptValue(ctx context.Context, _ string) (string, error) {
	return "", errors.New("failed to decrypt: incorrect passphrase, please set CODEINFRA_CONFIG_PASSPHRASE to the " +
		"correct passphrase or set CODEINFRA_CONFIG_PASSPHRASE_FILE to a file containing the passphrase")
}

func (ec *errorCrypter) BulkDecrypt(ctx context.Context, _ []string) (map[string]string, error) {
	return nil, errors.New("failed to decrypt: incorrect passphrase, please set CODEINFRA_CONFIG_PASSPHRASE to the " +
		"correct passphrase or set CODEINFRA_CONFIG_PASSPHRASE_FILE to a file containing the passphrase")
}
