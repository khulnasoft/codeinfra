...
func TestDeletingComponentResourceProducesResourceOutputsEvent(t *testing.T) {
	...
	step := deploy.NewDeleteStep(&deploy.Deployment{}, map[resource.URN]bool{}, &resource.State{
		URN:      resource.URN("urn:codeinfra:stack::project::my:example:Foo::foo"),
		ID:       "foo",
		Custom:   false,
		Provider: "unimportant",
	})
	acts.Seen[resource.URN("urn:codeinfra:stack::project::my:example:Foo::foo")] = step
	...
	case ResourceOutputsEvent:
		...
		assert.True(t, ok)
		assert.True(t, e.Metadata.URN == "urn:codeinfra:stack::project::my:example:Foo::foo")
	default:
