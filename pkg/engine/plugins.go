...
func (p PluginSet) Deduplicate() PluginSet {
	...
	add := func(p workspace.PluginSpec) {
		...
		if ok {
			...
			// depended on a language package `codeinfra-pkg` with version `x.y.z`.
...
func (p PackageSet) UpdatesTo(old PackageSet) []PackageUpdate {
	...
	for _, value := range p {
		for _, otherValue := range old {
			...
			// only really be one instance of a package name in a Codeinfra program.
...
func EnsurePluginsAreInstalled(ctx context.Context, opts *deploymentOptions, d diag.Sink, plugins PluginSet,
	...
	for _, plug := range plugins.Values() {
		if plug.Name == "codeinfra" && plug.Kind == apitype.ResourcePlugin {
			logging.V(preparePluginLog).Infof("ensurePluginsAreInstalled(): codeinfra is a builtin plugin")
			continue
		}
		...
		if workspace.IsPluginBundled(plug.Kind, plug.Name) {
			return fmt.Errorf(
				"the %v %v plugin is bundled with Codeinfra, and cannot be directly installed."+
					" Reinstall Codeinfra via your package manager or install script",
				plug.Name,
				plug.Kind,
			)
		}
...
func installPlugin(
	...
	if err := plugin.InstallWithContext(
		...
		return fmt.Errorf("installing plugin; run `codeinfra plugin install %s %s v%s` to retry manually: %w",
