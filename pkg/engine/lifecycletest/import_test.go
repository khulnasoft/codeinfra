...
func TestImportUpdatedID(t *testing.T) {
	...
	stackURN := resource.URN("urn:codeinfra:stack::stack::codeinfra:codeinfra:Stack::foo")
...
func TestImportPlanExistingImport(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		resp, err := monitor.RegisterResource("codeinfra:codeinfra:Stack", "test", false)
...
func TestImportPlanSpecificProvider(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		_, err := monitor.RegisterResource("codeinfra:providers:pkgA", "provA", true)
		assert.NoError(t, err)
		return nil
	})
	hostF := deploytest.NewPluginHostF(nil, nil, programF, loaders...)
...
func TestImportComponent(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::my-component::comp"), comp.URN)
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::my-component$pkgA:m:typA::resB"), custom.URN)
...
func TestImportRemoteComponent(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::mlc:index:Component::comp"), comp.URN)
	// Ensure it's marked as a component.
	assert.False(t, comp.Custom, "expected component resource to not be marked as custom")

	// Ensure resource 4 is the custom resource
	custom := snap.Resources[4]
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::mlc:index:Component$pkgA:m:typA::resB"), custom.URN)
	// Ensure it's marked as custom.
	assert.True(t, custom.Custom, "expected custom resource to be marked as custom")
}

// Regression test for https://github.com/khulnasoft/codeinfra-gcp/issues/1900
