...
func TestSingleResourceDefaultProviderGolangLifecycle(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		ctx, err := codeinfra.NewContext(context.Background(), codeinfra.RunInfo{
		...
		return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
			var resA testResource
			err := ctx.RegisterResource("pkgA:m:typA", "resA", &testResourceInputs{
				Foo: codeinfra.String("bar"),
			}, &resA)
			assert.NoError(t, err)
	...
	setupAndRunProgram := func(ignoreChanges []string) *deploy.Snapshot {
		programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
			...
			return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
				...
				err := ctx.RegisterResource("pkgA:m:typA", "resA", nil, &res, codeinfra.IgnoreChanges(ignoreChanges))
				assert.NoError(t, err)
...
func TestExplicitDeleteBeforeReplaceGoSDK(t *testing.T) {
	...
	inputsA := &testResourceInputs{Foo: codeinfra.String("foo")}
	...
	var stackURN, provURN, urnA resource.URN = "urn:codeinfra:test::test::codeinfra:codeinfra:Stack::test-test",
		"urn:codeinfra:test::test::codeinfra:providers:pkgA::provA", "urn:codeinfra:test::test::pkgA:m:typA::resA"
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		ctx, err := codeinfra.NewContext(context.Background(), codeinfra.RunInfo{
			Project:     info.Project,
			Stack:       info.Stack,
		...
		return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
			provider := &codeinfra.ProviderResourceState{}
			...
			err = ctx.RegisterResource("pkgA:m:typA", "resA", inputsA, &res,
				codeinfra.Provider(provider), codeinfra.DeleteBeforeReplace(getDbr()))
			assert.NoError(t, err)
	...
	inputsA.Foo = codeinfra.String("bar")
	p.Steps = []lt.TestStep{{
		Op: Update,
	...
	dbrA, inputsA.Foo = &dbrValue, codeinfra.String("baz")
	p.Steps = []lt.TestStep{{
		Op: Update,
...
func TestReadResourceGolangLifecycle(t *testing.T) {
	...
	var stackURN, defaultProviderURN, urnA resource.URN = "urn:codeinfra:test::test::codeinfra:codeinfra:Stack::test-test",
		"urn:codeinfra:test::test::codeinfra:providers:pkgA::default", "urn:codeinfra:test::test::pkgA:m:typA::resA"
	...
	setupAndRunProgram := func() *deploy.Snapshot {
		programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
			...
			return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
				var res codeinfra.CustomResourceState
				err := ctx.ReadResource("pkgA:m:typA", "resA", codeinfra.ID("someId"), nil, &res)
				assert.NoError(t, err)
...
// ensures that RegisterResource, ReadResource (TODO https://github.com/khulnasoft/codeinfra/issues/3562),
...
func TestProviderInheritanceGolangLifecycle(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
			...
			var providerA codeinfra.ProviderResourceState
			err := ctx.RegisterResource(string(providers.MakeProviderType("pkgA")), "prov1",
				&testResourceInputs{
					Foo: codeinfra.String("1"),
				}, &providerA)
			assert.NoError(t, err)
			var providerB codeinfra.ProviderResourceState
			err = ctx.RegisterResource(string(providers.MakeProviderType("pkgB")), "prov2",
				&testResourceInputs{
					Bar:  codeinfra.String("2"),
					Bang: codeinfra.String(""),
				}, &providerB)
			assert.NoError(t, err)
			var providerBOverride codeinfra.ProviderResourceState
			err = ctx.RegisterResource(string(providers.MakeProviderType("pkgB")), "prov3",
				&testResourceInputs{
					Bar:  codeinfra.String(""),
					Bang: codeinfra.String("3"),
				}, &providerBOverride)
			assert.NoError(t, err)
			parentProviders := make(map[string]codeinfra.ProviderResource)
			parentProviders["pkgA"] = &providerA
			parentProviders["pkgB"] = &providerB
			// create a parent resource that uses provider map
			var parentResource codeinfra.CustomResourceState
			err = ctx.RegisterResource("pkgA:m:typA", "resA", nil, &parentResource, codeinfra.ProviderMap(parentProviders))
			assert.NoError(t, err)
			...
			var childResource codeinfra.CustomResourceState
			err = ctx.RegisterResource("pkgB:m:typB", "resBChild", nil, &childResource, codeinfra.Parent(&parentResource))
			assert.NoError(t, err)
			...
			var childWithOverride codeinfra.CustomResourceState
			err = ctx.RegisterResource("pkgB:m:typB", "resBChildOverride", nil, &childWithOverride,
				codeinfra.Parent(&parentResource), codeinfra.Provider(&providerBOverride))
			assert.NoError(t, err)
			...
			// pass in a fake ID
			testID := codeinfra.ID("testID")

			// read a resource that uses provider map
			var rereadParent codeinfra.CustomResourceState
			err = ctx.ReadResource("pkgA:m:typA", "readResA", testID, nil, &rereadParent, codeinfra.ProviderMap(parentProviders))
			assert.NoError(t, err)
			// parent uses specified provider from map
			...
			var rereadChild codeinfra.CustomResourceState
			err = ctx.ReadResource("pkgB:m:typB", "readResBChild", testID, nil, &rereadChild, codeinfra.Parent(&parentResource))
			assert.NoError(t, err)
			...
			var rereadChildWithOverride codeinfra.CustomResourceState
			...
			err = ctx.Invoke("pkgB:do:something", invokeArgs{
				Bang: "3",
			}, &invokeResult, codeinfra.Provider(&providerBOverride))
			assert.NoError(t, err)

			// invoke with parent
			err = ctx.Invoke("pkgB:do:something", invokeArgs{
				Bar: "2",
			}, &invokeResult, codeinfra.Parent(&parentResource))
			assert.NoError(t, err)

			// invoke with parent and provider
			err = ctx.Invoke("pkgB:do:something", invokeArgs{
				Bang: "3",
			}, &invokeResult, codeinfra.Parent(&parentResource), codeinfra.Provider(&providerBOverride))
			assert.NoError(t, err)
	...
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
			...
			err := ctx.RegisterResource("pkgA:m:typA", "resA", resourceProperties, &res,
				codeinfra.ReplaceOnChanges([]string{"foo"}))
			assert.NoError(t, err)
...
type remoteComponentArgs struct {
	Foo codeinfra.URN `codeinfra:"foo"`
	Bar *string    `codeinfra:"bar"`
}

type remoteComponentInputs struct {
	Foo codeinfra.URNInput       `codeinfra:"foo"`
...
type remoteComponent struct {
	...
	Foo codeinfra.StringOutput `codeinfra:"foo"`
	Baz codeinfra.StringOutput `codeinfra:"baz"`
}
...
func TestRemoteComponentGolang(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
			var resB codeinfra.CustomResourceState
			err := ctx.RegisterResource("pkgA:index:typA", "resA", codeinfra.Map{}, &resB)
			require.NoError(t, err)
