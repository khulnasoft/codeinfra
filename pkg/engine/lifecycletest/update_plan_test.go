...
func TestPlannedUpdate(t *testing.T) {
	...
	validate := ExpectDiagMessage(t, regexp.QuoteMeta(
		"<{%reset%}>resource urn:codeinfra:test::test::pkgA:m:typA::resA violates plan: "+
	...
	plan.ResourcePlans["urn:codeinfra:test::test::codeinfra:providers:pkgA::default"].Ops = []display.StepOp{deploy.OpSame}
...
func TestExpectedDelete(t *testing.T) {
	...
	p.Options.Plan = plan.Clone()
	validate := ExpectDiagMessage(t, regexp.QuoteMeta(
		"<{%reset%}>resource urn:codeinfra:test::test::pkgA:m:typA::resB violates plan: "+
...
func TestExpectedCreate(t *testing.T) {
	...
	validate := ExpectDiagMessage(t, regexp.QuoteMeta(
		"<{%reset%}>expected resource operations for "+
			"urn:codeinfra:test::test::pkgA:m:typA::resB but none were seen<{%reset%}>\n"))
	snap, err = lt.TestOp(Update).RunStep(project, p.GetTarget(t, snap), p.Options, false, p.BackendClient, validate, "1")
	assert.NotNil(t, snap)
	assert.NoError(t, err)
...
func TestPlannedPreviews(t *testing.T) {
	...
	p.Options.Plan = plan.Clone()
	validate := ExpectDiagMessage(t, regexp.QuoteMeta(
		"<{%reset%}>resource urn:codeinfra:test::test::pkgA:m:typA::resA violates plan: properties changed: "+
...
func TestAliasWithPlans(t *testing.T) {
	...
	aliases[0] = resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA")
	plan, err := lt.TestOp(Update).Plan(project, p.GetTarget(t, nil), p.Options, p.BackendClient, nil)
	assert.NotNil(t, plan)
	assert.NoError(t, err)
...
func TestPlannedUpdateWithCheckFailure(t *testing.T) {
	// Regression test for https://github.com/khulnasoft/codeinfra/issues/9247
	...
	plan, err = lt.TestOp(Update).Plan(project, p.GetTarget(t, nil), p.Options, p.BackendClient, nil)
	assert.NotNil(t, plan)
	assert.Contains(t, plan.ResourcePlans, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"))
	assert.NoError(t, err)
...
func TestPlannedUpdateWithDependentDelete(t *testing.T) {
	...
	assert.Equal(t, 3, len(plan.ResourcePlans["urn:codeinfra:test::test::pkgA:m:typA::resA"].Ops))
	assert.Equal(t, 3, len(plan.ResourcePlans["urn:codeinfra:test::test::pkgA:m:typB::resB"].Ops))
	...
	plan, err := lt.TestOp(Update).Plan(project, p.GetTarget(t, nil), lt.TestUpdateOptions{
		...
		UpdateOptions: UpdateOptions{
			...
			Targets: deploy.NewUrnTargetsFromUrns([]resource.URN{
				resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"),
			}),
		},
	}, p.BackendClient, nil)
	assert.NoError(t, err)
