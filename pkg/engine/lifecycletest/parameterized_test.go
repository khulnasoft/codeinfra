...
func TestReplacementParameterizedProvider(t *testing.T) {
			...
			return &deploytest.Provider{
				...
				CallF: func(_ context.Context, req plugin.CallRequest, _ *deploytest.ResourceMonitor) (plugin.CallResponse, error) {
					...
					assert.Equal(t, map[resource.PropertyKey][]resource.URN{
						"input": {"urn:codeinfra:stack::m::typA::resB"},
					...
					return plugin.CallResponse{
						...
						ReturnDependencies: map[resource.PropertyKey][]resource.URN{
							"output": {"urn:codeinfra:stack::m::typA::resB"},
						},
						Failures: nil,
					}, nil
				},
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		extRef, err := monitor.RegisterPackage("pkgA", "1.0.0", "", nil, &codeinfrarpc.Parameterization{
			Name:    "pkgExt",
			Version: "0.5.0",
		...
		prov, err := monitor.RegisterResource("codeinfra:providers:pkgExt", "provider", true, deploytest.ResourceOptions{
			PackageRef: extRef,
		})
		assert.NoError(t, err)
		provID := prov.ID
	...
	assert.Equal(t, tokens.Type("codeinfra:providers:pkgExt"), prov.Type)
