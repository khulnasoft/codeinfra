...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/15763. Check that if a resource gets implicated in a
...
func TestDBRProtect(t *testing.T) {
	...
	assert.ErrorContains(t, err, "unable to replace resource \"urn:codeinfra:test::test::pkgA:index:typ::resB\""+
		" as part of replacing \"urn:codeinfra:test::test::pkgA:index:typ::resA\" as it is currently marked for protection.")
