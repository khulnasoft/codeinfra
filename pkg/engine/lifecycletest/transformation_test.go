...
func TransformFunction(
	...
	return func(request []byte) (proto.Message, error) {
		var transformationRequest codeinfrarpc.TransformRequest
		...
		return &codeinfrarpc.TransformResponse{
			Properties: mret,
			Options:    opts,
		}, nil
	}
...
func TransformInvokeFunction(
	f func(
		token string, props resource.PropertyMap, opts *codeinfrarpc.TransformInvokeOptions,
	) (resource.PropertyMap, *codeinfrarpc.TransformInvokeOptions, error),
) func([]byte) (proto.Message, error) {
	return func(request []byte) (proto.Message, error) {
		var transformationRequest codeinfrarpc.TransformInvokeRequest
		err := proto.Unmarshal(request, &transformationRequest)
		if err != nil {
			return nil, fmt.Errorf("unmarshaling request: %w", err)
		}
		...
		return &codeinfrarpc.TransformInvokeResponse{
			Args:    mret,
			Options: opts,
		}, nil
	}
...
func TestRemoteTransforms(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), res.URN)
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typA::resB"), res.URN)
	// Check it's final input properties match what we expected from the transformations
	assert.Equal(t, resource.PropertyMap{
...
func TestRemoteTransformationsConstruct(t *testing.T) {
	...
	res := snap.Resources[2]
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typC$pkgA:m:typA::resA"), res.URN)
	// Check it's final input properties match what we expected from the transformations
	assert.Equal(t, resource.PropertyMap{
...
func TestRemoteTransformsOptions(t *testing.T) {
	...
	urnB := "urn:codeinfra:test::test::pkgA:m:typA::resB"
	urnC := "urn:codeinfra:test::test::pkgA:m:typA::resC"
	urnD := "urn:codeinfra:test::test::pkgA:m:typA::resD"
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		callback1, err := callbacks.Allocate(
			TransformFunction(func(name, typ string, custom bool, parent string,
				...
				assert.Equal(t, urnB, opts.Aliases[0].Alias.(*codeinfrarpc.Alias_Urn).Urn)
				...
				opts = &codeinfrarpc.TransformResourceOptions{
					...
					Aliases: []*codeinfrarpc.Alias{
						{Alias: &codeinfrarpc.Alias_Urn{Urn: urnB}},
					},
...
func TestRemoteTransformsDependencies(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resC"), res.URN)
	// Check it's final input properties match what we expected from the transformations
	assert.Equal(t, resource.PropertyMap{
		"foo": resource.NewNumberProperty(1),
	}, res.Inputs)
	...
	assert.Equal(t, map[resource.PropertyKey][]resource.URN{
		"foo": {resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA")},
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/15843. Ensure that if a component resource has a
...
func TestTransformsProviderOpt(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		resp, err := monitor.RegisterResource("codeinfra:providers:pkgA", "explicit", true)
		require.NoError(t, err)
		explicitProvider = string(resp.URN) + "::" + resp.ID.String()

		resp, err = monitor.RegisterResource("codeinfra:providers:pkgA", "implicit", true)
		require.NoError(t, err)
	...
	assert.Equal(t, urn.URN("urn:codeinfra:test::test::pkgA:m:typA::explicitProvider"), snap.Resources[0].URN)
	assert.Equal(t, explicitProvider, snap.Resources[0].Provider)
	assert.Equal(t, urn.URN("urn:codeinfra:test::test::pkgA:m:typA::explicitProvidersMap"), snap.Resources[1].URN)
	assert.Equal(t, explicitProvider, snap.Resources[1].Provider)
	assert.Equal(t, urn.URN("urn:codeinfra:test::test::pkgA:m:typA::implicitProvider"), snap.Resources[2].URN)
	assert.Equal(t, implicitProvider, snap.Resources[2].Provider)
	assert.Equal(t,
		urn.URN("urn:codeinfra:test::test::xmy:component:resource$pkgA:m:typA::parentedResource"),
		snap.Resources[5].URN)
	assert.Equal(t, explicitProvider, snap.Resources[5].Provider)
	assert.Equal(t,
		urn.URN("urn:codeinfra:test::test::ymy:component:resource$pkgA:m:typA::parentedResource"),
