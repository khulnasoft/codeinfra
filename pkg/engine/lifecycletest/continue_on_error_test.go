...
func TestDestroyContinueOnError(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::codeinfra:providers:pkgA::default"), snap.Resources[0].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::dependency"), snap.Resources[1].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::codeinfra:providers:pkgB::default"), snap.Resources[2].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgB:m:typB::failing"), snap.Resources[3].URN)
}
...
func TestUpContinueOnErrorCreate(t *testing.T) {
	...
	// the create step's Apply method, which was overlooked and caused https://github.com/khulnasoft/codeinfra/issues/16373.
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SUCCESS, respIndependent1.Result)
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SUCCESS, respIndependent2.Result)
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SUCCESS, respIndependent3.Result)
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SKIP, respDepOnFailing.Result)
	...
	expectedURNs := []string{
		"urn:codeinfra:test::test::codeinfra:providers:pkgB::default",
		"urn:codeinfra:test::test::codeinfra:providers:pkgA::default",
		"urn:codeinfra:test::test::pkgA:m:typA::independent1",
		"urn:codeinfra:test::test::pkgA:m:typA::independent2",
		"urn:codeinfra:test::test::pkgA:m:typA::independent3",
		"urn:codeinfra:test::test::pkgB:m:typB::failing2",
		"urn:codeinfra:test::test::pkgB:m:typB::failing3",
	}
...
func TestUpContinueOnErrorUpdate(t *testing.T) {
	...
	// and caused https://github.com/khulnasoft/codeinfra/issues/16373.
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		assert.NoError(t, err)
		if update {
			assert.Equal(t, codeinfrarpc.Result_FAIL, resp.Result)
		} else {
			// On creation we expect to succeed
			assert.Equal(t, codeinfrarpc.Result_SUCCESS, resp.Result)
		}
		...
		assert.NoError(t, err)
		if update {
			assert.Equal(t, codeinfrarpc.Result_FAIL, resp2.Result)
		} else {
			// On creation we expect to succeed
			assert.Equal(t, codeinfrarpc.Result_SUCCESS, resp2.Result)
		}
	...
	expectedURNs := []string{
		...
		"urn:codeinfra:test::test::pkgA:m:typA::independent2",
		"urn:codeinfra:test::test::pkgA:m:typA::independent3",
		"urn:codeinfra:test::test::pkgB:m:typB::failing",
		"urn:codeinfra:test::test::pkgB:m:typB::failing2",
	}
	...
	for _, urn := range expectedURNs {
		...
		switch urn {
		case "urn:codeinfra:test::test::pkgB:m:typB::failing", "urn:codeinfra:test::test::pkgB:m:typB::failing2":
...
func TestUpContinueOnErrorUpdateWithRefresh(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::codeinfra:providers:pkgB::default"), snap.Resources[0].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::codeinfra:providers:pkgA::default"), snap.Resources[1].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::independent1"), snap.Resources[2].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::independent2"), snap.Resources[3].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::independent3"), snap.Resources[4].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgB:m:typB::failing"), snap.Resources[5].URN)
...
func TestDestroyContinueOnErrorDeleteAfterFailedUp(t *testing.T) {
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::codeinfra:providers:pkgA::default"), snap.Resources[0].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::willBeDeleted"), snap.Resources[1].URN)
...
func TestUpContinueOnErrorFailedDependencies(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_FAIL, parent.Result)
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SKIP, child.Result)

		deletedWith, err := monitor.RegisterResource("pkgB:m:typB", "deletedWith", true, deploytest.ResourceOptions{
			SupportsResultReporting: true,
		})
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_FAIL, deletedWith.Result)
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SKIP, deletedWithDep.Result)

		propDep, err := monitor.RegisterResource("pkgB:m:typB", "propDep", true, deploytest.ResourceOptions{
			SupportsResultReporting: true,
		})
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_FAIL, propDep.Result)
		...
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SKIP, propDepChild.Result)

		independent, err := monitor.RegisterResource("pkgA:m:typA", "independent", true, deploytest.ResourceOptions{
			SupportsResultReporting: true,
		})
		assert.NoError(t, err)
		assert.Equal(t, codeinfrarpc.Result_SUCCESS, independent.Result)
	...
	expectedURNs := []string{
		"urn:codeinfra:test::test::codeinfra:providers:pkgB::default",
		"urn:codeinfra:test::test::codeinfra:providers:pkgA::default",
		"urn:codeinfra:test::test::pkgA:m:typA::independent",
	}
