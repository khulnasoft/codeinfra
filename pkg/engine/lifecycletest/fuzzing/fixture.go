...
var urnPattern = regexp.MustCompile(`urn:codeinfra:[^:]+::[^:]+::[^\s,.'"]+`)

// writeReproTest writes the given string to a file in the directory specified by
// CODEINFRA_LIFECYCLE_TEST_FUZZING_REPRO_DIR (which will be created if it does not exist), or to a temporary directory if
...
func writeReproTest(reproTest string) (string, error) {
	reproDir := os.Getenv("CODEINFRA_LIFECYCLE_TEST_FUZZING_REPRO_DIR")
	if reproDir != "" {
