...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/display"
	. "github.com/khulnasoft/codeinfra/pkg/v3/engine" //nolint:revive
	lt "github.com/khulnasoft/codeinfra/pkg/v3/engine/lifecycletest/framework"
	"github.com/khulnasoft/codeinfra/pkg/v3/resource/deploy"
	"github.com/khulnasoft/codeinfra/pkg/v3/resource/deploy/deploytest"
	"github.com/khulnasoft/codeinfra/pkg/v3/resource/deploy/providers"
...
func aliasesFromAliases(aliases []resource.Alias) []*codeinfrarpc.Alias {
	result := make([]*codeinfrarpc.Alias, len(aliases))
	for i, alias := range aliases {
		if alias.URN != "" {
			result[i] = &codeinfrarpc.Alias{
				Alias: &codeinfrarpc.Alias_Urn{
					Urn: string(alias.URN),
				},
			}
		} else {
			spec := &codeinfrarpc.Alias_Spec{
				Name:    alias.Name,
				Type:    alias.Type,
			...
			if alias.Parent != "" {
				spec.Parent = &codeinfrarpc.Alias_Spec_ParentUrn{
					ParentUrn: string(alias.Parent),
				}
			} else if alias.NoParent {
				spec.Parent = &codeinfrarpc.Alias_Spec_NoParent{
					NoParent: alias.NoParent,
				}
			}

			result[i] = &codeinfrarpc.Alias{
				Alias: &codeinfrarpc.Alias_Spec_{
					Spec: spec,
				},
			}
		}
...
func makeUrnAlias(urn string) *codeinfrarpc.Alias {
	return &codeinfrarpc.Alias{
		Alias: &codeinfrarpc.Alias_Urn{
			Urn: urn,
		},
	}
...
func makeSpecAlias(name, typ, project, stack string) *codeinfrarpc.Alias {
	return &codeinfrarpc.Alias{
		Alias: &codeinfrarpc.Alias_Spec_{
			Spec: &codeinfrarpc.Alias_Spec{
				Name:    name,
...
func makeSpecAliasWithParent(name, typ, project, stack, parent string) *codeinfrarpc.Alias {
	return &codeinfrarpc.Alias{
		Alias: &codeinfrarpc.Alias_Spec_{
			Spec: &codeinfrarpc.Alias_Spec{
				Name:    name,
				...
				Project: project,
				Stack:   stack,
				Parent: &codeinfrarpc.Alias_Spec_ParentUrn{
					ParentUrn: parent,
				},
			},
		},
...
func makeSpecAliasWithNoParent(name, typ, project, stack string, parent bool) *codeinfrarpc.Alias {
	return &codeinfrarpc.Alias{
		Alias: &codeinfrarpc.Alias_Spec_{
			Spec: &codeinfrarpc.Alias_Spec{
				Name:    name,
				...
				Project: project,
				Stack:   stack,
				Parent: &codeinfrarpc.Alias_Spec_NoParent{
					NoParent: parent,
				},
			},
		},
					...
					Validate: func(project workspace.Project, target deploy.Target, entries JournalEntries,
						...
						for _, event := range events {
							if event.Type == ResourcePreEvent {
								...
								if payload.Metadata.Type == "codeinfra:providers:pkgA" {
									continue
								}
						...
						for _, entry := range entries {
							if entry.Step.Type() == "codeinfra:providers:pkgA" {
								continue
							}
							switch entry.Kind {
...
func TestAliases(t *testing.T) {
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		name: "n3",
		aliases: []resource.Alias{
			{URN: "urn:codeinfra:test::test::pkgA:index:t1::n2"},
			{Name: "n1"},
		},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		name: "n1",
		aliases: []resource.Alias{
			{URN: "urn:codeinfra:test::test::pkgA:index:t1::n3"},
			{Name: "n2"},
		},
	...
	snap = updateProgramWithResource(nil, []Resource{{
		...
		t:            "pkgA:index:t2",
		name:         "n2",
		dependencies: []resource.URN{"urn:codeinfra:test::test::pkgA:index:t1::n1"},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		t:            "pkgA:index:t2-new",
		name:         "n2-new",
		dependencies: []resource.URN{"urn:codeinfra:test::test::pkgA:index:t1-new::n1-new"},
		aliases: []resource.Alias{
	...
	snap = updateProgramWithResource(nil, []Resource{{
		...
		t:      "pkgA:index:t2",
		name:   "n2",
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1::n1"),
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		t:      "pkgA:index:t2-new",
		name:   "n2-new",
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1-new::n1-new"),
		aliases: []resource.Alias{
			{Type: "pkgA:index:t2", Name: "n2"},
		},
	}}, []display.StepOp{deploy.OpSame, deploy.OpReplace, deploy.OpCreateReplacement, deploy.OpDeleteReplaced},
	...
	snap = updateProgramWithResource(nil, []Resource{{
		...
		t:      "pkgA:index:t3",
		name:   "n1-sub-sub",
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1$pkgA:index:t2::n1-sub"),
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		t:      "pkgA:index:t3",
		name:   "n1-new-sub-sub",
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1-new$pkgA:index:t2::n1-new-sub"),
		aliases: []resource.Alias{
	...
	snap = updateProgramWithResource(nil, []Resource{{
		...
		t:      "pkgA:index:t2-v0",
		name:   "n1-sub",
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1-v0::n1"),
	}, {
		t:      "pkgA:index:t3",
		name:   "n1-sub-sub",
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1-v0$pkgA:index:t2-v0::n1-sub"),
	...
	for i := 0; i < 100; i++ {
		n1Aliases = append(n1Aliases, resource.Alias{URN: resource.URN(
			fmt.Sprintf("urn:codeinfra:test::test::pkgA:index:t1-v%d::n1", i),
		)})

		for j := 0; j < 10; j++ {
			n2Aliases = append(n2Aliases, resource.Alias{
				URN: resource.URN(fmt.Sprintf("urn:codeinfra:test::test::pkgA:index:t1-v%d$pkgA:index:t2-v%d::n1-sub", i, j)),
			})
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		t:       "pkgA:index:t2-v10",
		name:    "n1-new-sub",
		parent:  resource.URN("urn:codeinfra:test::test::pkgA:index:t1-v100::n1-new"),
		aliases: n2Aliases,
	}, {
		t:       "pkgA:index:t3",
		name:    "n1-new-sub-sub",
		parent:  resource.URN("urn:codeinfra:test::test::pkgA:index:t1-v100$pkgA:index:t2-v10::n1-new-sub"),
		aliases: n3Aliases,
	...
	snap = updateProgramWithResource(nil, []Resource{{
		...
		t:      "pkgA:index:child",
		parent: "urn:codeinfra:test::test::pkgA:index:parent::parent",
		name:   "child",
	...
	snap = updateProgramWithResource(nil, []Resource{{
		...
		t:      "pkgA:index:t1",
		parent: "urn:codeinfra:test::test::pkgA:index:t1::parent",
		name:   "sub",
	}, {
		t:      "pkgA:index:t1",
		parent: "urn:codeinfra:test::test::pkgA:index:t1$pkgA:index:t1::sub",
		name:   "sub-sub",
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		t:      "pkgA:index:t2",
		parent: "urn:codeinfra:test::test::pkgA:index:t1::one",
		name:   "two",
		aliases: []resource.Alias{
			{NoParent: true},
		},
...
func TestAliasesNodeJSBackCompat(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:               "Old Node SDK",
			grpcRequestHeaders: map[string]string{"codeinfra-runtime": "nodejs"},
...
func TestAliasURNs(t *testing.T) {
	...
	snap = updateProgramWithResource(snap, []Resource{{
		t:         "pkgA:index:t1",
		name:      "n2",
		aliasURNs: []resource.URN{"urn:codeinfra:test::test::pkgA:index:t1::n1"},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		name: "n3",
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1::n1",
			"urn:codeinfra:test::test::pkgA:index:t1::n2",
		},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		name: "n1",
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1::n3",
			"urn:codeinfra:test::test::pkgA:index:t1::n2",
			"urn:codeinfra:test::test::pkgA:index:t1::n1",
		},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1::n1",
			"urn:codeinfra:test::test::pkgA:index:t2::n1",
		},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1::n1",
			"urn:codeinfra:test::test::pkgA:othermod:t3::n1",
			"urn:codeinfra:test::test::pkgA:index:t2::n1",
		},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t4::n2",
		},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t5::n3",
		},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		deleteBeforeReplace: true,
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t6::n4",
		},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		dependencies: []resource.URN{"urn:codeinfra:test::test::pkgA:index:t1-new::n1-new"},
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t2::n2",
		},
	}}, []display.StepOp{deploy.OpSame, deploy.OpReplace, deploy.OpCreateReplacement, deploy.OpDeleteReplaced},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1-new::n1-new"),
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1$pkgA:index:t2::n2",
		},
	}}, []display.StepOp{deploy.OpSame, deploy.OpReplace, deploy.OpCreateReplacement, deploy.OpDeleteReplaced},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1-new::n1-new"),
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1$pkgA:index:t2::n1-sub",
		},
	}, {
		t:      "pkgA:index:t3",
		name:   "n1-new-sub-sub",
		parent: resource.URN("urn:codeinfra:test::test::pkgA:index:t1-new$pkgA:index:t2::n1-new-sub"),
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1$pkgA:index:t2$pkgA:index:t3::n1-sub-sub",
		},
	...
	for i := 0; i < 100; i++ {
		...
		for j := 0; j < 10; j++ {
			n2Aliases = append(n2Aliases, resource.URN(
				fmt.Sprintf("urn:codeinfra:test::test::pkgA:index:t1-v%d$pkgA:index:t2-v%d::n1-sub", i, j)))

			n3Aliases = append(n3Aliases, resource.URN(
				fmt.Sprintf("urn:codeinfra:test::test::pkgA:index:t1-v%d$pkgA:index:t2-v%d$pkgA:index:t3::n1-sub-sub", i, j)))
		}
	}
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		name:   "childnew",
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:parent$pkgA:index:child::child",
		},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		name:   "childnew",
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:parent$pkgA:index:child::childnew",
		},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		name:   "sub-sub-new",
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1$pkgA:index:t1$pkgA:index:t1::sub-sub",
		},
	...
	snap = updateProgramWithResource(snap, []Resource{{
		...
		name:   "two",
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t2::two",
		},
	...
	_ = updateProgramWithResource(snap, []Resource{{
		...
		name: "two",
		aliasURNs: []resource.URN{
			"urn:codeinfra:test::test::pkgA:index:t1$pkgA:index:t2::two",
		},
...
func TestDuplicatesDueToAliases(t *testing.T) {
	...
	// This is a test for https://github.com/khulnasoft/codeinfra/issues/11173
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), snap.Resources[1].URN)
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resB"), snap.Resources[1].URN)
}

func TestCorrectResourceChosen(t *testing.T) {
	t.Parallel()

	// This is a test for https://github.com/khulnasoft/codeinfra/issues/13848
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), snap.Resources[1].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typA::resB"), snap.Resources[2].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resB"), snap.Resources[3].URN)
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), snap.Resources[1].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resB"), snap.Resources[2].URN)
	assert.Len(t, snap.Resources[2].Aliases, 0)
}

func TestComponentToCustomUpdate(t *testing.T) {
	// Test for https://github.com/khulnasoft/codeinfra/issues/12550, check that if we change a component resource
...
func TestParentAlias(t *testing.T) {
	// Test for https://github.com/khulnasoft/codeinfra/issues/13324, check that if we change a parent resource which
...
func TestSplitUpdateComponentAliases(t *testing.T) {
	...
	// This is a test for https://github.com/khulnasoft/codeinfra/issues/13903 to check that if a component is
	...
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		case 1:
			...
			_, err := monitor.RegisterResource(
				...
				deploytest.ResourceOptions{
					AliasURNs: []resource.URN{
						"urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typB::resB",
					},
				})
			assert.NoError(t, err)
	...
	assert.Len(t, snap.Resources, 4)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), snap.Resources[1].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typB::resB"), snap.Resources[2].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), snap.Resources[2].Parent)
	assert.Equal(t,
		resource.URN("urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typB$pkgA:m:typC::resC"),
		snap.Resources[3].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typB::resB"), snap.Resources[3].Parent)
	...
	assert.Nil(t, snap.VerifyIntegrity())
	assert.Len(t, snap.Resources, 4)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typB::resB"), snap.Resources[0].URN)
	assert.Equal(t, resource.URN(""), snap.Resources[0].Parent)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), snap.Resources[2].URN)
	// Even though we didn't register C its URN must update to take the re-parenting into account.
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typB$pkgA:m:typC::resC"), snap.Resources[3].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typB::resB"), snap.Resources[3].Parent)
	...
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typB::resB"), snap.Resources[0].URN)
	assert.Equal(t, resource.URN(""), snap.Resources[0].Parent)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typB$pkgA:m:typC::resC"), snap.Resources[2].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typB::resB"), snap.Resources[2].Parent)
}

func TestFailDeleteDuplicateAliases(t *testing.T) {
	t.Parallel()

	// This is a test for https://github.com/khulnasoft/codeinfra/issues/14041 to check that we don't courrupt state files when
	...
	assert.Nil(t, snap.VerifyIntegrity())
	assert.Len(t, snap.Resources, 2)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resAX"), snap.Resources[1].URN)
	...
	assert.Len(t, snap.Resources, 3)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resA"), snap.Resources[1].URN)
	assert.Equal(t, resource.URN("urn:codeinfra:test::test::pkgA:m:typA::resAX"), snap.Resources[2].URN)
}
...
func TestAliasesInProvidersAreNormalized(t *testing.T) {
	...
	setupProgramF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		prov, err := monitor.RegisterResource("codeinfra:providers:pkgA", "prov", true, deploytest.ResourceOptions{
			Parent: res1.URN,
		})
		require.NoError(t, err)
