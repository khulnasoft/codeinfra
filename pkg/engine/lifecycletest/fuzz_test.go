...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/engine/lifecycletest/fuzzing"
...
// Usage:
//
// This test will be skipped unless the `CODEINFRA_LIFECYCLE_TEST_FUZZ` environment variable is set. This is because
...
func TestFuzz(t *testing.T) {
	...
	shouldFuzz := os.Getenv("CODEINFRA_LIFECYCLE_TEST_FUZZ")
	if shouldFuzz == "" {
		t.Skip("CODEINFRA_LIFECYCLE_TEST_FUZZ not set")
	}
...
// `codeinfra stack export` command. It can be used to try and reproduce errors that have occurred in a user's stack
// without having access to the actual program that ran, or logs that might help infer the program that ran.
//
// Usage:
//
// This test will be skipped unless the `CODEINFRA_LIFECYCLE_TEST_FUZZ_FROM_STATE_FILE` environment variable is set. The
...
func TestFuzzFromStateFile(t *testing.T) {
	...
	stateFile := os.Getenv("CODEINFRA_LIFECYCLE_TEST_FUZZ_FROM_STATE_FILE")
	if stateFile == "" {
		t.Skip("CODEINFRA_LIFECYCLE_TEST_FUZZ_FROM_STATE_FILE not set")
	}
