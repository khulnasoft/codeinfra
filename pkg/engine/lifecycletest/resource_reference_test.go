...
// TestResourceReferences_GetResource tests that invoking the built-in 'codeinfra:codeinfra:getResource' function
...
func TestResourceReferences_GetResource(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		// Expect the `child` property from `resContainer`'s state to come back from 'codeinfra:codeinfra:getResource'
		// as a resource reference.
		result, failures, err := monitor.Invoke("codeinfra:codeinfra:getResource", resource.PropertyMap{
