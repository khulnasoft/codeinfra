...
func TestDefaultProviderDiff(t *testing.T) {
	...
	// engine gracefully react to changes in the default provider in this manner. See codeinfra/codeinfra#2753 for what
...
func TestPluginDownloadURLPassthrough(t *testing.T) {
	...
	pkgAPluginDownloadURL := "get.codeinfra.com/${VERSION}"
...
func TestMultipleResourceDenyDefaultProviderLifecycle(t *testing.T) {
	...
	for _, tt := range cases {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			c := config.Map{}
			k := config.MustMakeKey("codeinfra", "disable-default-providers")
...
func TestProviderVersionAssignment(t *testing.T) {
	...
	prog := func(opts ...deploytest.ResourceOptions) deploytest.ProgramFunc {
		return func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
			...
			_, err = monitor.RegisterResource("codeinfra:providers:pkgA", "provA", true, opts...)
			if err != nil {
				return err
			}
			return nil
...
func TestComponentProvidersInheritance(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		resp, err := monitor.RegisterResource("codeinfra:providers:pkg", "provA", true)
		assert.NoError(t, err)
		...
		deploytest.NewProviderLoader("pkg", semver.MustParse("1.0.0"), func() (plugin.Provider, error) {
			return &deploytest.Provider{
				...
				ConstructF: func(
					...
					if req.Name == "resB" {
						assert.Contains(t, req.Options.Providers["pkgA"], "urn:codeinfra:test::test::codeinfra:providers:pkg::provA::")
					} else {
...
// https://github.com/khulnasoft/codeinfra/issues/16757.
func TestRefreshLegacyState(t *testing.T) {
	...
	snapshot := &deploy.Snapshot{
		Resources: []*resource.State{
			{
				Type: "providers:codeinfra:pkgA",
				URN:  p.NewURN("providers:codeinfra:pkgA", "prov", ""),
				Inputs: map[resource.PropertyKey]resource.PropertyValue{
					"version":           resource.NewPropertyValue("1.3.0"),
