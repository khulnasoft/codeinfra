...
func TestConstructCallReturnDependencies(t *testing.T) {
	...
	test := func(t *testing.T, opt deploytest.PluginOption) {
			...
			deploytest.NewProviderLoader("pkgA", semver.MustParse("1.0.0"), func() (plugin.Provider, error) {
				return &deploytest.Provider{
					...
					CallF: func(
						...
						assert.Equal(t,
							[]resource.URN{"urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typA::resA-a"},
		...
		programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
			...
			urn := resource.URN("urn:codeinfra:test::test::pkgA:m:typA$pkgA:m:typA::resA-a")
			...
			deploytest.NewProviderLoader("pkgA", semver.MustParse("1.0.0"), func() (plugin.Provider, error) {
				return &deploytest.Provider{
					...
					ConstructF: func(
						...
						// Arg was sent as an output but the dependency map should still be filled in for providers to look at
						assert.Equal(t,
							[]resource.URN{"urn:codeinfra:test::test::pkgA:m:typC::resC"},
	...
	loaders := []*deploytest.ProviderLoader{
		deploytest.NewProviderLoader("pkgA", semver.MustParse("1.0.0"), func() (plugin.Provider, error) {
			...
			call := func(
				...
				result, _, err := monitor.Invoke("codeinfra:codeinfra:getResource", resource.PropertyMap{
...
// returned registered resource reference correctly rehydrated by the program using the codeinfra:codeinfra:getResource
// invoke.
func TestComponentRegisteredResourceOutputCanBeHydratedByProgram(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		state, _, err := monitor.Invoke(
			"codeinfra:codeinfra:getResource",
...
// Tests that a remote component (that is, one implemented by a provider's Construct method) can be created and a
// returned registered resource reference correctly rehydrated by another component using the codeinfra:codeinfra:getResource
// invoke.
...
// Tests that a remote component (that is, one implemented by a provider's Construct method) can be created and a
// returned read resource reference correctly rehydrated by the program using the codeinfra:codeinfra:getResource invoke.
...
// Tests that a remote component (that is, one implemented by a provider's Construct method) can be created and a
// returned read resource reference correctly rehydrated by another component using the codeinfra:codeinfra:getResource
// invoke.
