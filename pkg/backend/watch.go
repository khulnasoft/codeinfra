...
func watchPaths(root string, paths []string) (chan string, func(), error) {
	...
	err = cmd.Start()
	if err != nil {
		return nil, nil, fmt.Errorf("error starting codeinfra-watch: %w", err)
	}

	stop := func() {
		err := cmd.Process.Kill()
		contract.AssertNoErrorf(err, "Unexpected error stopping codeinfra-watch process: %v", err)
	}
...
func getWatchUtil() (string, error) {
	program := "codeinfra-watch"
	if runtime.GOOS == windowsGOOS {
		program = "codeinfra-watch.exe"
	}

	watchCmd, err := exec.LookPath("codeinfra-watch")
	if err == nil {
		return watchCmd, nil
	}
	...
	if exeErr == nil {
		...
		if fullErr == nil {
			...
			if stat, err := os.Stat(candidate); err == nil {
				...
				return "", fmt.Errorf("Could not locate an executable codeinfra-watch, found %v without execute bit", fullPath)
			}
		}
	}

	return "", errors.New("Could not locate codeinfra-watch binary")
}
