...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/authhelpers"
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/resource/edit"
...
func newDIYBackend(
	...
	//
	//  - The state has a single .codeinfra/meta.yaml file
	...
	default:
		return nil, fmt.Errorf(
			"state store unsupported: 'meta.yaml' version (%d) is not supported "+
				"by this version of the Codeinfra CLI", meta.Version)
	}
	...
	// or migrates it to project mode with `codeinfra state upgrade`,
	...
	msg.WriteString("Please run 'codeinfra state upgrade' to migrate them to the new format.\n")
	msg.WriteString("Set CODEINFRA_DIY_BACKEND_NO_LEGACY_WARNING=1 to disable this warning.")
...
func (b *diyBackend) Upgrade(ctx context.Context, opts *UpgradeOptions) error {
	...
	// (e.g., we can write to .codeinfra/*/*" but not ".codeinfra/*.")
...
func massageBlobPath(path string) (string, error) {
	...
	// See also https://github.com/khulnasoft/codeinfra/issues/15352
...
// If the CWD is not in a Codeinfra project, does not contradict.
// If the project name in Codeinfra.yaml is "foo", a stack with a name of bar/foo should not work.
...
func (b *diyBackend) CreateStack(
	...
	if currentProjectContradictsWorkspace(diyStackRef) {
		return nil, fmt.Errorf("provided project name %q doesn't match Codeinfra.yaml", diyStackRef.project)
	}
...
func (b *diyBackend) apply(
	...
	if currentProjectContradictsWorkspace(diyStackRef) {
		return nil, nil, fmt.Errorf("provided project name %q doesn't match Codeinfra.yaml", diyStackRef.project)
	}
	...
	if !op.Opts.Display.SuppressPermalink && opts.ShowLink && !op.Opts.Display.JSONDisplay {
		...
		if strings.HasPrefix(b.url, FilePathPrefix) {
			...
			if err != nil {
				...
				// codeinfra with an error code.
				// printing a statefile perma link happens after all the providers have finished
				// deploying the infrastructure, failing the codeinfra update because there was a
				...
				cmdutil.Diag().Warningf(diag.Message("", "Unable to create signed url for current backend to "+
					"create a Permalink. Please visit https://www.codeinfra.com/docs/troubleshooting/ "+
