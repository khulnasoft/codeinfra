...
func TestDisabledFullyQualifiedStackNames(t *testing.T) {
	...
	pyaml := filepath.Join(projectDir, "Codeinfra.yaml")
...
func TestListStacksWithMultiplePassphrases(t *testing.T) {
	...
	defer func() {
		t.Setenv("CODEINFRA_CONFIG_PASSPHRASE", "abc123")
	...
	defer func() {
		t.Setenv("CODEINFRA_CONFIG_PASSPHRASE", "123abc")
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/10439
...
func TestDIYBackendRejectsStackInitOptions(t *testing.T) {
	...
	// • Simulate `codeinfra stack init`, passing non-nil init options
...
func TestLegacyFolderStructure(t *testing.T) {
	...
	tmpDir := t.TempDir()
	err := os.MkdirAll(path.Join(tmpDir, ".codeinfra", "stacks"), os.ModePerm)
	require.NoError(t, err)
	err = os.WriteFile(path.Join(tmpDir, ".codeinfra", "stacks", "a.json"), []byte("{}"), 0o600)
	require.NoError(t, err)
	...
	assert.FileExists(t, path.Join(tmpDir, ".codeinfra", "stacks", "b.json"))
}
...
func TestOptIntoLegacyFolderStructure(t *testing.T) {
	...
	require.NoError(t, err)
	assert.FileExists(t, filepath.Join(tmpDir, ".codeinfra", "stacks", "foo.json"))
}
...
func TestProjectFolderStructure(t *testing.T) {
	...
	tmpDir := t.TempDir()
	err := os.MkdirAll(path.Join(tmpDir, ".codeinfra", "plugins"), os.ModePerm)
	require.NoError(t, err)
	err = os.MkdirAll(path.Join(tmpDir, ".codeinfra", "stacks"), os.ModePerm)
	require.NoError(t, err)
	err = os.WriteFile(path.Join(tmpDir, ".codeinfra", "stacks", "a.txt"), []byte("{}"), 0o600)
	require.NoError(t, err)
	...
	err = os.MkdirAll(path.Join(tmpDir, ".codeinfra", "stacks", "testproj"), os.ModePerm)
	assert.NoError(t, err)
	err = os.WriteFile(path.Join(tmpDir, ".codeinfra", "stacks", "testproj", "a.json"), []byte("{}"), 0o600)
	assert.NoError(t, err)
	...
	assert.FileExists(t, path.Join(tmpDir, ".codeinfra", "stacks", "testproj", "b.json"))
}
...
func TestProjectNameMustMatch(t *testing.T) {
	...
	assert.FileExists(t, path.Join(tmpDir, ".codeinfra", "stacks", "my-project", "a.json"))
	...
	assert.FileExists(t, path.Join(tmpDir, ".codeinfra", "stacks", "my-project", "c.json"))
}
...
func TestNew_legacyFileWarning(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			files: map[string]string{
				...
				".codeinfra/foo/extraneous_file": "",
			},
		},
		{
			desc: "legacy stacks",
			files: map[string]string{
				".codeinfra/stacks/a.json":     "{}",
				".codeinfra/stacks/b.json":     "{}",
				".codeinfra/stacks/c.json.bak": "{}", // should ignore backup files
			},
			wantOut: "warning: Found legacy stack files in state store:\n" +
				"  - a\n" +
				"  - b\n" +
				"Please run 'codeinfra state upgrade' to migrate them to the new format.\n" +
				"Set CODEINFRA_DIY_BACKEND_NO_LEGACY_WARNING=1 to disable this warning.\n",
		},
		{
			...
			env: map[string]string{
				"CODEINFRA_DIY_BACKEND_NO_LEGACY_WARNING": "true",
			},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			ctx := context.Background()
			require.NoError(t,
				bucket.WriteAll(ctx, ".codeinfra/meta.yaml", []byte("version: 1"), nil),
				"write meta.yaml")
...
func TestLegacyUpgrade(t *testing.T) {
	...
	err := os.MkdirAll(path.Join(tmpDir, ".codeinfra", "stacks"), os.ModePerm)
	require.NoError(t, err)
	err = os.WriteFile(path.Join(tmpDir, ".codeinfra", "stacks", "a.json"), []byte(`{
		"latest": {
			"resources": [
				{
	...
	err = os.WriteFile(path.Join(tmpDir, ".codeinfra", "stacks", "b.json"), []byte(`{
		"latest": {
			"resources": [
				{
					"type": "package:module:resource",
					"urn": "urn:codeinfra:stack::other-project::package:module:resource::name"
				}
			]
		}
	}`), 0o600)
...
func TestLegacyUpgrade_partial(t *testing.T) {
	...
	ctx := context.Background()
	require.NoError(t,
		bucket.WriteAll(ctx, ".codeinfra/stacks/foo.json", []byte(`{
		"latest": {
			"resources": [
				{
					"type": "package:module:resource",
	...
	require.NoError(t,
		...
		bucket.WriteAll(ctx, ".codeinfra/stacks/bar.json",
	...
	exists, err := bucket.Exists(ctx, ".codeinfra/stacks/project/foo.json")
	require.NoError(t, err)
...
func TestLegacyUpgrade_ProjectsForDetachedStacks(t *testing.T) {
	...
	for _, stack := range []string{"foo", "bar", "baz"} {
		statePath := path.Join(".codeinfra", "stacks", stack+".json")
		require.NoError(t,
	...
	wantFiles := []string{
		".codeinfra/stacks/proj1/foo.json",
		".codeinfra/stacks/proj2/bar.json",
		".codeinfra/stacks/baz.json",
	}
...
func TestLegacyUpgrade_ProjectsForDetachedStacks_error(t *testing.T) {
	...
	wantFiles := []string{
		".codeinfra/stacks/foo.json",
		".codeinfra/stacks/bar.json",
	}
	for _, file := range wantFiles {
		exists, err := bucket.Exists(ctx, file)
		require.NoError(t, err, "exists(%q)", file)
...
func TestLegacyUpgrade_writeMetaError(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(filepath.Join(stateDir, ".codeinfra", "meta.yaml"), 0o755))
	...
	assert.FileExists(t, filepath.Join(stateDir, ".codeinfra", "stacks", "foo.json"),
...
func TestNew_unsupportedStoreVersion(t *testing.T) {
	...
	ctx := context.Background()
	require.NoError(t,
		bucket.WriteAll(ctx, ".codeinfra/meta.yaml", []byte("version: 999999999"), nil))
...
func TestUpgrade_manyFailures(t *testing.T) {
	...
	for i := 0; i < numStacks; i++ {
		stackPath := path.Join(".codeinfra", "stacks", fmt.Sprintf("stack-%d.json", i))
...
func TestCreateStack_gzip(t *testing.T) {
	...
	// With CODEINFRA_DIY_BACKEND_GZIP enabled,
	// we'll store state into gzipped files.
	assert.FileExists(t, filepath.Join(stateDir, ".codeinfra", "stacks", "testproj", "foo.json.gz"))
}
...
func TestCreateStack_retainCheckpoints(t *testing.T) {
	...
	// With CODEINFRA_RETAIN_CHECKPOINTS enabled,
	...
	got, err := filepath.Glob(
		filepath.Join(stateDir, ".codeinfra", "stacks", "testproj", "foo.json.*"))
	require.NoError(t, err)
...
func TestCreateStack_WritesInitialState(t *testing.T) {
		...
		},
		{
			...
			state: &apitype.UntypedDeployment{
				...
				Deployment: []byte(`{
					"resources": [
						{
							"urn": "urn:codeinfra:stack::proj::type::name1",
							"type": "type",
							"parent": "urn:codeinfra:stack::proj::type::name2"
						},
						{
							"urn": "urn:codeinfra:stack::proj::type::name2",
							"type": "type"
						}
					]
				}`),
			},
			contains: "urn:codeinfra:stack::proj::type::name2",
		},
		{
			name: "valid",
			state: &apitype.UntypedDeployment{
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			stateDir := t.TempDir()
			stateFile := path.Join(stateDir, ".codeinfra", "stacks", project, stack+".json")
...
func TestDisableIntegrityChecking(t *testing.T) {
	...
	require.ErrorContains(t, err,
		"child resource urn:codeinfra:stack::proj::type::name1's parent urn:codeinfra:stack::proj::type::name2 comes after it")
	assert.Nil(t, snap)
