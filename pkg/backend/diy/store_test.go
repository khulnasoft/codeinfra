...
func TestLegacyReferenceStore_referencePaths(t *testing.T) {
	...
	assert.Equal(t, ".codeinfra/stacks/foo", ref.StackBasePath())
	assert.Equal(t, ".codeinfra/history/foo", ref.HistoryDir())
	assert.Equal(t, ".codeinfra/backups/foo", ref.BackupDir())
}
...
func TestProjectReferenceStore_referencePaths(t *testing.T) {
	...
	assert.Equal(t, ".codeinfra/stacks/myproject/mystack", ref.StackBasePath())
	assert.Equal(t, ".codeinfra/history/myproject/mystack", ref.HistoryDir())
	assert.Equal(t, ".codeinfra/backups/myproject/mystack", ref.BackupDir())
}
...
func TestLegacyReferenceStore_ListReferences(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			files: []string{
				".codeinfra/stacks/foo.json",
				".codeinfra/stacks/bar.json/baz.json", // not a file
			},
			want: []tokens.QName{"foo"},
		},
	}
...
func TestProjectReferenceStore_List(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			desc: "json",
			files: []string{
				".codeinfra/stacks/proj/foo.json",
			},
		...
		},
		{
			desc: "gzipped",
			files: []string{
				".codeinfra/stacks/foo/bar.json.gz",
			},
		...
		},
		{
			desc: "multiple",
			files: []string{
				".codeinfra/stacks/a/foo.json",
				".codeinfra/stacks/b/bar.json.gz",
				".codeinfra/stacks/c/baz.json",
			},
			stacks: []tokens.QName{
				"organization/a/foo",
				"organization/b/bar",
		...
		},
		{
			...
			files: []string{
				".codeinfra/stacks/a/foo.json",
				".codeinfra/stacks/foo.json",
				".codeinfra/stacks/bar/baz/qux.json", // nested too deep
				".codeinfra/stacks/a b/c.json",       // bad project name
			},
...
func TestProjectReferenceStore_ProjectExists(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			files: []string{
				".codeinfra/stacks/b/a", // Project name exist, but as a subproject
			},
			projectName: "a",
			exist:       false,
		},
