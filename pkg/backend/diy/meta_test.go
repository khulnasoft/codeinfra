...
func TestEnsureCodeinfraMeta(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			desc: "future version",
			give: map[string]string{
				".codeinfra/meta.yaml": `version: 42`,
			},
			want: codeinfraMeta{Version: 42},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			state, err := ensureCodeinfraMeta(ctx, b, env.NewEnv(tt.env))
...
func TestEnsureCodeinfraMeta_corruption(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			desc:    "empty",
			give:    ``,
			wantErr: `corrupt store: missing version in ".codeinfra/meta.yaml"`,
		},
		{
			desc:    "other fields",
			give:    `foo: bar`,
		...
		},
		{
			...
			give:    `version: foo`,
			wantErr: `corrupt store: unmarshal ".codeinfra/meta.yaml"`,
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			b := memblob.OpenBucket(nil)
			ctx := context.Background()
			require.NoError(t, b.WriteAll(ctx, ".codeinfra/meta.yaml", []byte(tt.give), nil))

			_, err := ensureCodeinfraMeta(context.Background(), b, env.NewEnv(nil))
...
func TestMeta_roundTrip(t *testing.T) {
	...
	tests := []struct {
		...
		{desc: "zero", give: codeinfraMeta{Version: 0}},
		{desc: "one", give: codeinfraMeta{Version: 1}},
		{desc: "future", give: codeinfraMeta{Version: 42}},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			require.NoError(t,
				b.WriteAll(context.Background(), ".codeinfra/stacks/dev.json", []byte("bar"), nil))

			ctx := context.Background()
			require.NoError(t, tt.give.WriteTo(ctx, b))

			got, err := ensureCodeinfraMeta(ctx, b, env.NewEnv(nil))
			require.NoError(t, err)
...
func TestMeta_WriteTo_zero(t *testing.T) {
	...
	ctx := context.Background()
	require.NoError(t, (&codeinfraMeta{
		Version: 0,
	}).WriteTo(ctx, bucket))

	assert.NoFileExists(t, filepath.Join(tmpDir, ".codeinfra", "meta.yaml"))
}
...
func TestNew_noMetaOnInit(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t,
		bucket.WriteAll(context.Background(), ".codeinfra/stacks/dev.json", []byte("bar"), nil))
