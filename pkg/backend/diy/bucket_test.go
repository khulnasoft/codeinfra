...
func TestWrappedBucket(t *testing.T) {
	...
	// Initialize a diy backend, using the default Codeinfra directory.
	...
	t.Run("SanityCheck", func(t *testing.T) {
		...
		err := wrappedBucket.WriteAll(ctx, ".codeinfra/bucket-test/foo", randomData, &blob.WriterOptions{})
		mustNotHaveError(t, "WriteAll", err)

		readData, err := wrappedBucket.ReadAll(ctx, `.codeinfra\bucket-test\foo`)
		...
		err = wrappedBucket.Delete(ctx, ".codeinfra/bucket-test/foo")
		mustNotHaveError(t, "Delete", err)

		exists, err := wrappedBucket.Exists(ctx, ".codeinfra/bucket-test/foo")
	...
	t.Run("ListObjects", func(t *testing.T) {
		...
		for _, filename := range filenames {
			key := ".codeinfra\\bucket-test\\" + filename
		...
		objects, err := listBucket(ctx, wrappedBucket, `.codeinfra\bucket-test`)
