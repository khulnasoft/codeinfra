...
func GetEnvironmentTagsForCurrentStack(root string,
	...
	// Grab any `codeinfra:tag` config values and use those to update the stack's tags.
	configTags, has, err := cfg.Get(config.MustParseKey(apitype.CodeinfraTagsConfigKey), false)
	contract.AssertNoErrorf(err, "Config.Get(\"%s\") failed unexpectedly", apitype.CodeinfraTagsConfigKey)
	if has {
		configTagInterface, err := configTags.ToObject()
		if err != nil {
			return nil, fmt.Errorf("%s must be an object of strings", apitype.CodeinfraTagsConfigKey)
		}
		...
		for name, value := range configTagObject {
			...
			if !ok {
				return nil, fmt.Errorf("%s[%s] must be a string", apitype.CodeinfraTagsConfigKey, name)
			}
