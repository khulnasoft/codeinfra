...
const (
	...
	// defaultAPIDomainPrefix is the assumed Cloud URL prefix for typical Codeinfra Cloud API endpoints.
	defaultAPIDomainPrefix = "api."
	// defaultConsoleDomainPrefix is the assumed Cloud URL prefix typically used for the Codeinfra Console.
	defaultConsoleDomainPrefix = "app."
)

// cloudConsoleURL returns a URL to the Codeinfra Cloud Console, rooted at cloudURL. If there is
// an error, returns "".
func cloudConsoleURL(cloudURL string, paths ...string) string {
	...
	case os.Getenv(ConsoleDomainEnvVar) != "":
		// Honor a CODEINFRA_CONSOLE_DOMAIN environment variable to override the
		// default behavior. Since we identify a backend by a single URI, we
		// cannot know what the Codeinfra Console is hosted at...
