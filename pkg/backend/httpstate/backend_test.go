...
func TestEnabledFullyQualifiedStackNames(t *testing.T) {
	...
	_, err := NewLoginManager().Login(ctx, CodeinfraCloudURL, false, "", "", nil, true, display.Options{})
	require.NoError(t, err)

	b, err := New(diagtest.LogSink(t), CodeinfraCloudURL, &workspace.Project{Name: "testproj"}, false)
	require.NoError(t, err)
...
func TestValueOrDefaultURL(t *testing.T) {
	t.Run("TestValueOrDefault", func(t *testing.T) {
		...
		assert.Equal(t, "https://api-test1.codeinfra.com", ValueOrDefaultURL(mock, "https://api-test1.codeinfra.com/"))

		// Validate no-op case
		assert.Equal(t, "https://api-test2.codeinfra.com", ValueOrDefaultURL(mock, "https://api-test2.codeinfra.com"))

		// Validate trailing slash in pre-set env var is unchanged
		t.Setenv("CODEINFRA_API", "https://api-test3.codeinfra.com/")
		assert.Equal(t, "https://api-test3.codeinfra.com/", ValueOrDefaultURL(mock, ""))
		t.Setenv("CODEINFRA_API", "")

		// Validate current credentials URL is used
		current = "https://api-test4.codeinfra.com"
		assert.Equal(t, "https://api-test4.codeinfra.com", ValueOrDefaultURL(mock, ""))

		// Unless the current credentials URL is a filestate url
		current = "s3://test"
		assert.Equal(t, "https://api.codeinfra.com", ValueOrDefaultURL(mock, ""))
	})
}
