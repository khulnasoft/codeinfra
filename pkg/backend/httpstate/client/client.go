...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/util/validation"
...
// newClient creates a new Codeinfra API client with the given URL and API token. It is a variable instead of a regular
...
// NewClient creates a new Codeinfra API client with the given URL and API token.
...
// restCall makes a REST-style request to the Codeinfra API using the given method, path, query object, and request
...
// updateRESTCall makes a REST-style request to the Codeinfra API using the given method, path, query object, and request
...
// listPolicyGroupsPath returns the path for an API call to the Codeinfra service to list the Policy Groups
// in a Codeinfra organization.
func listPolicyGroupsPath(orgName string) string {
	return fmt.Sprintf("/api/orgs/%s/policygroups", orgName)
}

// listPolicyPacksPath returns the path for an API call to the Codeinfra service to list the Policy Packs
// in a Codeinfra organization.
func listPolicyPacksPath(orgName string) string {
	return fmt.Sprintf("/api/orgs/%s/policypacks", orgName)
}

// publishPolicyPackPath returns the path for an API call to the Codeinfra service to publish a new Policy Pack
// in a Codeinfra organization.
func publishPolicyPackPath(orgName string) string {
	return fmt.Sprintf("/api/orgs/%s/policypacks", orgName)
}

// updatePolicyGroupPath returns the path for an API call to the Codeinfra service to update a PolicyGroup
// for a Codeinfra organization.
func updatePolicyGroupPath(orgName, policyGroup string) string {
	return fmt.Sprintf(
		"/api/orgs/%s/policygroups/%s", orgName, policyGroup)
}

// deletePolicyPackPath returns the path for an API call to the Codeinfra service to delete
// all versions of a Policy Pack from a Codeinfra organization.
...
// deletePolicyPackVersionPath returns the path for an API call to the Codeinfra service to delete
// a version of a Policy Pack from a Codeinfra organization.
...
// publishPolicyPackPublishComplete returns the path for an API call to signal to the Codeinfra service
// that a PolicyPack to a Codeinfra organization.
...
// getAIPromptPath returns the API path to create a Codeinfra AI prompt.
...
// Copied from https://github.com/khulnasoft/codeinfra-service/blob/master/pkg/apitype/users.go#L7-L16
...
// Copied from https://github.com/khulnasoft/codeinfra-service/blob/master/pkg/apitype/users.go#L20-L37
...
// Copied from https://github.com/khulnasoft/codeinfra-service/blob/master/pkg/apitype/users.go#L39-L43
...
func (pc *Client) GetCLIVersionInfo(ctx context.Context) (semver.Version, semver.Version, semver.Version, error) {
	...
	// https://github.com/khulnasoft/codeinfra-service/pull/17429 yet
...
// requires that the Codeinfra program is uploaded, the provided getContents callback will be invoked to fetch the
// contents of the Codeinfra program.
...
func (pc *Client) ListPolicyGroups(ctx context.Context, orgName string, inContToken *string) (
	...
	// NOTE: The ListPolicyGroups API on the Codeinfra Service is not currently paginated.
...
func (pc *Client) ListPolicyPacks(ctx context.Context, orgName string, inContToken *string) (
	...
	// NOTE: The ListPolicyPacks API on the Codeinfra Service is not currently paginated.
...
// PublishPolicyPack publishes a `PolicyPack` to the Codeinfra service. If it successfully publishes
...
func (pc *Client) PublishPolicyPack(ctx context.Context, orgName string,
	...
	// Print a publishing message. We have to handle the case where an older version of codeinfra/policy
	...
	// If the version tag is empty, an older version of codeinfra/policy is being used and
	// we therefore need to use the version provided by the codeinfra service.
...
// as it is likely an older version of codeinfra/policy that does not gather the version.
...
// ApplyPolicyPack enables a `PolicyPack` to the Codeinfra organization. If policyGroup is not empty,
...
// DisablePolicyPack disables a `PolicyPack` to the Codeinfra organization. If policyGroup is not empty,
...
func (pc *Client) CreateDeployment(ctx context.Context, stack StackIdentifier,
	...
	err := pc.restCallWithOptions(ctx, http.MethodPost, getDeploymentPath(stack), nil, req, &resp, httpCallOptions{
		Header: map[string][]string{
			"X-Codeinfra-Deployment-Initiator": {deploymentInitiator},
		},
	})
	if err != nil {
...
func getCodeinfraOrgSearchPath(baseURL string, orgName string) string {
...
func (pc *Client) GetSearchQueryResults(
	ctx context.Context, orgName string, queryParams *apitype.CodeinfraQueryRequest, baseURL string,
	...
	resp.URL = fmt.Sprintf("%s?query=%s", getCodeinfraOrgSearchPath(baseURL, orgName), url.QueryEscape(queryParams.Query))
	return &resp, nil
}

func (pc *Client) GetNaturalLanguageQueryResults(
	ctx context.Context, orgName string, queryString string,
) (*apitype.CodeinfraQueryResponse, error) {
	var resp apitype.CodeinfraQueryResponse
	queryParamObject := apitype.CodeinfraQueryRequest{
		Query: queryString,
	}
