...
func UserAgent() string {
	return fmt.Sprintf("codeinfra-cli/1 (%s; %s)", version.Version, runtime.GOOS)
}
...
// accessTokenKind is enumerates the various types of access token used with the Codeinfra API. These kinds correspond
// directly to the "method" piece of an HTTP `Authorization` header.
type accessTokenKind string

const (
	// accessTokenKindAPIToken denotes a standard Codeinfra API token.
...
// accessToken is an abstraction over the two different kinds of access tokens used by the Codeinfra API.
...
// apiAccessToken is an implementation of accessToken for Codeinfra API tokens (i.e. tokens of kind
...
// codeinfraAPICall makes an HTTP request to the Codeinfra API.
func codeinfraAPICall(ctx context.Context,
	...
	req.Header.Set("Accept", "application/vnd.codeinfra+8")
	...
	if tracingOptions.TracingHeader != "" {
		req.Header.Set("X-Codeinfra-Tracing", tracingOptions.TracingHeader)
	}
	...
	logging.V(apiRequestLogLevel).Infof("Making Codeinfra API call: %s", url)
	if logging.V(apiRequestDetailLogLevel) {
		logging.V(apiRequestDetailLogLevel).Infof(
			"Codeinfra API call details (%s): headers=%v; body=%v", url, req.Header, string(body))
	}
	...
	logging.V(apiRequestLogLevel).Infof("Codeinfra API call response code (%s): %v", url, resp.Status)

	requestSpan.SetTag("responseCode", resp.Status)

	if warningHeader, ok := resp.Header["X-Codeinfra-Warning"]; ok {
	...
	if resp.StatusCode == 401 && tok.Kind() == accessTokenKindAPIToken && creds == "" {
		return "", nil, errors.New("this command requires logging in; try running `codeinfra login` first")
	}

	// Provide a better error if rate-limit is exceeded(429: Too Many Requests)
	if resp.StatusCode == 429 {
		return "", nil, errors.New("codeinfra service: request rate-limit exceeded")
	}
...
// restClient is an abstraction for calling the Codeinfra REST API.
...
// defaultRESTClient is the default implementation for calling the Codeinfra REST API.
type defaultRESTClient struct {
	client httpClient
}

// Call calls the Codeinfra REST API marshalling reqObj to JSON and using that as
...
func (c *defaultRESTClient) Call(ctx context.Context, diag diag.Sink, cloudAPI, method, path string, queryObj, reqObj,
	...
	if logging.V(apiRequestDetailLogLevel) {
		logging.V(apiRequestDetailLogLevel).Infof("Codeinfra API call response body (%s): %v", url, string(respBody))
	}
