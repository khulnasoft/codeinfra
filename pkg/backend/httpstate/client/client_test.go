...
func TestAPIErrorResponses(t *testing.T) {
	...
	t.Run("TestAuthError", func(t *testing.T) {
		...
		assert.EqualError(t, unauthorizedErr, "this command requires logging in; try running `codeinfra login` first")
	})
	t.Run("TestRateLimitError", func(t *testing.T) {
		...
		assert.EqualError(t, rateLimitErr, "codeinfra service: request rate-limit exceeded")
	})
...
func TestDeploymentSettingsApi(t *testing.T) {
    ...
    "sourceContext": {
        "git": {
            "repoUrl": "git@github.com:codeinfra/test-repo.git",
            "branch": "main",
            "repoDir": ".",
            "gitAuth": {
                "basicAuth": {
    ...
    "agentPoolID": "51035bee-a4d6-4b63-9ff6-418775c5da8d"
		...
		assert.Equal(t, "git@github.com:codeinfra/test-repo.git", resp.SourceContext.Git.RepoURL)
