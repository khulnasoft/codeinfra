...
import (
	...
	resourceanalyzer "github.com/khulnasoft/codeinfra/pkg/v3/resource/analyzer"
...
func (rp *cloudRequiredPolicy) Install(ctx context.Context) (string, error) {
	...
	// codeinfra/policy that do not have a version tag.
...
// cloudBackendPolicyPackReference is a reference to a PolicyPack implemented by the Codeinfra service.
...
// cloudPolicyPack is a the Codeinfra service implementation of the PolicyPack interface.
type cloudPolicyPack struct {
	// ref uniquely identifies the PolicyPack in the Codeinfra service.
...
func (pack *cloudPolicyPack) Publish(
	...
	if strings.EqualFold(runtime, "nodejs") {
		...
		// the same for other runtimes. That way, after unpacking, we can look for the CodeinfraPolicy.yaml inside the
	...
	fmt.Println("Uploading policy pack to Codeinfra service")
...
func installRequiredPolicy(ctx context.Context, finalDir string, tgz io.ReadCloser) error {
	...
	projPath := filepath.Join(finalDir, "CodeinfraPolicy.yaml")
...
func completePythonInstall(ctx context.Context, finalDir, projPath string, proj *workspace.PolicyPackProject) error {
	const venvDir = "venv"
	// TODO[codeinfra/codeinfra/issues/16286]: Allow using different toolchains for policy packs.
