...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/channel"
...
// recordEngineEvents will record the events with the Codeinfra Service, enabling things like viewing
...
// posting them to the Codeinfra service.
func (u *cloudUpdate) RecordAndDisplayEvents(
	...
	// them to the console or persist them on the Codeinfra Service. Both should terminate as soon as
...
func (b *cloudBackend) newUpdate(ctx context.Context, stackRef backend.StackReference, op backend.UpdateOperation,
	...
	if token != "" {
		// TODO[codeinfra/codeinfra#10482] instead of assuming
	...
	if err != nil {
		...
		case stack.ErrDeploymentSchemaVersionTooNew:
			return nil, fmt.Errorf("the stack '%s' is newer than what this version of the Codeinfra CLI understands. "+
				"Please update your version of the Codeinfra CLI", stackRef.Name())
		default:
...
// Codeinfra Service. This is the data that powers the logs display.
func persistEngineEvents(
	...
	// Maximum number of concurrent requests to the Codeinfra Service to persist
	// engine events.
	...
	// We maintain a sequence counter for each event to ensure that the Codeinfra Service can
	...
	transmitBatch := func() {
		...
		// Codeinfra Service, if an update is particularly chatty.
