...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/backend"
	"github.com/khulnasoft/codeinfra/pkg/v3/backend/display"
	"github.com/khulnasoft/codeinfra/pkg/v3/backend/diy"
	"github.com/khulnasoft/codeinfra/pkg/v3/backend/httpstate/client"
	sdkDisplay "github.com/khulnasoft/codeinfra/pkg/v3/display"
	"github.com/khulnasoft/codeinfra/pkg/v3/engine"
	"github.com/khulnasoft/codeinfra/pkg/v3/operations"
	"github.com/khulnasoft/codeinfra/pkg/v3/resource/deploy"
	"github.com/khulnasoft/codeinfra/pkg/v3/secrets"
	"github.com/khulnasoft/codeinfra/pkg/v3/util/nosleep"
	pkgWorkspace "github.com/khulnasoft/codeinfra/pkg/v3/workspace"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/apitype"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/diag"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/diag/colors"
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/result"
...
const (
	...
	defaultURLEnvVar = "CODEINFRA_API"
...
var codeinfraAILanguageMap = map[string]CodeinfraAILanguage{
	"typescript": CodeinfraAILanguageTypeScript,
	"javascript": CodeinfraAILanguageJavaScript,
	"python":     CodeinfraAILanguagePython,
	"go":         CodeinfraAILanguageGo,
	"c#":         CodeinfraAILanguageCSharp,
	"java":       CodeinfraAILanguageJava,
	"yaml":       CodeinfraAILanguageYAML,
}

// All of the languages supported by Codeinfra AI.
var CodeinfraAILanguageOptions = []CodeinfraAILanguage{
	CodeinfraAILanguageTypeScript,
	CodeinfraAILanguageJavaScript,
	CodeinfraAILanguagePython,
	CodeinfraAILanguageGo,
	CodeinfraAILanguageCSharp,
	CodeinfraAILanguageJava,
	CodeinfraAILanguageYAML,
}
...
func (e *CodeinfraAILanguage) Set(v string) error {
	value, ok := codeinfraAILanguageMap[strings.ToLower(v)]
	if !ok {
		return fmt.Errorf("must be one of %s", CodeinfraAILanguagesClause)
	}
	*e = value
	return nil
}
...
// Name validation rules enforced by the Codeinfra Service.
var stackOwnerRegexp = regexp.MustCompile("^[a-zA-Z0-9][a-zA-Z0-9-_]{1,38}[a-zA-Z0-9]$")

// DefaultURL returns the default cloud URL.  This may be overridden using the CODEINFRA_API environment
// variable.  If no override is found, and we are authenticated with a cloud, choose that.  Otherwise,
// we will default to the https://api.codeinfra.com/ endpoint.
...
func ValueOrDefaultURL(ws pkgWorkspace.Context, cloudURL string) string {
	...
	// Otherwise, respect the CODEINFRA_API override.
...
func loginWithBrowser(
	...
	// browser to a special endpoint on the Codeinfra.com console, passing the generated nonce as well as the port of the
	// webserver we launched. This endpoint does the OAuth flow and when it completes, redirects to localhost passing
	// the nonce and the codeinfra access token we created as part of the OAuth flow. If the nonces match, we set the
	// access token that was passed to us and the redirect to a special welcome page on Codeinfra.com
	...
	username, organizations, tokenInfo, err := client.NewClient(
		cloudURL, accessToken, insecure, cmdutil.Diag()).GetCodeinfraAccountDetails(ctx)
	if err != nil {
		return nil, err
	}
...
func (m defaultLoginManager) Current(
	...
	// either matches CODEINFRA_ACCESS_TOKEN or CODEINFRA_ACCESS_TOKEN
	// is not set use it.  If CODEINFRA_ACCESS_TOKEN does not match,
	// we prefer that.
	...
	// We either have no token saved, or CODEINFRA_ACCESS_TOKEN
	// doesn't match what we have saved.  Prefer the new
	// CODEINFRA_ACCESS_TOKEN.
	if accessToken == "" {
...
func (m defaultLoginManager) Login(
	...
	// In the case where we could not construct a link to the codeinfra console based on the API server's hostname,
...
func WelcomeUser(opts display.Options) {
  ...
  Codeinfra helps you create, deploy, and manage infrastructure on any cloud using
  your favorite language. You can get started today with Codeinfra at:

      https://www.codeinfra.com/docs/get-started/

  %s Resources you create with Codeinfra are given unique names (a randomly
  generated suffix) by default. To learn more about auto-naming or customizing resource
  names see https://www.codeinfra.com/docs/intro/concepts/resources/#autonaming.


`,
		opts.Color.Colorize(colors.SpecHeadline+"Welcome to Codeinfra!"+colors.Reset),
...
func (b *cloudBackend) Name() string {
	if b.url == CodeinfraCloudURL {
		return "codeinfra.com"
	}
...
func (b *cloudBackend) currentUser(ctx context.Context) (string, []string, *workspace.TokenInformation, error) {
	...
	return b.client.GetCodeinfraAccountDetails(ctx)
}
...
// qualifiedStackReference describes a qualified stack on the Codeinfra Service. The Owner or Project
// may be "" if unspecified, e.g. "codeinfra/production" specifies the Owner and Name, but not the
...
func (b *cloudBackend) ValidateStackName(s string) error {
	...
	// The Codeinfra Service enforces specific naming restrictions for organizations,
...
// a stack may exist within, which for the Codeinfra Service is the user account or organization.
...
//
// NOTE: Be careful when requiring a project name be valid. The Codeinfra.yaml file may contain
// an invalid project name like "r@bid^W0MBAT!!", but we try to err on the side of flexibility by
// implicitly "cleaning" the project name before we send it to the Codeinfra Service. So when we go
...
// Codeinfra.yaml file be updated.
//
...
// Confirm the specified stack's project doesn't contradict the Codeinfra.yaml of the current project.
// if the CWD is not in a Codeinfra project,
//
//	does not contradict
//
// if the project name in Codeinfra.yaml is "foo".
//
...
func (b *cloudBackend) CreateStack(
	...
	if currentProjectContradictsWorkspace(b.currentProject, stackID) {
		return nil, fmt.Errorf("provided project name %q doesn't match Codeinfra.yaml", stackID.Project)
	}
...
func (b *cloudBackend) ListStacks(
	...
	// Sanitize the project name as needed, so when communicating with the Codeinfra Service we
	// always use the name the service expects. (So that a similar, but not technically valid
	// name may be put in Codeinfra.yaml without causing problems.)
...
func (b *cloudBackend) RenameStack(ctx context.Context, stack backend.Stack,
	...
	// e.g. if you want to change the project name on the Codeinfra Console to reflect a
	// new value in Codeinfra.yaml.
	...
	if stackID.Owner != newIdentity.Owner {
		...
		errMsg += "       You cannot transfer stack ownership via a rename. If you wish to transfer ownership\n" +
			"       of a stack to another organization, you can do so in the Codeinfra Console by going to the\n" +
...
func (b *cloudBackend) NaturalLanguageSearch(
	...
	requestBody := apitype.CodeinfraQueryRequest{Query: parsedResults.Query}
...
func (b *cloudBackend) createAndStartUpdate(
	...
	if currentProjectContradictsWorkspace(op.Proj, stackID) {
		return client.UpdateIdentifier{}, updateMetadata{}, fmt.Errorf(
			"provided project name %q doesn't match Codeinfra.yaml", stackID.Project)
	}
	...
	//
	// TODO[codeinfra-service#3745]: Move this to the plugin-gathering routine when we have a dedicated
	...
	if isCopilotEnabled {
		if env.SuppressCopilotLink.Value() {
			...
			continuationString = " but the environment variable CODEINFRA_SUPPRESS_COPILOT_LINK" +
...
// apply actually performs the provided type of update on a stack hosted in the Codeinfra Cloud.
...
// getPermalink returns a link to the update in the Codeinfra Console.
...
// query executes a query program against the resource outputs of a stack hosted in the Codeinfra
// Cloud.
...
func (b *cloudBackend) runEngineAction(
	...
	// displayEvents renders the event to the console and Codeinfra service. The processor for the
...
func (b *cloudBackend) ExportDeploymentForVersion(
	...
	// The Codeinfra Console defines versions as a positive integer. Parse the provided version string and
	// ensure it is valid.
	//
...
// waitForUpdate waits for the current update of a Codeinfra program to reach a terminal state. Returns the
...
// tryNextUpdate tries to get the next update for a Codeinfra program.  This may time or error out, which results in a
...
func (b *cloudBackend) tryNextUpdate(ctx context.Context, update client.UpdateIdentifier, continuationToken *string,
	...
	if errResp, ok := err.(*apitype.ErrorResponse); ok {
		if errResp.Code == 504 {
			// If our request to the Codeinfra Service returned a 504 (Gateway Timeout), ignore it and keep
...
// IsValidAccessToken tries to use the provided Codeinfra access token and returns if it is accepted
...
func IsValidAccessToken(ctx context.Context, cloudURL string,
	...
	// it has a githubLogin field that is non-empty (like the Codeinfra Service would return).
	username, organizations, tokenInfo, err := client.NewClient(cloudURL, accessToken,
		insecure, cmdutil.Diag()).GetCodeinfraAccountDetails(ctx)
	if err != nil {
	...
	for {
		...
		for _, l := range logs.Lines {
			if l.Header != "" {
				...
				// If we see it's a Codeinfra operation, rather than outputting the deployment logs,
				// find the associated update and show the normal rendering of the operation's events.
				if l.Header == fmt.Sprintf("codeinfra %v", req.Op) {
					fmt.Println()
...
func (c httpstateBackendClient) GetStackOutputs(ctx context.Context, name string) (resource.PropertyMap, error) {
	...
	if strings.Count(name, "/") != 2 {
		return nil, errors.New("a stack reference's name should be of the form '<organization>/<project>/<stack>'. " +
			"See https://www.codeinfra.com/docs/using-codeinfra/stack-outputs-and-references/#using-stack-references " +
...
func doDetectCapabilities(ctx context.Context, d diag.Sink, client *client.Client) capabilities {
	...
	// remains necessary while CODEINFRA_OPTIMIZED_CHECKPOINT_PATCH has higher memory requirements on the client and
	// may cause out-of-memory issues in constrained environments.
	switch strings.ToLower(os.Getenv("CODEINFRA_OPTIMIZED_CHECKPOINT_PATCH")) {
	case "0", "false":
