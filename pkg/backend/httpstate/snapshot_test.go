	...
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		return codeinfra.RunWithContext(ctx, func(ctx *codeinfra.Context) error {
			...
			for i := 0; i < resourceCount; i++ {
				...
				err = ctx.RegisterResourceOutputs(&dummy, codeinfra.Map{
					"deadweight": codeinfra.String(pseudoRandomString(r, resourcePayloadBytes)),
				})
				if err != nil {
					return err
				}
...
func init() {
	for _, c := range strings.Split(os.Getenv("CODEINFRA_TEST_CHECKPOINT_DIFFS"), ",") {
		if c != "" {
