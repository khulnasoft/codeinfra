...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/backend/display/internal/terminal"
...
func printPermalink(out io.Writer, opts Options, message, permalink string) {
	if !opts.SuppressPermalink && permalink != "" {
		// Print a URL at the beginning of the update pointing to the Codeinfra Service.
...
func ShowEvents(
	...
	streamPreview := cmdutil.IsTruthy(os.Getenv("CODEINFRA_ENABLE_STREAMING_JSON_PREVIEW"))
...
func startEventLogger(events <-chan engine.Event, done chan<- bool, opts Options) (<-chan engine.Event, chan<- bool) {
	...
	// https://github.com/khulnasoft/codeinfra/issues/6768
