...
// Note: to regenerate the baselines for these tests, run `go test` with `CODEINFRA_ACCEPT=true`.
	...
	makeDisplayWithDiagnostic := func(sev diag.Severity) *ProgressDisplay {
		return &ProgressDisplay{
			eventUrnToResourceRow: map[resource.URN]ResourceRow{
				"urn:codeinfra:test::test::codeinfra:codeinfra:Stack::test": &resourceRowData{
					diagInfo: &DiagInfo{
