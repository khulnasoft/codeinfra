...
import (
	...
	codeasset "github.com/khulnasoft/codeinfra/pkg/v3/asset"
...
func getIndent(step engine.StepEventMetadata, seen map[resource.URN]engine.StepEventMetadata) int {
	...
	for p := step.Res.Parent; p != ""; {
		...
		if !has {
			...
			// TODO[codeinfra/codeinfra#340]: we need to figure out how best to display this sequence; at the very
...
// massageStackPreviewOutputDiff removes any adds of unknown values nested inside Codeinfra resources present in a stack's
// outputs.
func massageStackPreviewOutputDiff(diff *resource.ObjectDiff, inResource bool) {
	...
	_, isResource := diff.Adds["@isCodeinfraResource"]
	if isResource {
		delete(diff.Adds, "@isCodeinfraResource")
...
func (p *propertyPrinter) printPrimitivePropertyValue(v resource.PropertyValue) {
	...
	if v.IsNull() {
		...
		// All codeinfra numbers are IEEE doubles really (even in languages where we codegen integers the wire
		...
		// non-fractional). See https://github.com/khulnasoft/codeinfra/issues/13016 for context.
		...
		// better job here (codeinfra/codeinfra#234).
		if p.planning {
