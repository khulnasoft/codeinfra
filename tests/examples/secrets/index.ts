...
// A plaintext message.  We could turn this into a secret after the fact by passing it to `codeinfra.secret` if we wished.
const message = c.require("message");

// Secrets are viral. When you combine secrets with `codeinfra.all`, if any of the input values are secret, the entire
...
const combined = codeinfra.all([apiKey, message]).apply(([s, p]) => {
    return p;
})
...
export const richStructure = new ReflectResource("rValue", {
    plain: codeinfra.output("plaintext"),
    secret: codeinfra.secret("secret value"),
}).value;
