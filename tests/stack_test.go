...
func TestStackCommands(t *testing.T) {
	...
	t.Run("StackSelect", func(t *testing.T) {
		...
		integration.CreateBasicCodeinfraRepo(e)
		e.SetBackend(e.LocalURL())
		e.RunCommand("codeinfra", "stack", "init", "blighttown")
		e.RunCommand("codeinfra", "stack", "init", "majula")
		e.RunCommand("codeinfra", "stack", "init", "lothric")
		...
		// Select works
		e.RunCommand("codeinfra", "stack", "select", "blighttown")
		stacks, current = integration.GetStacks(e)
		if current == nil {
			t.Fatalf("No stack was labeled as current among: %v", stacks)
		} else {
		...
		// Error
		out, err := e.RunCommandExpectError("codeinfra", "stack", "select", "anor-londo")
		assert.Empty(t, out)
		// local: "no stack with name 'anor-londo' found"
		// cloud: "Stack 'integration-test-59f645ba/codeinfra-test/anor-londo' not found"
	...
	t.Run("StackInitNoSelect", func(t *testing.T) {
		...
		integration.CreateBasicCodeinfraRepo(e)
		e.SetBackend(e.LocalURL())
		e.RunCommand("codeinfra", "stack", "init", "first")
		e.RunCommand("codeinfra", "stack", "init", "second")
		...
		e.RunCommand("codeinfra", "stack", "init", "third", "--no-select")
		stacks, current = integration.GetStacks(e)
		if current == nil {
			t.Fatalf("No stack was labeled as current among: %v", stacks)
		} else {
	...
	t.Run("StackUnselect", func(t *testing.T) {
		...
		integration.CreateBasicCodeinfraRepo(e)
		e.SetBackend(e.LocalURL())
		e.RunCommand("codeinfra", "stack", "init", "one")
		e.RunCommand("codeinfra", "stack", "init", "two")
		...
		e.RunCommand("codeinfra", "stack", "unselect")
	...
	t.Run("StackRm", func(t *testing.T) {
		...
		e.RunCommand("codeinfra", "stack", "rm", "majula", "--yes")
		stacks, _ = integration.GetStacks(e)
		assert.Equal(t, 2, len(stacks))
		assert.Contains(t, stacks, "blighttown")
		assert.Contains(t, stacks, "lothric")

		e.RunCommand("codeinfra", "stack", "rm", "lothric", "--yes")
		stacks, _ = integration.GetStacks(e)
		assert.Equal(t, 1, len(stacks))
		assert.Contains(t, stacks, "blighttown")

		e.RunCommand("codeinfra", "stack", "rm", "blighttown", "--yes")
		stacks, _ = integration.GetStacks(e)
		assert.Equal(t, 0, len(stacks))

		// Error
		out, err := e.RunCommandExpectError("codeinfra", "stack", "rm", "anor-londo", "--yes")
		assert.Empty(t, out)
		// local: .codeinfra/stacks/codeinfra-test/anor-londo.json: no such file or directory
		// cloud:  Stack 'integration-test-59f645ba/codeinfra-test/anor-londo' not found
		assert.Contains(t, err, "anor-londo")
	})
	...
	t.Run("CheckpointVersioning", func(t *testing.T) {
		...
		for _, deploymentVersion := range versions {
			t.Run(fmt.Sprintf("Version%d", deploymentVersion), func(t *testing.T) {
				...
				integration.CreateBasicCodeinfraRepo(e)
				e.SetBackend(e.LocalURL())
				e.RunCommand("codeinfra", "stack", "init", "the-abyss")
				stacks, _ := integration.GetStacks(e)
				assert.Equal(t, 1, len(stacks))

				stackFile := path.Join(e.RootPath, "stack.json")
				e.RunCommand("codeinfra", "stack", "export", "--file", "stack.json")
				...
				stdout, stderr := e.RunCommandExpectError("codeinfra", "stack", "import", "--file", "stack.json")
				assert.Empty(t, stdout)
				switch {
				case deploymentVersion > apitype.DeploymentSchemaVersionCurrent:
					assert.Contains(t, stderr, "the stack 'the-abyss' is newer than what this version of the Codeinfra CLI understands")
	...
	t.Run("FixingInvalidResources", func(t *testing.T) {
		...
		t.Setenv("CODEINFRA_CONFIG_PASSPHRASE", "correct horse battery staple")
		...
		os.Unsetenv("CODEINFRA_CONFIG_PASSPHRASE")
		_, stderr := e.RunCommand("codeinfra", "stack", "import", "--file", "stack.json")
		...
		e.RunCommand("codeinfra", "up", "--non-interactive", "--yes", "--skip-preview")
		e.RunCommand("codeinfra", "stack", "rm", "--yes", "--force")
	})
}
...
func TestStackBackups(t *testing.T) {
	t.Run("StackBackupCreatedSanityTest", func(t *testing.T) {
		...
		// Now run codeinfra destroy.
		before = time.Now().UnixNano()
		e.RunCommand("codeinfra", "destroy", "--non-interactive", "--yes", "--skip-preview")
		after = time.Now().UnixNano()
...
func TestDestroySetsEncryptionsalt(t *testing.T) {
	...
	const stackName = "imulup"
	stackFile := filepath.Join(e.RootPath, "Codeinfra.imulup.yaml")
	var expectedSalt string

	// Set up the environment.
	{
		e.Setenv("CODEINFRA_CONFIG_PASSPHRASE", "")
		...
		e.RunCommand("codeinfra", "config", "set", "--secret", "token", "cookie")
	...
	// Remove `encryptionsalt` from `Codeinfra.imulup.yaml`.
	preamble := "secretsprovider: passphrase\n"
	err := os.WriteFile(stackFile, []byte(preamble), 0o600)
	assert.NoError(t, err, "writing Codeinfra.imulup.yaml")
...
func TestStackRenameAfterCreate(t *testing.T) {
	...
	e.RunCommand("codeinfra", "stack", "rename", newName)
}
...
func TestStackRenameAfterCreateServiceBackend(t *testing.T) {
	...
	username, _ := e.RunCommand("codeinfra", "whoami")
	...
	e.RunCommand("codeinfra", "config", "set", "xyz", "abc")

	// Try to rename the stack to itself. This should fail.
	e.RunCommandExpectError("codeinfra", "stack", "rename", stackName)
	...
	e.RunCommandExpectError("codeinfra", "stack", "rename", "fakeorg/"+stackRenameBase)

	// Next perform a legal rename. This should work.
	e.RunCommand("codeinfra", "stack", "rename", stackRenameBase)
	stdoutXyz1, _ := e.RunCommand("codeinfra", "config", "get", "xyz")
	...
	e.RunCommand("codeinfra", "stack", "rename", orgName+"/"+stackRenameBase+"2")
	stdoutXyz2, _ := e.RunCommand("codeinfra", "config", "get", "xyz")
	assert.Equal(t, "abc", strings.Trim(stdoutXyz2, "\r\n"))
}

func TestLocalStateLocking(t *testing.T) {
	t.Skip() // TODO[codeinfra/codeinfra#7269] flaky test
	t.Parallel()
	e := ptesting.NewEnvironment(t)
	defer e.DeleteIfNotFailed()
	...
	for i := 0; i < count; i++ {
		go func() {
			_, stderr, err := e.GetCommandResults("codeinfra", "up", "--non-interactive", "--skip-preview", "--yes")
			if err == nil {
	...
	for i := 0; i < count; i++ {
		go func() {
			_, stderr, err := e.GetCommandResults("codeinfra", "preview", "--non-interactive")
			if err == nil {
				stderrs <- "" // success marker
			} else {
				stderrs <- stderr
...
func stackFileFormatAsserters(t *testing.T, e *ptesting.Environment, projectName, stackName string) (func(), func()) {
	stacksDir := filepath.Join(".codeinfra", "stacks", projectName)
...
func TestLocalStateGzip(t *testing.T) { //nolint:paralleltest
	...
	codeinfraUp := func() { e.RunCommand("codeinfra", "up", "--non-interactive", "--yes", "--skip-preview") }

	// Test "codeinfra up" with gzip compression on and off.
	...
	switchGzipOn()
	codeinfraUp()
	// Running "codeinfra up" 2 times is important because normally, first the
	...
	rawHistory, _ := e.RunCommand("codeinfra", "stack", "history", "--json")
...
func TestStackTags(t *testing.T) {
	...
	e.RunCommand("codeinfra", "stack", "tag", "set", "tagA", "valueA")
	e.RunCommand("codeinfra", "stack", "tag", "set", "tagB", "valueB")

	lsTags := func() map[string]string {
		stdout, _ := e.RunCommand("codeinfra", "stack", "tag", "ls", "--json")
	...
	e.RunCommand("codeinfra", "stack", "tag", "rm", "tagA")
	tags = lsTags()
...
func TestNewStackConflictingOrg(t *testing.T) {
	...
	orgs := []string{"moolumi", "codeinfra-test"}
	for _, org := range orgs {
		stackRef := fmt.Sprintf("%s/%s/stack", org, project)
		// Ensure projects no longer exists. Ignoring errors.
		_, _, err := e.GetCommandResults("codeinfra", "stack", "rm", "-s", stackRef)
		_ = err
	}
	for _, org := range orgs {
		stackRef := fmt.Sprintf("%s/%s/stack", org, project)
		e.RunCommand("codeinfra", "new", "yaml", "-s", stackRef, "--yes", "--force")
		e.RunCommand("codeinfra", "up", "--yes")
		e.RunCommand("codeinfra", "destroy", "--yes", "--remove")
	}
}
