// Copyright 2023-2024, KhulnaSoft Ltd.
//
...
//
//nolint:paralleltest // codeinfra new is not parallel safe
func TestLanguageNewSmoke(t *testing.T) {
	...
	for _, runtime := range Runtimes {
		t.Run(runtime, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "new", "random-"+Languages[runtime], "--yes")
			e.RunCommand("codeinfra", "up", "--yes")
			e.RunCommand("codeinfra", "destroy", "--yes")
		})
	}
}
...
func TestLanguageConvertSmoke(t *testing.T) {
	...
	for _, runtime := range Runtimes {
		...
		t.Run(runtime, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "stack", "init", "test")
...
func TestLanguageConvertLenientSmoke(t *testing.T) {
	...
	for _, runtime := range Runtimes {
		...
		t.Run(runtime, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
			e.RunCommand(
				"codeinfra", "convert", "--generate-only",
				"--language", Languages[runtime], "--from", "pcl", "--out", "out")
...
func TestLanguageConvertComponentSmoke(t *testing.T) {
	...
	for _, runtime := range Runtimes {
		...
		t.Run(runtime, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
			e.RunCommand("codeinfra", "convert", "--language", Languages[runtime], "--from", "pcl", "--out", "out")
			e.CWD = filepath.Join(e.RootPath, "out")
			e.RunCommand("codeinfra", "stack", "init", "test")

			// TODO(https://github.com/khulnasoft/codeinfra/issues/14339): This doesn't work for Go yet because the
...
func TestLanguageGenerateSmoke(t *testing.T) {
	...
	for _, runtime := range Runtimes {
		...
		t.Run(runtime, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "package", "gen-sdk", "--language", runtime, "schema.json")
		})
	}
}
...
func TestPackageGetSchema(t *testing.T) {
	...
	removeRandomFromLocalPlugins := func() {
		e.RunCommand("codeinfra", "plugin", "rm", "resource", "random", "--all", "--yes")
	}
	...
	out, _ := e.RunCommand("codeinfra", "plugin", "ls")
	if strings.Contains(out, "random  resource") {
		removeRandomFromLocalPlugins()
	}

	// get the schema and bind it
	schemaJSON, _ := e.RunCommand("codeinfra", "package", "get-schema", "random")
	bindSchema("random", schemaJSON)

	// try again using a specific version
	removeRandomFromLocalPlugins()
	schemaJSON, _ = e.RunCommand("codeinfra", "package", "get-schema", "random@4.13.0")
	bindSchema("random", schemaJSON)
	...
	binaryPath := filepath.Join(
		...
		"codeinfra-resource-random")
	if runtime.GOOS == "windows" {
		binaryPath += ".exe"
	}

	schemaJSON, _ = e.RunCommand("codeinfra", "package", "get-schema", binaryPath)
	bindSchema("random", schemaJSON)

	// Now try and get the parameterized schema from the test-provider
	providerDir, err := filepath.Abs("testprovider")
	require.NoError(t, err)
	schemaJSON, _ = e.RunCommand("codeinfra", "package", "get-schema", providerDir, "parameter")
...
func TestPackageGetMappingToFile(t *testing.T) {
	...
	stdout, result := e.RunCommand("codeinfra",
...
func TestPackageGetMapping(t *testing.T) {
	...
	schema, result := e.RunCommand("codeinfra", "package", "get-mapping", "terraform", "random@4.13.0")
	require.Contains(t, result, "random@4.13.0 maps to provider random")
...
func TestLanguageImportSmoke(t *testing.T) {
	...
	for _, runtime := range Runtimes {
		t.Run(runtime, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
			e.RunCommand("codeinfra", "new", Languages[runtime], "--yes")
			e.RunCommand("codeinfra", "import", "--yes", "random:index/randomId:RandomId", "identifier", "p-9hUg")
		})
	}
}

// Test that CODEINFRA_DISABLE_AUTOMATIC_PLUGIN_ACQUISITION disables plugin acquisition in convert.
//
...
func TestConvertDisableAutomaticPluginAcquisition(t *testing.T) {
	...
	e.RunCommand("codeinfra", "plugin", "rm", "--all", "--yes")
	// Disable acquisition.
	e.SetEnvVars("CODEINFRA_DISABLE_AUTOMATIC_PLUGIN_ACQUISITION=true")

	// This should fail because of no terraform converter
	_, stderr := e.RunCommandExpectError(
		"codeinfra", "convert",
		"--language", "pcl", "--from", "terraform", "--out", "out")
	assert.Contains(t, stderr, "no converter plugin 'codeinfra-converter-terraform' found")

	// Install a _specific_ version of the terraform converter (so this test doesn't change due to a new release)
	e.RunCommand("codeinfra", "plugin", "install", "converter", "terraform", "v1.0.8")
...
func TestPreviewImportFile(t *testing.T) {
	...
	// Make sure random is installed
	e.RunCommand("codeinfra", "plugin", "install", "resource", "random", "4.12.0")

	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "test")
	e.RunCommand("codeinfra", "install")
	e.RunCommand("codeinfra", "preview", "--import-file", "import.json")
...
// https://github.com/khulnasoft/codeinfra/issues/15467.
...
// Quick sanity tests for https://github.com/khulnasoft/codeinfra/issues/16248. Ensure we can run plugins and auto-fetch them.
//
...
func TestPluginRun(t *testing.T) {
	...
	_, stderr := e.RunCommandExpectError("codeinfra", "plugin", "run", "--kind=resource", "random", "--", "--help")
...
func TestInstall(t *testing.T) {
	...
	for _, runtime := range Runtimes {
		...
		t.Run(runtime, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "new", "random-"+Languages[runtime], "--yes", "--generate-only")

			// Ensure `install` works and subsequent `up` and `destroy` operations work.
			_, stderr := e.RunCommand("codeinfra", "install")
...
func TestSecretsProvidersInitializationSmoke(t *testing.T) {
	...
	t.Setenv("CODEINFRA_CONFIG_PASSPHRASE", "test-passphrase")
	...
	for _, runtime := range Runtimes {
		for _, c := range cases {
			...
			t.Run(name, func(t *testing.T) {
				...
				projectYAML := filepath.Join(projectDir, "Codeinfra.yaml")
...
backend:
  url: '%s'`,
				...
				stackYAML := filepath.Join(projectDir, "Codeinfra.dev.yaml")
				...
				e.RunCommand("codeinfra", initArgs...)
				...
				stackJSONStr, _ := e.RunCommand("codeinfra", "stack", "export")
...
// or empty and the CODEINFRA_FALLBACK_TO_STATE_SECRETS_MANAGER environment variable
// is set.
//
//nolint:paralleltest // codeinfra new is not parallel safe, and we set environment variables
func TestSecretsProvidersFallbackSmoke(t *testing.T) {
	...
	t.Setenv("CODEINFRA_FALLBACK_TO_STATE_SECRETS_MANAGER", "true")
	...
	for _, runtime := range Runtimes {
		t.Run(runtime, func(t *testing.T) {
			...
			for _, operation := range operations {
				os.Remove(filepath.Join(projectDir, "Codeinfra.dev.yaml"))
				e.RunCommand("codeinfra", operation...)

				stackYamlStr, err := os.ReadFile(filepath.Join(projectDir, "Codeinfra.dev.yaml"))
				require.NoError(t, err)
...
func TestImportVersionSmoke(t *testing.T) {
	...
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "new", "random-go", "--yes")

	// Install the version of the random provider used by the template and another
	e.RunCommand("codeinfra", "plugin", "install", "resource", "random", "4.16.7")
	e.RunCommand("codeinfra", "plugin", "install", "resource", "random", "4.13.0")
	...
	stack, _ := e.RunCommand("codeinfra", "stack", "export")
...
func TestRefreshUpgradeWarning(t *testing.T) {
	...
	stdout, _ := e.RunCommand("codeinfra", "refresh", "--yes")
...
func TestDestroyUpgradeWarningParameterized(t *testing.T) {
	...
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "new", "random-typescript", "--yes")

	// Change to use the parameterised version of the provider
	e.RunCommand("codeinfra", "plugin", "install", "resource", "terraform-provider", "0.3.1")
	e.RunCommand("codeinfra", "package", "add", "terraform-provider", "hashicorp/random", "3.6.0")
	...
	replaceStringInFile(filepath.Join(projectDir, "package.json"), "\"@codeinfra/random\": \"^4.13.0\",", "")
	...
	// Update the provider to a new version
	e.RunCommand("codeinfra", "package", "add", "terraform-provider", "hashicorp/random", "3.6.3")

	// Run a destroy and check that the warning is shown
	stdout, _ := e.RunCommand("codeinfra", "destroy", "--yes")
