	...
	for _, command := range commands {
		...
		for name, tc := range tests {
			...
			t.Run(command+"_"+name, func(t *testing.T) {
				...
				// Remote flags currently require CODEINFRA_EXPERIMENTAL.
				e.Env = append(e.Env, "CODEINFRA_EXPERIMENTAL=true")

				args := []string{command, "--remote"}
				_, err := e.RunCommandExpectError("codeinfra", append(args, tc.args...)...)
...
func TestRemoteLifecycle(t *testing.T) {
	// This test requires the service with access to Codeinfra Deployments.
	// Set CODEINFRA_ACCESS_TOKEN to an access token with access to Codeinfra Deployments,
	// set CODEINFRA_TEST_OWNER to the organization to use for the fully qualified stack,
	// and set CODEINFRA_TEST_DEPLOYMENTS_API to any value to enable the test.
	...
	if os.Getenv("CODEINFRA_TEST_DEPLOYMENTS_API") == "" {
		t.Skipf("Skipping: CODEINFRA_TEST_DEPLOYMENTS_API is not set")
	}
	t.Parallel()
	...
	e.RunCommand("codeinfra", "stack", "init", "--no-select", "--stack", fullyQualifiedStack)
	...
	e.RunCommand("codeinfra", args("preview")...)
	e.RunCommand("codeinfra", args("up")...)
	e.RunCommand("codeinfra", args("refresh")...)
	e.RunCommand("codeinfra", args("destroy")...)

	e.RunCommand("codeinfra", "stack", "rm", "--stack", fullyQualifiedStack, "--yes")
}
