...
func TestConstructComponentConfigureProviderGo(t *testing.T) {
	...
	codeinfraGoSDK := filepath.Join(codeinfraRoot, "sdk")
	componentSDK := filepath.Join(codeinfraRoot, "tests/testdata/codegen/methods-return-plain-resource/go")
	sdkPkg := "github.com/khulnasoft/codeinfra/tests/testdata/codegen/methods-return-plain-resource/go"
	...
	cmd.Stdout = stdout
	cmd.Stderr = stderr
	cmd.Dir = filepath.Join(codeinfraRoot, "pkg", "codegen", "go")
	cmd.Env = os.Environ()
	cmd.Env = append(cmd.Env, "CODEINFRA_ACCEPT=1")
	err = cmd.Run()
	...
	opts = opts.With(integration.ProgramTestOptions{
		...
		Dependencies: []string{
			"github.com/khulnasoft/codeinfra/sdk/v3=" + codeinfraGoSDK,
