...
func NewComponentComponent(ctx *codeinfra.Context, name string, opts ...codeinfra.ResourceOption) (*Component, error) {
	component := &Component{}
	err := ctx.RegisterComponentResource(providerName+":index:ComponentComponent", name, component, opts...)
	if err != nil {
		return nil, err
	}
	err = ctx.RegisterRemoteComponentResource("testcomponent:index:Component", name+"-child", codeinfra.Map{
		"echo": codeinfra.String("checkExpected"),
	}, &Component{}, codeinfra.Parent(component))
	if err != nil {
		return nil, err
	}

	err = ctx.RegisterResourceOutputs(component, codeinfra.Map{})
