...
async def fn():
    ...
    return 42
b = codeinfra.Output.from_input(asyncio.to_thread(fn))
...
while True:
    ...
    codeinfra.runtime.sync_await._sync_await(asyncio.sleep(1))
    with codeinfra.runtime.settings.SETTINGS.lock:
        if len(codeinfra.runtime.settings.SETTINGS.outputs) == 0:
            break
