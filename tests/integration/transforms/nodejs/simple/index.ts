...
const res3 = new Random("res3", { length: codeinfra.secret(5) });
...
const res6 = new Random("res6", { length: 10 }, {
    ...
    transforms: [
        async ({ type, props, opts }) => {
            ...
            return {
                props: props,
                opts: codeinfra.mergeOptions(opts, { provider: provider2 }),
            };
        },
    ],
});
...
codeinfra.runtime.registerInvokeTransform(async ({ token, args, opts }) => {
    return {
	args: { ...args, length: 11 },
	opts: opts,
    };
...
const res8 = new Random("res8", { length: codeinfra.secret(5) });
const args = {
    length: 10,
    prefix: "test",
};
