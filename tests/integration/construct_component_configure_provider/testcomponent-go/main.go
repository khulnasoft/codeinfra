...
func call(ctx *codeinfra.Context, tok string, args codeinfraprovider.CallArgs) (*codeinfraprovider.CallResult, error) {
	switch tok {
	case tlsProviderMethodToken:
		...
		return codeinfraprovider.NewSingletonCallResult(result)
...
func construct(ctx *codeinfra.Context, typ, name string, inputs codeinfraprovider.ConstructInputs,
...
func main() {
	...
	codeinfra.RegisterResourceModule(providerName, mainModule, &module{semver.MustParse(version)})
