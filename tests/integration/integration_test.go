...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/codegen/testing/test"
...
func TestStackTagValidation(t *testing.T) {
	...
	t.Run("Error_StackName", func(t *testing.T) {
		...
		stdout, stderr := e.RunCommandExpectError("codeinfra", "stack", "init", "invalid name (spaces, parens, etc.)")
	...
	t.Run("Error_DescriptionLength", func(t *testing.T) {
		...
		if os.Getenv("CODEINFRA_TEST_OWNER") == "" {
			t.Skipf("Skipping: CODEINFRA_TEST_OWNER is not set")
		}
		...
		prefix = prefix + prefix + prefix + prefix // 416 + the current Codeinfra.yaml's description

		// Change the contents of the Description property of Codeinfra.yaml.
		yamlPath := filepath.Join(e.CWD, "Codeinfra.yaml")
		err = integration.ReplaceInFile("description: ", "description: "+prefix, yamlPath)
		assert.NoError(t, err)

		stdout, stderr := e.RunCommandExpectError("codeinfra", "stack", "init", stackName)
		assert.Equal(t, "", stdout)
		assert.Contains(t, stderr, "error: could not create stack:")
		assert.Contains(t, stderr, "validating stack properties:")
		assert.Contains(t, stderr, "stack tag \"codeinfra:description\" value is too long (max length 256 characters)")
	})
}
...
func TestStackInitValidation(t *testing.T) {
	...
	t.Run("Error_InvalidStackYaml", func(t *testing.T) {
		...
		stdout, stderr := e.RunCommandExpectError("codeinfra", "stack", "init", "valid-name")
		assert.Equal(t, "", stdout)
...
func TestConfigPaths(t *testing.T) {
	...
	assert.NoError(t, err)
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "testing")
	...
	validateConfigGet := func(key string, value string, path bool) {
		...
		stdout, stderr := e.RunCommand("codeinfra", args...)
	...
	for _, ns := range namespaces {
		for _, badKey := range badKeys {
			...
			stdout, stderr := e.RunCommandExpectError("codeinfra", "config", "set", "--path", key, "value")
			assert.Equal(t, "", stdout)
...
func testDestroyStackRef(e *ptesting.Environment, organization string) {
	...
	e.RunCommand("codeinfra", "destroy", "--skip-preview", "--yes", "-s", stackRef)
	e.RunCommand("codeinfra", "stack", "rm", "--yes", "-s", stackRef)
}
...
func TestDestroyStackRef_LocalNonProject_NewEnv(t *testing.T) {
	...
	t.Setenv("CODEINFRA_DIY_BACKEND_LEGACY_LAYOUT", "true")
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
...
func TestDestroyStackRef_LocalNonProject_OldEnv(t *testing.T) {
	...
	t.Setenv("CODEINFRA_SELF_MANAGED_STATE_LEGACY_LAYOUT", "true")
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	testDestroyStackRef(e, "")
}
...
func TestDestroyStackRef_Cloud(t *testing.T) {
	...
	output, _ := e.RunCommand("codeinfra", "whoami")
...
func TestProviderDownloadURL(t *testing.T) {
	...
	validate := func(t *testing.T, stdout []byte) {
		...
		for _, resource := range data.Resources {
			...
			case providers.IsProviderType(resource.Type):
				assert.Equalf(t, "get.codeinfra.test/providers", getPluginDownloadURL(resource.Inputs), "Inputs")
				assert.Equalf(t, "", getPluginDownloadURL(resource.Outputs), "Outputs")
				assert.Equalf(t, nil, resource.Inputs[urlKey], "Outputs")
				assert.Equalf(t, nil, resource.Outputs[urlKey], "Outputs")
			default:
	...
	languages := []struct {
		...
		{"nodejs", "@codeinfra/codeinfra"},
		{"go", "github.com/khulnasoft/codeinfra/sdk/v3"},
	}
...
func TestExcludeProtected(t *testing.T) {
	...
	stdout, _ := e.RunCommand("codeinfra", "destroy", "--skip-preview", "--yes", "--exclude-protected")
...
name: invalid-plugin
runtime: yaml
description: A Codeinfra program referencing an invalid plugin.
plugins:
  providers:
    - name: fakeplugin
...
`
	...
	integration.CreateCodeinfraRepo(e, codeinfraProject)
	e.SetBackend(e.LocalURL())
	{
		_, stderr := e.RunCommandExpectError("codeinfra", "stack", "init", "invalid-resources")
		assert.NotContains(t, stderr, "panic: ")
		assert.Contains(t, stderr, "error: ")
	}
	{
		_, stderr := e.RunCommandExpectError("codeinfra", "pre")
		assert.NotContains(t, stderr, "panic: ")
		assert.Contains(t, stderr, "error: ")
	}
}

// Regression test for https://github.com/khulnasoft/codeinfra/issues/12632.
...
runtime: yaml
description: A Codeinfra program testing passphrase config.
`

	integration.CreateCodeinfraRepo(e, codeinfraProject)
	e.SetBackend(e.LocalURL())
	// Init a new stack, then set a secret config value, then try to get it.
	e.RunCommand("codeinfra", "stack", "init", "passphrase-test")
	// Clear the config file so that "config set-all" has to re-initialize the passphrase config.
	err := os.Remove(filepath.Join(e.RootPath, "Codeinfra.passphrase-test.yaml"))
	require.NoError(t, err)
	// Set a secret config value, then try to get it.
	e.RunCommand("codeinfra", "config", "set-all", "--secret", "foo=bar")
	stdout, _ := e.RunCommand("codeinfra", "config", "get", "foo")
...
name: config-rm-test
runtime: yaml
description: A Codeinfra program testing config rm and config rm-all.
`

	integration.CreateCodeinfraRepo(e, codeinfraProject)
	e.SetBackend(e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "config-rm-test")
	// Clear the config file so that "config set" has to re-initialize the passphrase config.
	err := os.Remove(filepath.Join(e.RootPath, "Codeinfra.config-rm-test.yaml"))
	require.NoError(t, err)
	...
	e.RunCommand("codeinfra", configSet("foo", "bar")...)

	e.RunCommand("codeinfra", configRm("foo")...)
	_, stderr = e.RunCommandExpectError("codeinfra", configGet("foo")...)
	assert.Contains(t, stderr, "'foo' not found")

	// Set a single secret value, then try to remove it.
	e.RunCommand("codeinfra", configSet("--secret", "foo", "bar")...)
	...
	e.RunCommand("codeinfra", configSetAll("--plaintext", "foo=bar", "--secret", "baz=quux")...)

	e.RunCommand("codeinfra", configRm("foo")...)
	_, stderr = e.RunCommandExpectError("codeinfra", configGet("foo")...)
	assert.Contains(t, stderr, "'foo' not found")
	stdout, _ = e.RunCommand("codeinfra", configGet("baz")...)
	assert.Contains(t, stdout, "quux")

	// Set multiple values, then remove one secret value.
	e.RunCommand("codeinfra", configSetAll("--plaintext", "foo=bar", "--secret", "baz=quux")...)

	e.RunCommand("codeinfra", configRm("baz")...)
	stdout, _ = e.RunCommand("codeinfra", configGet("foo")...)
	assert.Contains(t, stdout, "bar")
	_, stderr = e.RunCommandExpectError("codeinfra", configGet("baz")...)
	assert.Contains(t, stderr, "'baz' not found")

	// Set multiple values, then remove them all.
	e.RunCommand("codeinfra", configSetAll("--plaintext", "foo=bar", "--secret", "baz=quux")...)

	e.RunCommand("codeinfra", configRmAll("foo", "baz")...)
	_, stderr = e.RunCommandExpectError("codeinfra", configGet("foo")...)
	assert.Contains(t, stderr, "'foo' not found")
	_, stderr = e.RunCommandExpectError("codeinfra", configGet("baz")...)
	assert.Contains(t, stderr, "'baz' not found")
...
`
	...
	e.SetBackend(e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "config-rm-test")
	configFile := t.TempDir() + "/Codeinfra.config-rm-test.yaml"
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/12593.
//
...
func testProjectRename(e *ptesting.Environment, organization string) {
	...
	e.RunCommand("codeinfra", "stack", "rename", stackRef)

	// Rename the project name in the yaml file
	projFilename := filepath.Join(e.CWD, "Codeinfra.yaml")
	...
	e.RunCommand("codeinfra", "up", "--skip-preview", "--yes", "--expect-no-changes", "-s", stackRef)
	e.RunCommand("codeinfra", "stack", "rm", "--force", "--yes", "-s", stackRef)
}
...
func TestParentRename_issue13179(t *testing.T) {
	...
	// https://github.com/khulnasoft/codeinfra/issues/13179.
	//
	// It creates a stack with a resource that has a parent
	// and then renames the parent resource with 'codeinfra state rename'.
	...
	require.NoError(t,
		pt.RunCodeinfraCommand("state", "rename", "-y", string(parentURN), "newParent"),
		"rename failed")
}
...
func testStackRmConfig(e *ptesting.Environment, organization string) {
	...
	e.RunCommand("codeinfra", "stack", "init", stackName)
	// Create a config value to ensure there's a Codeinfra.<name>.yaml file.
	e.RunCommand("codeinfra", "config", "set", "key", "value")
	...
	// And check that Codeinfra.<name>.yaml file is still there for the js project
	_, err = os.Stat(filepath.Join(jsDir, "Codeinfra."+stackName+".yaml"))
...
func TestAdvisoryPolicyPack(t *testing.T) {
	...
	stdout, _, err := e.GetCommandResults(
		"codeinfra", "up", "--skip-preview", "--yes", "--policy-pack", "advisory_policy_pack")
	assert.NoError(t, err)
...
func TestMandatoryPolicyPack(t *testing.T) {
	...
	stdout, _, err := e.GetCommandResults(
		"codeinfra", "up", "--skip-preview", "--yes", "--policy-pack", "mandatory_policy_pack")
	assert.Error(t, err)
...
func TestMultiplePolicyPacks(t *testing.T) {
	...
	stdout, _, err := e.GetCommandResults("codeinfra", "up", "--skip-preview", "--yes",
...
// regresses https://github.com/khulnasoft/codeinfra/issues/11092
//
//nolint:paralleltest // uses parallel programtest
func TestPolicyPluginExtraArguments(t *testing.T) {
	...
	_, _, err = e.GetCommandResults("codeinfra", "--logflow", "preview", "--logtostderr",
...
func TestPolicyPackNewGenerateOnly(t *testing.T) {
	...
	stdout, _ := e.RunCommand("codeinfra", "policy", "new", "aws-python", "--force", "--generate-only")
	require.Contains(t, stdout, "To install dependencies for the Policy Pack, run `codeinfra install`")
	require.False(t, e.PathExists("venv"))
}
...
func TestPolicyPackNew(t *testing.T) {
	...
	defer e.DeleteIfNotFailed()
	require.False(t, e.PathExists("venv"))
	stdout, _ := e.RunCommand("codeinfra", "policy", "new", "aws-python", "--force")
	require.NotContains(t, stdout, "To install dependencies for the Policy Pack, run `codeinfra install`")
...
func TestPolicyPackInstallDependencies(t *testing.T) {
	...
	require.False(t, e.PathExists("venv"))
	stdout, _ := e.RunCommand("codeinfra", "install")
	require.Contains(t, stdout, "Finished creating virtual environment")
	require.Contains(t, stdout, "Finished installing dependencies")
	require.True(t, e.PathExists("venv"))
}
...
func TestInstallDependenciesForPolicyPackWithParentProject(t *testing.T) {
	e := ptesting.NewEnvironment(t)
	defer e.DeleteIfNotFailed()
	// create a directory structure with a codeinfra project and a nested policy pack
	...
	// chdir to the policy pack directory and run codeinfra install
...
func TestInstallDependenciesProjectWithParentPolicyPack(t *testing.T) {
	...
	// chdir to the project directory and run codeinfra install
	e.CWD = projectPath
	stdout, _ := e.RunCommand("codeinfra", "install")
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/17862
//
//nolint:paralleltest // uses parallel programtest
func TestCodeinfraNewLocalTemplatePath(t *testing.T) {
	e := ptesting.NewEnvironment(t)
	defer e.DeleteIfNotFailed()
	...
	stdout, stderr := e.RunCommand("codeinfra", "new", templatePath, "--generate-only", "--yes", "--force")
