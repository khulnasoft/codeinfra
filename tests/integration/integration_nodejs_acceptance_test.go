...
func TestConstructComponentConfigureProviderNode(t *testing.T) {
	...
	// component's node SDK, and the test program need to agree on the version of codeinfra-tls
	// dependency, and when there are discrepancies two versions of codeinfra-tls may be installed
	...
	componentSDK := filepath.Join(codeinfraRoot, "pkg/codegen/testing/test/testdata/methods-return-plain-resource/nodejs")
	...
	cmd.Stdout = stdout
	cmd.Stderr = stderr
	cmd.Dir = filepath.Join(codeinfraRoot, "pkg", "codegen", "nodejs")
	cmd.Env = os.Environ()
	...
	t.Logf("yarn run tsc # precompile @codeinfra/metaprovider")
	cmd2 := exec.Command("yarn", "run", "tsc")
	cmd2.Dir = filepath.Join(componentSDK)
	err = cmd2.Run()
	require.NoError(t, err)

	t.Logf("yarn link # prelink @codeinfra/metaprovider")
...
func TestNewNodejsUsesNpmByDefault(t *testing.T) {
	...
	e.RunCommand("codeinfra", "new", "typescript", "--force", "--non-interactive", "--yes", "--generate-only")
