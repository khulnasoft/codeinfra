...
import (
	...
	"github.com/khulnasoft/codeinfra/pkg/v3/secrets/cloud"
	"github.com/khulnasoft/codeinfra/pkg/v3/secrets/passphrase"
...
func TestEngineEventPerf(t *testing.T) {
	t.Skip() // TODO[codeinfra/codeinfra#7883]

	// Prior to codeinfra/codeinfra#2303, a preview or update would take ~40s.
...
func TestEngineEvents(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
			...
			assert.Contains(t, preEventResourceTypes, "codeinfra:codeinfra:Stack")
			assert.Contains(t, preEventResourceTypes, "codeinfra-nodejs:dynamic:Resource")
			assert.Contains(t, preEventResourceTypes, "codeinfra:providers:codeinfra-nodejs")
		},
	})
}
...
func TestProjectMainNodejs(t *testing.T) {
	...
	t.Run("AbsolutePath", func(t *testing.T) {
		...
		// write a new Codeinfra.yaml using the absolute path of the environment as "main"
		yamlPath := filepath.Join(e.RootPath, "Codeinfra.yaml")
		absYamlContents := fmt.Sprintf(
			"name: project_main_abs\ndescription: A program with an absolute entry point\nruntime: nodejs\nmain: %s\n",
			e.RootPath,
		)
		t.Logf("writing new Codeinfra.yaml: \npath: %s\ncontents:%s", yamlPath, absYamlContents)
		...
		e.RunCommand("codeinfra", "stack", "init", "main-abs")
		e.RunCommand("codeinfra", "preview")
	...
	t.Run("ParentFolder", func(t *testing.T) {
		...
		// yarn link first
		e.RunCommand("yarn", "link", "@codeinfra/codeinfra")
		// then virtually change directory to the location of the nested Codeinfra.yaml
		e.CWD = filepath.Join(e.RootPath, "foo", "bar")

		e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
		e.RunCommand("codeinfra", "stack", "init", "main-parent")
		e.RunCommand("codeinfra", "preview")
		e.RunCommand("codeinfra", "stack", "rm", "--yes")
	})
}

// TestStackProjectName ensures we can read the Codeinfra stack and project name from within the program.
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
...
func TestRemoveWithResourcesBlocked(t *testing.T) {
	...
	e.RunCommand("yarn", "link", "@codeinfra/codeinfra")
	e.RunCommand("codeinfra", "up", "--non-interactive", "--yes", "--skip-preview")
	_, stderr := e.RunCommandExpectError("codeinfra", "stack", "rm", "--yes")
...
func TestStackOutputsJSON(t *testing.T) {
	...
	e.RunCommand("yarn", "link", "@codeinfra/codeinfra")
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "stack-outs")
	e.RunCommand("codeinfra", "up", "--non-interactive", "--yes", "--skip-preview")
	stdout, _ := e.RunCommand("codeinfra", "stack", "output", "--json")
	assert.Equal(t, `{
  "foo": 42,
  "xyz": "ABC"
}
...
// Tests basic configuration from the perspective of a Codeinfra program.
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
...
// Tests configuration error from the perspective of a Codeinfra program.
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
func TestConfigMissingJS(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
			...
			text2 := "\tplease set a value using the command `codeinfra config set --secret config_missing_js:notFound <value>`"
...
func TestConfigSecretsWarnNodeJS(t *testing.T) {
	...
	t.Skip("Temporarily skipping test until we've re-enabled the warning - codeinfra/codeinfra#7127")
	integration.ProgramTest(t, &integration.ProgramTestOptions{
...
func TestResourceWithSecretSerializationNodejs(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
			...
			//   1. One named `withSecret` who's prefix property should be secret, specified via `codeinfra.secret()`.
...
func TestPasswordlessPassphraseSecretsProvider(t *testing.T) {
	testOptions := integration.ProgramTestOptions{
		...
		Env:             []string{"CODEINFRA_CONFIG_PASSPHRASE=\"\""},
		Secrets: map[string]string{
	...
	workingTestOptions := testOptions.With(integration.ProgramTestOptions{
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
			t.Setenv("CODEINFRA_CONFIG_PASSPHRASE", "password")
...
func TestCloudSecretProvider(t *testing.T) {
	...
	awsKmsKeyAlias := os.Getenv("CODEINFRA_TEST_KMS_KEY_ALIAS")
	if awsKmsKeyAlias == "" {
		t.Skipf("Skipping: CODEINFRA_TEST_KMS_KEY_ALIAS is not set")
	}

	azureKeyVault := os.Getenv("CODEINFRA_TEST_AZURE_KEY")
	if azureKeyVault == "" {
		t.Skipf("Skipping: CODEINFRA_TEST_AZURE_KEY is not set")
	}

	gcpKmsKey := os.Getenv("CODEINFRA_TEST_GCP_KEY")
	if gcpKmsKey == "" {
		t.Skipf("Skipping: CODEINFRA_TEST_GCP_KEY is not set")
	}
	...
	// Run with default Codeinfra service backend
	//
	//nolint:paralleltest // ProgramTest calls t.Parallel()
...
func TestEnumOutputNode(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
			...
			assert.Equal(t, "Codeinfra Planters Inc.foo", stack.Outputs["myTreeFarmChanged"])
			assert.Equal(t, "My Burgundy Rubber tree is from Codeinfra Planters Inc.", stack.Outputs["mySentence"])
		},
	})
}
...
func TestConstructUnknownNode(t *testing.T) {
	t.Parallel()
	testConstructUnknown(t, "nodejs", "@codeinfra/codeinfra")
}
...
func TestConstructMethodsUnknownNode(t *testing.T) {
	t.Parallel()
	testConstructMethodsUnknown(t, "nodejs", "@codeinfra/codeinfra")
}

func TestConstructMethodsResourcesNode(t *testing.T) {
	t.Parallel()
	testConstructMethodsResources(t, "nodejs", "@codeinfra/codeinfra")
}

func TestConstructMethodsErrorsNode(t *testing.T) {
	t.Parallel()
	testConstructMethodsErrors(t, "nodejs", "@codeinfra/codeinfra")
}

func TestConstructMethodsProviderNode(t *testing.T) {
	t.Parallel()
	testConstructMethodsProvider(t, "nodejs", "@codeinfra/codeinfra")
}
...
func TestComponentProviderSchemaNode(t *testing.T) {
	t.Parallel()
	path := filepath.Join("component_provider_schema", "testcomponent", "codeinfra-resource-testcomponent")
...
func TestMainOverridesPackageJSON(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
			assert.NotNil(t, stack.Outputs)
			assert.Equal(t, "This is the entrypoint from Codeinfra.yaml", stack.Outputs["text"])
		},
	})
}
...
func TestNpmWorkspace(t *testing.T) {
	preparePropject := func(projinfo *engine.Projinfo) error {
		...
		// want to test `codeinfra install` and ensure that it works with npm
		// workspaces.
		return nil
	}
	...
	require.NoError(t, pt.RunCodeinfraCommand("install"), "install")
...
func TestYarnWorkspace(t *testing.T) {
	t.Setenv("CODEINFRA_PREFER_YARN", "true")
	preparePropject := func(projinfo *engine.Projinfo) error {
		// The default nodejs prepare uses yarn to link dependencies.
		// For this test we don't want to test the current SDK, instead we
		// want to test `codeinfra install` and ensure that it works with yarn
		// workspaces.
		return nil
	}
	pt := integration.ProgramTestManualLifeCycle(t, &integration.ProgramTestOptions{
...
func TestPnpmWorkspace(t *testing.T) {
	preparePropject := func(projinfo *engine.Projinfo) error {
		// The default nodejs prepare uses yarn to link dependencies.
		// For this test we don't want to test the current SDK, instead we
		// want to test `codeinfra install` and ensure that it works with pnpm
		// workspaces.
		return nil
	}
	pt := integration.ProgramTestManualLifeCycle(t, &integration.ProgramTestOptions{
...
func TestInstallWithMain(t *testing.T) {
	preparePropject := func(projinfo *engine.Projinfo) error {
		// The default nodejs prepare uses yarn to link dependencies.
		// For this test we don't want to test the current SDK, instead we
		// want to test `codeinfra install` and ensure that it works for projects
		// with a `main` property.
		return nil
	}
	pt := integration.ProgramTestManualLifeCycle(t, &integration.ProgramTestOptions{
		Dir:            filepath.Join("nodejs", "codeinfra-main"),
		Quick:          true,
		PrepareProject: preparePropject,
	})
...
func TestTranspileOnly(t *testing.T) {
	...
	for _, test := range []string{"tsconfig-no-check", "swc"} {
		...
		t.Run(test, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
			e.RunCommand("codeinfra", "stack", "init", stackName)
			e.RunCommand("codeinfra", "stack", "select", stackName)
			e.RunCommand("codeinfra", "up", "--yes")
			_, _ = e.RunCommand("codeinfra", "destroy", "--skip-preview", "--refresh=true")
		})
	}
}
...
func TestNoNegativeTimingsOnRefresh(t *testing.T) {
	...
	e.RunCommand("yarn", "install")
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "negative-timings")
	e.RunCommand("codeinfra", "stack", "select", "negative-timings")
	e.RunCommand("codeinfra", "up", "--yes")
	stdout, _ := e.RunCommand("codeinfra", "destroy", "--skip-preview", "--refresh=true")
	// Assert there are no negative times in the output.
	assert.NotContainsf(t, stdout, " (-",
		"`codeinfra destroy --skip-preview --refresh=true` contains a negative time")
}
...
func TestAboutNodeJS(t *testing.T) {
	...
	e.RunCommand("yarn", "install")
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "about-nodejs")
	e.RunCommand("codeinfra", "stack", "select", "about-nodejs")
	stdout, stderr := e.RunCommand("codeinfra", "about")
	e.RunCommand("codeinfra", "stack", "rm", "--yes")
...
func TestConstructOutputValuesNode(t *testing.T) {
	t.Parallel()
	testConstructOutputValues(t, "nodejs", "@codeinfra/codeinfra")
}
...
func TestTSConfigOption(t *testing.T) {
	...
	e.RunCommand("yarn", "install")
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "stack", "select", "tsconfg", "--create")
	e.RunCommand("codeinfra", "preview")
}
...
func TestUnsafeSnapshotManagerRetainsResourcesOnError(t *testing.T) {
	...
	t.Run("Check with experimental flag", func(t *testing.T) {
		integration.ProgramTest(t, &integration.ProgramTestOptions{
			...
			Dependencies: []string{"@codeinfra/codeinfra"},
			Env: []string{
				"CODEINFRA_EXPERIMENTAL=1",
				"CODEINFRA_SKIP_CHECKPOINTS=1",
			},
			Quick: true,
...
// TestResourceRefsGetResourceNode tests that invoking the built-in 'codeinfra:codeinfra:getResource' function
...
func TestCustomResourceTypeNameDynamicNode(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
			...
			typ := urn.Type().String()
			assert.Equal(t, "codeinfra-nodejs:dynamic/custom-provider:CustomResource", typ)
		},
	})
}
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/12301
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
...
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
func TestCodeinfraConfig(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		Dir:          filepath.Join("dynamic", "nodejs-codeinfra-config"),
		Dependencies: []string{"@codeinfra/codeinfra"},
		Config: map[string]string{
			"codeinfra-nodejs:id": "testing123",
		},
		ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
			assert.Len(t, stack.Outputs, 1)
...
func TestConstructProviderPropagationNode(t *testing.T) {
	...
	testConstructProviderPropagation(t, "nodejs", []string{"@codeinfra/codeinfra"})
}

func TestConstructProviderExplicitNode(t *testing.T) {
	t.Parallel()

	testConstructProviderExplicit(t, "nodejs", []string{"@codeinfra/codeinfra"})
}

// Regression test for https://github.com/khulnasoft/codeinfra/issues/7376
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
...
// Tests basic environments from the perspective of a Codeinfra program.
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
		...
		CreateEnvironments: []integration.Environment{{
			...
			Definition: map[string]any{
				"values": map[string]any{
					"codeinfraConfig": map[string]any{
						"aConfigValue": "this value is a value",
...
// Tests merged environments from the perspective of a Codeinfra program.
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
...
func TestParameterizedNode(t *testing.T) {
	...
	t.Setenv("CODEINFRA_TEST_MULTI_ARGUMENT_INPUTS", "true")
	...
	_, _ = e.RunCommand("codeinfra", "package", "gen-sdk", "../../../testprovider", "pkg", "--language", "nodejs", "--local")
	...
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		PrePrepareProject: func(project *engine.Projinfo) error {
			// Patch up the local SDK's package.json so its codeinfra dependency points to the local core SDK
			coreSDK, err := filepath.Abs(filepath.Join("..", "..", "sdk", "nodejs", "bin"))
			if err != nil {
				return err
			}
			...
			deps["@codeinfra/codeinfra"] = "file:" + coreSDK
...
func TestPackageAddNode(t *testing.T) {
	...
	for _, packageManager := range []string{"npm", "yarn", "pnpm"} {
		t.Run(packageManager, func(t *testing.T) {
			...
			_, _ = e.RunCommand("codeinfra", "plugin", "install", "resource", "random")
			_, _ = e.RunCommand("codeinfra", "package", "add", "random")
			...
			assert.True(t, ok)
			cf, ok := dependencies["@codeinfra/random"]
			assert.True(t, ok)
			cf, ok = cf.(string)
			assert.True(t, ok)
...
func TestConvertTerraformProviderNode(t *testing.T) {
	...
	_, _ = e.RunCommand("codeinfra", "plugin", "install", "converter", "terraform")
	_, _ = e.RunCommand("codeinfra", "plugin", "install", "resource", "terraform-provider")
	_, _ = e.RunCommand("codeinfra", "convert", "--from", "terraform", "--language", "typescript", "--out", "nodedir")
	...
	dependencies, ok := packagesJSON["dependencies"].(map[string]any)
	assert.True(t, ok)
	cf, ok := dependencies["@codeinfra/supabase"]
	assert.True(t, ok)
	cf, ok = cf.(string)
	assert.True(t, ok)
...
func TestConstructFailuresNode(t *testing.T) {
	t.Parallel()
	testConstructFailures(t, "nodejs", "@codeinfra/codeinfra")
}

func TestCallFailuresNode(t *testing.T) {
	t.Parallel()
	testCallFailures(t, "nodejs", "@codeinfra/codeinfra")
}
...
func TestAutonaming(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			config: map[string]string{
				"codeinfra:autonaming.mode": "verbatim",
			},
			experimental: false,
			autoName:     "default-name",
		},
		...
		},
		{
			...
			config: map[string]string{
				"codeinfra:autonaming.providers.testprovider.pattern": "${config.foo}-${name}",
				"foo": "bar",
			},
			experimental: true,
			autoName:     "bar-test1",
	...
	for _, tc := range testCases {
		...
		env := []string{}
		if tc.experimental {
			env = append(env, "CODEINFRA_EXPERIMENTAL=1")
		}
