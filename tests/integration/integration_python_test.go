...
import (
	...
	pygen "github.com/khulnasoft/codeinfra/pkg/v3/codegen/python"
...
func TestConfigMissingPython(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
			...
			text2 := "\tplease set a value using the command `codeinfra config set --secret config_missing_py:notFound <value>`"
...
func TestPython3NotInstalled(t *testing.T) {
	// TODO[codeinfra/codeinfra#6304]
	t.Skip("Temporarily skipping failing test - codeinfra/codeinfra#6304")
	stderr := &bytes.Buffer{}
	...
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		Quick: true,
		Env: []string{
			// Note: we use CODEINFRA_PYTHON_CMD to override the default behavior of searching
			...
			"CODEINFRA_PYTHON_CMD=" + badPython,
		},
		ExpectFailure: true,
		Stderr:        stderr,
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
...
func TestCustomResourceTypeNameDynamicPython(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
			...
			typ := urn.Type().String()
			assert.Equal(t, "codeinfra-python:dynamic/custom-provider:CustomResource", typ)
		},
	})
}
...
func TestPythonPylint(t *testing.T) {
	t.Skip("Temporarily skipping test - codeinfra/codeinfra#4849")
...
func TestConstructSlowPython(t *testing.T) {
	...
	// module to run `yarn install && yarn link @codeinfra/codeinfra` in the Python program's directory, allowing
...
func TestConstructComponentWithIdOutputPython(t *testing.T) {
	...
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
			...
			stack := findResource("codeinfra:codeinfra:Stack", stackInfo.Deployment.Resources)
...
func TestPythonAwaitOutputs(t *testing.T) {
	...
	t.Run("CreateWithinApply", func(t *testing.T) {
		integration.ProgramTest(t, &integration.ProgramTestOptions{
			...
			ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
				...
				for _, evt := range stack.Events {
					if evt.DiagnosticEvent != nil {
						if strings.Contains(evt.DiagnosticEvent.Message, "codeinfra-python:dynamic:Resource::magic_string") {
							sawUrn = true
						}
					}
				}
	...
	// https://github.com/khulnasoft/codeinfra/issues/16055.
	//
	//nolint:paralleltest // ProgramTest calls t.Parallel()
...
func TestComponentProviderSchemaPython(t *testing.T) {
	t.Parallel()
	// TODO[https://github.com/khulnasoft/codeinfra/issues/12365] we no longer have shim files so there's no native
	...
	path := filepath.Join("component_provider_schema", "testcomponent-python", "codeinfra-resource-testcomponent")
	if runtime.GOOS == WindowsOS {
		path += ".cmd"
	}
...
func TestAboutPython(t *testing.T) {
	...
	stdout, _ := e.RunCommand("codeinfra", "about")
	// Assert we parsed the dependencies
	assert.Contains(t, stdout, "codeinfra-kubernetes")
...
// TestResourceRefsGetResourcePython tests that invoking the built-in 'codeinfra:codeinfra:getResource' function
...
// Regression test for https://github.com/khulnasoft/codeinfra/issues/9411
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
...
func TestFailsOnImplicitDependencyCyclesPython(t *testing.T) {
	t.Skip("Temporarily skipping flakey test - codeinfra/codeinfra#14708")
...
func TestParameterizedPython(t *testing.T) {
	...
	_, _ = e.RunCommand("codeinfra", "package", "gen-sdk", "../../../testprovider", "pkg", "--language", "python", "--local")
	...
	for _, pm := range []struct {
		...
		t.Run(pm.packageManager, func(t *testing.T) {
			...
			if pm.usePyProject {
				...
				pkgSpec, ok := data["codeinfra-random"]
				assert.True(t, ok)
...
func TestConvertTerraformProviderPython(t *testing.T) {
	...
	_, _ = e.RunCommand("codeinfra", "convert", "--from", "terraform", "--language", "python", "--out", "pydir")
...
func TestConfigGetterOverloads(t *testing.T) {
	...
	e.RunCommand("codeinfra", "config", "set", "foo", "bar")
	e.RunCommand("codeinfra", "config", "set", "foo_int", "42")
	e.RunCommand("codeinfra", "config", "set", "--secret", "foo_secret", "3")
...
func TestDynamicProviderPython(t *testing.T) {
	...
	for _, toolchain := range []string{"pip", "uv", "poetry"} {
		...
		t.Run(toolchain, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "stack", "init", stackName)
			defer e.RunCommand("codeinfra", "stack", "rm", "--yes", "--stack", stackName)
			e.RunCommand("codeinfra", "up", "--yes", "--skip-preview")
			e.RunCommand("codeinfra", "dn", "--yes", "--skip-preview")
		})
	}
}
...
// to stderr.
// https://github.com/khulnasoft/codeinfra/issues/18176
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
func TestRegress18176(t *testing.T) {
	...
	stderr := &bytes.Buffer{}
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		Env: []string{"CODEINFRA_DISABLE_AUTOMATIC_PLUGIN_ACQUISITION=false"},
