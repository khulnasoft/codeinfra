...
func main() {
	codeinfra.Run(func(ctx *codeinfra.Context) error {
		...
		_, err := NewRandom(ctx, "res1", &RandomArgs{Length: codeinfra.Int(5)}, codeinfra.Transformations([]codeinfra.ResourceTransformation{
			func(rta *codeinfra.ResourceTransformationArgs) *codeinfra.ResourceTransformationResult {
				fmt.Printf("res1 transformation")
				return &codeinfra.ResourceTransformationResult{
					Props: rta.Props,
					Opts:  append(rta.Opts, codeinfra.AdditionalSecretOutputs([]string{"result"})),
				}
			},
		}))
		if err != nil {
		...
		_, err = NewMyComponent(ctx, "res2", codeinfra.Transformations([]codeinfra.ResourceTransformation{
			func(rta *codeinfra.ResourceTransformationArgs) *codeinfra.ResourceTransformationResult {
		...
		err = ctx.RegisterStackTransformation(func(rta *codeinfra.ResourceTransformationArgs) *codeinfra.ResourceTransformationResult {
			...
			if rta.Type == "testprovider:index:Random" {
				...
				props.Prefix = codeinfra.String("stackDefault")
		...
		_, err = NewMyComponent(ctx, "res4", codeinfra.Transformations([]codeinfra.ResourceTransformation{
			func(rta *codeinfra.ResourceTransformationArgs) *codeinfra.ResourceTransformationResult {
				fmt.Printf("res4 transformation")
				if rta.Type == "testprovider:index:Random" {
					props := rta.Props.(*RandomArgs)
					props.Prefix = codeinfra.String("default1")
			...
			func(rta *codeinfra.ResourceTransformationArgs) *codeinfra.ResourceTransformationResult {
				...
				if rta.Type == "testprovider:index:Random" {
					props := rta.Props.(*RandomArgs)
					props.Prefix = codeinfra.String("default2")
		...
		transformChild1DependsOnChild2 := func(rta *codeinfra.ResourceTransformationArgs) *codeinfra.ResourceTransformationResult {
			if strings.HasSuffix(rta.Name, "-child2") {
				...
				child2Result := codeinfrax.Flatten[string, codeinfrax.Output[string]](
					codeinfrax.ApplyErr[int, codeinfrax.Output[string]](
						props.Length.ToIntOutput().ToOutput(ctx.Context()),
						func(input int) (codeinfrax.Output[string], error) {
							var none codeinfrax.Output[string]
				...
				props.Prefix = child2Result.Untyped().(codeinfra.StringInput)
		...
		_, err = NewMyOtherComponent(ctx, "res5", codeinfra.Transformations([]codeinfra.ResourceTransformation{transformChild1DependsOnChild2}))
		if err != nil {
			return err
		}
