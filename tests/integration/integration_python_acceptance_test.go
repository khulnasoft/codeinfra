	...
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		EditDirs: []integration.EditDir{{
			...
			ExtraRuntimeValidation: func(t *testing.T, stack integration.RuntimeValidationStackInfo) {
				...
				// Regression testing the workaround for https://github.com/khulnasoft/codeinfra/issues/8265
...
func TestConstructComponentConfigureProviderPython(t *testing.T) {
	...
	runComponentSetup(t, testDir)
	codeinfraRoot, err := filepath.Abs("../..")
	require.NoError(t, err)
	codeinfraPySDK := filepath.Join("..", "..", "sdk", "python")
	componentSDK := filepath.Join(codeinfraRoot, "pkg/codegen/testing/test/testdata/methods-return-plain-resource/python")
	...
	opts = opts.With(integration.ProgramTestOptions{
		...
		Dependencies: []string{codeinfraPySDK, componentSDK},
		NoParallel:   true,
	})
	integration.ProgramTest(t, &opts)
}

// Regresses https://github.com/khulnasoft/codeinfra/issues/6471
func TestAutomaticVenvCreation(t *testing.T) {
	...
	// handling by the codeinfra CLI itself.
	...
	check := func(t *testing.T, venvPathTemplate string, dir string) {
		...
		// replace "virtualenv: venv" with "virtualenv: ${venvPath}" in Codeinfra.yaml
		codeinfraYaml := filepath.Join(e.RootPath, "Codeinfra.yaml")

		oldYaml, err := os.ReadFile(codeinfraYaml)
		if err != nil {
			t.Error(err)
			return
		}
		...
		if err := os.WriteFile(codeinfraYaml, newYaml, 0o600); err != nil {
			t.Error(err)
			return
		}

		t.Logf("Wrote Codeinfra.yaml:\n%s\n", string(newYaml))
		...
		e.RunCommand("codeinfra", "stack", "init", "teststack")
	...
	t.Run("TestInitVirtualEnvBeforePythonVersionCheck", func(t *testing.T) {
		...
		e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
		e.RunCommand("codeinfra", "stack", "init", "teststack")
		stdout, stderr, _ := e.GetCommandResults("codeinfra", "preview")
		// codeinfra/codeinfra#9175
...
//nolint:paralleltest // Poetry causes issues when run in parallel on windows. See codeinfra/codeinfra#17183
...
func TestPoetryInstallParentDirectory(t *testing.T) {
	...
	// Run from the subdir with the Codeinfra.yaml file
...
func TestUv(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// for Poetry https://github.com/khulnasoft/codeinfra/pull/17337
		//nolint:paralleltest
		t.Run(test.template, func(t *testing.T) {
			...
			e.RunCommand("codeinfra", "install")
			e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
			e.RunCommand("codeinfra", "stack", "init", ptesting.RandomStackName())
			e.RunCommand("codeinfra", "preview")
...
func TestMypySupport(t *testing.T) {
	...
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		// mypy doesn't really support editable packages, so we install codeinfra normally from pip for this test.
		Quick:                  true,
		ExpectFailure:          true,
...
func TestPyrightSupport(t *testing.T) {
	...
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		// to match the mypy test we install codeinfra normally from pip for this test.
		Quick:                  true,
		ExpectFailure:          true,
		ExtraRuntimeValidation: validation,
	})
...
func TestNewPythonUsesPip(t *testing.T) {
	...
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	stdout, _ := e.RunCommand("codeinfra", "new", "python", "--force", "--non-interactive", "--yes", "--generate-only")

	require.Contains(t, stdout, "codeinfra install")
...
func TestNewPythonUsesPipNonInteractive(t *testing.T) {
	...
	t.Setenv("CODEINFRA_TEST_INTERACTIVE", "1")

	e := ptesting.NewEnvironment(t)
	defer e.DeleteIfNotFailed()

	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	stdout, _ := e.RunCommand("codeinfra", "new", "python", "--force", "--yes", "--generate-only")
...
func TestNewPythonChoosePoetry(t *testing.T) {
	...
	e.RunCommand("codeinfra", "new", "python", "--force", "--generate-only",
		"--name", "test_project",
...
func TestNewPythonRuntimeOptions(t *testing.T) {
	...
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "new", "python", "--force", "--non-interactive", "--yes", "--generate-only",
		"--name", "test_project",
		"--description", "A python test using poetry as toolchain",
		"--stack", "test",
...
in-project = true`
	...
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	out, _ := e.RunCommand("codeinfra", "new", template, "--force", "--non-interactive", "--yes",
		"--name", "test_project",
		"--description", "A python test using poetry as toolchain",
		"--stack", "test",
...
codeinfra = ">=3.0.0,<4.0.0"
python = "^3.9"
`, string(b))
}

// Regression test for https://github.com/khulnasoft/codeinfra/issues/17877
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
func TestUvWindowsError(t *testing.T) {
	...
	// This test will hang on Windows without the fix for https://github.com/khulnasoft/codeinfra/issues/17877
	done := make(chan struct{})
	var stdout string
	go func() {
		e.ImportDirectory(filepath.Join("python", "uv-with-error"))
		e.RunCommand("codeinfra", "install")
		e.RunCommand("codeinfra", "plugin", "install", "resource", "random")
		e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
		e.RunCommand("codeinfra", "stack", "init", ptesting.RandomStackName())
		stdout, _ = e.RunCommandExpectError("codeinfra", "preview")
