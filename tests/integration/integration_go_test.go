...
import (
	...
	"github.com/grapl-security/codeinfra-hcp/sdk/go/hcp"
	...
	"github.com/khulnasoft/codeinfra/sdk/v3/go/auto"
...
// This checks that the buildTarget option for Codeinfra Go programs does build a binary.
func TestBuildTarget(t *testing.T) {
	...
	e.RunCommand("codeinfra", "stack", "init", "go-build-target-test-stack")
	e.RunCommand("codeinfra", "stack", "select", "go-build-target-test-stack")
...
// This checks that the CODEINFRA_GO_USE_RUN=true flag is triggering go run by checking the `exit status`
...
func TestGoRunEnvFlag(t *testing.T) {
	stderr := &bytes.Buffer{}
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		Env: []string{"CODEINFRA_GO_USE_RUN=true"},
		Dir: filepath.Join("go", "go-exit-5"),
		Dependencies: []string{
			"github.com/khulnasoft/codeinfra/sdk/v3",
		},
...
// TestEmptyGoRunMain exercises the 'go run' invocation path with a 'main' entrypoint specified in Codeinfra.yml
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
...
// Tests basic configuration from the perspective of a Codeinfra Go program.
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
func TestConfigBasicGo(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		Dir:          filepath.Join("config_basic", "go"),
		Dependencies: []string{"github.com/khulnasoft/codeinfra/sdk/v3"},
		Quick:        true,
...
// Tests configuration error from the perspective of a Codeinfra Go program.
//
//nolint:paralleltest // ProgramTest calls t.Parallel()
func TestConfigMissingGo(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
			...
			text2 := "\tplease set a value using the command `codeinfra config set --secret config_missing_go:notFound <value>`"
...
func TestConstructSlowGo(t *testing.T) {
	...
	// Until we've addressed this, set CODEINFRA_TEST_YARN_LINK_CODEINFRA, which tells the integration test
	// module to run `yarn install && yarn link @codeinfra/codeinfra` in the Go program's directory, allowing
	// the Node.js dynamic provider plugin to load.
	// When the underlying issue has been fixed, the use of this environment variable inside the integration
	// test module should be removed.
	const testYarnLinkCodeinfraEnv = "CODEINFRA_TEST_YARN_LINK_CODEINFRA=true"

	testDir := "construct_component_slow"
	runComponentSetup(t, testDir)

	opts := &integration.ProgramTestOptions{
		Env: []string{testYarnLinkCodeinfraEnv},
...
func TestConstructPlainGo(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// When the underlying issue has been fixed, the use of this environment variable inside the integration
			// test module should be removed.
			env: []string{"CODEINFRA_TEST_YARN_LINK_CODEINFRA=true"},
		},
		{
...
func TestConstructUnknownGo(t *testing.T) {
	t.Parallel()
	testConstructUnknown(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}
		...
		t.Run(test.componentDir, func(t *testing.T) {
			...
			integration.ProgramTest(t, &integration.ProgramTestOptions{
				...
				ExtraRuntimeValidation: func(t *testing.T, stackInfo integration.RuntimeValidationStackInfo) {
					...
					// TODO[codeinfra/codeinfra#12471]: Only the Go SDK has been fixed such that rehydrated
...
func TestConstructMethodsUnknownGo(t *testing.T) {
	t.Parallel()
	testConstructMethodsUnknown(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}

func TestConstructMethodsResourcesGo(t *testing.T) {
	t.Parallel()
	testConstructMethodsResources(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}

func TestConstructMethodsErrorsGo(t *testing.T) {
	t.Parallel()
	testConstructMethodsErrors(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}

func TestConstructMethodsProviderGo(t *testing.T) {
	t.Parallel()
	testConstructMethodsProvider(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}
...
func TestGetResourceGo(t *testing.T) {
	...
	t.Setenv("CODEINFRA_DISABLE_AUTOMATIC_PLUGIN_ACQUISITION", "false")
...
func TestComponentProviderSchemaGo(t *testing.T) {
	t.Parallel()
	// TODO[https://github.com/khulnasoft/codeinfra/issues/12365] We no longer build the go-component in
	...
	path := filepath.Join("component_provider_schema", "testcomponent-go", "codeinfra-resource-testcomponent")
...
func TestTracePropagationGo(t *testing.T) {
	...
	store, err := ReadMemoryStoreFromFile(filepath.Join(dir, "codeinfra-update-initial.trace"))
	assert.NoError(t, err)
...
func TestAboutGo(t *testing.T) {
	...
	e.RunCommand("codeinfra", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("codeinfra", "stack", "init", "about-stack")
	e.RunCommand("codeinfra", "stack", "select", "about-stack")
	stdout, _ := e.RunCommand("codeinfra", "about", "-t")

	// Assert we parsed the dependencies
	assert.Contains(t, stdout, "github.com/khulnasoft/codeinfra/sdk/v3")
...
func TestConstructOutputValuesGo(t *testing.T) {
	t.Parallel()
	testConstructOutputValues(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}
...
// TestResourceRefsGetResourceGo tests that invoking the built-in 'codeinfra:codeinfra:getResource' function
...
func TestConstructProviderPropagationGo(t *testing.T) {
	...
	testConstructProviderPropagation(t, "go", []string{"github.com/khulnasoft/codeinfra/sdk/v3"})
}

func TestConstructResourceOptionsGo(t *testing.T) {
	t.Parallel()

	testConstructResourceOptions(t, "go", []string{"github.com/khulnasoft/codeinfra/sdk/v3"})
}

// Regression test for https://github.com/khulnasoft/codeinfra/issues/13301.
// The reproduction is a bit involved:
//
//   - Set up a fake Codeinfra Go project that imports a specific non-Codeinfra plugin.
//     Specifically, the plugin MUST NOT be imported by any Go file in the project.
//   - Install that plugin with 'codeinfra plugin install'.
//   - Run a Go Automation program that uses that plugin.
//
// The issue in #13301 was that this plugin would not be downloaded by `codeinfra plugin install`,
...
func TestAutomation_externalPluginDownload_issue13301(t *testing.T) {
	...
	// Plugins are installed globally in CODEINFRA_HOME.
	// We will set that to a temporary directory,
	// so this is not polluted by other tests.
	codeinfraHome := filepath.Join(e.RootPath, ".codeinfra-home")
	require.NoError(t, os.MkdirAll(codeinfraHome, 0o700))

	// The commands that follow will make gRPC requests that
	// we don't have a lot of visibility into.
	// If we run the Codeinfra CLI with CODEINFRA_DEBUG_GRPC set to a path,
	...
	e.Env = append(e.Env,
		"CODEINFRA_HOME="+codeinfraHome,
		"CODEINFRA_DEBUG_GRPC="+grpcLog)
	e.RunCommand("codeinfra", "plugin", "install")
	...
	ws, err := auto.NewLocalWorkspace(ctx,
		...
		auto.WorkDir(e.CWD),
		auto.CodeinfraHome(codeinfraHome),
		auto.EnvVars(map[string]string{
			"CODEINFRA_CONFIG_PASSPHRASE": "not-a-real-passphrase",
			"CODEINFRA_DEBUG_COMMANDS":    "true",
			"CODEINFRA_CREDENTIALS_PATH":  e.RootPath,
			"CODEINFRA_DEBUG_GRPC":        grpcLog,
		}),
	)
	require.NoError(t, err)

	ws.SetProgram(func(ctx *codeinfra.Context) error {
...
func TestConstructProviderExplicitGo(t *testing.T) {
	...
	testConstructProviderExplicit(t, "go", []string{"github.com/khulnasoft/codeinfra/sdk/v3"})
}
...
func TestParameterizedGo(t *testing.T) {
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		PrePrepareProject: func(info *engine.Projinfo) error {
			...
			_, _ = e.RunCommand("codeinfra", "package", "gen-sdk", path, "pkg", "--language", "go")
			return nil
		},
	})
}
...
func TestPackageAddGoParameterized(t *testing.T) {
	...
	_, _ = e.RunCommand("codeinfra", "package", "add", "terraform-provider", "hashicorp/random")
	...
	assert.NoError(t, err)
	assert.Equal(t, "github.com/khulnasoft/codeinfra-terraform-provider/sdks/go/random/v3", packageMod.Module.Mod.Path)
	...
	for _, r := range gomod.Replace {
		if r.New.Path == "./sdks/random" && r.Old.Path == "github.com/khulnasoft/codeinfra-terraform-provider/sdks/go/random/v3" {
			containsRename = true
		}
	}
...
func TestConvertTerraformProviderGo(t *testing.T) {
	...
	_, _ = e.RunCommand("codeinfra", "plugin", "install", "resource", "terraform-provider", "0.6.0")
	_, _ = e.RunCommand("codeinfra", "convert", "--from", "terraform", "--language", "go", "--out", "godir")
	...
	for _, r := range gomod.Replace {
		if r.New.Path == "./sdks/supabase" && r.Old.Path ==
			"github.com/khulnasoft/codeinfra-terraform-provider/sdks/go/supabase" {
			containsRename = true
		}
	}
...
func TestDebuggerAttach(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		e.Env = append(e.Env, "CODEINFRA_DEBUG_COMMANDS=true")
		e.RunCommand("codeinfra", "stack", "init", "debugger-test")
		e.RunCommand("codeinfra", "stack", "select", "debugger-test")
		e.RunCommand("codeinfra", "preview", "--attach-debugger",
...
func TestConstructFailuresGo(t *testing.T) {
	t.Parallel()
	testConstructFailures(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}

func TestCallFailuresGo(t *testing.T) {
	t.Parallel()
	testCallFailures(t, "go", "github.com/khulnasoft/codeinfra/sdk/v3")
}
...
func TestRunPlugin(t *testing.T) {
	...
	e.RunCommand("go", "mod", "edit", "-replace=github.com/khulnasoft/codeinfra/sdk/v3="+sdkPath)
	e.RunCommand("go", "mod", "tidy")
	e.RunCommand("codeinfra", "stack", "init", "runplugin-test")
	e.RunCommand("codeinfra", "stack", "select", "runplugin-test")
	e.RunCommand("codeinfra", "preview")
}
