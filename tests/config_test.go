...
const codeinfraTestOrg = "moolumi"
...
func TestConfigCommands(t *testing.T) {
	...
	t.Run("SanityTest", func(t *testing.T) {
		...
		stdout, _ := e.RunCommand("codeinfra", "config")
		assert.Equal(t, "KEY  VALUE", strings.Trim(stdout, "\r\n"))

		// set a bunch of config
		e.RunCommand("codeinfra", "config", "set-all",
		...
		stdout, _ = e.RunCommand("codeinfra", "config", "get", "key1")
		assert.Equal(t, "value1", strings.Trim(stdout, "\r\n"))

		stdout, _ = e.RunCommand("codeinfra", "config", "get", "outer.inner")
		assert.Equal(t, "value2", strings.Trim(stdout, "\r\n"))

		stdout, _ = e.RunCommand("codeinfra", "config", "get", "my_token")
		assert.Equal(t, "my_secret_token", strings.Trim(stdout, "\r\n"))

		stdout, _ = e.RunCommand("codeinfra", "config", "get", "myList[0]")
		assert.Equal(t, "foo", strings.Trim(stdout, "\r\n"))

		// check that the nested config does not exist because we didn't use path
		_, stderr := e.RunCommandExpectError("codeinfra", "config", "get", "outer")
		assert.Equal(t,
			"error: configuration key 'outer' not found for stack '"+stackName+"'",
			strings.Trim(stderr, "\r\n"))

		_, stderr = e.RunCommandExpectError("codeinfra", "config", "get", "myList")
		assert.Equal(t,
			"error: configuration key 'myList' not found for stack '"+stackName+"'",
			strings.Trim(stderr, "\r\n"))

		// set the nested config using --path
		e.RunCommand("codeinfra", "config", "set-all", "--path",
			"--plaintext", "outer.inner=value2",
			"--plaintext", "myList[0]=foo")

		// check that the nested config now exists
		stdout, _ = e.RunCommand("codeinfra", "config", "get", "outer")
		assert.Equal(t, "{\"inner\":\"value2\"}", strings.Trim(stdout, "\r\n"))

		stdout, _ = e.RunCommand("codeinfra", "config", "get", "myList")
		assert.Equal(t, "[\"foo\"]", strings.Trim(stdout, "\r\n"))

		// remove the nested config values
		e.RunCommand("codeinfra", "config", "rm-all", "--path", "outer.inner", "myList[0]")
		...
		e.RunCommand("codeinfra", "config", "rm-all",
		...
		stdout, _ = e.RunCommand("codeinfra", "config", "get", "=some-weird=key=")
		assert.Equal(t, "value3", strings.Trim(stdout, "\r\n"))

		// remove the config key
		e.RunCommand("codeinfra", "config", "rm-all", "foo=", "=some-weird=key=")
	...
	t.Run("YAMLTest", func(t *testing.T) {
		...
		e.RunCommand("codeinfra", "config", "set",
			"a", "A",
		)
...
config:
  codeinfra-test:a: A
$`
		b, err := os.ReadFile(filepath.Join(e.CWD, "Codeinfra."+stackName+".yaml"))
...
config:
  first-value:
	...
	stackName := ptesting.RandomStackName()
	e.RunCommand("codeinfra", "stack", "init", stackName)
	stdout, _ := e.RunCommand("codeinfra", "config", "get", "first-value")
...
runtime: go`
	...
	e.RunCommand("codeinfra", "stack", "init", stackName)
	e.RunCommand("codeinfra", "config", "set", "Bconfig", "one")
	e.RunCommand("codeinfra", "config", "set", "Aconfig", "shouldBeAtEnd")
	e.RunCommand("codeinfra", "config", "set", "Cconfig", "shouldAlsoBeAtEnd")
	e.RunCommand("codeinfra", "config", "set", "Bconfig", "shouldOverWrite")
...
config:
  codeinfra-test:Bconfig: shouldOverWrite
  codeinfra-test:Aconfig: shouldBeAtEnd
  codeinfra-test:Cconfig: shouldAlsoBeAtEnd`
...
runtime: go`
	...
	e.RunCommand("codeinfra", "stack", "init", stackName)
	e.RunCommand("codeinfra", "config", "set", "Aconfig", "shouldBeRemoved")
	e.RunCommand("codeinfra", "config", "set", "Bconfig", "shouldRemain")
	e.RunCommand("codeinfra", "config", "rm", "Aconfig")
	...
	expectedRegex := `encryptionsalt: .*
config:
  codeinfra-test:Bconfig: shouldRemain`

	assert.Regexp(t, regexp.MustCompile(expectedRegex), strings.TrimSpace(string(b)))

	e.RunCommand("codeinfra", "config", "rm", "Bconfig")
...
config:
  ...
  third-value:
	...
	e.RunCommand("codeinfra", "stack", "init", stackName)
	e.RunCommand("codeinfra", "config", "set", "second-value", "second")
	stdout, _ := e.RunCommand("codeinfra", "config", "--json")
	...
	assert.Equal(t, "first", config["codeinfra-test:first-value"].(map[string]interface{})["value"])
	assert.Equal(t, "second", config["codeinfra-test:second-value"].(map[string]interface{})["value"])
	thirdValue := config["codeinfra-test:third-value"].(map[string]interface{})
...
func TestConfigCommandsUsingEnvironments(t *testing.T) {
	...
	integration.CreateBasicCodeinfraRepo(e)
	e.RunCommand("codeinfra", "org", "set-default", getTestOrg())
	stackName := ptesting.RandomStackName()
	e.RunCommand("codeinfra", "stack", "init", stackName)
	...
	e.RunCommand("codeinfra", "config", "env", "add", "secrets-test-env-DO-NOT-DELETE", "--yes")

	// just `codeinfra config`
	stdout, _ = e.RunCommand("codeinfra", "config")
	assert.Equal(t, `KEY          VALUE
test_secret  [secret]`, strings.Trim(stdout, "\r\n"))

	// `codeinfra config --show-secrets`
	stdout, _ = e.RunCommand("codeinfra", "config", "--show-secrets")
	assert.Equal(t, `KEY          VALUE
test_secret  this_is_my_secret`, strings.Trim(stdout, "\r\n"))

	// `codeinfra config --open`
	stdout, _ = e.RunCommand("codeinfra", "config", "--open")
	assert.Equal(t, `KEY          VALUE
test_secret  [secret]`, strings.Trim(stdout, "\r\n"))

	// `codeinfra config --show-secrets --open`
	stdout, _ = e.RunCommand("codeinfra", "config", "--show-secrets", "--open")
	assert.Equal(t, `KEY          VALUE
test_secret  this_is_my_secret`, strings.Trim(stdout, "\r\n"))

	// `codeinfra config --show-secrets --open=false`
	stdout, _ = e.RunCommand("codeinfra", "config", "--show-secrets", "--open=false")
	assert.Equal(t, `KEY          VALUE
test_secret  [unknown]`, strings.Trim(stdout, "\r\n"))

	// `codeinfra config get`
	stdout, _ = e.RunCommand("codeinfra", "config", "get", "test_secret")
	assert.Equal(t, "this_is_my_secret", strings.Trim(stdout, "\r\n"))

	// `codeinfra config get --open=false`
	stdout, _ = e.RunCommand("codeinfra", "config", "get", "test_secret", "--open=false")
	assert.Equal(t, "[unknown]", strings.Trim(stdout, "\r\n"))

	// delete the stack
	e.RunCommand("codeinfra", "stack", "rm", "-s", stackName, "--yes")
}
