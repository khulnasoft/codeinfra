...
func main() {
	codeinfra.Run(func(ctx *codeinfra.Context) error {
		...
		_ = bucket.ID().ApplyT(func(id string) (codeinfra.String, error) {
			return codeinfra.String(base64.StdEncoding.EncodeToString([]byte(id))), nil
		}).(codeinfra.StringOutput)
		_ = bucket.ID().ApplyT(func(id string) (codeinfra.String, error) {
			value, _ := base64.StdEncoding.DecodeString(id)
			return codeinfra.String(value), nil
		}).(codeinfra.StringOutput)
		secretValue := codeinfra.ToSecret("hello").(codeinfra.StringOutput)
		_ = codeinfra.Unsecret(secretValue).(codeinfra.StringOutput)
		...
		_, err = s3.NewBucketObject(ctx, "first", &s3.BucketObjectArgs{
			Bucket:      bucket.ID(),
			Source:      codeinfra.NewStringAsset(filebase64OrPanic("./base64.txt")),
			ContentType: codeinfra.String(fileMimeType),
			Tags: codeinfra.StringMap{
				"stack":   codeinfra.String(currentStack),
				"project": codeinfra.String(currentProject),
				"cwd":     codeinfra.String(workingDirectory),
			},
		})
		if err != nil {
			return err
		...
		_, err = s3.NewBucketObject(ctx, "second", &s3.BucketObjectArgs{
			Bucket: bucket.ID(),
			Source: codeinfra.NewStringAsset(filebase64sha256OrPanic("./base64.txt")),
		})
		if err != nil {
			return err
		}
		...
		_, err = s3.NewBucketObject(ctx, "third", &s3.BucketObjectArgs{
			Bucket: bucket.ID(),
			Source: codeinfra.NewStringAsset(sha1Hash("content")),
		})
		if err != nil {
			return err
		}
