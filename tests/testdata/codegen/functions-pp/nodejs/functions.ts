...
const secretValue = codeinfra.secret("hello");
const plainValue = codeinfra.unsecret(secretValue);
const currentStack = codeinfra.getStack();
const currentProject = codeinfra.getProject();
const workingDirectory = process.cwd();
const fileMimeType = mimeType("./base64.txt");
// using the filebase64 function
const first = new aws.s3.BucketObject("first", {
    bucket: bucket.id,
    source: new codeinfra.asset.StringAsset(fs.readFileSync("./base64.txt", { encoding: "base64" })),
...
const second = new aws.s3.BucketObject("second", {
    bucket: bucket.id,
    source: new codeinfra.asset.StringAsset(computeFilebase64sha256("./base64.txt")),
});
// using the sha1 function
const third = new aws.s3.BucketObject("third", {
    bucket: bucket.id,
    source: new codeinfra.asset.StringAsset(crypto.createHash('sha1').update("content").digest('hex')),
});
