...
index_html = aws.s3.BucketObject("index.html",
    bucket=my_bucket.id,
    source=codeinfra.FileAsset("./index.html"),
    content_type="text/html",
    acl="public-read",
    opts = codeinfra.ResourceOptions(depends_on=[
            public_access_block,
            ownership_controls,
        ]))
codeinfra.export("bucketName", my_bucket.id)
codeinfra.export("bucketEndpoint", my_bucket.website_endpoint.apply(lambda website_endpoint: f"http://{website_endpoint}"))
