...
describe("output-funcs", () => {
    it("funcWithAllOptionalInputsOutput", (done) => {
        checkTable(done, res => JSON.parse(res.r), [
            {given: sut.funcWithAllOptionalInputsOutput({}),
             expect: {}},
            {given: sut.funcWithAllOptionalInputsOutput({a: codeinfra.output("my-a")}),
             expect: {"a": "my-a"}},
            {given: sut.funcWithAllOptionalInputsOutput({a: codeinfra.output("my-a"),
                                                         b: codeinfra.output("my-b")}),
             expect: {"a": "my-a", "b": "my-b"}}
        ]);
    });

    // TODO[codeinfra/codeinfra#7973] Node codegen does not respect default
    // values at the moment, otherwise "b" parameter would receive the
    // default value from the schema.
    it("funcWithDefaultValueOutput", (done) => {
        checkTable(done, res => JSON.parse(res.r), [
            {given: sut.funcWithDefaultValueOutput({"a": codeinfra.output("my-a")}),
             expect: {"a": "my-a"}},
            {given: sut.funcWithDefaultValueOutput({"a": codeinfra.output("my-a"),
                                                    "b": codeinfra.output("my-b")}),
             expect: {"a": "my-a", "b": "my-b"}}
    ...
    it("funcWithListParamOutput", (done) => {
        ...
        checkTable(done, res => JSON.parse(res.r), [
            {given: sut.funcWithListParamOutput({}),
             expect: {}},
            {given: sut.funcWithListParamOutput({"a": codeinfra.output(l)}),
             expect: {"a": l}},
            {given: sut.funcWithListParamOutput({"a": codeinfra.output(l),
                                                 "b": codeinfra.output("my-b")}),
             expect: {"a": l, "b": "my-b"}},
    ...
    it("funcWithDictParamOutput", (done) => {
        ...
        checkTable(done, res => JSON.parse(res.r), [
            {given: sut.funcWithDictParamOutput({}),
             expect: {}},
            {given: sut.funcWithDictParamOutput({"a": codeinfra.output(d)}),
             expect: {"a": d}},
            {given: sut.funcWithDictParamOutput({"a": codeinfra.output(d),
                                                 "b": codeinfra.output("my-b")}),
             expect: {"a": d, "b": "my-b"}},
    ...
    it("listStorageAccountKeysOutput with unknown inputs returns unknown", (done) => {
        const output = sut.listStorageAccountKeysOutput({
            accountName: codeinfra.output("my-account-name"),
            resourceGroupName: codeinfra.output("my-resource-group-name"),
            expand: codeinfra.unknown as any,
        });
    ...
    it("getIntegrationRuntimeObjectMetadatumOutput", (done) => {
        checkTable(
            ...
            [{given: sut.getIntegrationRuntimeObjectMetadatumOutput({
                factoryName: codeinfra.output("my-factory-name"),
                integrationRuntimeName: codeinfra.output("my-integration-runtime-name"),
                metadataPath: codeinfra.output("my-metadata-path"),
                resourceGroupName: codeinfra.output("my-resource-group-name")}),
