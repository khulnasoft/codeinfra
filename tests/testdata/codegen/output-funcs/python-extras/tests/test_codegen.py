...
from codeinfra_mypkg import *


@pytest.fixture
def my_mocks():
    old_settings = codeinfra.runtime.settings.SETTINGS
    try:
        mocks = MyMocks()
        codeinfra.runtime.mocks.set_mocks(mocks)
        yield mocks
    finally:
        codeinfra.runtime.settings.configure(old_settings)
...
def my_preview_mocks():
    ...
    try:
        mocks = MyMocks()
        codeinfra.runtime.mocks.set_mocks(mocks, preview=True)
        yield mocks
    finally:
        codeinfra.runtime.settings.configure(old_settings)
...
class MyMocks(codeinfra.runtime.Mocks):
    def call(self, args):
        ...
        if args.token == 'mypkg::listStorageAccountKeys':
            ...
            keys_value = "{}" if codeinfra.contains_unknowns(args.args) else jstr(args.args)
            return {'keys': [
...
def test_func_with_default_value(my_mocks):
    ...
    # https://github.com/khulnasoft/codeinfra/issues/7815
...
def out(x):
    return codeinfra.Output.from_input(x).apply(lambda x: x)
