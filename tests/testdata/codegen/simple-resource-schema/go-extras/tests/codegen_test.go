...
func codeinfraTest(t *testing.T, body func(ctx *codeinfra.Context) error) {
	err := codeinfra.RunErr(body, codeinfra.WithMocks("project", "stack", mocks(0)))
...
func TestCanOverrideDefaultPluginDownloadURL(t *testing.T) {
	codeinfraTest(t, func(ctx *codeinfra.Context) error {
		r, err := example.NewResource(ctx, "resource", &example.ResourceArgs{},
			codeinfra.PluginDownloadURL("example.com/other"))
		assert.NoError(t, err)
