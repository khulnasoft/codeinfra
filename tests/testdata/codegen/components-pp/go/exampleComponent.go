...
type DeploymentZonesArgs struct {
	Zone codeinfra.StringInput
}

type GithubAppArgs struct {
	Id            codeinfra.StringInput
	KeyBase64     codeinfra.StringInput
	WebhookSecret codeinfra.StringInput
}
...
type ExampleComponentArgs struct {
	Input           codeinfra.StringInput
	CidrBlocks      map[string]codeinfra.StringInput
	GithubApp       *GithubAppArgs
	Servers         []*ServersArgs
	DeploymentZones map[string]*DeploymentZonesArgs
	IpAddress       []codeinfra.IntInput
}

type ExampleComponent struct {
	codeinfra.ResourceState
	Result codeinfra.AnyOutput
}
...
func NewExampleComponent(
	...
	for key0, val0 := range args.DeploymentZones {
		__res, err := random.NewRandomPassword(ctx, fmt.Sprintf("%s-zonePasswords-%v", name, key0), &random.RandomPasswordArgs{
			Length:          codeinfra.Int(16),
			Special:         codeinfra.Bool(true),
			OverrideSpecial: codeinfra.String(val0),
		}, codeinfra.Parent(&componentResource))
		if err != nil {
			return nil, err
		}
	...
	_, err = NewSimpleComponent(ctx, fmt.Sprintf("%s-simpleComponent", name), nil, codeinfra.Parent(&componentResource))
	if err != nil {
		return nil, err
	}
