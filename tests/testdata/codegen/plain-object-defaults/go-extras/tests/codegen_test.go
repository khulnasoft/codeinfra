...
func TestDefaultResource(t *testing.T) {
	t.Setenv("CODEINFRA_K8S_CLIENT_BURST", "42")
	codeinfra.Run(func(ctx *codeinfra.Context) error {
		_, err := example.NewFoo(ctx, "foo", &example.FooArgs{
			...
			BackupKubeClientSettings: &example.KubeClientSettingsArgs{Qps: codeinfra.Float64(7)},
		})
		assert.NoError(t, err)
		return nil
	}, codeinfra.WithMocks("example", "stack", mocks(0)))
}
