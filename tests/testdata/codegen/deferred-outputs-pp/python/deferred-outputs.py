...
second_password_length, resolve_second_password_length = codeinfra.deferred_output()
first = First("first", {
    'passwordLength': second_password_length})
second = Second("second", {
    'petName': first.pet_name})
resolve_second_password_length(second.password_length)
looping_over_many, resolve_looping_over_many = codeinfra.deferred_output()
...
resolve_looping_over_many(codeinfra.Output.from_input([v["passwordLength"] for _, v in many]))
