...
func TestEnumUsage(t *testing.T) {
	...
	t.Run("StringsForRelaxedEnum", func(t *testing.T) {
		require.NoError(t, codeinfra.RunErr(func(ctx *codeinfra.Context) error {
			rubberTree, err := tree.NewRubberTree(ctx, "blah", &tree.RubberTreeArgs{
				Container: plant.ContainerArgs{
					Color:    codeinfra.String("Magenta"),
					Material: codeinfra.String("ceramic"),
			...
			wg.Wait()
			return nil
		}, codeinfra.WithMocks("project", "stack", mocks(1))))
	})
		...
		require.NoError(t, codeinfra.RunErr(func(ctx *codeinfra.Context) error {
			rubberTree, err := tree.NewRubberTree(ctx, "blah", &tree.RubberTreeArgs{
				Container: plant.ContainerArgs{
					...
					Material: codeinfra.String("ceramic").ToStringOutput(),
