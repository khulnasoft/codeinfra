...
func TestGetAmiIdsWorks(t *testing.T) {
	makeFilter := func(n int) mypkg.GetAmiIdsFilterInput {
		return &mypkg.GetAmiIdsFilterArgs{
			Name: codeinfra.Sprintf("filter-%d-name", n),
			Values: codeinfra.StringArray{
				codeinfra.Sprintf("value-%d-1", n),
				codeinfra.Sprintf("value-%d-2", n),
			},
		}
	}
	...
	codeinfraTest(t, func(ctx *codeinfra.Context) error {
		output := mypkg.GetAmiIdsOutput(ctx, mypkg.GetAmiIdsOutputArgs{
			NameRegex:     codeinfra.String("[a-z]").ToStringPtrOutput(),
			SortAscending: codeinfra.Bool(true).ToBoolPtrOutput(),
			Owners: codeinfra.StringArray{
				codeinfra.String("owner-1"),
				codeinfra.String("owner-2"),
			}.ToStringArrayOutput(),
