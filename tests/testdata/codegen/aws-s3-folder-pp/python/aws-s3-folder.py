...
for range in [{"key": k, "value": v} for [k, v] in enumerate(os.listdir(site_dir))]:
    files.append(aws.s3.BucketObject(f"files-{range['key']}",
        ...
        key=range["value"],
        source=codeinfra.FileAsset(f"{site_dir}/{range['value']}"),
        content_type=range["value"],
        opts = codeinfra.ResourceOptions(deleted_with=site_bucket)))
# set the MIME type of the file
# Set the access policy for the bucket so all objects are readable
bucket_policy = aws.s3.BucketPolicy("bucketPolicy",
    bucket=site_bucket.id,
    policy=codeinfra.Output.json_dumps({
        "Version": "2012-10-17",
        "Statement": [{
            "Effect": "Allow",
            "Principal": "*",
...
codeinfra.export("bucketName", site_bucket.bucket)
codeinfra.export("websiteUrl", site_bucket.website_endpoint)
