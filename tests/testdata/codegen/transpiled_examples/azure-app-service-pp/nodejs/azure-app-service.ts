...
const sqlAdmin = config.get("sqlAdmin") || "codeinfra";
...
const blobAccessToken = codeinfra.secret(codeinfra.all([sa.name, appservicegroup.name, sa.name, container.name]).apply(([saName, appservicegroupName, saName1, containerName]) => azure_native.storage.listStorageAccountServiceSASOutput({
    accountName: saName,
...
const blob = new azure_native.storage.Blob("blob", {
    ...
    source: new codeinfra.asset.FileArchive("./www"),
});
...
const app = new azure_native.web.WebApp("app", {
    ...
    siteConfig: {
        appSettings: [
            {
                ...
                value: codeinfra.interpolate`https://${sa.name}.blob.core.windows.net/${container.name}/${blob.name}?${blobAccessToken}`,
            },
            {
            ...
            },
            {
                ...
                value: codeinfra.interpolate`InstrumentationKey=${appInsights.instrumentationKey}`,
            },
            {
        ...
        connectionStrings: [{
            ...
            connectionString: codeinfra.interpolate`Server= tcp:${sqlServer.name}.database.windows.net;initial catalog=${db.name};userID=${sqlAdmin};password=${sqlPassword.result};Min Pool Size=0;Max Pool Size=30;Persist Security Info=true;`,
        }],
    },
});
