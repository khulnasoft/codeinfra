...
eks_vpc = aws.ec2.Vpc("eksVpc",
    ...
    tags={
        "Name": "codeinfra-eks-vpc",
    })
eks_igw = aws.ec2.InternetGateway("eksIgw",
    vpc_id=eks_vpc.id,
    tags={
        "Name": "codeinfra-vpc-ig",
    })
...
for range in [{"key": k, "value": v} for [k, v] in enumerate(zones.names)]:
    vpc_subnet.append(aws.ec2.Subnet(f"vpcSubnet-{range['key']}",
        ...
        tags={
            "Name": f"codeinfra-sn-{range['value']}",
        }))
rta = []
for range in [{"key": k, "value": v} for [k, v] in enumerate(zones.names)]:
...
node_group = aws.eks.NodeGroup("nodeGroup",
    ...
    node_group_name="codeinfra-eks-nodegroup",
    node_role_arn=ec2_role.arn,
    subnet_ids=subnet_ids,
    tags={
        "Name": "codeinfra-cluster-nodeGroup",
    },
    scaling_config={
        "desired_size": 2,
        "max_size": 2,
...
codeinfra.export("clusterName", eks_cluster.name)
codeinfra.export("kubeconfig", codeinfra.Output.json_dumps({
    "apiVersion": "v1",
    "clusters": [{
        "cluster": {
