...
func editSnapshot(snapshotDirectory string, edits []compiledReplacement) (string, error) {
	...
	if len(edits) > 0 {
		var err error
		result, err = os.MkdirTemp("", "codeinfra-test-language")
		if err != nil {
...
// Do a snapshot check of the generated source code against the snapshot code. If CODEINFRA_ACCEPT is true just
// write the new files instead.
func doSnapshot(
	disableSnapshotWriting bool,
	sourceDirectory, snapshotDirectory string,
) ([]string, error) {
	if !disableSnapshotWriting && cmdutil.IsTruthy(os.Getenv("CODEINFRA_ACCEPT")) {
		// Write files
