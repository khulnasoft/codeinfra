...
import (
	...
	"github.com/khulnasoft/codeinfra/cmd/codeinfra-test-language/tests"
	"github.com/khulnasoft/codeinfra/pkg/v3/backend"
	backendDisplay "github.com/khulnasoft/codeinfra/pkg/v3/backend/display"
	...
	b64secrets "github.com/khulnasoft/codeinfra/pkg/v3/secrets/b64"
	...
	testingrpc "github.com/khulnasoft/codeinfra/sdk/v3/proto/go/testing"
...
func (l *providerLoader) LoadPackageReferenceV2(
	...
	if descriptor.Name == "codeinfra" {
		return schema.DefaultCodeinfraPackage.Reference(), nil
	}
...
func (eng *languageTestServer) PrepareLanguageTests(
	...
	languageClient := plugin.NewLanguageRuntimeClient(
		pctx, req.LanguagePluginName, codeinfrarpc.NewLanguageRuntimeClient(conn))
...
// TODO(https://github.com/khulnasoft/codeinfra/issues/13944): We need a RunLanguageTest(t *testing.T) function that
...
func (eng *languageTestServer) RunLanguageTest(
	...
	languageClient := plugin.NewLanguageRuntimeClient(
		pctx, token.LanguagePluginName, codeinfrarpc.NewLanguageRuntimeClient(conn))
	...
	for i, run := range test.Runs {
		...
		for _, pkg := range pkgs {
			if pkg.Name() == "codeinfra" {
				// No need to write the codeinfra package, it's builtin to core SDKs
				continue
			}
	...
	// We always override the core "codeinfra" package to point to the local core SDK we built as part of test
	// setup.
	localDependencies := map[string]string{}
	if token.CoreArtifact != "" {
		localDependencies["codeinfra"] = token.CoreArtifact
	}
	for _, pkg := range packages {
		...
		//
		// TODO[codeinfra/issues/16079]: This could probably be a per-sdk lock to be more fine grained and allow more
		// parallelism.
	...
	for i, run := range test.Runs {
		...
		// Generate the project and read in the Codeinfra.yaml
		...
		// TODO(https://github.com/khulnasoft/codeinfra/issues/13940): We don't report back warning diagnostics here
		...
		project, err := workspace.LoadProject(filepath.Join(projectDir, "Codeinfra.yaml"))
		if err != nil {
		...
		// TODO(https://github.com/khulnasoft/codeinfra/issues/13941): We don't capture stdout/stderr from the language
		...
		// TODO(https://github.com/khulnasoft/codeinfra/issues/13942): This should only add new things, don't modify

		// Query the language plugin for what it thinks the project dependencies are, we expect to see codeinfra and the SDKs.
		...
		if token.CoreVersion != "" {
			expectedDependencies = append(expectedDependencies, plugin.DependencyInfo{
				Name: "codeinfra", Version: token.CoreVersion,
			})
		}
		for _, pkg := range programPackages {
			if pkg.Name() == "codeinfra" {
				// Skip the codeinfra package, the version for that is handled above.
				continue
			}
		...
		for _, expectedDependency := range expectedDependencies {
			...
			// library, which is generally _not_ just the plugin name. e.g. "@codeinfra/aws" for the nodejs aws library.
