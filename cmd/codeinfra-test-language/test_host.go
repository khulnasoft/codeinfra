...
import (
	...
	"github.com/khulnasoft/codeinfra/cmd/codeinfra-test-language/providers"
...
func (h *testHost) GetProjectPlugins() []workspace.ProjectPlugin {
	...
	// just reading off Codeinfra.yaml.
	return nil
}
...
func newProviderServer(provider plugin.Provider) codeinfrarpc.ResourceProviderServer {
...
func wrapProviderWithGrpc(provider plugin.Provider) (plugin.Provider, io.Closer, error) {
	...
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		...
		Init: func(srv *grpc.Server) error {
			codeinfrarpc.RegisterResourceProviderServer(srv, newProviderServer(provider))
			return nil
		},
	...
	wrapped := plugin.NewProviderWithClient(
		nil, provider.Pkg(), codeinfrarpc.NewResourceProviderClient(conn), false)
