...
```bash
go test github.com/khulnasoft/codeinfra/sdk/python/cmd/codeinfra-language-python/v3 -count 1
```
...
```bash
go test github.com/khulnasoft/codeinfra/sdk/python/cmd/codeinfra-language-python/v3 -count 1 -run TestLanguage/classes/l1-output-string
```

To update the snapshots for generated code, run the tests with the `CODEINFRA_ACCEPT` environment variable set to a truthy value:

```bash
CODEINFRA_ACCEPT=1 go test github.com/khulnasoft/codeinfra/sdk/python/cmd/codeinfra-language-python/v3 -count 1
```

## Architecture

Test providers are defined in
<gh-file:codeinfra#cmd/codeinfra-test-language/providers>. PCL programs for language
conformance tests are defined in
<gh-file:codeinfra#cmd/codeinfra-test-language/testdata>.
<gh-file:codeinfra#cmd/codeinfra-test-language/tests.go> then references these
...
<gh-file:codeinfra#sdk/nodejs/cmd/codeinfra-language-nodejs/language_test.go> for
NodeJS/TypeScript and
<gh-file:codeinfra#sdk/python/cmd/codeinfra-language-python/language_test.go> for
Python. This function works as follows:

* The relevant [language runtime](language-runtimes) (e.g.
  `codeinfra-language-nodejs` for NodeJS) is booted up as a separate process.
* The `codeinfra-test-language` executable is booted up, with the language
...
* In preparation for test execution, the language test host retrieves the list
  of tests from the `codeinfra-test-language` server. It
  [](codeinfrarpc.LanguageRuntime.Pack)s the core SDK (e.g. `@codeinfra/codeinfra` in
  TypeScript/NodeJS).
* For each test:
  * SDKs required by the test are generated by calling the language host's
    [](codeinfrarpc.LanguageRuntime.GeneratePackage) method. The generated code is
    written to a temporary directory where it is
    [](codeinfrarpc.LanguageRuntime.Pack)ed for use in the test.
  * The [](codeinfrarpc.LanguageRuntime.GenerateProject) method is invoked to
    convert the test's PCL code into a program in the target language.
    Dependencies are installed with
    [](codeinfrarpc.LanguageRuntime.InstallDependencies) and the test program is
    [](codeinfrarpc.LanguageRuntime.Run).
...
sequenceDiagram
    ...
    participant PTL as codeinfra-test-language
    participant LH as Language host

    LTH->>+PTL: Start codeinfra-test-language process
    PTL-->>-LTH: Return gRPC server address

    note right of LTH: All future calls to<br>codeinfra-test-language are via gRPC
