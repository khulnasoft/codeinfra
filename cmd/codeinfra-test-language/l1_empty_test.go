...
func (h *L1EmptyLanguageHost) Pack(ctx context.Context, req *codeinfrarpc.PackRequest) (*codeinfrarpc.PackResponse, error) {
...
func (h *L1EmptyLanguageHost) Run(ctx context.Context, req *codeinfrarpc.RunRequest) (*codeinfrarpc.RunResponse, error) {
	if req.Info.RootDirectory != filepath.Join(h.tempDir, "projects", "l1-empty") {
...
func TestL1Empty_BadSnapshot(t *testing.T) {
	...
	assert.Contains(t, runResponse.Messages[0],
		"program snapshot validation failed:\nexpected file Codeinfra.yaml does not match actual file")
}
