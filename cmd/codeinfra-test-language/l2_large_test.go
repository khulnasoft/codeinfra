// Copyright 2016-2024, KhulnaSoft Ltd.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package main

import (
	"context"
	"errors"
	"fmt"
	"os"
	"path/filepath"
	"testing"

	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/apitype"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/resource"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/util/rpcutil"
	"github.com/khulnasoft/codeinfra/sdk/v3/go/common/workspace"
	codeinfrarpc "github.com/khulnasoft/codeinfra/sdk/v3/proto/go"
	testingrpc "github.com/khulnasoft/codeinfra/sdk/v3/proto/go/testing"
	"github.com/segmentio/encoding/json"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
	"google.golang.org/grpc"
	"google.golang.org/grpc/credentials/insecure"
	"google.golang.org/protobuf/types/known/structpb"
	"gopkg.in/yaml.v2"
)

type L2LargeLanguageHost struct {
	codeinfrarpc.UnimplementedLanguageRuntimeServer

	tempDir string
}

func (h *L2LargeLanguageHost) Pack(
	ctx context.Context, req *codeinfrarpc.PackRequest,
) (*codeinfrarpc.PackResponse, error) {
	if req.DestinationDirectory != filepath.Join(h.tempDir, "artifacts") {
		return nil, fmt.Errorf("unexpected destination directory %s", req.DestinationDirectory)
	}

	if req.PackageDirectory == filepath.Join(h.tempDir, "sdks", "large-4.3.2") {
		return &codeinfrarpc.PackResponse{
			ArtifactPath: filepath.Join(req.DestinationDirectory, "large-4.3.2.sdk"),
		}, nil
	} else if req.PackageDirectory != filepath.Join(h.tempDir, "sdks", "core") {
		return &codeinfrarpc.PackResponse{
			ArtifactPath: filepath.Join(req.DestinationDirectory, "core.sdk"),
		}, nil
	}

	return nil, fmt.Errorf("unexpected package directory %s", req.PackageDirectory)
}

func (h *L2LargeLanguageHost) GenerateProject(
	ctx context.Context, req *codeinfrarpc.GenerateProjectRequest,
) (*codeinfrarpc.GenerateProjectResponse, error) {
	if req.LocalDependencies["codeinfra"] != filepath.Join(h.tempDir, "artifacts", "core.sdk") {
		return nil, fmt.Errorf("unexpected core sdk %s", req.LocalDependencies["codeinfra"])
	}
	if req.LocalDependencies["large"] != filepath.Join(h.tempDir, "artifacts", "large-4.3.2.sdk") {
		return nil, fmt.Errorf("unexpected large sdk %s", req.LocalDependencies["large"])
	}
	if !req.Strict {
		return nil, errors.New("expected strict to be true")
	}
	if req.TargetDirectory != filepath.Join(h.tempDir, "projects", "l2-large-string") {
		return nil, fmt.Errorf("unexpected target directory %s", req.TargetDirectory)
	}
	var project workspace.Project
	if err := json.Unmarshal([]byte(req.Project), &project); err != nil {
		return nil, err
	}
	if project.Name != "l2-large-string" {
		return nil, fmt.Errorf("unexpected project name %s", project.Name)
	}
	project.Runtime = workspace.NewProjectRuntimeInfo("mock", nil)
	projectYaml, err := yaml.Marshal(project)
	if err != nil {
		return nil, fmt.Errorf("could not marshal project: %w", err)
	}

	// Write the minimal project file.
	if err := os.WriteFile(filepath.Join(req.TargetDirectory, "Codeinfra.yaml"), projectYaml, 0o600); err != nil {
		return nil, err
	}

	return &codeinfrarpc.GenerateProjectResponse{}, nil
}

func (h *L2LargeLanguageHost) GeneratePackage(
	ctx context.Context, req *codeinfrarpc.GeneratePackageRequest,
) (*codeinfrarpc.GeneratePackageResponse, error) {
	if req.LocalDependencies["codeinfra"] != filepath.Join(h.tempDir, "artifacts", "core.sdk") {
		return nil, fmt.Errorf("unexpected core sdk %s", req.LocalDependencies["codeinfra"])
	}
	if req.Directory != filepath.Join(h.tempDir, "sdks", "large-4.3.2") {
		return nil, fmt.Errorf("unexpected directory %s", req.Directory)
	}

	// Write the minimal package code.
	if err := os.WriteFile(filepath.Join(req.Directory, "test.txt"), []byte("testing"), 0o600); err != nil {
		return nil, err
	}

	return &codeinfrarpc.GeneratePackageResponse{}, nil
}

func (h *L2LargeLanguageHost) GetRequiredPlugins(
	ctx context.Context, req *codeinfrarpc.GetRequiredPluginsRequest,
) (*codeinfrarpc.GetRequiredPluginsResponse, error) {
	if req.Info.ProgramDirectory != filepath.Join(h.tempDir, "projects", "l2-large-string") {
		return nil, fmt.Errorf("unexpected directory to get required plugins %s", req.Info.ProgramDirectory)
	}

	return &codeinfrarpc.GetRequiredPluginsResponse{
		Plugins: []*codeinfrarpc.PluginDependency{
			{
				Name:    "large",
				Kind:    string(apitype.ResourcePlugin),
				Version: "4.3.2",
			},
		},
	}, nil
}

func (h *L2LargeLanguageHost) GetProgramDependencies(
	ctx context.Context, req *codeinfrarpc.GetProgramDependenciesRequest,
) (*codeinfrarpc.GetProgramDependenciesResponse, error) {
	if req.Info.ProgramDirectory != filepath.Join(h.tempDir, "projects", "l2-large-string") {
		return nil, fmt.Errorf("unexpected directory to get program dependencies %s", req.Info.ProgramDirectory)
	}

	return &codeinfrarpc.GetProgramDependenciesResponse{
		Dependencies: []*codeinfrarpc.DependencyInfo{
			{
				Name:    "codeinfra_codeinfra",
				Version: "1.0.1",
			},
			{
				Name:    "codeinfra_large",
				Version: "4.3.2",
			},
		},
	}, nil
}

func (h *L2LargeLanguageHost) InstallDependencies(
	req *codeinfrarpc.InstallDependenciesRequest, server codeinfrarpc.LanguageRuntime_InstallDependenciesServer,
) error {
	if req.Info.RootDirectory != filepath.Join(h.tempDir, "projects", "l2-large-string") {
		return fmt.Errorf("unexpected root directory to install dependencies %s", req.Info.RootDirectory)
	}
	if req.Info.ProgramDirectory != req.Info.RootDirectory {
		return fmt.Errorf("unexpected program directory to install dependencies %s", req.Info.ProgramDirectory)
	}
	if req.Info.EntryPoint != "." {
		return fmt.Errorf("unexpected entry point to install dependencies %s", req.Info.EntryPoint)
	}
	return nil
}

func (h *L2LargeLanguageHost) Run(
	ctx context.Context, req *codeinfrarpc.RunRequest,
) (*codeinfrarpc.RunResponse, error) {
	if req.Info.RootDirectory != filepath.Join(h.tempDir, "projects", "l2-large-string") {
		return nil, fmt.Errorf("unexpected root directory to run %s", req.Info.RootDirectory)
	}
	if req.Info.ProgramDirectory != req.Info.RootDirectory {
		return nil, fmt.Errorf("unexpected program directory to run %s", req.Info.ProgramDirectory)
	}
	if req.Info.EntryPoint != "." {
		return nil, fmt.Errorf("unexpected entry point to run %s", req.Info.EntryPoint)
	}

	conn, err := grpc.NewClient(
		req.MonitorAddress,
		grpc.WithTransportCredentials(insecure.NewCredentials()),
		rpcutil.GrpcChannelOptions(),
	)
	if err != nil {
		return nil, fmt.Errorf("could not connect to resource monitor: %w", err)
	}
	defer conn.Close()

	monitor := codeinfrarpc.NewResourceMonitorClient(conn)

	stackResource, err := monitor.RegisterResource(ctx, &codeinfrarpc.RegisterResourceRequest{
		Type: string(resource.RootStackType),
		Name: req.Stack,
	})
	if err != nil {
		return nil, fmt.Errorf("could not register stack: %w", err)
	}

	stringResource, err := monitor.RegisterResource(ctx, &codeinfrarpc.RegisterResourceRequest{
		Type:   "large:index:String",
		Custom: true,
		Name:   "res",
		Object: &structpb.Struct{
			Fields: map[string]*structpb.Value{
				"value": structpb.NewStringValue("hello world"),
			},
		},
	})
	if err != nil {
		return nil, fmt.Errorf("could not register resource: %w", err)
	}

	_, err = monitor.RegisterResourceOutputs(ctx, &codeinfrarpc.RegisterResourceOutputsRequest{
		Urn: stackResource.Urn,
		Outputs: &structpb.Struct{
			Fields: map[string]*structpb.Value{
				"output": stringResource.Object.Fields["value"],
			},
		},
	})
	if err != nil {
		return nil, fmt.Errorf("could not register stack outputs: %w", err)
	}

	return &codeinfrarpc.RunResponse{}, nil
}

// Run a successful test with a mocked runtime that returns large string.
//
// TODO(https://github.com/khulnasoft/codeinfra/issues/13945): enable parallel tests
//
//nolint:paralleltest // These aren't yet safe to run in parallel
func TestL2LargeString(t *testing.T) {
	ctx := context.Background()
	tempDir := t.TempDir()
	engine := &languageTestServer{}
	runtime := &L2LargeLanguageHost{tempDir: tempDir}
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		Init: func(srv *grpc.Server) error {
			codeinfrarpc.RegisterLanguageRuntimeServer(srv, runtime)
			return nil
		},
	})
	require.NoError(t, err)

	prepareResponse, err := engine.PrepareLanguageTests(ctx, &testingrpc.PrepareLanguageTestsRequest{
		LanguagePluginName:   "mock",
		LanguagePluginTarget: fmt.Sprintf("127.0.0.1:%d", handle.Port),
		TemporaryDirectory:   tempDir,
		SnapshotDirectory:    "./tests/testdata/snapshots",
		CoreSdkDirectory:     "sdk/dir",
		CoreSdkVersion:       "1.0.1",
	})
	require.NoError(t, err)
	assert.NotEmpty(t, prepareResponse.Token)

	runResponse, err := engine.RunLanguageTest(ctx, &testingrpc.RunLanguageTestRequest{
		Token: prepareResponse.Token,
		Test:  "l2-large-string",
	})
	require.NoError(t, err)
	t.Logf("stdout: %s", runResponse.Stdout)
	t.Logf("stderr: %s", runResponse.Stderr)
	assert.Empty(t, runResponse.Messages)
	assert.True(t, runResponse.Success)
}
